...
当前步骤：B2 · 文件、内存、资源容器与公共解压 · DBCS 文本缓冲对象
...
7. `[>]` B2：公共解压 B2.1、参数重排包装器 B2.2、80 字节文件对象 B2.3a 和全局调试内存管理器 B2.3b 已闭环，20,091 个真实 TSW 帧通过包装接口后的六包哈希不变；可配置数据根目录与进程级内存管理器生命周期 smoke 通过；Windows LLVM `core`/`app` 构建与 27 项 CTest 通过，当前执行 `0x004389D0..0x00438AF0` DBCS 文本缓冲对象。
