...
当前步骤：B5.7 · stream manager 与剩余游戏侧 stream wrapper
...
11. `[>]` B5：机械候选 76 项中已确认 73 项属于 B5，`0x004841B0/0x00484230/0x00484500` 三项 battle helper 移交 B10；唯一工作包已固定接口、状态和生命周期并达到开始条件。参数转换、SND 固定索引/sample buffer、sample manager、旧 wave 格式协商、SDL3 PCM sample backend 和六个游戏侧 sample wrapper 已按 LST 与平台边界实现；真实 `all.snd` 的全部 loader view、13 种 PCM 组合和异常 data 标签通过，Linux `core` 70/70、Linux/Windows `app` 74/74 CTest 通过。当前恢复 stream manager 及剩余 stream wrapper。
