...
当前步骤：B2 · 文件、内存、资源容器与公共解压 · 虚拟页/内存对象族
...
7. `[>]` B2：公共解压 B2.1、参数重排包装器 B2.2 和 80 字节文件对象 B2.3a 已闭环，20,091 个真实 TSW 帧通过包装接口后的六包哈希不变；可配置数据根目录已通过命令行、TOML 和任意工作目录窗口 smoke；Windows LLVM `core`/`app` 构建与 26 项 CTest 通过，当前执行 `0x00438650` 起的虚拟页/内存对象族。
