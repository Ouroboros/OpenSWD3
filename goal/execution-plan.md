...
当前步骤：B2 · LMF 地图主读取链已闭环 · 执行有限模块收口审计
...
7. `[>]` B2：B2.1 至 B2.20 已闭环；`huge.lmf` 地图主读取链已完成物理容器解析，`+0x1C` 的 329 条五 word/NUL 记录及其 `+0x20` 边界通过验证。下一步按既定 63 项函数范围执行一次有限收口审计，只补真实缺口；满足移交条件后进入 B3。
...
当前只执行 B2 收口审计，不切换到其他模块，也不继续 opcode 125 起的逐值恢复。审计不得扩展为新的全局资源调研；只允许核对既定范围、补真实缺口并判定模块移交条件。
