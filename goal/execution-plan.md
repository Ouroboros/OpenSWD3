...
10. `[>]` B4：B4.2 像素转换、B4.3 owned framebuffer 与 B4.4 稀疏分派及四条基础 copy 已闭环；B4.5 已继续闭环颜色运算、run 边缘和 raw/RLE 透明度单元，现转入 `0x30` 邻像素涂抹；当前 Linux `core` 42/42、Linux/Windows `app` 44/44 CTest 基线保持通过。
