...
当前步骤：B4.3 · owned framebuffer、显式 pitch 与行偏移
...
10. `[>]` B4：接口级逆向已满足单模块开始条件；B4.2 的 16 位像素转换、选择状态、穷举 UT 和当前 CM 真实资产验证已经静态闭环，现执行 B4.3 owned framebuffer、显式 pitch 与行偏移。
