...
当前步骤：B2 · LMF 地图头 `+0x18` 索引对象目录与解压已闭环 · 继续沿 `+0x1C` 原样记录目录推进
...
7. `[>]` B2：B2.1 至 B2.19 已闭环；`huge.lmf` 已推进到地图头 `+0x18` 索引对象目录，16 条对象流的 0x1A 字节头、实际压缩读取量和 40,741,028 字节解压输出通过验证。继续沿 `+0x1C` 原样记录目录推进。
