...
当前步骤：B4.7 · 画面动作、effect 更新与 present 请求
...
10. `[>]` B4：B4.2–B4.7 已闭环像素转换、owned framebuffer、全部当前资产可达 blitter、原版字形 Atlas、文字链、画面效果、21 个 primary 提交合同、BMP 截图、格式化文字和倒计时。B4.8 已令 SDL3 直接上传 owned framebuffer 和实际 pitch，建立固定 logical hash，并接通纹理重建、重新上传及保留缩放窗口尺寸的恢复路径；Linux `core` 54/54、Linux/Windows LLVM `app` 56/56 CTest 通过。继续实现 partial RECT 与 primary surface 状态；`sub_430BE0` 属于 `runtime_platform`，不插入 B4。
