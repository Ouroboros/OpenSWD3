...
当前步骤：B2 · `Env.dat` 物理 codec 与旧布局迁移已闭环 · 阶段提交边界
...
7. `[>]` B2：B2.1 至 B2.6 已闭环；B2.7a 已按完整汇编实现 `Env.dat` 当前/旧布局、迁移清零 BUG 和写回前缀的物理 codec，当前 63 字节样本与 Windows LLVM `core`/`app` 30/30 CTest 均通过。当前位于 B2.7a 阶段提交边界；提交后以 B2.7b 实现首次打开、迁移写回、按旧路径重开、返回值和不截断文件的调用顺序。
