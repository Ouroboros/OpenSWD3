...
9. `[>]` B3：26 项函数范围、状态所有权、平台端口与确定性测试策略已经建立；两套 RNG、帧时钟、默认绑定、DIK 快照、鼠标合同和整帧 20 条输入记录已实现，Windows LLVM `core` 为 38/38、`app` 为 40/40 CTest。当前实现 DBCS/IME 编辑驱动并接入 UTF-16 边界前的旧字节缓冲。
