...
10. `[>]` B4：B4.2 像素转换、B4.3 owned framebuffer 与 B4.4 稀疏分派及四条基础 copy 已闭环；B4.5 已继续闭环颜色运算、run 边缘、raw/RLE 透明度和 `0x30` 的 17 拍邻像素涂抹，现转入当前真实调用可达的 raw `0x88` 常量纵向淡出；当前 Linux `core` 42/42、Linux/Windows `app` 44/44 CTest 保持通过。
