...
9. `[>]` B3：26 项函数范围、状态所有权、平台端口与确定性测试策略已经建立；两套 RNG、帧时钟、默认绑定、单记录状态机和 DIK 快照已实现，Windows LLVM `core` 为 38/38、`app` 为 39/39 CTest。当前实现鼠标采样、坐标夹取、rebase 和灵敏度合同。
