...
10. `[>]` B4：B4.2–B4.7 已闭环像素转换、owned framebuffer、全部当前资产可达 blitter、原版字形 Atlas、文字链、画面效果、21 个 primary 提交合同、BMP 截图、格式化文字和倒计时。当前 Linux `core` 54/54、Windows LLVM `app` 56/56 CTest 通过；进入 B4.8，把 SDL3 smoke 的临时像素 vector 收口到 owned framebuffer，再完成上传、恢复生命周期与 framebuffer 哈希回放。`sub_430BE0` 已确认属于 `runtime_platform`，不插入 B4。
