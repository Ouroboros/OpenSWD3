...
当前步骤：B4.4 · 稀疏 blitter 分派与基础 copy 路径
...
10. `[>]` B4：B4.2 像素转换和 B4.3 owned framebuffer、显式 pitch、行偏移已经静态闭环；Linux `core` 41/41、Linux/Windows `app` 43/43 CTest 通过，现执行 B4.4 稀疏 blitter 分派与基础 copy 路径。
