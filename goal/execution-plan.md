...
以 `swd3.exe` 完整汇编为唯一行为真值，用 C++20、CMake 和 SDL3 实现可独立运行的 OpenSWD3，并继续读取原始游戏资产和存档。
...
- 汇编优先级：完整汇编高于 IDA 伪码、符号名、字符串解释和主观推断。
...
- 公共解压、主要资源容器、16 位软件像素规则、输入和时间的静态规格已经形成；原程序动态 oracle 因缺少运行后端仍受阻。
...
- `[>]` 覆盖进程入口、初始化、消息泵、单帧主循环、普通世界、特殊模式、战斗、保存/读取和退出。
- `[ ]` 把每条顶层路径落到明确模块。
- `[ ]` 记录帧内调用顺序以及互斥、等待和提前返回关系。

### A2 · 函数归属

- `[ ]` 为现有函数目录中的每个函数机械分配模块候选或 `unresolved`，本阶段不逐函数恢复语义。
- `[ ]` 区分游戏自有函数、编译器/CRT 代码和第三方库边界；后两类只恢复游戏实际依赖的调用合同，不冒充自有模块实现范围。
- `[ ]` 只人工复核 39 个已确认 ABI 合同、全部顶层直接调用和跨模块边界调用。
- `[ ]` `unresolved` 项必须记录调用者、被调者和以后负责处理的模块；只要不影响模块边界和首个模块接口，就允许留到对应模块处理。

### A3 · 状态所有权

- `[ ]` 为已有所有权目录中的状态和会跨模块传递的候选结构记录创建、读取、写入及销毁方；不在本阶段恢复每个内部字段。
- `[ ]` 找出剧情、动作、世界、特殊模式、战斗和存档的交叉写入。
- `[ ]` 区分状态所有者与临时借用者，避免新工程复制成无所有权的全局变量集合。

### A4 · 依赖、生命周期与平台边界

- `[ ]` 建立模块依赖方向和循环依赖清单。
- `[ ]` 恢复初始化、每帧更新、场景切换、战斗切换、失焦恢复和退出销毁顺序。
- `[ ]` 把 Win32、DirectDraw、DirectInput、Miles、Bink 和 GDI 字形来源与业务核心分开。

### A5 · 原程序模块到重写模块的映射

- `[ ]` 确定模块职责、输入输出和允许的依赖方向。
- `[ ]` 确定首轮实现顺序和每个模块的验证入口。
- `[ ]` 固定正式源码/测试目录、CMake 入口和首个模块采用的测试框架，避免实现时临时决定工程骨架。
- `[ ]` 只固定接口和所有权，不提前设计不受汇编证据支持的复杂类层次。
- `[ ]` 冻结首版架构基线；满足阶段 A 完成条件后，下一项必须是首个模块的接口级逆向与工程建立，不得转去全量深挖其他业务模块。
...
1. `[>]` A1：恢复顶层执行路径和首版模块骨架。
2. `[ ]` A2：函数归属。
3. `[ ]` A3：状态所有权。
4. `[ ]` A4：依赖、生命周期和平台边界。
5. `[ ]` A5：原程序模块到重写模块映射，并冻结首轮模块顺序。

在 A1 完成前不切换到其他调研项，也不继续 opcode 125 起的逐值恢复。
