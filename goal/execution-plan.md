...
10. `[>]` B4：B4.2–B4.8 已闭环像素转换、owned framebuffer、全部当前资产可达 blitter、原版字形 Atlas、文字链、画面效果、21 个 primary 提交合同、BMP 截图、格式化文字、倒计时以及 SDL3 primary 上传与恢复。Linux `core` 54/54、Linux/Windows LLVM `app` 56/56 CTest 通过；Windows OpenSWD3 的 `1000×750` 缩放、最小化、恢复、尺寸保持和零退出 smoke 通过。继续逐项审计 151 个 `rendering` 函数的实现映射；`sub_430BE0` 属于 `runtime_platform`，不插入 B4。
