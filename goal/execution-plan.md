...
12. `[>]` B6：78 个自有函数、TSW/ACT/ANI 运行时、公共动作记录、缓存状态、SND
    ...
    保留和原版先淘汰顺序也已形成最小闭环。六个真实 ACT 固定切片哈希通过，Linux
    `core` 83/83、Windows LLVM `app` 87/87 CTest 通过。当前进入 `0x98` 动作记录
    初始化与更新器，按命令小组边逆向边实现，不先全量深挖 ANI 或其他资产内部。
