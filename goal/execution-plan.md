...
当前步骤：B2 · `Env.dat` B2.7 闭环 · 阶段提交边界
...
7. `[>]` B2：B2.1 至 B2.7 已闭环；新无 FLIRT 汇编纠正了 `0x00424390` 的 CRT 误识别，`Env.dat` 带标记/无标记物理 codec、默认按键迁移、首次打开、写回、重开失败旧窗口语义、原返回值和不截断顺序均已实现。当前 63 字节样本与 Windows LLVM `core`/`app` 30/30 CTest 均通过，`openswd3.exe` 已重新链接；当前处于阶段提交边界。
8. `[ ]` 日志基础设施：在 B2.7 阶段提交后独立实现和测试完整日志合同，再继续 B2 的下一个可独立验证单元；不得与 `Env.dat` 变更混成同一提交。

当前只执行 B2，不切换到其他模块，也不继续 opcode 125 起的逐值恢复。B2 达到单模块开始条件后立即实现首个可独立验证单元，不等待全部资源格式内部语义恢复。
