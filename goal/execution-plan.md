...
当前步骤：B2 · 特殊模式 Act 初始化归属修正已闭环 · 阶段提交边界
...
7. `[>]` B2：B2.1 至 B2.5 已闭环；B2.6 已按完整汇编确认 `0x00439DE0..0x00439FC8` 实际初始化特殊模式的 18 个 Act 记录和三组回调表，已从 `resource_io` 改归 `special_modes` 并延后实现。当前位于 B2.6 阶段提交边界；提交后对资源模块尚未人工复核的候选 `0x00423FB0` 做接口级逆向。
