...
13. `[>]` B7：114 个函数的单模块工作包和真实地址范围已经建立，开始条件已满足；
    ...
    `sub_405500` 的首个可运行解释器切片随后恢复 opcode `0/4/5/7/8`，并完整恢复
    `sub_406580` 的 72 槽路线运动准备；普通角色现在能够按真实 PATH.DAT 执行等待、
    请求 A*、逐帧移动、终点清槽和脚本回绕，并保持 `sub_405430` 逐角色选择普通/队伍
    owner 的原始顺序。helper 已从入口到全部出口完成第二次
    汇编核对；解释器只对五个已恢复分支声明精确，其余 32 个分支保持光标并输出一次
    延期警告。当前 MAPS 的 136 个非零初始 Path 全部通过 opcode 5 真实数据回归，
    Linux/Windows LLVM 完整应用均为 178/178 CTest 通过，Windows EXE 成功链接；
    未启动任何原版或重写版 EXE。
