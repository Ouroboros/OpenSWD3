...
当前步骤：B4.9d · 固定 writer 与 packed-row 像素 helper
...
10. `[>]` B4：B4.2–B4.8 已闭环；五个 packed image command-stream 函数、三个基础绘制 helper，以及 `0x00411FA0/0x00414B60/0x00414CE0/0x00414E50/0x004153D0` 暂停层、动作精灵、行特效和限时消息协调器均已通过完整 LST、独立 UT、Linux `core` 60/60 与 Windows LLVM `app` 62/62 CTest。151 项有限收口矩阵现有 14 项真实缺口、5 项延后接线和 1 项归属修正；当前实现 `0x00417050/0x004174D0..0x00417650/0x00417DE0` 固定 writer 与 packed-row 像素 helper。`sub_430BE0` 属于 `runtime_platform`，不插入 B4。
