...
状态：执行中，P0/P1 已完成，当前步骤 P2；当前 handler `0x00429AD2`（opcode 64）
...
- 当前已实现的 90 个 opcode 不继承完成状态，必须随所属 handler 组重新审计和验证。
...
值、窗口、公共 join/yield 与返回路径。P2当前人工语义已随审计增至127行。前47行已独立
...
同步prefix到interval/remaining并快照left/top；成功同调用继续，超56项原地yield重试。对外进度为
已实现90/198、已验收70/198；内部workpack为47/146，即
`5 assembly_exact + 42 platform_adapted + 99 pending_audit`。下一行只审计
`0x00429AD2`下的opcode64。
