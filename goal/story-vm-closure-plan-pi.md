...
状态：执行中，P0/P1 已完成，当前步骤 P2；当前 handler `0x0042890F`（opcode 34）
...
- 当前已实现的 72 个 opcode 不继承完成状态，必须随所属 handler 组重新审计和验证。
...
值、窗口、公共 join/yield 与返回路径。P2当前人工语义已随审计增至126行。前二十四行已独立
...
以及共享opcodes29–33全局整数set/add/sub与无符号条件跳转。当前为24/146；下一行只审计
`0x0042890F`的opcode34 handler。
