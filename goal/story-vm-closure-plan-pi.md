...
状态：执行中，P0/P1 已完成，当前步骤 P2；当前 handler `0x0042829C`（opcode 14）
...
- 当前已实现的 58 个 opcode 不继承完成状态，必须随所属 handler 组重新审计和验证。
...
值、窗口、公共 join/yield 与返回路径。P2 前九行已独立关闭：默认非法与共享对话两组、
opcode7/9 的 bit31/bit30 clear、opcode8 lifetime、opcode10/11 action、opcode12 position 与
opcode13 role step。当前为 9/146；下一行只审计 `0x0042829C` 的 opcode14。
