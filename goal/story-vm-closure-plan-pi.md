...
状态：执行中，P0/P1 已完成，当前步骤 P2；当前 handler `0x0042A54C`（opcode 84）
...
值、窗口、公共 join/yield 与返回路径。P2当前人工语义已随审计增至127行。前64行已独立
...
opcode83创建packed-row效果，保持同ID先删除、staged读取、矩形门、mode/数组初始化与unsafe边界。
已实现101/198、已验收88/198；内部workpack为64/146，即
`8 assembly_exact + 56 platform_adapted + 82 pending_audit`。下一行只审计`0x0042A54C`下的opcode84。
