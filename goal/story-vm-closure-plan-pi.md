...
状态：执行中，P0/P1 已完成，当前步骤 P2；当前 handler `0x00428310`（opcode 15）
...
值、窗口、公共 join/yield 与返回路径。P2 前十行已独立关闭：默认非法与共享对话两组、
opcode7/9 的 bit31/bit30 clear、opcode8 lifetime、opcode10/11 action、opcode12 position、
opcode13 role step 与 opcode14 action wait。当前为 10/146；下一行只审计 `0x00428310` 的 opcode15。
