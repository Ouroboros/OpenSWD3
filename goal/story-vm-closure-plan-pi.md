...
状态：执行中，P0/P1 已完成，当前步骤 P2；当前共享 handler `0x00428D18`（opcodes 42/43）
...
- 当前 C++ 接入 77 个显式 opcode。
- 当前资产静态控制流观察到 143 个 opcode，其中仍有 73 个尚未实现。
- 另有 55 个 opcode 未在当前资产静态控制流中观察到；未观察不等于不可达或可以删除。
- `0..124` 已有人工汇编语义；`125..193` 目前只有分派、长度和保守 CFG，尚不能直接翻译为 C++。
- 当前已实现的 77 个 opcode 不继承完成状态，必须随所属 handler 组重新审计和验证。
...
值、窗口、公共 join/yield 与返回路径。P2当前人工语义已随审计增至126行。前三十二行已独立
...
opcode40角色重定位/路径完成/MAPS坐标fallback，以及opcode41共享selector索引目标并重载TALK窗口。
当前为32/146，即`5 assembly_exact + 27 platform_adapted + 114 pending_audit`；下一行只审计
`0x00428D18`的共享opcodes42/43 handler。
