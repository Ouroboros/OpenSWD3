...
状态：执行中，P0/P1 已完成，当前步骤 P2；当前 handler `0x00429AE8`（opcode 65）
...
- 当前已实现的 91 个 opcode 不继承完成状态，必须随所属 handler 组重新审计和验证。
...
值、窗口、公共 join/yield 与返回路径。P2当前人工语义已随审计增至127行。前48行已独立
...
只把64-word选择滚动表清为CFCF，保留interval、remaining、cursor与视口快照后同调用继续。对外进度为
已实现91/198、已验收71/198；内部workpack为48/146，即
`5 assembly_exact + 43 platform_adapted + 98 pending_audit`。下一行只审计
`0x00429AE8`下的opcode65。
