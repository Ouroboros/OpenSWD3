...
状态：执行中，P0/P1 已完成，当前步骤 P2；当前 handler `0x00427EC2`（opcode 9）
...
值、窗口、公共 join/yield 与返回路径。P2 前四行已独立关闭：`0x0042D230` 覆盖 opcode 0
及完整默认域；`0x00427B8F` 覆盖 `1-6,89-90`；`0x00427E72` 覆盖 opcode7；
`0x00427E9A` 覆盖 opcode8 的 u16 零扩展、one-shot lifetime、previous 与同帧 continue。
当前为 4/146；下一行只审计 `0x00427EC2` 的 opcode9。
