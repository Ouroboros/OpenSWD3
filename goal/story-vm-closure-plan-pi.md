...
状态：执行中，P0/P1 已完成，当前步骤 P2；当前 handler `0x004285ED`（opcode 24）
...
- 当前已实现的 64 个 opcode 不继承完成状态，必须随所属 handler 组重新审计和验证。
...
schedule、共享opcode21/22全局bit条件跳转，以及opcode23全局bit列表全真跳转。当前为18/146；
下一行只审计`0x004285ED`的opcode24 handler。
