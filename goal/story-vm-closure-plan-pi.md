...
状态：执行中，P0/P1 已完成，当前步骤 P2；当前 handler `0x004289BE`（opcode 37）
...
- 当前已实现的 75 个 opcode 不继承完成状态，必须随所属 handler 组重新审计和验证。
...
值、窗口、公共 join/yield 与返回路径。P2当前人工语义已随审计增至126行。前二十七行已独立
...
opcode35脚本时钟低位字节条件跳转，以及opcode36脚本时钟相对快照条件跳转。当前为27/146；
下一行只审计`0x004289BE`的opcode37 handler。
