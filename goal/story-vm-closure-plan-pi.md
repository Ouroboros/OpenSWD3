...
状态：执行中，P0/P1 已完成，当前步骤 P2；当前 handler `0x0042A341`（opcode 83）
...
值、窗口、公共 join/yield 与返回路径。P2当前人工语义已随审计增至127行。前63行已独立
...
opcode82遣出首匹配头像动作，保持staged读取、unsafe缺失推进、motion分支、previous与same-call。
已实现100/198、已验收87/198；内部workpack为63/146，即
`8 assembly_exact + 55 platform_adapted + 83 pending_audit`。下一行只审计`0x0042A341`下的opcode83。
