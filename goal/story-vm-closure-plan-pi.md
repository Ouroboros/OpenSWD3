...
状态：执行中，P0/P1 已完成，当前步骤 P2；当前 handler `0x00428DB8`（opcode 44）
...
值、窗口、公共 join/yield 与返回路径。P2当前人工语义已随审计增至126行。前三十三行已独立
...
以及共享opcodes42/43设置/清除dialog counter共用的interaction lock并由42重置受控角色base variant。
当前为33/146，即`5 assembly_exact + 28 platform_adapted + 113 pending_audit`；下一行只审计
`0x00428DB8`的opcode44 handler。
