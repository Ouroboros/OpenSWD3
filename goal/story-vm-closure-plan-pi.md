...
opcode175恢复完整ANI control dword的bit4置位，actual activity owner写入后+2、previous并
same-call，不audio；其余31位及幂等语义保持。ANI flags恢复u32并由窄port连接SDL实际owner。
线性资产为0条，以absence锁定；ANI owner依赖与Story VM共4/4、Linux core186/186及
app192/192通过。已实现178/198、已验收176/198；内部workpack为131/146，即
`21 assembly_exact + 110 platform_adapted + 15 pending_audit`。
opcode176恢复完整ANI control dword的bit4清除，actual activity owner写入后+2、previous、
audio一次并yield，不读取后继；其余31位及幂等语义保持。线性资产为0条，以absence锁定；
...
opcode178恢复角色路径碰撞绕过位：FFF0替换为controlled index低word后仍作ordinary GUID
lookup，FFFE由helper直选controlled；命中只置actual role flag，miss静默，+4、previous并
same-call，无audio。6条真实线性记录全部base raw并逐条精确尾回放。role lookup与Story VM
共4/4、SDL app编译、Linux core完整门186/186与app完整门192/192通过。已实现181/198、已验收179/198；
内部workpack为134/146，即`23 assembly_exact + 111 platform_adapted + 12 pending_audit`。
opcode179复用actual framebuffer deformation与CRT RNG owner，恢复四项signed operand、固定
640×480 surface、相对origin、双倍field和固定半径24注入，成功后才头插；+10、previous并
same-call，无audio。线性资产为0条，以absence与synthetic锁定。frame-deformation、world-frame
consumer与Story VM共5/5、SDL app编译、Linux core完整门186/186与app完整门192/192通过。已实现182/198、已验收
180/198；内部workpack为135/146，即`23 assembly_exact + 112 platform_adapted + 11 pending_audit`。
opcode180恢复actual process-level完整u32主帧执行门清零、+2、previous、audio一次并yield；
不读取后继。现代复用F8/frame-dispatch/runtime integration同一owner，缺binding在原写点
停止。线性资产为0条，以absence与synthetic锁定。主帧门依赖与Story VM共6/6、SDL app编译
通过，Linux完整门core 186/186、app 192/192通过。已实现183/198、已验收181/198；内部workpack为136/146，即
`23 assembly_exact + 113 platform_adapted + 10 pending_audit`。
shared opcodes181–185恢复完整u32宽值set/add/sub-sign-clamp与unsigned GE/LE同文件重载，复用
actual 64项脚本变量owner；value/target顺序、variable0共享clamp、高index重试和negative index
unsafe边界均已锁定。唯一真实opcode184记录双向回放与Story VM 3/3通过，SDL app编译通过，
Linux完整门core 186/186、app 192/192通过。已实现188/198、已验收186/198；内部workpack为137/146，即
`23 assembly_exact + 114 platform_adapted + 9 pending_audit`。
shared opcodes186–187恢复第二项0x38 party-member记录的17字段getter与signed GE/LE同文件
重载；完整字段宽度、negative/default、高selector重试、not-taken未读target、taken双audio-yield
和checked load失败均已锁定。两opcode线性资产为0；Story VM 3/3和SDL app编译通过，Linux完整门
core 186/186、app 192/192通过。已实现190/198、已验收188/198；内部workpack为138/146，即
`23 assembly_exact + 115 platform_adapted + 8 pending_audit`。
shared opcodes188–190恢复第二项0x38 party-member记录17字段的set/add/sub写入、getter extension、
i32回绕和目标宽度；field16先写低byte，再以可失败B10 LEVEL窄port成功更新field14，SDL不
伪造LEVEL值。三opcode线性资产为0；Story VM 3/3、SDL app编译、Linux core 186/186与app
192/192完整门全部通过。
...
opcode192恢复完整u32音乐transition mode/current divisor等待：mode2或current非零原地等待，
否则推进2；三路均previous/audio/yield且不fetch后继。线性资产为0，以absence和synthetic锁定，
归assembly exact。Story VM 3/3、SDL app编译、Linux core 186/186与app 192/192完整门通过。
已实现195/198、
