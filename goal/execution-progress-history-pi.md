# OpenSWD3 详细执行进度历史（Pi）

状态：历史归档，不是当前执行顺序、状态或断点的权威来源。
...
本轮再完成`audit_order=276`的`0x00478250`战斗角色公共前部初始化函数。完整权威LST主体`0x00478250..0x004782FF`从proc到endp共63个物理行、35条实际指令、0个call、0个跳转、0个局部标签和1个返回点，没有外部chunk。函数保存this到EDX并严格按原顺序执行53次dword、10次word和1次byte写：先把四目标槽写全一，再写回合/动作/高偏移控制字段、四个动作文本dword，最后以41-dword `rep stosd`清零`+0x10..+0xB3` definition；正常返回`EAX=this,ECX=0,EDX=this`。实现以公共typed视图复用组B现有definition、动作文本、动作种类及执行状态唯一owner，组A和固定单例补齐同一物理前部owner；对象前缀、动作文本和definition的每个可达原写访问均typed-stop，保留已完成写入、EAX/ECX/EDX及未到达说明owner，不增加短对象继续路径。`0x00451875`单例尾跳、`0x0046E494`组A调用和`0x00475564`组B调用三处物理xref均删除本函数opaque边界并直接组合；公共前部停止分别阻断`_atexit`注册、组A尾字段/分配和组B分配。验证：定向测试`2/2`、AddressSanitizer`197/197`、Linux core`197/197`、Linux app`203/203`、连续10轮完整core、clang-format和release审计全部通过，最终日志零OpenSWD3源码warning、测试失败、sanitizer finding或runtime error；未启动原版或OpenSWD3游戏程序。工作包为`276/422 = 266 platform_adapted + 10 assembly_exact + 146 pending_audit`；生成器连续双跑逐字节一致，SHA256为`6ef0923452f60cf13342d07e9bd6ef861b3b5100602169723c1ee53b61679cd0`。动态差分因原版组A/组B/单例完整对象、异常写访问、definition说明堆及三caller联合寄存器捕获后端缺失而登记为`blocked_runtime_oracle`。
...
本轮再完成`audit_order=282`的`0x004783B0`战斗角色状态选择坐标查询函数。完整权威LST主体`0x004783B0..0x004783F7`共72字节、16条实际指令、0个call、1个条件分支和2个`retn 8`，没有外部chunk；入口以角色`+0x26D8`完整word选择primary `+0x0D66/+0x0D68`或alternate `+0x0D86/+0x0D88`，严格按X源读取、X低word写入、Y源读取、Y低word写入顺序执行，保留CMP flags、分支专有EAX/ECX/EDX残值、同址别名和Y故障时已提交的X写入。typed实现复用startup/action和Group-B lifecycle canonical owner，不建立平行角色数组；固定token、非对齐、越界、源读和输出写都只在原真实访问点停止。24个物理caller共35个callsite中，17个已有modern caller的28处站点分四个REVIEW全部直接组合，覆盖角色指标、脚本、效果、动作、特殊动作和目标就绪；其余7处属于尚未实现的caller workpack，不保留本函数opaque生产边界。caller回收同时恢复既有word/dword/stack参数槽低16位别名、高word保留、action 402两次查询共享槽及各路径故障后缀阻断。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。连续10轮完整core、changed-range clang-format、零源码warning/测试失败/sanitizer finding和TMP分类门亦通过；未启动原版或OpenSWD3游戏程序。工作包为`282/422 = 272 platform_adapted + 10 assembly_exact + 140 pending_audit`；生成器连续双跑逐字节一致，SHA256为`9103b39a7077fb0c5759decd99d269657ce48d4343ee1d29db5264a4a5cb82b0`。动态差分因原版组A/组B完整对象、异常内存页及35处callsite联合寄存器/SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=283`的`0x00478400`战斗角色绘制偏移查询函数。完整权威LST主体`0x00478400..0x00478464`共101字节、25条实际指令、0个call、3个条件分支和1个`retn 8`，没有外部chunk；函数严格按X后Y执行基础word写入，`actor+0x2A87` bit1置位时重复写覆盖X/Y，`actor+0x2B08`完整dword等于1且基础X非零时再以`*(actor+0x2548)+0x0C`宽度减基础X写回镜像X。typed实现保留输出dword高word、相同输出及输出/actor字段别名、逐访问部分提交、ESI局部低word与恢复、TEST/CMP/SUB flags及null render-source真实解引用停止；owner复用startup/action与Group-B lifecycle/action-composition唯一状态。6个已关闭caller中的8个物理callsite按效果、选择框和动作三轮全部删除opaque边界：效果保留各自资源/sample/render前后缀，选择框复用共享坐标槽并保留重置/快照/动作6顺序，动作十三/十四复用零初始化局部dword、mixed-zero canonical回退、覆盖/镜像偏移、frame-source和raster/audio/render后缀。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。四份最终stderr为空；连续10轮完整core、changed-range clang-format及TMP分类/迁移终检通过，零源码warning、测试失败或sanitizer finding；未启动原版或OpenSWD3游戏程序。工作包为`283/422 = 273 platform_adapted + 10 assembly_exact + 139 pending_audit`；生成器连续双跑逐字节一致，SHA256为`722bf3c15e501a11000b52758726196d3e52a90b10e048a3ec3e4ce8bc9f9a83`。动态差分因原版组A/组B完整对象、render source异常内存页及8处callsite联合寄存器/SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=284`的`0x00478470`战斗角色基准坐标查询函数。完整权威LST主体`0x00478470..0x0047849C`共45字节、9条实际指令、0个call、0个条件分支和1个`retn 8`，没有外部chunk；函数严格按X源、首输出指针、X减数、X低word写入、Y源、Y减数、次输出指针、Y低word写入的真实顺序执行`position_x-source_y_offset`与`position_y-low16(target_phase_y_adjustment)`。typed实现复用既有actor coordinate view及startup/action与Group-B lifecycle canonical owner，保留EAX高word、ECX/EDX输出指针残值、两次16-bit SUB flags、同输出指针和输出/actor别名、八个真实访问停点以及X提交后的Y故障前缀。三个REVIEW共回收7个已关闭caller中的8个现代物理callsite：三个效果、脚本Group-A/Group-B动态文字、目标阶段和动作十三/十四；各路径保留原非零门、低word dword槽别名、高word来源、资源或坐标前缀、caller flags、X后Y部分提交及后缀抑制。目标阶段直接复用目标参数槽低word并在演出记录清零前停机；动作十三/十四仅在双非零偏移路径查询基准坐标，保留两套相反局部槽顺序与不同EAX高word来源，mixed-zero继续走canonical坐标查询。生产源码仅保留reserved地址值且对`0x00478470`零调用；`audit_order=419 / 0x00484020`延期caller已登记为其自身工作包必须复用typed接口。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。四份正式stderr为空；连续10轮完整core、changed-range clang-format、inventory双生成及TMP分类门均通过，未启动原版或OpenSWD3应用程序。工作包为`284/422 = 274 platform_adapted + 10 assembly_exact + 138 pending_audit`，inventory SHA-256为`e945ecc6cf0c24204114f7eff9006d5d8ddbf4d0822403e40a426e459af38601`。动态差分因原版组A/组B完整对象、异常内存页及9处物理callsite联合寄存器/SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=285`的`0x004784A0`战斗角色当前动作帧边界查询函数。完整权威LST主体`0x004784A0..0x0047859B`共252字节、68条实际指令、2个call、6个条件分支、1个无条件跳转和1个`retn 4`，没有外部chunk；两个callee依次为已关闭动作记录更新`0x004321E0`和frame piece provider `0x004315D0`。函数先执行完整`0x98`动作记录清零与三项角色门，再按mode选择X anchor；临时记录`+0x4C/+0x4A`两次dword读取物理重叠，provider只消费低word。mirror完整dword读取先于frame-token提交；输出首指针延迟读取，四项完整dword严格按X、Y、width、height提交，width/height前分别从actor重载frame token，保留输出与actor字段别名及逐槽部分提交。首个actor读取前原prologue已使EAX为0、ECX为`0x26`且flags来自`xor eax,eax`，只有EDX保留caller入口值；两条正常早退与所有typed-stop均保留对应寄存器/flags。typed实现复用startup/action与Group-B lifecycle canonical owner以及既有action updater/frame provider，不新增平行actor数组。五个已关闭caller中的15个物理callsite分三轮全部直接组合：frame input 3处、菜单后退/前进各3处、target refresh 2处和selection frame 4处；各caller复用自身四dword块，保留入口token算术、共享输出、正常早退继续、部分提交和后缀抑制。旧frame input、菜单、target runtime与selection frame snapshot槽只保留reserved ordinal且生产零调用。验证：最终定向`1/1`、Linux core`199/199`、AddressSanitizer`199/199`、Linux app`205/205`、连续10轮完整core、changed-range clang-format、inventory双生成及release审计全部通过；最终日志零OpenSWD3源码warning、测试失败、sanitizer finding或runtime error，未启动原版或OpenSWD3应用程序。工作包为`285/422 = 275 platform_adapted + 10 assembly_exact + 137 pending_audit`；inventory SHA-256为`26476ff99b1f4ae5e64a878a72a6e1ef489b7ae3bf89cb029d57629345528cf1`。动态差分因原版完整Group-A/Group-B actor、动作资源、frame provider、异常内存页及15处物理callsite联合寄存器/SEH捕获后端缺失而登记为`blocked_runtime_oracle`。
...
本轮再完成`audit_order=288`的`0x00478600`战斗角色当前坐标查询函数。完整权威LST主体`0x00478600..0x0047861E`共31字节、7条实际指令、0个call、0个分支和1个`retn 8`，没有外部chunk；函数严格按第一输出指针读取、actor X读取、X低word写入、actor Y读取、第二输出指针读取、Y低word写入执行，正常返回保留入口EAX高word并令`AX=Y`、`EDX=out_x token`、`ECX=out_y token`，全部MOV与RET保持入口flags。typed实现保留六个真实访问停点、输出互相及输出/actor别名、Y侧故障时X部分提交，并与`0x004783B0` mode-gated selector query保持独立。三个REVIEW回收六个caller函数中的二十一个物理callsite：效果步进4处、debug overlay 1处、target-selection entry 1处、script dispatch 13处、turn gate 1处与Group-B action17 1处；Group-A复用startup party并在独立helper无startup时回退action owner，Group-B复用lifecycle action-execution owner。最后两处caller保留栈局部位形、EAX/ECX/EDX、到达call前flags、`-0x10/+0x10`与`+0x19/-0x19`调整、typed publication及query stop后的frame source、blit、倒计时和父级后缀抑制；reserved枚举ordinal保持，脚本SDL和opponent adapter空槽生产零raw调用。验证：最终定向`1/1`、Linux core`199/199`、AddressSanitizer/UBSan`199/199`、Linux app`205/205`、连续10轮完整core、changed-range clang-format、inventory双生成、TMP分类及完整unstaged/staged发布审计均通过，fresh reviewer最终返回PASS；最终日志零OpenSWD3源码warning、测试失败、sanitizer finding或runtime error，未启动原版或OpenSWD3应用程序。工作包为`288/422 = 278 platform_adapted + 10 assembly_exact + 134 pending_audit`；inventory SHA-256为`23c9d965fd4b9c964bc2803605411e7d9a7b50dbfbc94dc2993d603d7bc65ed7`。动态差分因原版完整Group-A/Group-B actor、异常栈与输出内存页、寄存器/flags/SEH及二十一处caller联合捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=289`的`0x00478620`战斗角色帧资源准备函数。完整权威LST主体`0x00478620..0x0047866C`共77字节、29条实际指令、2个call、1个条件分支和2个普通`retn`，没有外部chunk；函数保存EBX/ESI/EDI后以DF=0执行38次正向`REP MOVSD`，直接组合已关闭动作更新与帧查询，完整EAX零早退，非零路径只替换EAX/ECX低word并无条件发布provider token。typed实现复用Group-A action dispatch与Group-B lifecycle内十八槽物理动作记录及canonical frame-token owner，保留重叠传播、source/destination部分提交、两次CALL隐式返回地址、两个RET、寄存器、ESP和TEST/ADD flags。三个REVIEW回收五个物理caller：frame input三处、Group-A目标演出一处与`0x00484020`前缀中的Group-B目标演出一处；最后一处复用source×target二维phase、source共享`+0x255C`、target动作owner、坐标/phase清零/资源/解码/属性/host-surface顺序，并校正外层真实`0x004787F0` mode调用及其EAX/ECX/EDX残值。生产`0x00478620`零raw调用；`audit_order=419 / 0x00484020`自身继续`pending_audit`。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。连续10轮完整core、changed-range clang-format、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP分类及完整release审计全部通过；未启动原版或OpenSWD3游戏程序。工作包为`289/422 = 279 platform_adapted + 10 assembly_exact + 133 pending_audit`；inventory SHA-256为`ad09b8c14fa5660fcda7a0a5f5b02ec7528f1c3dd042f7af4c10c07b1c2324d7`。动态差分因原版完整Group-A/Group-B actor十八槽动作记录、重叠与异常内存页、callee寄存器/flags、DF/SEH及五处caller联合捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=290`的`0x00478670`战斗角色资料记录选择函数。完整权威LST主体`0x00478670..0x0047868A`共27字节、12条实际指令、0个call、3个条件分支和2个`retn 4`，没有外部chunk或中段入口；函数先读取完整32位参数并TEST，参数零只读取actor `+0x04` live来源记录token，参数非零只读取`+0x00`角色基础记录token，第二次TEST后非零直接返回、零值执行`xor eax,eax`再返回。typed实现直接复用`LegacyBattleGroupAConfigurationState`首双token唯一owner，保留互斥读取、相同token动态别名、两个RET真实返回地址、ESP、EAX/ECX/EDX与TEST/XOR flags，并为参数、两个字段和两个RET分别建模typed-stop。startup随机/顺序补位的`0x00452511/0x00452646`两处当前物理caller均直接组合typed leaf，以固定参数1读取首个Group-A actor基础记录，保持所选token先于新角色token进入materialization；旧seed ordinal改为reserved且生产零调用。`0x00481109`参数0与`0x00481ADD`参数1两处后续caller已登记精确复用合同，当前不伪造尚未实现的生产路径。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。连续10轮完整core、全量/changed-range clang-format、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP分类及发布前审计亦通过；未启动原版或OpenSWD3游戏程序。工作包为`290/422 = 280 platform_adapted + 10 assembly_exact + 132 pending_audit`；inventory SHA-256为`6e832b90dd4b17830f2b63f7c1ca2ce7b98b7e534ad12f1b92e895f761bc6386`。动态差分因原版完整Group-A actor首双token、异常栈/字段内存页及四处caller联合寄存器/flags/SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=291`的`0x00478690`战斗角色回合完成latch查询函数。完整权威LST主体`0x00478690..0x00478696`共7字节、2条实际指令、0个call、0个分支和1个普通`retn`，没有外部chunk或中段入口；函数只把actor `+0x2AAC`完整dword读入EAX后执行普通RET，ECX、EDX与全部flags保持，正常ESP增加4。typed实现直接借用Group-A action-execution十槽与startup Group-B lifecycle八槽中的同一`turn_completion_latch` owner，保留字段与RET两个真实停点、完整寄存器、ESP/EIP、flags和访问计数，不增加平行actor或latch数组。Group-A frame的one-based Group-A目标、Group-B动作目标及Group-B frame的Group-A候选三处物理caller全部直接组合typed leaf，保留各自地址算术或excluded查询的EAX/EDX、SUB/CMP flags、真实返回地址和post-call TEST；字段停止抑制当前分支、剩余循环与frame后缀，生产`0x00478690`零raw调用。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。连续10轮完整core、全量/changed-range clang-format、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP分类及完整release审计均通过；未启动原版或OpenSWD3游戏程序。工作包为`291/422 = 281 platform_adapted + 10 assembly_exact + 131 pending_audit`；inventory SHA-256为`6f3b2d2ac6ed80eeae52310b85a946e408c4ce5846452bd2c0a82175d5283119`。动态差分因原版完整Group-A/Group-B actor完成latch、异常字段/栈内存页及三处caller联合寄存器/flags/SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=292`的`0x004786A0`战斗角色空闲状态查询函数。完整权威LST主体`0x004786A0..0x004786A6`共7字节、2条实际指令、0个call、0个分支和1个普通`retn`，没有外部chunk或中段入口；函数只把actor `+0x2AB4`完整dword读入EAX后执行普通RET，ECX、EDX与全部flags保持，正常ESP增加4。typed实现把字段命名为`idle_state_latch`并直接复用Group-A action-execution十槽及startup Group-B lifecycle八槽canonical owner，保留字段与RET两个真实停点、完整寄存器、ESP/EIP、flags和访问计数，不增加平行actor、第二套latch或token槽。Group-A frame的actor启动、peer扫描、直接空闲门和selected-target四处及Group-B frame的target scan、selection和action-decision三处物理caller全部直接组合typed leaf，保留各自前置callee或比较的EAX/EDX、TEST/CMP flags、七个真实返回地址以及post-call完整TEST或`CMP 1`；typed-stop阻断当前项、剩余循环与各自后缀，生产`0x004786A0`零raw调用。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。连续10轮完整core、全量/changed-range clang-format、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP分类及完整release审计均通过；未启动原版或OpenSWD3游戏程序。工作包为`292/422 = 282 platform_adapted + 10 assembly_exact + 130 pending_audit`；inventory SHA-256为`9eacf35fec012c1c797eed0020ee0b2f6b46525ceda67c2e84f5a8d5112c0373`。动态差分因原版完整Group-A/Group-B actor空闲latch、异常字段/栈内存页及七处caller联合寄存器/flags/SEH捕获后端缺失而登记为`blocked_runtime_oracle`。
...
本轮再完成`audit_order=295`的`0x004786D0`战斗角色启动门word查询函数。完整权威LST主体`0x004786D0..0x004786D7`共8字节、2条实际指令、0个call、0个分支和1个普通`retn`，没有外部chunk或中段入口；函数只把actor `+0x2A74`完整word读入AX，保留EAX高16位、ECX、EDX和全部flags后普通RET。typed实现直接复用Group-A action-execution十槽和startup Group-B lifecycle八槽中的canonical启动门owner，保留字段与RET两个真实停点、完整寄存器、ESP/EIP、flags和访问计数，不增加平行actor数组、第二套word缓存或token槽。Group-A frame、Group-B frame及调试叠加层Group-B/Group-A行四处物理caller全部直接组合typed leaf，保留两类对象地址算术、命令mask、level或绘制callee残值形成的EAX/EDX/flags、四个真实返回地址，以及frame post-call TEST与调试post-call低word mask；typed-stop抑制当前caller后缀，调试端口ordinal保留为reserved且生产`0x004786D0`零raw调用。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。连续10轮完整core、全量/changed-range clang-format、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP分类及完整release审计亦通过；未启动原版或OpenSWD3游戏程序。工作包为`295/422 = 285 platform_adapted + 10 assembly_exact + 127 pending_audit`；生成器连续双跑逐字节一致，inventory SHA-256为`f75543c6335679e3e34b8c7ea7782cce30f8d0e5960c8cb4a2820182bf6f005a`。动态差分因原版完整Group-A/Group-B actor、异常字段/栈内存页及四处caller联合寄存器/flags/SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=296`的`0x004786E0`战斗角色动作目标word查询函数及七类函数中的二十处直接caller。完整权威LST主体`0x004786E0..0x004786E7`共8字节、2条实际指令、0个call、0个分支和1个普通`retn`，没有外部chunk或中段入口；函数只把actor `+0x29A2`完整word读入AX，保留EAX高16位、ECX、EDX和全部flags后普通RET。typed实现直接复用Group-A action-execution十槽、startup Group-B lifecycle八槽与调试特殊actor的canonical owner，保留字段与RET两个真实停点、完整寄存器、ESP/EIP、flags和访问计数，不增加平行actor数组、第二套target缓存或token槽。动作分派2处、Group-A frame 4处、Group-B frame 2处、动作后处理1处、角色优先级2处、效果协调器8处与调试热键1处物理caller全部直接组合typed leaf，保留20个真实返回地址、调用前寄存器、符号扩展/哨兵比较、物理结果历史和typed-stop前缀/后缀；retired generic ordinal保留但生产`0x004786E0`零raw调用。尚待回收的generic setter继续执行原调用，并同步唯一canonical `actor+0x29A2`写效果；全局`word_4A7626`由`selected_target_index`独立持有。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。连续10轮完整core、全量/changed-range clang-format、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP分类及完整release审计亦通过；未启动原版或OpenSWD3游戏程序。工作包为`296/422 = 286 platform_adapted + 10 assembly_exact + 126 pending_audit`；inventory SHA-256为`394486f01599ad2d0a0f3051079e8bcae840da84bf7cb21c24c814a2f9e2dcb8`。动态差分因原版完整Group-A/Group-B及调试特殊actor、异常字段/栈内存页和二十处caller联合寄存器/flags/SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=297`的`0x004786F0`战斗角色首条动作记录清零函数及唯一直接caller。完整权威LST主体`0x004786F0..0x00478703`共20字节、8条实际指令、0个call、0个分支和1个普通`retn`，没有外部chunk或中段入口；函数保存actor token与EDI，以入口DF方向执行38次独立`STOSD`，只清零从actor `+0x02A0`开始的第0条`0x98`字节动作记录。typed实现直接复用`LegacyBattleActorActionRecordSlots::frame_source_action_record` canonical owner，保留DF正反地址序列、每个dword写fault与部分提交、XOR flags和AF未定义、PUSH/POP/RET栈访问、EAX/ECX/EDX/EDI/ESP/EIP，不使用高层数组清零或新增平行动作记录。主动作分派case 15的唯一物理caller `0x0045529A`直接组合typed leaf，保留`index*0xBCD` EAX、Group-A actor ECX、前序callee EDX、switch EDI=14、末次SUB flags和真实返回地址`0x0045529F`；typed-stop阻断清模式、召唤构造、动画与全部后缀，生产`0x004786F0`零raw调用。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。连续10轮完整core、全量/changed-range clang-format、零源码warning/测试失败/sanitizer finding、inventory双生成及TMP分类均通过；未启动原版或OpenSWD3游戏程序。工作包为`297/422 = 287 platform_adapted + 10 assembly_exact + 125 pending_audit`；inventory SHA-256为`d130ea0b32aa2d9b5fe244f54580c398fd2c8a086da71bb502fe469399d9c85e`。动态差分因原版完整Group-A actor前部物理内存、异常destination/栈页、DF、前序callee残值及唯一caller联合寄存器、flags与SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=298`的`0x00478710`战斗角色动作与显示模式设置函数及十一个caller函数中的四十一处物理CALL。完整权威LST主体`0x00478710..0x0047876D`共94字节、0个callee、六次入口CMP、一次额外CMP、两个OR、两个word写和两个`retn 4`出口；参数按完整dword依序比较`2/13/3/14/6/15`，默认写动作word，`2/13/3`写显示word并清动作word，`14/15/6`还先对模式byte执行保留旧bit的读改写。typed实现复用Group-A action/startup与Group-B lifecycle canonical owner，保留参数与RET栈读取、word截断、逐字段部分提交、ECX/EDX、ESP/EIP，以及默认/命中CMP、额外CMP和OR的最终flags。动作分派、对手动作、Group-A/Group-B frame、动作后处理、消息阶段、脚本分派、三个Group-B组合helper及待审actor-ready内部共41处真实返回地址均保留独立物理身份；共享helper不折叠CALL，嵌套结果和typed-stop向外层回卷，`0x00480220`reply只投影实际执行的八个固定槽。旧脚本枚举保留reserved地址与ordinal，生产generic `0x00478710`调用归零；SDL脚本继续绑定同一startup/action owner。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。连续10轮完整core、全量/changed-range clang-format、零OpenSWD3源码warning、测试失败、sanitizer finding、inventory双生成、TMP分类及完整release审计均通过；未启动原版或OpenSWD3游戏程序。工作包为`298/422 = 288 platform_adapted + 10 assembly_exact + 124 pending_audit`；inventory SHA-256为`8bf8a407bbf1781c98d2096019ff31a38a4fe7c17e1de2b00695457b94793b76`。动态差分因原版完整Group-A/Group-B actor、异常参数/字段/栈内存页及41处caller联合寄存器、flags与SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=299`的`0x00478770`战斗角色`+0x26B8`高位清除函数。完整权威LST主体`0x00478770..0x0047877A`共11字节、2条实际指令、0个callee、0个分支和1个普通`retn`，没有外部chunk或中段入口；函数对`actor+0x26B8`执行不可省略的完整dword读改写，只清bit31并保留低31位，即使bit31原清除也仍写回，随后普通RET。typed实现保留字段读、字段写与RET三处真实停止点，写fault不提交AND flags，RET fault保留字段及逻辑flags，EAX/ECX/EDX保持且ESP仅在正常RET增加4。字段以中性`field_26b8`归入既有Group-A action与Group-B lifecycle action-execution canonical owner，没有新增平行actor状态。唯一机器码caller为待审`sub_478B60`中的`0x00478CC8 -> 0x00478CCD`；完整LST确认EBX固定为1，只有`sub_47BA80`返回1且`cmp eax,ebx`相等时进入CALL，故入口EAX=1且flags来自`CMP 1,1`。Workpack 299不复制该1403行待审父函数；Group-A/Group-B frame通过窄reply只投影真实执行信息并组合typed leaf，现代frame typed-stop后缀已阻断，父函数`actor+0x2AC4`前缀和四POP/`retn 4`阻断合同延期到Workpack 315。生产`0x00478770` generic调用归零。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。连续10轮完整core、全量/changed-range clang-format、零OpenSWD3源码warning、测试失败、sanitizer finding、inventory双生成、TMP分类及完整release审计均通过；未启动原版或OpenSWD3游戏程序。工作包为`299/422 = 289 platform_adapted + 10 assembly_exact + 123 pending_audit`；inventory SHA-256为`3e37262d7ed737667d380c92eece59d05b4bd4d68b4fa941099029f7d2543de0`。动态差分因原版完整Group-A/Group-B actor、`actor+0x26B8`异常内存页、RET异常栈页及唯一caller联合寄存器、flags与SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=300`的`0x00478780`战斗角色`+0x26B8`高位设置函数及21个caller函数中的35处物理CALL。完整权威LST主体`0x00478780..0x004787B4`共53字节、11条实际指令、0个callee、2个条件分支和1个普通`retn`，没有外部chunk或中段入口；函数先以`actor+0x26C0`完整dword bit25为早退门，未阻断时读取`+0x26B8`，原bit31置位才依次清零`+0x2A78`与`+0x0542`两个word，最后无条件OR/write bit31。typed实现保留四字段访问顺序、两个word的部分提交、字段与RET六类停止点、EAX/ECX/EDX、ESP/EIP以及TEST/XOR/OR flags。`+0x26C0`收敛为共享backing，assignment保留destination alias；其余字段复用`field_26b8`、`summon_completion_word`和slot 4 `special_target_action_record.command_cursor`，Group-A/Group-B继续绑定action/startup lifecycle canonical owner。27个已关闭物理CALL全部在原控制流位置直接组合typed leaf，保留独立返回地址、入口寄存器、flags、条件可达性、嵌套trace顺序与typed-stop后缀；8个待审CALL保持精确延期，`sub_478B60`通过窄reply投影`0x00478BD6 -> 0x00478BDB`，第二pending槽保持独立actor和物理身份。生产`0x00478780` generic raw调用归零。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。连续10轮完整core、新文件全量与旧文件changed-range clang-format、零OpenSWD3源码warning、测试失败、sanitizer finding、inventory双生成、TMP分类及完整release审计均通过；未启动原版或OpenSWD3游戏程序。工作包为`300/422 = 290 platform_adapted + 10 assembly_exact + 122 pending_audit`；inventory SHA-256为`6b2a0955844e203c1cdec332519159fed4225d54e4b44f2408253fdd2f241306`。动态差分因原版完整Group-A/Group-B actor、四字段异常内存页、RET异常栈页及35处caller联合寄存器、flags与SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=301`的`0x004787C0`战斗角色`+0x26B8`高位查询函数及唯一物理CALL。完整权威LST主体`0x004787C0..0x004787C9`共10字节、3条实际指令、0个callee、0个分支和1个普通`retn`，没有外部chunk或中段入口；函数先把`actor+0x26B8`完整dword读入EAX，再执行`SHR 31`返回原bit31。typed实现复用Workpacks 299–300建立的Group-A action-execution与Group-B startup/lifecycle `field_26b8` canonical owner，保留字段读取与RET两个真实停止点、完整EAX覆盖、ECX actor token、入口EDX、ESP/EIP，以及SHR后CF=原bit30、ZF/SF/PF与AF/OF definedness，不新增布尔缓存或平行actor状态。唯一机器caller为frame coordinator `sub_453200`中的`0x00453409 -> 0x0045340E`；现代caller按原两次SHL和两次SUB形成`1007*index` EAX、`0x005029D0+0x2F34*index` ECX，保留最后SUB flags及九宫格callee后的EDX，直接组合typed leaf并执行真实post-call `TEST/JNZ`。物理CALL地址与返回地址保留为单元素trace，字段或RET typed-stop阻断TEST、独立动作帧和全部frame后缀；旧`actor_ready_query`槽保留reserved ordinal且生产零调用。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。连续10轮完整core、新文件全量与旧文件changed-range clang-format、零OpenSWD3源码warning、测试失败、sanitizer finding、inventory双生成、TMP分类及完整release审计均通过；未启动原版或OpenSWD3游戏程序。工作包为`301/422 = 291 platform_adapted + 10 assembly_exact + 121 pending_audit`；inventory SHA-256为`ed883a34c95dd0ca329382eb1c4941a62a26752ee7ced6bc73313efa8f15cbcd`。动态差分因原版完整Group-A actor、`actor+0x26B8`异常内存页、RET异常栈页及唯一caller联合寄存器、flags与SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=302`的`0x004787D0`战斗角色效果资源word槽写入函数及13个caller函数中的40处物理CALL。完整权威LST主体`0x004787D0..0x004787E8`共25字节、5条实际指令、0个callee、0个分支和1个`retn 4`，没有外部chunk或中段入口；函数按参数低word、actor cursor、目标word槽和`retn 4`顺序执行。typed实现复用Group-A action/startup与Group-B lifecycle canonical owner中的35个word槽和cursor，保留四个真实停点、EAX/ECX/EDX、ESP/EIP、XOR flags与部分提交；cursor大于等于35时只在原目标写点停止，故登记为`platform_adapted`。40处机器码CALL中，10个已关闭父函数内27处站点全部直接组合typed leaf并保留返回地址、actor、共享backing、寄存器、flags、trace顺序与后缀抑制；另外13处精确延期到`0x0047E5C0`、`0x00481010`与`0x00481A40`三个尚待审父函数。独立`0x0047CEC0` cursor更新保持参数1递增并夹34、其他参数按u16递减及下溢，奖励调用后同步canonical cursor。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过。连续10轮完整core、新文件全量与旧文件changed-range clang-format、零OpenSWD3源码warning、测试失败、sanitizer finding、inventory双生成及TMP分类/迁移终检均通过；未启动原版或OpenSWD3游戏程序。工作包为`302/422 = 292 platform_adapted + 10 assembly_exact + 120 pending_audit`；inventory SHA-256为`adeeb9eba674ec671ec51253acfda91f35d43be15135826a004f574400fab93e`。动态差分因原版完整Group-A/Group-B actor、35槽效果资源区、cursor、异常栈/字段内存页及40处caller联合寄存器、flags与SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=303`的`0x004787F0`战斗角色演出激活函数及三个已关闭父函数中的七处物理CALL。完整权威LST主体`0x004787F0..0x0047882D`共62字节、15条实际指令、0个callee、3个条件分支和1个`retn 4`，没有外部chunk或中段入口；函数按完整参数写`+0x2AB8`，保留原`+0x2A94` marker到DL，无条件写`+0x2ABC=1`，按原marker决定是否写6，并只在完整`+0x2AA0==1`且live-record token非零时清记录低word。typed实现复用Group-A startup party与Group-B lifecycle canonical owner，保留九个真实访问停点、原marker返回值、EAX/ECX/EDX、ESP/EIP、TEST/CMP flags和部分提交；固定actor或live token无法合法解析时只在对应原访问点停止，故登记为`platform_adapted`。动作分派三处、对手动作两处及脚本opcode 10两处物理CALL全部直接组合typed leaf，保留七组CALL/返回地址、入口寄存器、flags、canonical backing、分支可达性与父级后缀抑制；动作6两处CALL按启动/完成phase分别到达，脚本两分支按LST恢复入口算术，生产generic `0x004787F0`调用归零。验证：Linux core`199/199`、AddressSanitizer/UBSan`199/199`、Linux app`205/205`及连续10轮core均通过；新文件全量与历史文件changed-range clang-format、零OpenSWD3源码warning、测试失败、sanitizer finding、inventory双生成逐字节一致及TMP/release审计均通过，未启动原版或OpenSWD3游戏程序。工作包为`303/422 = 293 platform_adapted + 10 assembly_exact + 119 pending_audit`；inventory SHA-256为`5f746280df63b04b8f04a04b139170ee47fb6fc9f25e5e46aa5f89c6dea699cd`。动态差分因原版完整Group-A/Group-B actor、九个异常内存访问及七处caller联合寄存器、flags与SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=304`的`0x00478830`战斗角色二值状态切换函数及脚本opcode 29的两处物理CALL。完整权威LST主体`0x00478830..0x00478843`共20字节、6条实际指令、0个callee、0个跳转和1个`retn 4`，没有外部chunk或中段入口；函数读取actor `+0x2AF8`完整dword，旧值为零写1、任意非零写0，保留旧值到EDX及TEST flags，显式参数不读取但由`retn 4`消费。typed实现复用Group-A startup party与Group-B lifecycle action-configuration canonical owner，保留字段读、字段写与RET三个真实停止点、完整EAX/ECX/EDX、ESP/EIP、flags和部分提交。脚本Group-A `0x0046B04E -> 0x0046B053`与Group-B `0x0046B072 -> 0x0046B077`两处CALL均在原位置直接组合typed leaf，保留actor-code发布、地址算术、两组物理身份、Group-B byte按u8递增回绕、cursor `+4`、selected actor code清零和父级EAX=1；code 18与`0xFFFF`不提前拒绝，只在原字段读点停止。生产generic `0x00478830`调用归零。验证：Linux core`199/199`、AddressSanitizer/UBSan`199/199`、Linux app`205/205`及连续10轮完整core均通过；新文件全量与触碰文件clang-format、零源码warning、测试失败、sanitizer finding、inventory双生成、TMP分类及release审计全部通过，未启动原版或OpenSWD3游戏程序。工作包为`304/422 = 294 platform_adapted + 10 assembly_exact + 118 pending_audit`；inventory SHA-256为`17532962784376fa50d3dac230c609ed9b30309425db0a7f92628acd75b3bd55`。动态差分因原版完整Group-A/Group-B actor、三个异常内存访问及两处caller联合寄存器、flags与SEH捕获后端缺失而登记为`blocked_runtime_oracle`。

本轮再完成`audit_order=305`的`0x00478850`战斗角色运行状态重置函数。完整LST范围`0x00478850..0x00478A61`共112条指令，保留8段REP、两次304-dword重复清零、DF、RNG拒绝采样、寄存器、flags、栈和852个物理访问。typed实现复用Group-A startup/action与Group-B lifecycle canonical owner，只新增未建模residual backing；七个已关闭父函数中的17处物理CALL全部直接接入，`sub_479850`四处与`sub_47E880`一处CALL精确延期。最大路径逐访问停止、caller trace、nested request offset和父级后缀抑制均有测试。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过；连续10轮完整core、格式、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP和release audit亦通过，未启动原版或OpenSWD3游戏程序。工作包为`305/422 = 295 platform_adapted + 10 assembly_exact + 117 pending_audit`；inventory SHA-256为`6d2e8fc6ef51963ba50ab33c8d439dd34f0eb1f29f8e62fa9e782e098feec353`。原版动态差分因完整actor backing、DF/异常内存页、RNG联合状态和22处caller联合捕获后端缺失而保留`blocked_runtime_oracle`。

本轮再完成`audit_order=306`的`0x00478A70`战斗角色目标选择状态写入函数。完整LST范围`0x00478A70..0x00478A91`共5条指令，按参数低word、空闲latch写1、动作目标写入、进度低word清零和`retn 4`顺序执行。typed实现复用Group-A action/startup与Group-B lifecycle canonical owner，保留五个真实访问停点、前缀部分提交、EAX/ECX/EDX、flags、ESP/EIP和物理返回地址；六个已关闭父函数中的20处物理CALL全部直接接入，没有延期CALL。request offset、nested trace与父级后缀抑制均有测试。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过；连续10轮完整core、changed-range格式、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP和release audit亦通过，未启动原版或OpenSWD3游戏程序。工作包为`306/422 = 296 platform_adapted + 10 assembly_exact + 116 pending_audit`；inventory SHA-256为`b7a3c5db9f5df536b2800e1dc4c5016587ea8e503376cafd12fddc0afb8d186d`。原版动态差分因完整actor backing、异常参数/字段/RET栈页和20处caller联合捕获后端缺失而保留`blocked_runtime_oracle`。

本轮再完成`audit_order=307`的`0x00478AA0`战斗角色目标选择计数递增函数。完整LST范围`0x00478AA0..0x00478AA7`共2条指令，对actor `+0x2A76`执行16位INC并保留CF。typed实现复用Group-A action与Group-B lifecycle canonical owner，保留字段读写、RET、回绕、寄存器、flags、ESP/EIP和三个真实停止点；已关闭Group-A frame中的三处物理CALL全部直接接入，没有延期CALL。one-based地址算术、循环request序号、caller trace与父级后缀抑制均有测试。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过；连续10轮完整core、changed-range格式、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP和发布前静态审计亦通过，未启动原版或OpenSWD3游戏程序。工作包为`307/422 = 297 platform_adapted + 10 assembly_exact + 115 pending_audit`；inventory SHA-256为`4d285482fe5a5c218d62e0f07f923a3a1cf6bf8ca2484aafe4d5df05d297b372`。原版动态差分因完整actor backing、异常字段/RET栈页、INC flags和三处caller联合捕获后端缺失而保留`blocked_runtime_oracle`。

本轮再完成`audit_order=308`的`0x00478AB0`战斗角色目标选择计数查询函数。完整LST范围`0x00478AB0..0x00478AB7`共2条指令，只把actor `+0x2A76`完整word读入AX后普通返回。typed实现复用Workpack 307的Group-A action与Group-B lifecycle canonical owner，保留EAX高16位、ECX、EDX、flags、字段读取、RET和两个真实停止点；脚本分派case 61的唯一物理CALL直接接入，没有延期CALL。无符号actor code、Group-A/Group-B地址算术、调用前SUB flags、post-call `TEST AX,AX`、零计数读取后缀、非零跳过及typed-stop后缀抑制均有测试。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过；连续10轮完整core、全量/changed-range格式、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP和完整release审计亦通过，未启动原版或OpenSWD3游戏程序。工作包为`308/422 = 298 platform_adapted + 10 assembly_exact + 114 pending_audit`；inventory SHA-256为`e9336855b4153ed9cfaf00e7541b5ed9f2048b5c1623d122e22a88b0d9a2ce25`。原版动态差分因完整actor backing、异常字段/RET栈页及唯一caller联合捕获后端缺失而保留`blocked_runtime_oracle`。

本轮再完成`audit_order=309`的`0x00478AC0`战斗角色启动门递增与latch发布函数。完整LST范围`0x00478AC0..0x00478AD1`共3条指令，按启动门word读取、16位INC写回、latch完整dword写1和普通RET顺序执行。typed实现复用Group-A action与Group-B lifecycle canonical owner，把`+0x2AE0`收敛为同一action-execution状态并保留runtime reset映像路径，保留EAX/ECX/EDX、CF、INC flags、ESP/EIP、四个真实停止点和部分提交；Group-A frame两处、Group-B frame两处与script dispatch case 78一处物理CALL全部直接接入，没有延期CALL。五组call/return、父级地址算术、入口寄存器、flags与typed-stop后缀抑制均有测试。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过；格式化后完整门禁、连续10轮core、全量/changed-range格式、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP和完整release审计亦通过，未启动原版或OpenSWD3游戏程序。工作包为`309/422 = 299 platform_adapted + 10 assembly_exact + 113 pending_audit`；inventory SHA-256为`68a4b609093e1fcc352040de93133a266e3eb780b43e98aa34997e73ef9d5257`。原版动态差分因完整actor backing、两个异常字段页、RET异常栈页及五处caller联合捕获后端缺失而保留`blocked_runtime_oracle`。

本轮再完成`audit_order=310`的`0x00478AE0`战斗角色启动门与目标计数衰减函数。完整LST范围`0x00478AE0..0x00478B10`共13条指令，按两个word读取、非零时32位DEC并写回、latch完整dword清零和普通RET顺序执行。typed实现复用Group-A action、Group-B lifecycle与固定前一槽packed canonical owner，保留EAX高字、EDX清零、CMP/DEC flags、ESP/EIP、六个真实停止点和部分提交；action dispatch两处、Group-A frame四处、Group-B frame三处、post-action两处与debug hotkeys一处物理CALL全部直接接入，没有延期CALL。十二组call/return、全局request offset、嵌套dispatch、父级地址算术、入口状态与typed-stop后缀抑制均有测试。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过；格式化后完整门禁、连续10轮core、全量/changed-range格式、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP和完整release审计亦通过，未启动原版或OpenSWD3游戏程序。工作包为`310/422 = 300 platform_adapted + 10 assembly_exact + 112 pending_audit`；inventory SHA-256为`5d2cac8cfcd4a4f3475e611b3ebb7ad7ba9fa33ed352b3732676b56929d9c0b2`。原版动态差分因完整actor backing、三个异常字段页、RET异常栈页及十二处caller联合捕获后端缺失而保留`blocked_runtime_oracle`。下一条为`audit_order=311 / 0x00478B20 / sub_478B20`；按用户要求，Workpack 310发布后暂停，不进入Workpack 311。

本轮再完成`audit_order=311`的`0x00478B20`战斗角色动作目标清空函数。完整LST范围`0x00478B20..0x00478B29`共2条指令，只把actor `+0x29A2`完整word无条件写为`0xFFFF`后普通返回。typed实现复用Group-A action与Group-B lifecycle canonical action-target owner，保留字段写、RET读、EAX/ECX/EDX、flags、ESP/EIP、两个真实停止点及RET失败时的部分提交；Group-A frame两处、Group-B frame一处与post-action两处物理CALL全部直接接入，没有延期CALL。五组call/return、全局request offset、嵌套dispatch、父级入口状态与typed-stop后缀抑制均有测试；相邻post-action gate-decay的两组物理地址及EAX/EDX残值也按完整LST修正。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205 全部通过；格式化后完整门禁、连续10轮core、全量/changed-range格式、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP和完整release审计亦通过，未启动原版或OpenSWD3游戏程序。工作包为`311/422 = 301 platform_adapted + 10 assembly_exact + 111 pending_audit`；inventory SHA-256为`805e441154895295cfae0ded8ff186748d3058f355b22dcb07b2c84da06d19cb`。原版动态差分因完整Group-A/Group-B actor backing、`+0x29A2`异常字段页、RET异常栈页及五处caller联合捕获后端缺失而保留`blocked_runtime_oracle`。下一条为`audit_order=312 / 0x00478B30 / sub_478B30`。

本轮再完成`audit_order=312`的`0x00478B30`战斗角色目标选择latch设置函数。完整LST范围`0x00478B30..0x00478B3A`共2条指令，只把actor `+0x2AA8`完整dword无条件写为1后普通返回。typed实现把既有runtime-reset residual字段语义化为`target_selection_latch`，复用Group-A runtime-reset与Group-B lifecycle canonical owner，保留字段写、RET读、EAX/ECX/EDX、flags、ESP/EIP、两个真实停止点及RET失败时的部分提交；action dispatch、Group-A frame与Group-B frame三处物理CALL全部直接接入，没有延期CALL。三组call/return、全局request offset、Group-A嵌套dispatch、命中terminal回复与typed-stop后缀抑制均有测试。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205全部通过；格式化后完整门禁、连续10轮core、全量/changed-range格式、零源码warning/测试失败/sanitizer finding、inventory双生成、TMP和完整release审计亦通过，未启动原版或OpenSWD3游戏程序。工作包为`312/422 = 302 platform_adapted + 10 assembly_exact + 110 pending_audit`；inventory SHA-256为`bf69e0e06746fc36650e43f0edea4b3deb5b4b0885026b976e09246f6559dd9d`。原版动态差分因完整Group-A/Group-B actor backing、`+0x2AA8`异常字段页、RET异常栈页及三处caller联合捕获后端缺失而保留`blocked_runtime_oracle`。下一条为`audit_order=313 / 0x00478B40 / sub_478B40`。

本轮再完成`audit_order=313`的`0x00478B40`战斗角色目标选择latch查询函数。完整LST范围`0x00478B40..0x00478B46`共2条指令，只把actor `+0x2AA8`完整dword读入EAX后普通返回。typed实现复用Workpack 312的Group-A runtime-reset与Group-B lifecycle canonical owner、view和resolver，保留字段读、RET读、ESP/EIP、两个真实停止点及RET失败时已提交EAX的部分结果；Group-A frame三处和Group-B frame一处物理CALL全部直接接入，没有延期CALL。`0x00456F12`保留TEST flags穿过MOV驱动JNZ，其他三处保留完整dword `CMP 1`；Group-B typed-stop保留action-target clear前缀。验证：定向测试、AddressSanitizer、Linux core 199/199、Linux app 205/205全部通过；连续10轮完整core、全量/changed-range格式、零源码warning/测试失败/sanitizer finding、inventory双生成和release审计亦通过，未启动原版或OpenSWD3游戏程序。工作包为`313/422 = 303 platform_adapted + 10 assembly_exact + 109 pending_audit`；inventory SHA-256为`2a55f9cf35d6a4c01024b6e95ddc4c2344a9f4e7e9fd34eece7275514894f879`。原版动态差分因完整Group-A/Group-B actor backing、`+0x2AA8`异常字段页、RET异常栈页及四处caller联合捕获后端缺失而保留`blocked_runtime_oracle`。下一条为`audit_order=314 / 0x00478B50 / sub_478B50`。

本轮再完成`audit_order=314`的`0x00478B50`战斗角色启动门latch查询函数。完整LST范围`0x00478B50..0x00478B56`共2条指令，只把actor `+0x2AE0`完整dword读入EAX后普通返回。typed实现复用Group-A action-execution与Group-B startup/lifecycle canonical `start_gate_latch` owner，保留字段读、RET读、ESP/EIP、两个真实停止点及RET失败时已提交EAX的部分结果；已关闭Group-B frame中的唯一物理CALL `0x00457842 -> 0x00457847`直接接入，没有延期CALL。caller保留blocked查询EAX/EDX和比较flags，返回后只在完整latch值精确等于1时跳过候选后缀，其他完整dword继续回合完成查询；typed-stop保留前缀并抑制全部后缀。验证：定向测试、AddressSanitizer/UBSan 199/199、Linux core 199/199、Linux app 205/205全部通过；连续10轮完整core、全量/changed-range格式、零源码warning/测试失败/sanitizer finding/runtime error、inventory双生成和完整release审计亦通过，未启动原版或OpenSWD3游戏程序。工作包为`314/422 = 304 platform_adapted + 10 assembly_exact + 108 pending_audit`；inventory SHA-256为`350003b628090d8f7cc4d22b130ee5cd6d9326f2649f4ba5cc27e768f8f29b0a`。原版动态差分因完整Group-A/Group-B actor backing、`+0x2AE0`异常字段页、RET异常栈页及唯一caller联合捕获后端缺失而保留`blocked_runtime_oracle`。下一条为`audit_order=315 / 0x00478B60 / sub_478B60`。

本轮再完成`audit_order=315`的`0x00478B60`战斗角色行动表现主流程及Group-A、Group-B两个真实caller。完整LST范围`0x00478B60..0x00479849`共805条指令、31个静态CALL、119个跳转和六个`retn 4`；typed实现复用并扩展runtime-reset actor image到`0x2B1C`，绑定既有Group-A action/startup与Group-B lifecycle canonical owner，保留入口effect写、三种坐标mode、全部gate/action/snapshot/draw/decimal/sample/第二与附加记录分支、六个返回出口和逐访问partial commit。31个CALL逐项保留物理身份，三个closed child直接组合，其余精确延期，机械审计`31/31`零差异；两个caller直接接入并覆盖effect 0/1、trace merge和typed-stop后缀抑制。验证：定向测试、Linux core`199/199`、Linux app`205/205`、AddressSanitizer/UBSan`199/199`、连续十轮core、全量/changed-range格式、零warning/测试失败/sanitizer finding/runtime error、inventory双生成和release审计全部通过，未启动原版或OpenSWD3游戏程序。工作包为`315/422 = 305 platform_adapted + 10 assembly_exact + 107 pending_audit`；inventory SHA-256为`8f70033463b588e8a32cfdea2784c7b3142872a27a64d49bedcf51c8696858c6`。原版动态差分因完整actor backing、异常字段/nested-record/RET栈页及caller/callee联合捕获后端缺失而保留`blocked_runtime_oracle`。下一条为`audit_order=316 / 0x00479850 / sub_479850`；按用户要求，Workpack 315发布后暂停，不进入Workpack 316。
