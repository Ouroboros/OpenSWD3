...
优先级：高于 [`execution-plan.md`](execution-plan.md) 的当前执行队列

## 1. 与原 PLAN 的关系

本文件由 [`execution-plan.md`](execution-plan.md) 挂载并优先执行，不替代原 PLAN。
...
[`execution-plan.md`](execution-plan.md) 的后续模块队列。
