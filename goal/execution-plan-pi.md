...
本副本在原执行 GOAL 上增加一层 Pi 协作约束。项目目标、阶段顺序、真值来源、完成条件、
提交规则和其他技术决定仍以本文后续章节为准；本节只规定如何由主 Agent 主导并把有界工作
交给使用干净 Context 的子 Agent。

### 主 Agent 是唯一总负责人

- 主 Agent 独占任务编排、范围控制、阶段推进、设计裁决、完成判定和最终交付权；子 Agent
  只在授权边界内提供侦察、实现、审查或验证结果，不自行改变 GOAL、阶段或优先级。
- 主 Agent 必须先读取当前 GOAL、`HANDOFF.md`、仓库约束和本阶段权威证据，亲自掌握关键
  文件、真实执行断点、验证入口及工作区状态；不能把子 Agent 摘要当作唯一事实来源。
- 主 Agent 把当前阶段拆成可独立验收的最小工作包，为每个工作包定义输入、输出、非目标、
  写权限、停止线和验证合同；任何跨模块扩展、架构变化或用户权限决定都必须回到主 Agent。
- 主 Agent 负责综合相互冲突的子 Agent 结论，逐项决定采纳、修正、延期或拒绝；子 Agent
  的报告和测试结果都只是证据，不能自动升级为完成结论。
- 主 Agent 必须亲自检查最终 diff、关键证据和验证结果，并负责阶段状态、文档、精确暂存、
  `$commit` Skill 提交及推送。未达到本文原有停止线时不得因子 Agent 声称完成而提交。

### 子 Agent 一律使用干净 Context

- 每次子 Agent 运行必须显式使用 `context: "fresh"`；禁止依赖主 Agent 对话历史、使用
  `context: "fork"`，或让旧会话中的假设隐式进入当前工作包。
- 主 Agent 给出的任务提示必须自包含，至少写明：目标仓库与 `cwd`、当前分支/基线、工作包
  目标、应读取的计划与证据路径、允许修改的范围、禁止事项、完成条件、验证命令、输出格式
  以及遇到未批准决定时的停止/上报规则。
- 干净 Context 不等于脱离仓库事实。子 Agent 必须直接读取任务指定的源码、LST、测试、
  文档和当前 diff，独立复核输入，不得仅复述主 Agent 提供的摘要。
- 子 Agent 不得再启动或编排其他子 Agent，不得自行提交、推送、启动 EXE、扩大阶段范围，
  也不得替主 Agent作产品、架构、兼容例外或发布决定；遇到这些情况立即停止并上报。
- 子 Agent 的交接必须列出：实际读取的关键证据、修改文件、完成内容、未完成内容、执行过的
  命令及退出码、测试/验证证据、发现的风险，以及需要主 Agent裁决的问题。

### 单写者与并行规则

- 同一 checkout/worktree 同一时间只允许一个写者。默认由一个 fresh-context `worker` 承担
  当前工作包的全部源码、测试和文档修改；它运行时主 Agent 和其他子 Agent 不并行编辑该
  工作区。
- 侦察、证据核对、测试设计、审查和验证可以并行，但默认只读，并为每个子 Agent 分配互不
  重复的观察角度和明确输出。
- 只有主 Agent 事先划定互不重叠的文件与接口边界，并为每个写者创建独立 worktree 时，才
  能并行写入。各 worktree 的结果必须由主 Agent 分别审查、验证并串行集成。
- 如果主 Agent 亲自承担写入，则该时段不得另起写者子 Agent；如果子 Agent 已成为写者，主
  Agent 只进行不触碰该工作区的分析、验证准备和后续任务设计。

### 每个工作包的固定闭环

```text
主 Agent 锁定当前阶段、基线与验证合同
→ fresh-context scout/reviewer 独立读取仓库和权威证据，必要时并行侦察
→ 主 Agent 综合证据、消除冲突并批准唯一实施范围
→ 一个 fresh-context worker 作为唯一写者实施并提交完整交接
→ fresh-context reviewer/validator 从实际 diff 独立审查正确性、测试与简单性
→ 主 Agent 逐项处置发现；需要修正时只派一个 fresh-context fix worker
→ 发生实质修改则再次 fresh-context 审查
→ 主 Agent 亲自检查最终 diff，执行或确认本阶段要求的验证
→ 达到原 GOAL 停止线后更新状态，并按原规则提交、推送
```

审查默认至少覆盖三个互不重叠的角度：汇编/行为正确性与回归、测试与验证充分性、实现简单
性与可维护性。涉及安全、性能、平台、真实资产、用户流程或大范围改动时，由主 Agent 增加
对应的 fresh-context reviewer/validator。最多连续进行三轮“审查 → 单写者修正”；仍有阻断
项、范围争议或未批准决定时停止并向用户汇报，不能为追求形式上的闭环而无限循环。

### Pi 调度约定

- 执行前先列出并确认当前可用且未禁用的 Agent，只使用满足任务权限和工具要求的角色。
- 所有子 Agent 调用统一由主 Agent 通过 `workflowScript` 发起；单任务使用一个
  `runs.run(...)`，并行只读任务使用具有稳定且不同 key 的 `runs.all(...)`。
- 子 Agent 默认异步运行。主 Agent 在结果依赖点之前可以继续做无冲突的本地分析和验证准备，
  但不得轮询等待，也不得在写者工作区并行修改。
- 每次调用均显式设置 `context: "fresh"`。示意：

```text
subagent({
  workflowScript: "return runs.run('work-package', { agent: 'worker', task: '<自包含任务合同>' })",
  context: "fresh",
  async: true
})
```

- 子 Agent 完成后，主 Agent 必须消费其交接并检查仓库实物；异步运行完成通知、输出 artifact、
  CI、审查报告和命令退出码均是证据，不是自动验收权。
...
剩余风险或阻塞项以及下一步。汇报必须基于主 Agent 已检查的仓库实物和验证证据；子 Agent 的
完成声明不能直接作为汇报内容。命令执行失败时记录错误并明确告知用户，不得伪称已经送达。
