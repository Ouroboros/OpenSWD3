...
- 用户实际验证120 FPS后要求普通世界运动真正平滑。新增
    ...
    直接snap，绝不外推。插值时间原点已从world帧渲染完成时刻前移到accepted逻辑帧开始，原帧
    渲染耗时计入35/70ms区间，避免下一tick前残留尾段造成周期跳变。用户决定当前仅采用场景级
    插值，原生高频逻辑模拟留待整个执行计划完成后另行评估。定向UT覆盖配置开关、60/120/240
...
用户实际反馈当前世界运动插值有明显拖拉感。按用户要求，模块9在`16/227`暂停；恢复时下一
单元为`0x0043BB40`。在用户明确确认插值满意前，不启动任何其他模块工作。

下一工作包：定位并修正世界运动插值拖拉感，保持35/70ms逻辑时钟和可选配置不变。
