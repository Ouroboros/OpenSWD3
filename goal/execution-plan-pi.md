...
当前步骤：模块10 · Workpack 307 LST 审计与最终 REVIEW
...
- B7世界地图已有限收口，B8剧情VM已完成P1–P3验收，B9特殊模式227/227已关闭；B10战斗函数 inventory 已关闭至 `306/422 = 296 platform_adapted + 10 assembly_exact + 116 pending_audit`，当前执行 Workpack 307，存档业务字段由B11最终验收。
...
16. `[>]` B10：战斗状态机、AI与数值系统进行中；完整队列见[`battle-function-workpack.tsv`](../analysis/04-reverse-engineering/inventory/battle-function-workpack.tsv)。当前已关闭至 Workpack 306；当前只执行 `audit_order=307 / 0x00478AA0 / sub_478AA0`。
...
当前只执行B10，不并行展开B11。当前只执行 `audit_order=307 / 0x00478AA0 / sub_478AA0`；该工作包关闭后，才允许从 inventory 重新读取下一条 `pending_audit`。
...
当前工作包：`audit_order=307 / 0x00478AA0 / sub_478AA0`。

当前断点：Workpack 306 已完成最终 REVIEW 并关闭；Workpack 307 尚未开始生产修改。必须先从完整 LST 锁定函数物理边界、ABI、全部 caller、callee、共享状态和测试向量，再审计现有实现边界，不得从名称或相邻工作包猜测行为。

#### Workpack 307 REVIEW 划分

Workpack 307 采用一个最终 REVIEW 单元。该单元必须同时覆盖目标函数完整 LST 语义、全部作用域内 caller 与 callee、typed owner、生产接入、测试、唯一 evidence、inventory、模块文档和发布门禁；在该 REVIEW 通过前不得提交部分实现或前移游标。若完整 LST 证明存在多个能够独立实现、验证和回退的生产切片，必须在生产修改前原位修正本节并重新 REVIEW。
...
1. 从完整 LST 独立锁定 Workpack 307 的函数边界、ABI、基本块、callee、caller、共享状态和全部出口。
