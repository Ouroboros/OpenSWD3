...
本轮再完成`audit_order=92`的`0x0045DEE0`战斗调试叠加显示。完整权威LST主体`0x0045DEE0..0x0045E576`从proc到endp共736行、460条实际指令、45个call、15个分支、15个局部标签且无外部FUNCTION CHUNK。typed实现恢复显示总门、字体前后生命周期、两组角色生命/锁定/命令/等级、攻击/等待/选择三组动态顺序、当前角色、十项固定状态文字、255字节格式缓冲与全部CP950字面值；15次`wsprintfA`以等价十进制格式直接实现，18次文字绘制与12次字体/角色查询保留窄端口。生命输出未初始化栈局部只接收一次入口snapshot，callee写回后成为后续角色陈旧值；两组数量与三类顺序上界均在callee后动态重读，不加现代上限。组B双行标记从唯一framebuffer几何读取signed `Rect.right`，按低32位乘法、u16宽度和每列上行后下行顺序直接写`0xEEEE`；第一行成功、第二行越界时保留第一行。三类顺序分别在第19/11/19次真实读取处typed-stop，frame divisor零在最后signed `idiv`处停止且不执行字体恢复。角色数量、优先索引、启动记录、最终角色当前/发布值/顺序/预帧门/frame gate、共享message state、效果反馈、调试开关和framebuffer全部复用既有owner；新增叠加调用门与其余显示源按全局重置原写集合同步。唯一逐帧caller删除条件反向的opaque边界，改为门精确等于1时直连；子typed-stop阻断两个后续stage、颜色、surface和截图。定向测试`1/1`、独立AddressSanitizer `1/1`、Linux core `188/188`与Linux app `194/194`全部通过；battle聚合源码零warning，SDL上游缺少ALSA开发库的环境提示仍按项目既定策略保留。工作包正式推进到`92/422 = 87 platform_adapted + 5 assembly_exact + 330 pending_audit`；连续双跑逐字节一致，SHA256为`a5ae5e3fcb3f0fcce650c6d1087cdb7bf523813fb70d734256d424cc21a5c20f`。原版字体/文字、两组角色对象、生命查询、共享格式缓冲、缓存计数、完整叠加全局、framebuffer与寄存器联合捕获后端缺失，动态差分登记为`blocked_runtime_oracle`。
