...
当前步骤：模块10 · Workpack 313 已关闭；按用户要求暂停于 Workpack 314 前
...
- B7世界地图已有限收口，B8剧情VM已完成P1–P3验收，B9特殊模式227/227已关闭；B10战斗函数 inventory 已关闭至 `313/422 = 303 platform_adapted + 10 assembly_exact + 109 pending_audit`，下一游标为 Workpack 314，存档业务字段由B11最终验收。
...
16. `[>]` B10：战斗状态机、AI与数值系统进行中；完整队列见[`battle-function-workpack.tsv`](../analysis/04-reverse-engineering/inventory/battle-function-workpack.tsv)。当前已关闭至 Workpack 313；下一项为 `audit_order=314 / 0x00478B50 / sub_478B50`，按用户要求在本次提交发布后暂停。
...
当前只执行B10，不并行展开B11。Workpack 313 的生产实现、测试、证据、inventory 和最终门禁已经完成；下一游标为 `audit_order=314 / 0x00478B50 / sub_478B50`，但在 Workpack 313 提交、push 和阶段通知后按用户要求暂停，不开始 Workpack 314。
...
当前工作包：`audit_order=313 / 0x00478B40 / sub_478B40`，单一最终 REVIEW 已完成。

当前断点：inventory 为 `313/422 = 303 platform_adapted + 10 assembly_exact + 109 pending_audit`，SHA-256 为 `2a55f9cf35d6a4c01024b6e95ddc4c2344a9f4e7e9fd34eece7275514894f879`。Workpack 313 的 leaf、四处已关闭父 CALL、测试、evidence、双向 REVIEW、完整 Linux 门禁和 inventory 双生成已收敛。下一游标是 `audit_order=314 / 0x00478B50 / sub_478B50`；按用户要求，本次提交、push 和阶段通知完成后立即暂停，不建立 Workpack 314 REVIEW 计划。

#### Workpack 313 单一最终 REVIEW

1. `[x]` 从完整 LST 独立锁定目标函数范围、ABI、基本块、状态读写、调用者、全部出口和测试向量，并形成唯一 evidence。
2. `[x]` 审计 canonical owner、父调用边界和测试，锁定 typed 接口、停止点、caller 接入及无延期范围。
3. `[x]` 最小实现目标函数与四处已关闭父 CALL，补齐汇编独立测试，并清除 production raw getter 调用。
4. `[x]` 完成汇编—C++ 双向收敛、定向测试、Linux core/app、AddressSanitizer、十轮 core、格式与静态门禁、inventory 双生成和模块文档同步。
5. `[>]` 完成精确 staging、`$commit` Skill 提交、push 和固定五段 Telegram；随后按用户要求暂停。
