...
public:
  ...
  virtual void emit_role_particles(compat::i32 world_x, compat::i32 world_y,
                                   compat::u16 guid) noexcept = 0;
  [[nodiscard]] virtual std::span<const compat::u8>
  resolve_label_bytes(compat::u32 token) noexcept = 0;
  [[nodiscard]] virtual compat::u16 label_color(compat::u32 index) noexcept = 0;
  virtual void draw_label(std::span<const compat::u8> bytes, compat::i32 x,
                          compat::i32 y, compat::u16 color,
                          compat::u32 style) noexcept = 0;
};

class LegacyWorldRoleRenderPorts : public LegacyWorldRoleExternalPorts {
public:
  [[nodiscard]] virtual bool load_frame(compat::u16 resource_id,
                                        compat::u16 frame_index,
                                        LegacyWorldRoleFrame &frame) = 0;
  [[nodiscard]] virtual rendering::LegacyBlitExecutionStatus
  draw_frame(const LegacyWorldRoleFrame &frame,
             const LegacyWorldRoleBlitRequest &request,
             rendering::LegacyRleRowJitterState &jitter) noexcept = 0;
};
...
public:
  ...
  void play_positional_sample(compat::u16 sound_id, compat::i32 world_x,
                              compat::i32 world_y) noexcept override;
  [[nodiscard]] bool load_frame(compat::u16 resource_id,
                                compat::u16 frame_index,
                                LegacyWorldRoleFrame &frame) override;
  [[nodiscard]] rendering::LegacyBlitExecutionStatus
  draw_frame(const LegacyWorldRoleFrame &frame,
             const LegacyWorldRoleBlitRequest &request,
             rendering::LegacyRleRowJitterState &jitter) noexcept override;
  [[nodiscard]] const asset_runtime::LegacyActionRecord *
  resolve_overlay_action(compat::u32 token) noexcept override;
  void emit_role_particles(compat::i32 world_x, compat::i32 world_y,
                           compat::u16 guid) noexcept override;
  ...
  void draw_label(std::span<const compat::u8> bytes, compat::i32 x,
                  compat::i32 y, compat::u16 color,
                  compat::u32 style) noexcept override;
...
[[nodiscard]] LegacyWorldRoleDrawResult
...
struct LegacyWorldRolesResult {
  LegacyWorldRolesStatus status{LegacyWorldRolesStatus::invalid_spatial_index};
...
[[nodiscard]] LegacyWorldRolesResult
