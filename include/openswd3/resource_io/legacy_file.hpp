...
[[nodiscard]] bool legacy_exclusive_file_probe(
    const std::filesystem::path& path, std::span<char> error_buffer = {}
) noexcept;
...
public:
    ...
    map_view(compat::u32 offset = 0U, compat::u32 size = 0U) noexcept;
    ...
    seek_current_one_based(compat::i32 distance) noexcept;
    [[nodiscard]] compat::u32
    seek_begin_one_based(compat::i32 distance) noexcept;
    [[nodiscard]] compat::u32 seek_end_one_based(compat::i32 distance) noexcept;
    [[nodiscard]] bool current_position(compat::u32& position) noexcept;

    [[nodiscard]] bool write_exact(std::span<const compat::u8> buffer) noexcept;
...
private:
    ...
    [[nodiscard]] compat::u32
    seek_raw(SeekOrigin origin, compat::i32 distance) noexcept;
