...
EXPECTED_SNIPPETS = {
    ...
    0x00436B73: "cmp ecx, edx",
    0x00436B8D: "sub ecx, eax",
    0x00436B93: "add ecx, 2",
    0x00436B9F: "call sub_436EA0",
    0x00436BB4: "mov edx, [esi+0FE0h]",
    0x00436C05: "sub eax, edx",
    0x00436C07: "add ebp, eax",
    0x00436EEF: "cmp ebx, edx",
    0x00436F13: "mov [ebp+esi*2+0], di",
    0x00436F25: "cmp eax, edx",
}
...
def pipeline_rows(face: str) -> list[PipelineRow]:
    return [
        ...
        PipelineRow("character_parse", "0x00436B0A-0x00436B45", "byte<0x80 -> one byte plus zero; byte>=0x80 -> blindly consume the next byte, including the NUL after a dangling final lead; key is the little-endian raw u16", "this is a CP950-era byte protocol, not UTF-8 and not validated Big5"),
        PipelineRow("cache_lookup_insert", "0x00436980-0x00436AC2,0x00436B45-0x00436E70", "unsigned lookup; on miss insert/shift and clear slot, rasterize mask, fill background, run the selected writer, advance, then increment/cap count; hits skip rasterization and count mutation", "preserve call order and the 1999->1998 clear-after-draw behavior; do not replace it with an unbounded map"),
        PipelineRow("background_fill", "0x00436B65-0x00436BD7,0x00436CF7-0x00436D62,0x00436EA0-0x00436F66", "unless color==0xFFFE, fill before glyph pixels; non-last width=FE0, last width=font_width-ASCII_reduction+2; vertical-empty rectangles still write the clamped first row", "fill uses framebuffer bounds rather than glyph clip; a dangling final high lead consumes NUL and misses the last-character width branch"),
