...
3. 再写`+0x48=FFFFFFFF`。
4. slot7/8：读取seed `text_index`；null seed在此停止。非`FFDC`时在原位置直接组合已关闭`0x00477B40`，先扫描固定根`0x004B8A00`，再读取并清理MON定义，按`count | maximum<<16`写`+0x48`。
5. 再写`+0x4C=FFFFFFFF`。
...
- slot0固定曲线、slot7/8定义上限/固定数量查询或slot9/10固定计数链在真实访问点停止：在原调用位置停止，保留此前sentinel。

slot0、slot7/8与slot9/10分别直连已关闭`0x004779F0`、`0x00477B40`和`0x00477800`，共同复用唯一`LegacyBattleFixedObjectStatePort`；slot7/8另复用唯一`LegacyBattleMonDatabasePort`，旧pair query端口已经删除。不再传seed pointer或destination给opaque summary。
...
UT覆盖slot0固定曲线根命中及caller ECX高字/EDX token、slot7真实MON definition与固定根命中后packed `5678:1234`、slot9固定数量链根命中、无关slot、FFDC residual、三种null seed的sentinel前缀、三类固定链typed-stop和目标越界，并重验429B0全部owner及旧opaque边界零调用。定向测试通过。
