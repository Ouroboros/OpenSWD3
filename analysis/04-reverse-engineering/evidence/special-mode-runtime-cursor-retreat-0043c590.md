...
`0x0043BBC0`的指针/整数联合EAX被后续调用覆盖，不是组合器最终返回。未关闭callee继续由与`0x0043C520`共享的窄port隔离。

## 3. typed边界与caller回接

`retreat_legacy_standard_mode_runtime_cursor`复用已关闭的window retreat helper。selected index只在原`[entry_base + index*4]`读取点检查64项边界；停止时保留cursor后退、alias重建和page刷新，不执行消费、flags或sample。
