...
44A050删除`commit_transition_pair` opaque端口，在原flags bit4位置直接调用本typed helper；caller继续只计一次44A250调用并截取最终返回低字节。回调前释放、阶段写回和active owner清理均已完成，不在回调失败时回滚。
