...
函数以组B对象查询指标来源，再把来源返回EAX作为键，在原call位置直接查询共享固定键计数链。固定计数返回低word小于10时直接返回；否则查询当前值和上限，并以CP950固定前缀“生命:”及signed `%d/%d`写20-byte局部缓冲。固定链typed-stop阻断全部指标后缀；若第20字节后的首次写发生，typed实现保留此前字节并stop，不扩大栈缓冲或截断成功。
