...
效果阶段先从目标窄port取得双坐标，再按原顺序计算`positionX-offset+field76`与`positionY+field78-drawOffsetY`，随后递增`+0x2F26`word。待审效果更新callee接收目标token、`+0x630`唯一动作记录、零、运行word、两坐标、signed sourceY和一；返回非一时保留递增tick与两记录。
