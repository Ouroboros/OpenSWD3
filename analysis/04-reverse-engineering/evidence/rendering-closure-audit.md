...
- 78 项已有独立实现映射，16 项是已实现函数内部由跳转表选择的物理 PROC 分支；
...
五个 command-stream 函数、三项绘制 helper、暂停层和四项动作/消息协调器已经完成。
剩余 14 项按调用依赖合并为以下紧密行为
单元：

1. `0x00417050/0x004174D0..0x00417650/0x00417DE0`：描边包装、四个固定
   `16×16` writer 和 packed-row 效果；
2. `0x00420490..0x004207E0/0x00421FB0`：已经有静态规格、尚未转写的六个
   整帧颜色函数；
3. `0x00422C70/0x00423020`：两条 10.10 缩放 RLE writer。
...
下一组是 `0x00417050/0x004174D0..0x00417650/0x00417DE0`。其中
`0x00417DE0` 已作为本轮 packed-row 协调器的唯一底层端口暴露；按完整 LST 收口六个
软件 helper 后立即写 UT 与 C++20，不等待其余八项重新调研。
