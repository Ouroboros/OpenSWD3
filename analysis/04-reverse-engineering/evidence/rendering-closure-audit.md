...
- 70 项已有独立实现映射，16 项是已实现函数内部由跳转表选择的物理 PROC 分支；
...
首组五个 command-stream 函数已经完成。剩余 22 项按调用依赖合并为以下紧密行为
单元：

1. `0x0040DE50/0x0040E080/0x004117F0`：边框、缩略图和数字绘制 helper；
2. `0x00411FA0/0x00414B60..0x004153D0`：暂停及四组借用剧情状态的动作绘制；
3. `0x00417050/0x004174D0..0x00417650/0x00417DE0`：描边包装、四个固定
   `16×16` writer 和 packed-row 效果；
4. `0x00420490..0x004207E0/0x00421FB0`：已经有静态规格、尚未转写的六个
   整帧颜色函数；
5. `0x00422C70/0x00423020`：两条 10.10 缩放 RLE writer。
...
下一组是 `0x0040DE50/0x0040E080/0x004117F0`。按完整 LST 固定输入、状态读取、
调用顺序和像素结果后立即写 UT 与 C++20，不等待其余 19 项重新调研。
