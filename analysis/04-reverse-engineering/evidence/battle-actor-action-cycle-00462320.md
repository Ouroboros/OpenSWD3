...
三个静态caller均属于已关闭逐帧输入分派，地址分别位于记录2确认、记录4菜单后退和记录5菜单前进路径。两个callee尚未关闭：第一类按起点轮转到可用动作，第二类提交其返回动作；当前仍以typed port保留完整调用边界。
...
第一次callee的完整EAX/ECX/EDX成为第二次调用的预调用寄存器；第二次callee的唯一参数为第一次返回EAX。函数最后完整返回第二次callee的EAX/ECX/EDX。栈清理由caller执行且不改寄存器。

## 4. caller回收与陈旧值

`LegacyBattleInputDispatchCall`原动作确认槽保留相同数值并改为reserved，三个caller全部直接组合typed动作轮转：

- 记录2在写option word和action kind后调用；message为1时再进入已关闭目标选择，最后才把option word恢复全1；
- 记录4先完成已关闭菜单后退，动作轮转普通返回后才发布菜单动作1；菜单后退typed-stop会阻断动作轮转和尾写；
- 记录5在对话为空时先执行右向opaque动作，再进入动作轮转。
...
定向测试覆盖code 7与12默认返回、code 8..11四项起点、两级callee参数与完整寄存器传递、记录2短按和长按陈旧3、记录4普通直连与前置typed-stop阻断、记录5右向调用后的直连，以及reserved槽零调用。

当前缺少原版当前角色、两个callee内部动作表与角色队列共享副作用、三个caller输入记录及EAX/ECX/EDX联合捕获后端，`original_diff_verified`为`blocked_runtime_oracle`。
