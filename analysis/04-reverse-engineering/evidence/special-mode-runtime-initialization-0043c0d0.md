...
- 以mode0直接调用已关闭`0x0043C9C0`：执行signed classification与status两轮1..500扫描、文本/token生命周期，并由其清window/cursor/alias和刷新page。
...
- 无匹配classification时真实entry表全0、entry[0]消费顺序、mode flags最终清0和consumer EAX返回。
