...
入口固定先发送`0x1009`清空列表。现代直接复用已关闭的队伍字段getter `0x004112B0`和旧行替换`0x00410490`；三类战斗派生值查询继续由窄B10端口表达。
...
每次flags匹配都先把按位与结果的bit15清掉再与原mask精确比较；mask本身不清bit15。查询不可用时停在当前调用点，不绘制该行。
...
`special_modes.legacy_initial_menu`覆盖17字段页的原文字和signed/unsigned值、64整数页特殊名称、page0两节点查询覆盖与低ID最终覆盖、page1空附加值、invalid member page、链自环、首查询不可用和清空消息不可用前缀。
