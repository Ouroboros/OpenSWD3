...
入口固定先发送`0x1009`清空列表。现代直接复用已关闭的队伍字段getter `0x004112B0`和旧行替换`0x00410490`；第一类派生值在原位置直接组合已关闭固定键曲线查询`0x004779F0`，第二类直接组合已关闭定义上限与固定数量查询`0x00477B40`，第三类两个物理站点直接组合已关闭固定键计数查询`0x00477800`。
...
2. 匹配第二mask：先扫描固定根`0x004B8A00`，再读取并清理MON定义；definition `+0x44` maximum作分母，命中记录`+0x06` count作附加值，缺键count为零。
3. 匹配第三mask：第三查询，分母`-1`。
4. ID非零且无符号`<=500`：无条件再次执行第三查询，覆盖此前结果，分母`-1`。

每次flags匹配都先把按位与结果的bit15清掉再与原mask精确比较；mask本身不清bit15。第一类固定曲线、第二类定义上限/固定数量查询或第三类固定数量链在原访问点停止时均不绘制当前行；低ID最终覆盖仍会按LST执行第二次固定计数查询。
...
`special_modes.legacy_initial_menu`覆盖17字段页的原文字和signed/unsigned值、64整数页特殊名称、page0三类固定链直连、第二类两次MON定义读取、两节点查询覆盖与低ID最终覆盖、三类固定链typed-stop前缀、page1空附加值、invalid member page、链自环和清空消息不可用前缀。
