...
入口固定先发送`0x1009`清空列表。现代直接复用已关闭的队伍字段getter `0x004112B0`和旧行替换`0x00410490`；前两类战斗派生值查询继续由窄B10端口表达，第三类两个物理站点直接组合已关闭固定键计数查询`0x00477800`。
...
1. raw `+0x20` flags匹配第一mask：第一查询，分母0。
2. 匹配第二mask：pair查询，返回第一项作分母、第二项作附加值。
3. 匹配第三mask：第三查询，分母`-1`。
4. ID非零且无符号`<=500`：无条件再次执行第三查询，覆盖此前结果，分母`-1`。

每次flags匹配都先把按位与结果的bit15清掉再与原mask精确比较；mask本身不清bit15。前两类query不可用或第三类固定链访问typed-stop时均停在当前调用点，不绘制该行；低ID最终覆盖仍会按LST执行第二次固定计数查询。
...
`special_modes.legacy_initial_menu`覆盖17字段页的原文字和signed/unsigned值、64整数页特殊名称、page0第三类两个物理站点的固定计数直连、两节点查询覆盖与低ID最终覆盖、固定链typed-stop前缀、page1空附加值、invalid member page、链自环、首查询不可用和清空消息不可用前缀。
