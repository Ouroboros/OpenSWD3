...
只有随机值`27..32`进入。逐队员：mode返回1或actor特殊字段等于1则跳过；否则执行准备与reset，并扫描十槽数组，把第一个零槽写为`party_index+8`。数组已满则不写。随后逐敌人执行消息准备与刷新，并以另一固定文本token和相同参数直连已关闭文字消息入链；两个C++互斥分支对应同一汇编共享callsite，单次执行只追加一个节点。
