...
函数接收40字节输出资料和16位资料编号。它共享两个原版全局：MON.DAT惰性文件句柄与临时1024字节读取块。下一项`0x00476DB0`也使用这两个全局；本轮只关闭当前函数，不提前实现下一项，也不提前关闭共享文件会话。
...
临时1024字节块只在首tag非零或正常tag 5路径释放一次。open失败和分配零没有块可释放；访问typed-stop不提前释放。文件句柄属于跨调用唯一会话owner，当前函数不关闭它。
