...
### 工作包282：查询字段不能按源成员名推导

`0x00475739`把源`+0x16/+0x18`复制到角色`+0x0D66/+0x0D68`。
`0x0047574C`再复制到`+0x0D86/+0x0D88`。
已审计的坐标查询`0x004783B0`正是读取这些角色偏移，不能改读`+0x0D68/+0x0D6A`。
因此既有源类型中的`reserved_16`进入查询X，`position_x`进入查询Y；成员名称不改变字节事实。

本轮在两次原复制处同步既有`action_execution`运行期坐标字段，删除startup调用前按成员名复制坐标的临时接线。
源记录仍属于独立的固定全局记录表，不当成角色坐标查询的实时owner。
资源加载停止时，主、备用两组坐标都已复制，查询必须看见该前缀。
增加了源`+0x16=0x4567/+0x18=0x2345/+0x1A=0x3456`的独立测试，明确排除错用后两个字段。
本工作包验证尚未完成，下面既有验证记录不替代本轮门禁。
