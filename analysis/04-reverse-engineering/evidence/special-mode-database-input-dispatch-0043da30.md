...
唯一行为真值为`swd3.exe.lst`。函数范围`0x0043DA30..0x0043DD1F`，365行；由B480 callback表间接绑定，没有direct call caller。直接callee包括已关闭C090，以及尚未独立关闭的`DD20/DDF0/DED0/DFA0/E080/E170/E310/E3D0/E770`与外部物品查询。
...
未关闭callee用地址枚举port表达；已关闭DD20/DDF0直接回接typed helper。port同时取得state与input引用，使DA30能保留callee后重读全局X、button和toggle的顺序。C090直接回接typed availability helper；物品`0x1BA9`查询保留最小port。
...
上面板矩形为`x=41..414,y=13..291`。物品`0x1BA9`存在、runtime flag bit0清且button bit0置位时先调用E080；随后独立检查button bit1：toggle非零再次调用E080，否则调用E3D0。因此双button会保留`E080→E080`或`E080→E3D0`的同帧双调用BUG。
...
定向UT覆盖页1..6上界、24像素索引及signed count边界、strict hover、C090 typed-stop、phase1两条三callee直连/重读路径、phase2两套双调用BUG、物品查询ID、phase3/4/5和未知phase。
