...
- mode 1/page 2将窗口起点加5。新起点signed小于总数时，先按signed起点把当前窗口指针从目录owner前移`2 * 起点`，再从目录直接重算visible count并夹scroll；否则回滚起点并无条件写`visible_count-1`，保留0下溢。最后只对cursor flags低字节OR 30，保留高24位。
...
44B070的下方hover caller已删除opaque command并直接调用本typed helper；其input-status调用仍计入外层helper count。UT覆盖末页回滚与visible underflow、有效页窗口指针与可见数量直算、scroll夹值、AL OR保高位、行和确认光标夹值、19项回绕、完整sample owner及caller直连。
