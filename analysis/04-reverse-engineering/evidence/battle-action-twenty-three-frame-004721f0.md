...
实现严格保留动作号取`+0x2A0C`、variant 43、special mode、动作更新、frame读取与共享源发布。完成位非零时在frame发布后清零整条记录并返回一；否则按`+0x2B08`和`field_1c`执行低位翻转、bit15 latch与宽度镜像。目标坐标仍经未审计`0x004783B0`窄port读取，随后保留16位回绕和signed解释。
