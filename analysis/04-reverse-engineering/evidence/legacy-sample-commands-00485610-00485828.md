...
`0x12C`和live mix level直连`0x00485610`播放；角色升级属性提交则先按单sample停止语义停止`0x12C`，再以`0x12B`和live mix level播放升级提示；角色成长对照面板每次成功递减两条串行动画队列时，以`0x125`和live mix level播放推进提示。其他入口作为剧情、世界和战斗后续模块的
