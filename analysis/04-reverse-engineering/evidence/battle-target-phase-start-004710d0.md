...
每个group-B opponent record新增唯一`LegacyBattleTargetPhaseState`，持有资源token、精确`0x58`字节typed演出记录、mode byte与三段清零区，不复制第二份物理状态。`0x00478620`、`0x00478470`、`0x004019A0`和`0x0047CE70`尚未审计，继续作为窄callee port；不保留整个`0x004710D0` opaque adapter。

## 4. caller回收与验证

唯一caller为action dispatch `0x004539B0`的action 6阶段零分支。production现在按group-B index定位typed opponent owner并直接调用target-phase初始化，失败映射到caller typed-stop；后续演出门、暗化、阶段word与刷新保持原顺序。
