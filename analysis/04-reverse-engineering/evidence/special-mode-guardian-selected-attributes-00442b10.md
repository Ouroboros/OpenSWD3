...
442BC0与44D6E0后续均已独立关闭并由B10直接调用。名称端口只解析typed护驾贡献；B10把scratch前42字节映射为21个u16后直接执行资源与战斗属性应用。模板、destination、record/name与贡献解析在原读取点typed-stop，BC0目标越界则保留此前scratch、destination clear与成功合并。
