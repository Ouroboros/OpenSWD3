...
生产实现位于`legacy_battle_actor_frame_snapshot.hpp/.cpp`。leaf在首个actor读取前显式反映原prologue：EAX已由`xor eax,eax`清零、ECX已载入`0x26`用于`rep stosd`、flags来自该XOR，只有EDX保留caller入口值；因此首读typed-stop不得伪装成caller EAX/ECX/flags直通。action updater与frame provider的现代接口不携带全部寄存器/flags，因此调用方另行提供两段callee的原ABI残值；provider EAX token按原时点提交到actor `+0x254C`，不再伪造成actor字段地址。typed request分别控制`+0x4C/+0x4A`两次重叠local dword读取、输出指针、四次输出写、两次frame-token重载和frame宽高访问；已提交输出通过32位地址token与actor frame-token字段发生别名。
...
frame input原`prepare_actor_origin`、菜单前进/后退两个原点准备槽、target runtime原`build_selection_snapshot`和selection frame原`build_actor_snapshot`均已保留枚举ordinal并改名为reserved，生产调用数全部为0。五个caller中的15个物理callsite现均在原位置直接组合typed leaf，不保留generic `0x004784A0`生产边界。
...
REVIEW 3已回收`0x00462740`两处和`0x00464270`四处并关闭工作包。目标刷新复用同一个四dword输出块，分别固定`EDX=输出token`与`EDX=0xBCD*code`的入口形状，并保留两条success suffix差异。选择帧四处查询在原位置直接组合typed leaf，复用同一个四dword snapshot：Group-B/Group-A遍历标记保留完成查询或共享输出token残值，当前Group-B/Group-A目标保留两套索引算术、reset前后顺序及render-offset调用形状。任一snapshot typed-stop保留逐槽部分提交和返回寄存器/flags，并抑制对应reset、目标发布、动作6可用性及prepared-action绘制后缀。

## 8. 测试与动态差分点

REVIEW 1 typed leaf测试已覆盖：prologue清零EAX、ECX=`0x26`与XOR flags的首读停点、两条早退、mode非1、默认/actor/强制anchor、mirror完整值1与其他值、负位置符号扩展、四项dword结果、updater和provider调用、`+0x4C/+0x4A`重叠dword参数及两处独立local读取停点、显式frame token提交、成功寄存器/flags、每类真实读取/写入停点、X/Y/width部分提交、两次frame token重载及X/width输出与frame-token别名。
...
目标刷新与选择帧测试已补齐最后六处生产路径：覆盖两条目标刷新snapshot入口寄存器、正常早退继续、typed-stop后缀抑制；覆盖Group-B遍历标记Y输出故障时仅X槽提交、Group-A遍历标记输出token装入EAX、当前Group-B `0x565/0x159`与当前Group-A `0xBCD/共享输出token`入口算术、共享四dword输出、snapshot中心、signed render offset、reset顺序、动作6可用性、prepared-action绘制和reserved槽零调用。
