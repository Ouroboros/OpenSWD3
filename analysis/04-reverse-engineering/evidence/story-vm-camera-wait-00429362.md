...
任一值非零就不推进指令位置并让出；四值全零才推进 2 字节并在同一次调用继续。两路都会经过共同出口，把归一化后的有效 opcode 51 发布到 `dword_4CF6D8`。没有音频、helper 调用、operand 读取或镜头状态写入。
...
因此 previous opcode 发布不是完成路径专属，也不能在等待返回前省略。全零路径设置 `ESI=1` 后同调用取下一条；等待路径保持零并让出。

## C++ 差异与修正

审计前 C++ 已正确检查四个字段、等待时不推进、完成时推进 2 字节并继续，但两条路径都漏掉了共同出口对 `previous_opcode` 的发布。修正后：

- 任一字段非零：先发布有效 opcode51，再返回 `yielded`；
- 四字段全零：先推进 2 字节，再发布有效 opcode51，并同调用继续；
...
- 在 `0x7FFE` 且任一字段非零时，保留 IP `0x7FFE`，发布 previous 后让出；
...
代表性真实回放使用 `TALK1.DAT@0x000046C2: 33 00`。该记录紧随同文件 `0x000046B8` 的绝对镜头移动初始化：非零移动状态时原地让出并发布 previous，状态归零后推进 2 字节、发布 previous并同调用继续。
...
LST→C++：四字段读取集合与顺序、任一非零等待、全零推进 2、`ESI` 续取语义、两路 previous 发布和无额外副作用均有对应。

C++→LST：没有只检查位移余量、清零步长、主动推进镜头、循环阻塞、额外音频、operand 读取或等待路径漏发 previous。新增 owner 检查只隔离原版不可安全表达的全局地址失效域。
