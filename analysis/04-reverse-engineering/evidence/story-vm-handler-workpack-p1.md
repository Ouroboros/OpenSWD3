...
状态：P1 scope lock 完成；初始 146 个 handler 全部为 `pending_audit`，不继承旧语义、C++ case、资产观察或 CFG 的完成状态。P2 当前进度 21/146。
...
因此 P1 没有把任何一行标为已实现。`all` C++ case presence 也不能关闭 handler：共享入口仍可能有不同 operand、修饰位、等待、异常或窗口路径，旧实现同样必须按所属组重审。P2 已通过独立证据关闭前二十一行；生成器现在硬断言 21 closed / 125 pending，并拒绝无 override 的状态漂移。
...
- lifetime 暂存入口 `0x00427E9A` 的 opcode8：[`story-vm-dialog-lifetime-00427e9a.md`](story-vm-dialog-lifetime-00427e9a.md)；
...
- global-bit set入口`0x0042865B`的opcode25：[`story-vm-global-bit-set-0042865b.md`](story-vm-global-bit-set-0042865b.md)；
- global-bit clear入口`0x00428679`的opcode26：[`story-vm-global-bit-clear-00428679.md`](story-vm-global-bit-clear-00428679.md)。
...
opcode27尚未独立审计；现有C++与导航语义均不继承完成状态。
