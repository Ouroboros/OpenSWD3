...
状态：P1 scope lock 完成；初始 146 个 handler 全部为 `pending_audit`，不继承旧语义、C++ case、资产观察或 CFG 的完成状态。P2 当前进度 12/146。
...
因此 P1 没有把任何一行标为已实现。`all` C++ case presence 也不能关闭 handler：共享入口仍可能有不同 operand、修饰位、等待、异常或窗口路径，旧实现同样必须按所属组重审。P2 已通过独立证据关闭前十二行；生成器现在硬断言 12 closed / 134 pending，并拒绝无 override 的状态漂移。
...
- role-path conditional jump 入口 `0x00428318` 的 opcode16：[`story-vm-role-path-conditional-jump-00428318.md`](story-vm-role-path-conditional-jump-00428318.md)。
