...
`0x0043BB80`的指针/整数联合EAX在本函数中立即被后续载入和call覆盖，不是`0x0043C520`最终返回。alias重建与page刷新已分别直接复用关闭的`0x0043CC00`、`0x0043CBD0`typed helper；未关闭的entry消费和sample播放继续由共享窄port隔离。
