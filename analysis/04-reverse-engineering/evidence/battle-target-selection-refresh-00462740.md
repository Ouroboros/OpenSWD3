...
message 7未到alternate limit时固定动作25并查询当前group-B对象；完成对象按target cursor循环查找下一个未完成对象，直到live group-B count。count大于物理八对象时在第九次真实对象call或第九项target map读取停止，不添加步进上限。到limit时提交动作99并清选择缓存。
