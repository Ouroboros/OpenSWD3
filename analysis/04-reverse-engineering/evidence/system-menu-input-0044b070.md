...
- hover未早退后只读取一次flags低字节。右键单击/双击优先返回；mode 7先打开模式14，再重读输入锁和页面执行共享返回。mode 10在低四位非零时提交。
...
- mode 2仅“离开”页按unsigned矩形和Y除66写“确定/放弃”光标，并在写入后检查confirm bit。
- 所有乘法除法保留x86 `mul`的EAX低积残值；未调用helper的路径不伪造业务返回。

UT覆盖入口锁、查询后snapshot、signed动态边界、mode7模式14调用后右键返回、mode5主条目与固定条目、提交后sample owner重读、page 4选择与两步EAX、负row停止、value clamp、service移除/重加、共享值反向发布、row 6 fallback和离开页确认光标写入。顺带按LST `mov dword`纠正44B010消息参数：tail/shared及后三字段均为完整32位值，不采用反编译`char`注释。
