...
全局重置`0x0045B630`写到的18槽主记录、八条精确`0x98`强度效果记录、两组模式、参数数组、计数器、反馈actor与活动latch已从未映射字节像回收，直接同步同一协调器状态；强度record0前28字节同时供已关闭攻击顺序移除作固定一过尾源；攻击顺序出队在18槽全部跳过后也以本数组为无界28字节扫描的相邻物理owner。重置程序未写的scan limit、delay和反馈数组保持不变。
