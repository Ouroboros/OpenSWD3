...
原光标 API 依赖宿主 ACP。当前 DVD 资产是 CP950；继续使用现代宿主 ACP 会让字节边界随系统语言变化。B2 已按 `CharNextExA(950, ...)` 探针固定兼容规则，B3 的 previous helper从同一规则和缓冲起点扫描得到前一边界。该替换标记为 `platform_adapted`。
