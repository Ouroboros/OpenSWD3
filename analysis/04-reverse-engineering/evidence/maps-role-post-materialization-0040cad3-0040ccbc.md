...
`sub_40D610` 还有供剧情 opcode 65 等运行时调用者使用的非整格坐标路径。该路径已经由 `world-role-transfer-0040d610-0040d785.md` 独立锁定并实现共享 owner。它在本调用点仍不可达：`0x0040CA72..0x0040CA87` 刚把 MAPS 的 word tile X/Y 左移四位写入角色，至 `0x0040CC28` 前没有任何坐标修改。剧情解释器尚未接线，不能由此反推 opcode 65 已完成。
...
- 非整格分支转交共享 `legacy_world_role_transfer` owner；
