...
权威LST主体为`0x00468ED0..0x00468FEC`，从proc到endp共128行、84条带机器码和真实助记符的实际指令、9个静态call、2个跳转、1个局部标签和1个返回点，没有外部`FUNCTION CHUNK`。9个callsite依次为`wsprintfA` 1次、`lstrlenA` 2次、固定矩形1次、九宫格1次、stage推进1次、字体大小设置2次和文字绘制1次。
...
九宫格正常返回后，EAX改为局部缓冲token并执行第二次`lstrlenA`；ECX/EDX继承九宫格返回。随后直连`base=212,target=244,divisor=3`的共享stage推进，signed商非零、返回值不等于1时直接返回推进结果EAX/ECX及EDX余数，不修改字体也不绘文字。

stage商零时，以共享字体对象token把字体大小先设为17。该callee正常返回后，EDX改为共享framebuffer token、ECX重新装载字体token，并在`216,218`以颜色`0xFFC0`、字体参数16绘制完整局部文字。文字callee返回后再次装载字体token，把大小恢复为16；最终函数返回该恢复callee的完整EAX/ECX/EDX。

格式、两次长度、两次字体设置和文字绘制通过独立窄typed端口保留六处不同的预调用寄存器布局；stage推进直接组合assembly-exact实现。主帧适配使用显式格式文字和显式测量长度发布位，合法零长度不会与“未提供reply”混淆。
...
定向测试覆盖局部seed、mode精确1门、CP950完整字节、24-byte标题源边界、64-byte格式边界、两次长度、signed half、动态矩形/九宫格尺寸和资源高word、stage商非零、字体17→16、六处generic寄存器布局、九宫格stop、消息113直连/旧槽零调用/timer后置/子stop传播及主帧五类服务映射。验证：定向测试、AddressSanitizer、Linux core 188/188、Linux app 194/194 全部通过。源码构建零warning；app仅有既有ALSA开发库CMake提示。

当前缺少原版共享标题与transition联合状态、真实framebuffer/边框资源、字体对象、动态局部栈地址、`wsprintfA/lstrlenA`返回及EAX/ECX/EDX联合捕获后端，`original_diff_verified`为`blocked_runtime_oracle`。
