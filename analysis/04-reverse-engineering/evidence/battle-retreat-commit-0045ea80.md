...
失败分支严格先以固定CP950字节串`B5 4C AA 6B BA 4D B0 68 21 21`，即“無法撤退!!”，参数`(280,10,50,text,0x40000002)`直连已关闭文字消息入链；随后读取live样本混音等级，以编号`0x008C`播放。函数完整尾返回来自样本callee，文字返回不参与控制流；文字typed-stop阻断样本播放。失败分支不修改任何成功状态。
...
定向测试覆盖u32索引回绕、首callee返回0/1/2、mode bit阻断、固定CP950文字入链与五参数、文字typed-stop阻断样本、live signed混音等级、样本尾寄存器、成功九项写序/宽度、CL陈旧高位、callee后mode动态重读、全局重置别名，以及动作3 caller直连和旧地址调用清零。
