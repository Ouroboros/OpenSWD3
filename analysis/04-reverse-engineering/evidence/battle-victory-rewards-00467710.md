...
奖励word位15已置位时跳过全部音频、掉落、角色奖励和银币写入，只继续结算面板查询。位15未置位时严格执行：
...
因此唯一道具计数为10时，第十一个新掉落已经完成玩家道具副作用，随后在首次真实数量表访问typed-stop；计数、payload和编号尚未推进。`0x00524468..0x0052448F`现由十项payload数组唯一承接；上一工作包消息99写的是该数组第0项，不再误建单一标量owner。
...
最后固定以`212,284,2`查询结算面板；返回精确1才使用同一64-byte局部缓冲连续执行三次signed `%d`等价格式化和文字绘制：
