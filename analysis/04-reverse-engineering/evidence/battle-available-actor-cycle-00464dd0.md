...
权威LST主体为`0x00464DD0..0x00464E33`，从proc到endp共68行、54条实际指令、2个静态call、6个跳转、6个局部/返回标签、2个`retn`，没有外部`FUNCTION CHUNK`。
...
实现把审计到的八个连续dword保存为单一常量物理视图，不把无效起点回绕到主表，也不在候选code小于8时提前拒绝。若相邻候选命中actor order，已关闭候选可用性会完成u32组A地址计算并在首次真实对象call停止；若均未命中，则四次扫描后正常返回0。
