# `0x00439120..0x004399D2` 两套 LZO1X 压缩器
...
Windows LLVM `core-debug` 与 `app-debug` 均完成构建并通过 29/29 CTest。原程序动态输入捕获后端仍不可用，因此没有把汇编逐块复核、历史原流逐字节差分和真实存档验证升级为运行时 `original_diff_verified`；该项继续登记为 `blocked_runtime_oracle`。
