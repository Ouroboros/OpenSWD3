...
- 战斗调试叠加层的调用门、18项选择顺序、战斗选择、消息低byte、初始模式和战斗帧按原写宽度同步；当前角色、fMenu、mMove和MS分别复用最终角色当前值、共享message state、预帧门与已发布值，不保留叠加副本；255字节文字缓冲、战斗模式、选择低word、缓存计数、帧率除数与标记坐标保持入口值；
