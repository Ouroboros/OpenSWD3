...
- 效果总协调器的18槽主记录、八条强度效果记录、两组模式、计数器、反馈actor、参数数组和活动latch映射回唯一`LegacyBattleEffectCoordinatorState`；强度记录0同时是攻击顺序移除必读的一过尾七dword源；未写的扫描计时与反馈数组保持不变；
