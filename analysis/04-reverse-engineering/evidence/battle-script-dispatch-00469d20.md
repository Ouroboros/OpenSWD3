...
`script+4 <= 7`走组B主动方：先清`script+2`对应u16目标槽，再从`script+6`开始寻找首个可用目标。候选`>7`时按live组A数量随机或顺序推进，只有两个live actor门均为1才查询可用性；候选`<=7`时递增到7并跳过callee返回1的项。最终写回目标槽，设置两个选择门，给目标槽高byte或入`0x40`，调用组B动作准备并插入类型2攻击顺序。
