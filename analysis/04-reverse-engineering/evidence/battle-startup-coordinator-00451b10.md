...
modern以定长typed数组和字段建模全部块，不以宿主指针模拟旧地址。`LegacyBattleStartupResetRecord`现为精确`0x1C`布局；本函数只修改上述五个已证字段，`+04/+08/+10`保持入口值。战斗调试W键和全局重置执行完整126 dword清零时则覆盖全部字段。角色优先索引不再保留启动状态副本，本函数直接写唯一actor metric owner。测试先以非零值污染所有块，再逐块验证最终值与未写字段保留。
