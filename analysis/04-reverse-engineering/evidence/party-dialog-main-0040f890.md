...
状态：`platform_adapted`、`caller_refreshed`
...
- `0x3EC`：page<5先申请32字节scratch。page1..4只接受ID 1501..1999并强制数量1、附加值0；page0读取输入。调用玩家数量helper后，page0按第一、第二、第三mask及记录ID<=500顺序发布分类更新：第一类直接组合共享固定曲线count/percent设置helper；第二类以命令ID读取MON定义、清理临时说明并更新共享定义曲线；第三类直接组合共享固定键计数链设置helper。第三mask和低ID条件同时成立时可对同一命令键重复设置。最后严格为填页→清三编辑框→释放scratch。
...
- `0x3F6`：先申请32字节scratch。page9写64个全局i32；page0..4始终在玩家总库存链按低14位找ID，数量存在时先清first并把signed值仅按`>=90`上夹，page0再发布分类更新；第一类曲线maximum和第三类链键仍来自解析后的命令参数，第一类曲线键与低ID重复门读取记录自身字段，两者不得因低14位查找而合并。page5..8直接调用17字段setter。成功路径严格为释放scratch→填页→清三编辑框。
...
- 第一类固定曲线helper typed-stop时，保留库存修改并阻断第二、第三分类、填页、清框和scratch释放；masked flags高word、解析ID和附加值的完整callsite寄存器前缀继续显式传入。
- 第二类定义曲线helper typed-stop时，保留库存和第一分类前缀；MON正常打开失败仍完成清理并以零maximum继续，只有loader或固定记录原访问停止才阻断第三分类、填页、清框和scratch释放。
- 第三类固定计数helper typed-stop时，保留已完成的库存修改、第一类曲线和第二类定义曲线副作用，并阻断填页、清框和scratch释放。
- 新增与直接修改的释放时机不同，不得统一清理。

## 6. 验证

`special_modes.legacy_initial_menu`覆盖初始化10个原Big5页名、四列与三长度；有效/无效页签通知；关闭顺序；negative first补足残值；page0第一类共享fixed-curve、第二类MON定义曲线与第三类共享fixed-count节点；mask和低ID重复命中；命令键、记录曲线键与masked EDX高字分离；三条typed链的分配失败前缀；page1强制数量；删除打包高字；直接修改的negative u16与`lParam`残值；队员setter；全局stale值；越界、记录缺失、selector 17和scratch申请失败前缀。
