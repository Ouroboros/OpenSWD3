...
本证据块记录 OpenSWD3 如何把世界帧协调器、LMF indexed object、两条空间角色路径、
两条图片动作、两条 `0xB4` 动作链、七个环境效果、三个公共尾部阶段和软件鼠标链接成
一个可执行纵向切片。原调用顺序仍只以
...
| stage | 实际执行 |
|---|---|
| `pre_background_records_004151f0` | 31 个 ordinal 的 LMF indexed object 扫描、局部 clip 与 blitter |
...
| `world_indicator_004149b0` | `0x2329` 右边条与主软件鼠标、输入热区及特殊模式请求 |

normal 路径只剩 `0x0042ED40` 转交 `remaining_stages`；另有只在 activity 分支出现的
`0x004154A0` 尚未接线，因此整个 stage 枚举仍有两项外部边界。最终平台提交仍在
...
- LMF render session 拥有的 indexed object command stream，以及软件鼠标 owner 的两个
  `0x2329` 动作记录、滑动计数和旧变体；SDL 只借入当前 DIK、逻辑鼠标、左键记录及
  app 的实际特殊模式状态。
...
- normal 路径仍有十九次 stage 调用，其中十八个 stage 实际执行、一个明确转交；
