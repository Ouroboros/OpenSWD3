...
入口直接读取和写入高优先级菜单共享状态；公共输入`0x00406EB0`和子模式0 `0x00406F70`均已关闭并由本入口直接调用。子模式1 `0x004070A0`及活动画面`0x00408CF0`分别由跨B11 owner或后续工作包关闭，当前以逐调用窄端口保留原重读和停止位置。
...
公共输入的尾部状态处理、子模式0的故事位读取、子模式1或活动画面不可用时，只在对应原调用点停止；timer、frame count、activity 3→1、鼠标frame以及此前callee修改均不回滚。
