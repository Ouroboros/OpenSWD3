...
模式3保留slot复制、0x80/0x20 flag、inventory返回值、139/184音效和刷新顺序。装备复制所调用的44D6E0现已关闭：端口只提供四个目标slot的21-word属性记录，记录payload解析为typed贡献后直接应用，旧`copy_equipment_payload`行为端口已删除。模式4委派46FE0。模式5按动作0执行移除、184和刷新后回模式2。模式10分配216字节资源、加载选择+71、读取+0x48 flag、进入11并释放；失败回2。模式11先回10，列非零直接返回；列零执行资源确认，成功回2并按flag决定46FE0。模式15先移除2B9，再读取筛选记录并BFC0准备对话；typed vector清理后把secondary offset保留为原释放数量，清计数、455A0并清全局owner。模式17/18回2。
