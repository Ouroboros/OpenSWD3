...
坐标增减是 32 位回绕。四次调用共享同一组进程期状态，但每次正常返回都先经过
`0x00417494..0x004174C8`公共后缀：目标高度、水平位移、纵向phase、opacity、RGB
偏移与跳行状态清零，跨调用放大位保留。因此第二至第四遍必须看到第一遍完成后的
状态，不能重复入口snapshot。原函数没有返回状态分支；正常返回固定继续，裸源、目标
或routine故障则进程在该遍停止。现代以`pass_count`保存已尝试前缀，首个typed-stop
不执行剩余遍次，也不提前应用公共后缀；四遍均正常时最终EAX仍对应第四遍。
