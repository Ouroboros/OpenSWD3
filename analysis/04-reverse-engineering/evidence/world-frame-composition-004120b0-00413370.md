...
- stage port 为已有 B4/B6 renderer 和尚待恢复的调用保留原位置；实际 runtime adapter
  已在原槽接入 `0x00413EA0`、`0x00413870` 和两处 `0x004147E0`，其余十五个 stage
  继续逐项显式转交，
  不把空 stage 误报成已经绘制；
...
空间及图片动作 runtime 接线后，真实 TSW 双角色路径叠加底图的 framebuffer 哈希为
`0xA6144A91E57939F9`；Linux Clang `core` 为 155/155、Windows LLVM `app` 为
159/159 CTest。真实资产结果证明当前数据链可用，但尚未与原程序逐帧 framebuffer
