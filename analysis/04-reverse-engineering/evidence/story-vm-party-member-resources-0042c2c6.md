...
全局数组基址从`0x004AB790`开始，因此LST标签分别显示为`word_4AB794..word_4AB79E`和`word_4AB7B4`。`sub_411030`与`sub_4112B0`对同一`0x38`步长记录提供17种field set/get；战斗和持久化路径也读取这些记录。现代VM进程状态现承接完整中性0x38字段owner；本handler只修改上述资源子集，opcodes186–187独立读取17字段视图。真实B10/B11记录加载仍不在本工作包伪造。
