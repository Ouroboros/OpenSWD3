...
全局数组基址从`0x004AB790`开始，因此LST标签分别显示为`word_4AB794..word_4AB79E`和`word_4AB7B4`。`sub_411030`与`sub_4112B0`对同一`0x38`步长记录提供17种field set/get；战斗和持久化路径也读取这些记录。现代VM进程状态现承接完整中性0x38字段owner；战斗角色升级快照进一步证明`+0x2D..+0x37`尾11 byte必须包含在同一typed记录内，结构现以显式尾数组和`sizeof==0x38`断言锁定。第一等级byte位于独立`+0x2C`，不是`+0x1C`字段的低byte。本handler只修改上述资源子集，opcodes186–187独立读取17字段视图。真实B10/B11记录加载仍不在本工作包伪造。
