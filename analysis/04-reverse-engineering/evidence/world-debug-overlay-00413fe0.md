...
`0x004140D9..0x0041435D` 通过 `sub_4302F0` 输出七条固定基线文字，位置为
`x = 8`、`y = 0,16,...,96`，最终由 16 点 renderer 以 flags `0x10` 绘制：
...
FPS 仍执行无符号 `1000 / frame_interval`。OpenSWD3 对零除和旧版 256 字节
`vsprintf` 越界只建立受检失败边界；正常输入的格式、宽度、参数顺序和输出字节不变。
...
“不可通過/可通過”，再查低 16 位并选择“可點選/不可點選”，最后按格式中的低位组、
高位组顺序输出。
...
`y=400` 重复输出“角色重疊”。角色循环中的三个文字调用点可重复执行，因此静态共有
13 个 `sub_4302F0` call site，动态调用数不固定。
...
等一、七条固定文字、事件高位后低位查询、事件 Big5 文本、附近角色严格边界与重叠输出。
coordinator UT 另固定外层总门、原槽顺序和失败停止点。

Linux LLVM `core` 154/154、Windows LLVM `app` 158/158 CTest 通过，Windows app 已成功
链接；未启动原版或新版 EXE。原程序动态差分若需要，只准备 Frida spawn 工具并等待
用户执行，不由开发流程启动原版。
