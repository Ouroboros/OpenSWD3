...
modern state固定持有`0x190`字节cache；四party destination以offset表达，不暴露分配pointer。442A40 seed选择、442AA0 party population与442B10 selected combination后续均已独立闭环；mode0返回party表typed record、mode1直接索引链、其他mode返回null，四个party依次复制模板并合并16个名称，选中记录再按seed替换或跳过slot。剩余summary finalization待独立关闭并继续由强类型端口表达。typed seed pointer、party low16、guardian slot与`0x140`destination均按原顺序传递。
