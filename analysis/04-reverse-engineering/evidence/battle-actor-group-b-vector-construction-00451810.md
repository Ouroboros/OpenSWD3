...
构造迭代器和组B元素构造/析构回调仍为后续工作包，当前以共享vector construction port隔离MSVC/EH边界；request本身区分组A与组B。
...
当前没有原版编译器向量构造迭代器、组B元素构造/析构回调、全局数组字节与异常展开联合捕获后端，`original_diff_verified`为`blocked_runtime_oracle`。完整19行LST已完成固定参数闭环。
