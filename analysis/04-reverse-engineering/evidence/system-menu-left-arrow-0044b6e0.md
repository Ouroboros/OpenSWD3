...
44B070的上方hover caller已删除opaque command并直接调用本typed helper；外层input-status查询与内层重建/计数都纳入helper count。UT覆盖页0负值、负窗口归零、重建顺序、AL OR保高位、行`<=0`夹值与音效、确认光标夹前EAX、19项反向回绕、完整sample owner及caller直连。
