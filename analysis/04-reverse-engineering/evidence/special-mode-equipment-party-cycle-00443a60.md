...
唯一行为真值为`swd3.exe.lst`。物理范围`0x00443A60..0x00443B6B`，127行，无FUNCTION CHUNK。code caller为F40一处，3B480另绑定为动作callback；F40现已直接回收。BB40、B9A0、BC90、B9C0、B9E0已关闭并直接复用；444F00、444F60、444E80、444FB0继续保持最窄typed端口，sample107保留平台端口。
...
选定party后严格按LST顺序调用444F60、444E80。callee可重建total、offset、local、visible与record head，后续全部重读。随后直接调用BB40规范窗口，B9A0定位visible head，BC90最多计24项，B9C0按`offset+local`选中记录，B9E0发布共享文本。
