...
文件开头是否为 `0xFFFFFFFF` 决定读取偏移。这里仅按可证明的物理差异命名为“带标记”和“无标记”；现有汇编与单个 DVD 样本不能证明它们分别属于哪个历史发行版。无标记布局可能来自更早 CD 版，但当前只登记为 `hypothesis_only`。同一 EXE 的 `0x00423E00` 也会直接在文件起点写 16 个按键字节，因此不能把无标记前缀简单等同于“旧版格式”。
