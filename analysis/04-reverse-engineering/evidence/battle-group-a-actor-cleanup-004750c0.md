...
状态：`platform_adapted`。完整LST主体、typed实现、八处caller回收、定向测试、AddressSanitizer、Linux完整门与inventory双生成均已关闭。
...
`+0x0D90`块与既有action-execution字段、`+0x2630`块与final-processing pre-effect owner、`+0x2F10..+0x2F18`与workspace/final-processing/item-effect owner、`+0x29A4..+0x29AA`与item-effect/attribute-effect owner存在typed视图重叠。实现用一个helper按物理写入顺序同步所有已存在视图，不新增第二份持久状态；任何必要视图缺失时停在对应原版访问点之前，并保留此前已完成写入和寄存器状态。
