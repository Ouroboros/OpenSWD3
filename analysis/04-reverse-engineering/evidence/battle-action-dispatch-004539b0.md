...
首次对象访问直接组合已关闭的typed动作种类word查询，结果只取AX；旧`0x004786B0` opaque地址生产调用为零。随后调用角色终止查询；完整EAX等于1时立即返回1，第二参数完全不访问。动作word或返回地址typed-stop保留原入口部分寄存器与flags，并阻断终止查询、fallback和全部动作分派后缀。

主动作号为0时，直接组合已关闭的typed显示种类word查询；旧`0x004786C0` opaque地址生产调用为零。leaf入口EAX/EDX沿用终止查询回复，flags来自`test edi,edi`，真实返回地址为`0x00453A13`；返回后才mask为u16并写局部动作槽。显示种类仍为0时返回1，非零时进入后续分派。字段或RET typed-stop保留动作种类与终止查询前缀并阻断局部写入和switch。modern测试锁定“主0、显示种类5”必须进入case 5，而不是错误地继续使用陈旧主动作0。

角色组A索引越界只在首次typed动作种类word查询点停止。第二参数不做入口预验；每个case只在首次实际组A/组B对象、表或状态槽访问时检查。
...
- case 6目标阶段开始前的固定键计数查询，以及完成后的固定键计数链累加；
- mode 1五秒倒计时初始化；
- 两次0x98动作record清零；
- 状态指示器动画；
- 三档刻度扫描；
...
- case 6减到2消息终态、目标阶段前固定键计数查询阈值及typed-stop后缀阻断、完成时固定键计数链直连、缺键唯一分配及selector-one道具数量直连；
- case 13事件槽；
...
- 95个原始唯一callee中80个窄端口边界全部存在，另外15个已关闭callee全部直连；
