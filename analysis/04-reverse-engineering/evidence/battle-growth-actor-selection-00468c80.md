...
四项奖励计数、成长限制和成长道具码属于胜利奖励state的连续profile owner；全局重置按原写集合保留这三组profile。角色道具链与新节点继续复用`LegacyWorldItemListState`，不建立battle平行链。共享定义scratch由本state唯一持有，并由后续法宝成长结果角色选择继续复用；24-byte标题复用`LegacyBattleLevelAdvancementState::growth_caption_text`；transition mode/actor复用目标选择runtime。消息113的成长结果选择写入同一标题，法宝完全成长提示框随后直接消费，不复制scratch或文字owner。
