...
114 项全集复核当前已关闭 70 项：43 项 `assembly_exact`、27 项 `platform_adapted`；其余
44 项保持待审计。最新关闭 `sub_40ED60`：首次装载抑制、一次 RNG、渐变进度条、两条
动作记录、两次音频维护及画面提交顺序均已逐段完成双向追溯；DirectDraw 裸表面改由
受控 framebuffer 与统一 presentation 端口承担。中间可见进度调用仍由待审计的
`sub_425BE0/sub_426DF0` 持有，不在 SDL 层伪造。此前 `sub_40E0B0` 的剧情位、变量 0、
