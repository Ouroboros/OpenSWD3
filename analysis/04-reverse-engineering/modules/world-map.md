...
114 项全集复核当前已关闭 51 项：38 项 `assembly_exact`、13 项 `platform_adapted`；其余
63 项保持待审计。MAPS 角色 helper 组已独立关闭默认值、两种相机计算、切换前同步、
完整写回、选择性 patch 和源物化；复核中纠正了默认值对 `role+0x2C` 只写低 word，
并补上此前缺失的 `sub_40D560` 共享 owner。`sub_402F80` 仍有隐藏调试热键、列表仲裁、
随机遇敌运行时接线和完整入口审计缺口，不继承旧完成叙述。

当前回归基线为 Linux `core` 179/179、Windows LLVM `app` 184/184 CTest；Windows
EXE 只完成链接，未启动原版或重写版游戏。
