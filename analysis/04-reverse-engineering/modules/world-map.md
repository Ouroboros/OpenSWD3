...
114 项全集复核当前已关闭 44 项：36 项 `assembly_exact`、8 项 `platform_adapted`；其余
70 项保持待审计。继控制/碰撞组七个 helper 后，本轮又关闭地图私有 TSW、两项地表
占用和四项角色 GUID/selector helper；其中纠正了 GUID 查找失败被误写成角色索引零的
旧说明。`sub_402F80` 仍有隐藏调试热键、列表仲裁、随机遇敌运行时接线和完整入口审计
缺口，不继承旧完成叙述。
