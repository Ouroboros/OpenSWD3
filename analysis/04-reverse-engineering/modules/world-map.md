...
114 项全集复核当前已关闭 71 项：43 项 `assembly_exact`、28 项 `platform_adapted`；其余
43 项保持待审计。最新关闭 `sub_40EFD0`：MAPS `+0x50` 相对目录、16 位键对完整 32 位
参数的比较、未对齐 `%Q` 逐字节扫描、NUL 结尾复制、`0xFFFF` miss 和“`不知道`”回退
均已逐基本块完成双向追溯；当前游戏数据的 346 个唯一名称键及新游戏键 81 的 CP950
字节已经固定。特殊模式调用者留在其实际 owner 中继续接线。此前 `sub_40ED60` 的首次
