...
状态：B6 单模块开始条件已满足；TSW、ACT 与通用动作记录最小闭环已实现，当前单元为 ANI
...
5. `[>]` ANI：`0x24` 头、独占打开、一基帧节点链、首次排除/缓存包含 12 字节、
   ...
   6,057,767 个 span、178,426,082 个索引像素及每文件首帧哈希通过。Linux `core`
   87/87、Windows LLVM `app` 91/91 CTest 通过；当前继续 `0x004154A0` 活动时间线
   与释放生命周期。
