...
状态：B6 单模块开始条件已满足；缓存容量策略已实现，当前单元为 TSW 查询最小闭环
...
2. `[>]` TSW 查询最小闭环：六包、3000 物理槽、低 16 位键、帧记录和缓存所有权。
3. `[ ]` ACT 变体查询最小闭环：完整 32 位键、两级缓存和借用 stream。
4. `[ ]` `0x98` 动作记录初始化与更新器，按命令小组逐批实现和 UT。
5. `[ ]` ANI 打开、帧链、首次/重读差异、span 输出与活动生命周期。
...
- 动作记录：固定字节快照、fake ACT stream、跨帧等待、重置保留字段和父对象借用测试。
