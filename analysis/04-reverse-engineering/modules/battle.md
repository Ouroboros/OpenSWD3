...
当前关闭进度：`28/422`。现有资产读取与建场代码只是此前恢复的有限切片，不提前计入完整函数关闭。
...
`audit_order=27`的`0x004507A0`已关闭为`platform_adapted`。它只覆盖packed颜色槽高16位，发布值/X/Y并清leading，固定按十亿到一调用`0x00450900`十次；每次重读callee可变X并加EAX低16位，最后一位调用前强制leading=1，正常返回最后推进低字。callee当前以typed端口隔离，下一项关闭后必须回收直连。
