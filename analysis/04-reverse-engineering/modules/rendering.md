...
5. `[>]` B4.5：按效果族逐单元实现其余已赋值 blitter；`0x10/0x24/0x28` 目标颜色偏移、常量填充和灰度，raw `0x84/0x85` 色键复制、RLE `0x04/0x2C` 饱和加减色，`0x18` literal run 边缘覆盖，raw `0x94`、RLE `0x14/0x1C` 透明度，以及 `0x30` 的 17 拍邻像素涂抹单元已闭环；下一单元处理当前真实调用可达的 raw `0x88` 常量纵向淡出。
