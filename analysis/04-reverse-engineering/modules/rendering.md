...
当前单元：B4.3 owned framebuffer、显式 pitch 与行偏移
...
3. `[>]` B4.3：实现 owned framebuffer、显式 pitch 和行偏移合同。
4. `[ ]` B4.4：建立稀疏 blitter 分派与 RLE/span、原始 copy 基础路径。
5. `[ ]` B4.5：按效果族逐单元实现其余已赋值 blitter。
