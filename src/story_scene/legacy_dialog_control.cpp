...
decrement_flagged_dialog_counter(const u32 value) noexcept {
...
[[nodiscard]] constexpr u32
elapsed_deciseconds(const u32 current_tick, const u32 started_at) noexcept {
    return (current_tick - started_at) / 100U;
}

void accept_selection(
    LegacyDialogRecord32& record, LegacyDialogControlState& state
) noexcept {
...
LegacyDialogControlResult advance_legacy_dialog_control(
    ...
    if (input.transition_in_progress) {
        result.action = LegacyDialogControlAction::skip_text_during_transition;
        return result;
    }
...
LegacyDialogControlApplyResult apply_legacy_dialog_control_action(
    ...
    const LegacyDialogControlAction action,
