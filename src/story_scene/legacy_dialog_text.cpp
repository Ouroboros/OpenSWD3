...
[[nodiscard]] constexpr LegacyDialogTextFrameStatus
malformed_status(const LegacyDialogTextTokenStatus status) noexcept {
...
[[nodiscard]] constexpr bool marker(
    const std::span<const u8> text, const std::size_t index, const u8 second
) noexcept {
...
[[nodiscard]] LegacyDialogTextToken fixed_token(
    const std::span<const u8> text,
    const std::size_t index,
    const LegacyDialogTextTokenKind kind,
...
LegacyDialogTextToken next_legacy_dialog_text_token(
    ...
    if (marker(text, source_index, static_cast<u8>('Q'))) {
        return fixed_token(
            text, source_index, LegacyDialogTextTokenKind::terminator, 2U
        );
    }
    if (marker(text, source_index, static_cast<u8>('N'))) {
        return fixed_token(
            text, source_index, LegacyDialogTextTokenKind::line_break, 2U
        );
    }
    ...
    if (marker(text, source_index, static_cast<u8>('P'))) {
        return fixed_token(
            text, source_index, LegacyDialogTextTokenKind::page_break, 2U
        );
    }
    if (marker(text, source_index, static_cast<u8>('S'))) {
        return parameter_token(
            text, source_index, LegacyDialogTextTokenKind::character_delay
        );
    }
    if (marker(text, source_index, static_cast<u8>('C'))) {
        return parameter_token(
            text, source_index, LegacyDialogTextTokenKind::both_colors
        );
    }
    if (has_bytes(text, source_index, 2U) &&
        ...
        return parameter_token(
            text, source_index, LegacyDialogTextTokenKind::secondary_color
        );
    }
    if (marker(text, source_index, static_cast<u8>('G'))) {
        return parameter_token(
            text, source_index, LegacyDialogTextTokenKind::text_style
        );
    }
    if (marker(text, source_index, static_cast<u8>('B')) ||
        marker(text, source_index, static_cast<u8>('b'))) {
        return period_payload_token(
            text, source_index, LegacyDialogTextTokenKind::choice
        );
    }
    if (marker(text, source_index, static_cast<u8>('A')) ||
        marker(text, source_index, static_cast<u8>('a'))) {
        return period_payload_token(
            text, source_index, LegacyDialogTextTokenKind::sound
        );
    }
    if (marker(text, source_index, static_cast<u8>('K')) ||
        ...
        token.uppercase_variant =
            text[source_index + 1U] == static_cast<u8>('K');
        return token;
    }

    const std::size_t byte_count = (text[source_index] & 0x80U) != 0U ? 2U : 1U;
    ...
    return fixed_token(
        text, source_index, LegacyDialogTextTokenKind::text, byte_count
    );
}

LegacyDialogTextFrameResult update_legacy_dialog_text(
    LegacyDialogMessage& message,
    const LegacyDialogTextFrameInput& input,
    LegacyDialogTextPorts& ports
) noexcept {
    ...
    const auto emit = [&](const std::span<const u8> bytes_to_draw,
        ...
        const bool draw_ok = ports.draw_segment(
            LegacyDialogSegmentDrawRequest{
    ...
    const auto flush_segment = [&]() noexcept {
        ...
        emit(
            std::span<const u8>{segment.data(), segment_size + 1U},
            line_position,
            false
        );
    };
    ...
    const auto reset_after_flush = [&]() noexcept {
        line_position =
            wrapping_add(line_position, static_cast<i32>(segment_size));
        segment.fill(0U);
        segment_size = 0U;
    };
    ...
    for (;;) {
        ...
        if (token.kind == LegacyDialogTextTokenKind::terminator) {
            flush_segment();
            if ((record.flags & kLegacyDialogFlagAlternateDirectTransition) ==
                0U) {
            ...
            if (next_line_height >= static_cast<i32>(record.height)) {
                ...
                if (token.kind == LegacyDialogTextTokenKind::line_break) {
                    ...
                    message.page_stop_index =
                        message.text_cursor_index + consumed;
                }
                if (record.lifetime_started_at == 0U) {
                    record.lifetime_started_at = input.current_tick;
                }
                ...
                save_page_style();
                return finish(
                    LegacyDialogTextFrameStatus::page_boundary_reached
                );
            }
        ...
        case LegacyDialogTextTokenKind::both_colors:
            ...
            reset_after_flush();
            foreground = low_word(
                sign_extend_byte(token.parameter) - static_cast<i32>('0')
            );
        ...
        case LegacyDialogTextTokenKind::text_style: {
            ...
            const i32 raw_style =
                sign_extend_byte(token.parameter) - static_cast<i32>('0');
            style = raw_style == 0 ? static_cast<u8>(4U)
        ...
        case LegacyDialogTextTokenKind::choice: {
            ...
            for (std::size_t index = 0U; index < token.payload.size();
                 ++index) {
            ...
            emit(
                std::span<const u8>{
                    choice_text.data(), token.payload.size() + 1U
                },
                line_position,
                selected
            );
            ...
            if (token.uppercase_variant) {
                ...
                const i32 absolute_left =
                    wrapping_add(static_cast<i32>(record.left), relative_left);
                ...
                const i32 absolute_bottom = wrapping_add(
                    absolute_top, wrapping_multiply(2, input.scale)
                );
                try {
                    message.choices.push_back(
                        LegacyDialogChoiceHotspot{
                            .kind = 1U,
