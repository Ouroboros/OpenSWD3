...
requires_role_anchor(const LegacyDialogRecord32& record) noexcept {
    return (record.flags &
            (kLegacyDialogFlagDirectRectangle |
             kLegacyDialogFlagExplicitAnchor)) == 0U &&
        record.role_index != 0xFFFDU;
}

[[nodiscard]] constexpr bool
accepted_text_status(const LegacyDialogTextFrameStatus status) noexcept {
...
[[nodiscard]] constexpr bool
should_draw_indicator(const compat::u32 flags) noexcept {
    if ((flags &
...
LegacyDialogRuntimeResult update_draw_legacy_dialogs(
    LegacyDialogRuntimeState& state,
    const LegacyDialogRuntimeInput& input,
    LegacyDialogRuntimePorts& ports
) noexcept {
    ...
    if (!ports.begin_text_surface(
            kLegacyDialogSurfaceWidth, kLegacyDialogSurfaceHeight
        )) {
    ...
    for (LegacyDialogMessage& message : state.messages) {
        ...
        const LegacyDialogControlResult control = advance_legacy_dialog_control(
            ...
            LegacyDialogControlInput{
                ...
                .global_confirm_latch_state = input.global_confirm_latch_state,
        ...
        if (control.action == LegacyDialogControlAction::parse_text) {
            ...
            const LegacyDialogControlApplyResult applied =
                apply_legacy_dialog_control_action(
                    message, control.action, state.control, state.close, ports
                );
    ...
    ports.restore_text_destination(
        input.destination_width, input.destination_height
    );
