...
void advance_four_step_transition(
    LegacyDialogRecord32& record, bool& transition_in_progress
) noexcept {
    ...
    if ((record.flags & kLegacyDialogFlagDirectRectangle) != 0U) {
        ...
        if ((record.flags & kLegacyDialogFlagAlternateDirectTransition) != 0U) {
            if (record.display_counter < 8U) {
                result.opacity_step =
                    wrapping_multiply(zero_extend(record.display_counter), 2);
            }
        } else {
            advance_four_step_transition(record, result.transition_in_progress);
            result.opacity_step =
                wrapping_multiply(zero_extend(record.transition_step), 4);
        }
        ...
        if ((record.flags & kLegacyDialogFlagExplicitAnchor) != 0U) {
            ...
            panel_top = wrapping_add(
                wrapping_add(
                    anchor_top,
                    wrapping_multiply(
                        step, signed_quarter(wrapping_subtract(top, anchor_top))
                    )
                ),
                -4
            );
                ...
                wrapping_add(
                    ...
                    wrapping_multiply(
                        ...
                        signed_quarter(wrapping_subtract(
                            wrapping_add(input.camera_left, left), anchor_left
                        ))
                    )
                ),
                input.camera_left
                ...
                wrapping_subtract(
                    wrapping_add(
                        ...
                        wrapping_multiply(
                            step,
                            signed_quarter(wrapping_subtract(
                                wrapping_add(input.camera_top, top), anchor_top
                            ))
                        )
                    ),
                    input.camera_top
        ...
        panel_right = wrapping_add(
            panel_left,
            wrapping_multiply(
                step, signed_quarter(wrapping_add(width, twice_scale))
            )
        );
