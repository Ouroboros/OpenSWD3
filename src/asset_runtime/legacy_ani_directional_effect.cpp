...
[[nodiscard]] constexpr i32 move_toward(const i32 current,
                                        const i32 target) noexcept {
  if (current < target) {
...
[[nodiscard]] u32
random_bounded(input_time_rng::LegacySecondaryRng &random,
               const u32 upper_bound,
               LegacyAniDirectionalInitializationResult &result) noexcept {
...
[[nodiscard]] u32 random_bounded(input_time_rng::LegacySecondaryRng &random,
                                 const u32 upper_bound,
                                 LegacyAniDirectionalResult &result) noexcept {
  ++result.random_call_count;
  return random.next_bounded(upper_bound);
}
...
LegacyAniDirectionalRuntimePorts::draw_frame_piece(
    ...
    const i32 destination_y, const u32 flags, const i32 opacity_step,
    const i32 color_offset) noexcept {
...
LegacyAniDirectionalResult LegacyAniDirectionalEffect::update(
  ...
  for (std::size_t index = 0U; index < kLegacyAniDirectionalUpdatedSlotCount;
    ...
    const bool inside =
        motion.world_x > kMinimumX && motion.world_x < maximum_x &&
        motion.world_y > kMinimumY && motion.world_y < maximum_y;
    if (!inside) {
      if (frame_roll < 990U) {
      ...
      timing.variant = std::bit_cast<i32>(
          (static_cast<u32>(configuration.variant_count) * frame_roll) / 1000U +
          static_cast<u32>(configuration.base_variant));
      ...
      const u32 width_bound = std::bit_cast<u32>(map_width_pixels);
      if (!valid_random_bound(width_bound)) {
        result.status = LegacyAniDirectionalStatus::invalid_random_bound;
        result.failed_slot = static_cast<u32>(index);
        return result;
      }
      motion.world_x =
      ...
      if (configuration.spawn_direction < 2U) {
        motion.world_y = wrapping_add(map_height_pixels, kVerticalSpawnOffset);
      } else {
