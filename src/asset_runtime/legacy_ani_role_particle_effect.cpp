...
[[nodiscard]] constexpr i16 wrapping_add(const i16 left,
                                         const i16 right) noexcept {
  return std::bit_cast<i16>(
      static_cast<u16>(static_cast<u32>(std::bit_cast<u16>(left)) +
...
[[nodiscard]] bool
outside_cull_rectangle(const i32 world_x, const i32 world_y,
                       const LegacyAniRoleParticleViewport &viewport) noexcept {
...
[[nodiscard]] u32 random_bounded(input_time_rng::LegacySecondaryRng &random,
                                 const u32 upper_bound,
                                 LegacyAniRoleParticleResult &result) noexcept {
  ++result.random_call_count;
  return random.next_bounded(upper_bound);
}

void set_flag_bit_zero(LegacyAniRoleParticleEmitter &emitter,
...
void color_offsets(const i16 lifetime, const i32 legacy_map_id, i32 &red_offset,
                   i32 &green_offset, i32 &blue_offset) noexcept {
...
LegacyAniRoleParticleRuntimePorts::draw_frame_piece(
    const rendering::LegacyFramePiece &piece, const i32 destination_x,
    const i32 destination_y, const u32 flags, const i32 red_offset,
    const i32 green_offset, const i32 blue_offset) noexcept {
  effects_.red_offset = red_offset;
  effects_.green_offset = green_offset;
  effects_.blue_offset = blue_offset;
  return rendering::blit_legacy_copy_paths(framebuffer_, current_clip(raster_),
                                           piece.source,
...
LegacyAniRoleParticleResult LegacyAniRoleParticleEffect::update(
    const i32 role_world_x, const i32 role_world_y, const u16 role_selector,
    const i32 legacy_map_id, const LegacyAniRoleParticleViewport &viewport,
  ...
  const i16 frame_horizontal_drift =
      from_low_word(random_bounded(random, 5U, result) - static_cast<u32>(3U));
  ...
  rendering::LegacyFramePiece piece;
  if (!ports.load_frame_piece(shared_action_record.field_4a,
                              shared_action_record.field_4c, piece)) {
  ...
  for (LegacyAniRoleParticleEmitter &emitter : emitters_) {
    ...
    if (!positions.resolve_role_position(role_selector, current_role_x,
                                         current_role_y)) {
    ...
    if (emitter.field_08 < kSpawnFieldLimit && flag_bit_zero(emitter)) {
      ...
      const u32 spawn_x =
          random_bounded(random, 16U, result) +
      ...
      spawned->vertical_step = from_low_word(
          std::numeric_limits<u32>::max() - random_bounded(random, 1U, result));
    ...
    while (token != 0U) {
      ...
      const i16 horizontal_step =
          wrapping_add(node->horizontal_step_1_16, frame_horizontal_drift);
      node->world_y = wrapping_add(node->world_y, node->vertical_step);
      node->fixed_x_1_16 = wrapping_add(node->fixed_x_1_16, horizontal_step);
      ...
      i32 green_offset{};
      i32 blue_offset{};
      color_offsets(node->lifetime, legacy_map_id, red_offset, green_offset,
                    blue_offset);
      const i32 destination_y = wrapping_subtract(
          wrapping_subtract(static_cast<i32>(node->world_y),
                            field_as_i32(shared_action_record.draw_offset_y)),
          viewport.top);
      const i32 destination_x = wrapping_subtract(
          wrapping_subtract(static_cast<i32>(node->fixed_x_1_16) / 16,
                            field_as_i32(shared_action_record.draw_offset_x)),
          viewport.left);
      result.last_blit_status = ports.draw_frame_piece(
          piece, destination_x, destination_y, shared_action_record.mode_flags,
...
const std::array<LegacyAniRoleParticleEmitter,
