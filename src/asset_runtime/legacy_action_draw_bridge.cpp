...
load_and_draw(const u16 resource_id, const u16 frame_index, const i32 x,
              const i32 y, const u32 flags, const i32 opacity_step,
              LegacyActionDrawPorts &ports, LegacyActionDrawResult result) {
...
[[nodiscard]] LegacyActionDrawResult
update_and_draw(LegacyActionRecord &record, const i32 x, const i32 y,
                const u32 caller_flags, const bool mask_action_flags,
...
LegacyActionDrawRuntimePorts::draw_frame_piece(
    ...
    const i32 destination_y, const u32 flags, const i32 opacity_step) noexcept {
...
LegacyActionDrawResult update_draw_legacy_action(LegacyActionRecord &record,
                                                 const i32 x, const i32 y,
                                                 LegacyActionDrawPorts &ports) {
  return update_and_draw(record, x, y, 0U, false, ports);
}

LegacyActionDrawResult draw_legacy_tsw_frame(const u32 resource_id_slot,
                                             const u32 frame_index_slot,
                                             const i32 x, const i32 y,
                                             LegacyActionDrawPorts &ports) {
  return load_and_draw(static_cast<u16>(resource_id_slot),
                       static_cast<u16>(frame_index_slot), x, y, 0U, 0, ports,
                       LegacyActionDrawResult{});
}

LegacyActionDrawResult
update_draw_legacy_action_with_flags(LegacyActionRecord &record, const i32 x,
                                     const i32 y, const u32 caller_flags,
                                     LegacyActionDrawPorts &ports) {
