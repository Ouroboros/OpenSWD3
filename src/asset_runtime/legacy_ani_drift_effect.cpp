...
constexpr std::array<i32, 8U> kVerticalVariants{
    0x34, 0x34, 0x34, 0x36, 0x38, 0x38, 0x38, 0x36,
};
...
bool LegacyAniDriftRuntimePorts::load_frame_piece(
    const compat::u16 resource_id, const compat::u16 frame_index,
...
LegacyAniDriftRuntimePorts::draw_frame_piece(
    const rendering::LegacyFramePiece &piece, const compat::i32 destination_x,
    const compat::i32 destination_y, const compat::u32 flags) noexcept {
  return rendering::blit_legacy_copy_paths(
             framebuffer_, current_clip(raster_), piece.source,
...
LegacyAniDriftResult LegacyAniDriftEffect::update(
    const compat::i32 map_width_tiles, const compat::i32 map_height_tiles,
    const compat::i32 camera_x, const compat::i32 camera_y,
  ...
  for (std::size_t index = 0U; index < state_.slots.size(); ++index) {
    ...
    if (slot.x == kLegacyAniDriftInactiveX) {
      ...
      const i32 vertical_delta = static_cast<i32>(random.next_bounded(3U)) - 1;

      slot.velocity_y =
          std::clamp(wrapping_add(slot.velocity_y, vertical_delta), -3, 3);
      slot.velocity_x =
          std::clamp(wrapping_add(slot.velocity_x, horizontal_delta), -5, 5);
      if (slot.velocity_x == 0) {
    ...
    i32 variant = kVerticalVariants[static_cast<std::size_t>(variant_index)];
    if (slot.velocity_x > 0) {
      ++variant;
    }
...
LegacyAniDriftState &LegacyAniDriftEffect::state() noexcept { return state_; }
