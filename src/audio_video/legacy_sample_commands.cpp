...
[[nodiscard]] constexpr compat::i32
wrapping_shift_left(const compat::i32 value, const compat::u32 count) noexcept {
...
[[nodiscard]] LegacySamplePlayRequest
make_play_request(const compat::u32 sound_id, const compat::i32 volume) {
...
compat::i32 play_legacy_sample(
    ...
    return manager.play(make_play_request(
        low_word(raw_sound_id), scale_level_by_128_over_11(level)
    ));
}
...
compat::i32 stop_legacy_sample(
    LegacySampleManager& manager, const compat::u32 raw_sound_id
) {
...
compat::i32 play_legacy_spatial_sample(
    ...
    const compat::i32 distance = spatial_distance(
        state.listener_x, state.listener_y, target_x, target_y
    );
    if (distance >= 0x200) {
        return distance;
    }
    ...
    const compat::i32 remaining_level =
        wrapping_subtract(0x80, distance_attenuation);
    ...
    const compat::i32 horizontal_delta =
        wrapping_subtract(target_x, state.listener_x);
    const compat::i32 pan = wrapping_shift_left(horizontal_delta, 6U) / 0x200;
