...
bool LegacySndArchive::is_open() const noexcept {
    return open_;
}
...
LegacySndArchive::load_sample(const compat::u32 one_based_sound_id) noexcept {
    ...
    std::transform(
        std::begin(kRiffTemplate),
        std::end(kRiffTemplate),
        result.bytes.begin(),
        [](const char value) { return static_cast<compat::u8>(value); }
    );

    if (file_.seek_begin_one_based(
            std::bit_cast<compat::i32>(runtime_entry->file_offset)
        ) != runtime_entry->file_offset + 1U) {
        result.bytes.clear();
        result.status = LegacySndSampleStatus::sample_seek_failed;
        return result;
    }

    const std::span<compat::u8> payload{
        result.bytes.data() + prefix_size, read_size
    };
