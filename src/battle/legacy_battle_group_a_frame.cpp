...
[[nodiscard]] constexpr LegacyBattleActorCoordinateFlags
decrement_flags(const u32 value, const bool preserved_carry) noexcept {
    auto flags = subtract_flags(value, 1U);
    flags.carry = preserved_carry;
    return flags;
}
...
public:
    ...
    invoke_actor_target_preparation(
        ...
        const auto reply = port_.invoke({
            .callee_token = kCallQueryTerminal,
            .arguments = arguments,
            .eax = request.eax,
...
[[nodiscard]] bool decay_actor_gates(
    ...
    if (execute_legacy_battle_actor_gate_decay_call(
            ...
            context.actor_gate_decay_requests,
            {
                ...
                .fixed_pre_group_b_packed_counts = &state.shared_value_525454,
                .fixed_pre_group_b_start_gate_latch =
                    &state.shared_value_5254c0,
            },
            call_address,
            return_address,
            actor_token,
...
void merge_nested_result(
    ...
    for (u32 index = 0U; index < nested.actor_action_target_calls; ++index) {
        outer.actor_action_targets[outer.actor_action_target_calls + index] =
    ...
    for (std::size_t index = 0U; index < nested.actor_gate_decay.calls;
         ++index) {
        const std::size_t destination = outer.actor_gate_decay.calls + index;
        if (destination < outer.actor_gate_decay.call_addresses.size()) {
            outer.actor_gate_decay.call_addresses[destination] =
                nested.actor_gate_decay.call_addresses[index];
            outer.actor_gate_decay.return_addresses[destination] =
                nested.actor_gate_decay.return_addresses[index];
            outer.actor_gate_decay.actor_tokens[destination] =
                nested.actor_gate_decay.actor_tokens[index];
        }
    }
    outer.actor_gate_decay.calls += nested.actor_gate_decay.calls;
    if (nested.actor_gate_decay.calls != 0U) {
        outer.actor_gate_decay.last = nested.actor_gate_decay.last;
    }
    for (std::size_t index = 0U; index < nested.actor_action_target_clear.calls;
         ++index) {
        const std::size_t destination =
            outer.actor_action_target_clear.calls + index;
        if (destination <
            outer.actor_action_target_clear.call_addresses.size()) {
            outer.actor_action_target_clear.call_addresses[destination] =
                nested.actor_action_target_clear.call_addresses[index];
            outer.actor_action_target_clear.return_addresses[destination] =
                nested.actor_action_target_clear.return_addresses[index];
            outer.actor_action_target_clear.actor_tokens[destination] =
                nested.actor_action_target_clear.actor_tokens[index];
        }
    }
    outer.actor_action_target_clear.calls +=
    ...
    for (std::size_t index = 0U; index < nested.actor_target_selection.calls;
         ++index) {
        const std::size_t destination =
            outer.actor_target_selection.calls + index;
        if (destination < outer.actor_target_selection.call_addresses.size()) {
            outer.actor_target_selection.call_addresses[destination] =
                nested.actor_target_selection.call_addresses[index];
            outer.actor_target_selection.return_addresses[destination] =
                nested.actor_target_selection.return_addresses[index];
            outer.actor_target_selection.actor_tokens[destination] =
                nested.actor_target_selection.actor_tokens[index];
            outer.actor_target_selection.argument_values[destination] =
                nested.actor_target_selection.argument_values[index];
        }
    }
    outer.actor_target_selection.calls += nested.actor_target_selection.calls;
    if (nested.actor_target_selection.calls != 0U) {
        outer.actor_target_selection.last = nested.actor_target_selection.last;
    }
...
LegacyBattleTurnAdvanceResult advance_legacy_battle_turn_gate(
    ...
    if (progress->special_ready == 1U) {
        if (actor == nullptr) {
            result.status =
                LegacyBattleTurnAdvanceStatus::actor_state_typed_stop;
            return result;
        }
    ...
    auto invoke_turn = [&](const u32 callee,
    ...
    const u32 argument = request.argument;
    actor->turn_threshold = argument == 0U ? 2U : 6U;
    ++result.queue_completion_calls;
    ...
    actor->turn_action_record.external_mode =
        actor->special_mode == 1U ? 1U : 0U;
    eax = request.actor_token + 0x468U;
    ecx = actor->special_mode;
    ...
    actor->turn_frame_token = frame.eax;
    actor->turn_render_flags = actor->turn_action_record.mode_flags;
    ...
    static_cast<void>(invoke_turn(
        ...
        {render_x,
         render_y,
         static_cast<u16>(frame.outputs[1U]),
         static_cast<u16>(frame.outputs[2U]),
...
LegacyBattleActionDispatchResult advance_legacy_battle_group_a_frame(
    ...
    const LegacyBattleActorTargetSelectionOwners target_selection_owners{
        .action = &state.action,
        .startup = context.startup,
    };
    ...
    const auto select_actor_target = [&context,
                                      &result,
                                      &target_selection_owners,
                                      actor_token](
    ...
    if (state.ai_coordination_enabled == 1U &&
        ...
        if (progress.return_eax == 1U) {
            ...
            if (state.actor_ai_primary[group_a_index] == 1U ||
                ...
                u32 terminal_query_edx = 0U;
                if (!count_group_b_terminal(
                        state, port, result, terminal_count, terminal_query_edx
                    )) {
                    return result;
                }
                if (terminal_count < state.action.group_b_count) {
                    if (!set_availability_block(1U, terminal_query_edx)) {
                        return result;
                    }
                    while (true) {
                ...
                if (slot < state.final_actor_step.actor_order.size()) {
                    ...
                    if (queue_mode.eax == 1U) {
                        ...
                        result.actor_availability_block_calls +=
                            prepared.actor_availability_block_calls;
                        if (prepared.status !=
        ...
        if (actor.action_complete != 1U) {
            if (actor.mode_gate != 0U) {
                ...
                if (selected_turn_completion == 0U &&
                    ...
                    const auto action_mode_flags =
                        subtract_flags(result.actor_action_mode.return_eax, 1U);
            ...
            LegacyBattleActionCallReply selection_terminal{};
            ...
            const auto selection_complete =
                invoke(port, result, kCallSelectionComplete, {actor_token});
            if (selection_complete.eax == 1U) {
                if (state.actor_ai_primary[group_a_index] != 0U ||
                    state.actor_ai_secondary[group_a_index] != 0U) {
                    state.final_actor_step.selection_gate = 0U;
                    if (!set_availability_block(0U, selection_complete.edx)) {
                        return result;
                    }
                    state.selection_aux_gate = 0U;
    ...
    if (state.action.active_effect_target == group_a_index + 8U) {
        if (state.final_actor_step.action_execution_active != 0U) {
            ...
            action_target_request.entry_eax = prepared_action.eax;
            action_target_request.entry_edx = prepared_action.edx;
            action_target_request.entry_return_address = 0x004570D1U;
            ...
            const u16 target_index = low_word(action_target_value);
            ...
            const auto nested = dispatch_legacy_battle_action(
                state.action, port, nested_context, group_a_index, target_index
            );
            merge_nested_result(result, nested);
            ...
            if (nested.return_value == 1U) {
                action_target_request =
                    context.group_a_frame_action_target_requests[2U];
                action_target_request.actor_token = actor_token;
                action_target_request.entry_eax = nested.return_value;
                action_target_request.entry_return_address = 0x004570F1U;
                action_target_request.entry_flags =
                    subtract_flags(nested.return_value, 1U);
                ...
                const u16 completed_target = low_word(action_target_value);
                ...
                if (completed_target != 0xFFFFU) {
                    ...
                    if (selection_complete.eax == 1U) {
                        ...
                        for (i32 index = 0; index < state.action.group_b_count;
                            ...
                            decay_entry_flags = subtract_flags(
                                uindex + 1U, to_bits(state.action.group_b_count)
                            );
                        }
                    } else {
                        const u32 reset_index = to_bits(
                            static_cast<i32>(
                                std::bit_cast<i16>(completed_target)
                            )
                        );
                        u32 reset_token{};
                        u32 decay_entry_eax{};
                        ...
                        if (state.action_side != 0U) {
                            ...
                            const u32 times_sixty_three =
                                times_sixty_four - reset_index;
                            const u32 times_one_thousand_eight =
                                times_sixty_three << 4U;
                            const u32 times_one_thousand_seven =
                                times_one_thousand_eight - reset_index;
                            reset_token = group_a_token(reset_index);
                            decay_entry_eax = times_one_thousand_seven;
                            decay_entry_flags = subtract_flags(
                                times_one_thousand_eight, reset_index
                            );
                        } else {
                            const u32 times_three =
                                reset_index + reset_index * 2U;
                            const u32 times_twenty_four = times_three << 3U;
                            const u32 times_twenty_three =
                                times_twenty_four - reset_index;
                            ...
                            reset_token = group_b_token(reset_index);
                            decay_entry_eax =
                                times_one_thousand_three_hundred_eighty_one;
                            decay_entry_edx = times_three_hundred_forty_five;
                            decay_entry_flags =
                                subtract_flags(times_twenty_four, reset_index);
                        }
                        if (!decay_actor_gates(
                                state,
                                context,
                    ...
                    if (terminal.eax == 1U) {
                        ...
                        action_target_request.actor_token = completed_b;
                        action_target_request.entry_eax = terminal.eax;
                        action_target_request.entry_edx = terminal.edx;
                        ...
                        const u16 actor_target = low_word(action_target_value);
                        if (actor_target != 0xFFFFU) {
                            const u32 actor_target_index = to_bits(
                                static_cast<i32>(
                                    std::bit_cast<i16>(actor_target)
                                )
                            );
                            const u32 times_sixty_four = actor_target_index
                                << 6U;
                            const u32 times_sixty_three =
                                times_sixty_four - actor_target_index;
                            const u32 times_one_thousand_eight =
                                times_sixty_three << 4U;
                            const u32 times_one_thousand_seven =
                                times_one_thousand_eight - actor_target_index;
                            if (!decay_actor_gates(
                                    state,
                                    context,
                                    result,
                                    group_a_token(actor_target_index),
                                    0x0045720CU,
                                    0x00457211U,
                                    times_one_thousand_seven,
                                    result.actor_action_target.return_edx,
                                    subtract_flags(
                                        times_one_thousand_eight,
                                        actor_target_index
                                    )
                                )) {
                                return result;
            ...
            auto action_target_request =
                context.group_a_frame_action_target_requests[0U];
            action_target_request.actor_token = actor_token;
            action_target_request.entry_eax = state.action_aux_gate;
            action_target_request.entry_return_address = 0x00456ECEU;
            action_target_request.entry_flags =
                logical_flags(state.action_aux_gate);
            action_target_request.entry_flags_known = true;
            u32 queried_target_value{};
            if (!query_action_target(
                    result,
                    {.action = &state.action, .startup = context.startup},
                    action_target_request,
                    queried_target_value
                )) {
                return result;
            }
            const auto& queried_target_reply = result.actor_action_target;
            const u16 queried_target = low_word(queried_target_value);
            ...
            if (!query_turn_completion(
                    ...
                    queried_target_reply.return_edx,
                    subtract_flags(
            ...
            if (target_turn_completion == 0U) {
                    ...
                    if (invoke(port, result, kCallQueryTerminal, {target_token})
                        ...
                        if (selected >= state.action.group_b_count) {
                            ...
                            if (!clear_actor_action_target(
                                    ...
                                    0x00456F94U,
                                    0x00456F99U,
                                    first_nonterminal.eax,
                                    first_nonterminal.edx,
                                    subtract_flags(first_nonterminal.eax, 1U)
                                )) {
                                return result;
                            }
                            if (!decay_actor_gates(
                            ...
                            if (static_cast<u16>(selected) == 0xFFFFU) {
                                ...
                                if (!select_actor_target(
                                        ...
                                        result.actor_gate_decay.last.return_edx,
                                        subtract_word_flags(
                                            static_cast<u16>(selected), 0xFFFFU
                                        ),
                                        0x00456FC1U,
                                        0x00456FC6U
                                    )) {
                ...
                if (state.final_actor_step.action_execution_active == 1U) {
                    ...
                    if (!execute_group_a_actor_list_action(
                            context, result, group_a_index, actor_token
                        )) {
                        result.return_value =
    ...
    if (state.action_block_gate == 0U &&
        ...
        if ((turn & 0x4000U) != 0U) {
            ...
            if (result.turn_advance.return_eax == 1U) {
                replace_low_word(
