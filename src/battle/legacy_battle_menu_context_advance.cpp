...
LegacyBattleMenuContextAdvanceResult advance_legacy_battle_menu_context(
    const LegacyBattleMenuContextAdvanceBindings bindings,
    LegacyBattleInputDispatchPort& port,
    const LegacyBattleMenuContextAdvanceRequest& request
) {
    LegacyBattleMenuContextAdvanceResult result;
    ...
    const auto stop = [&](const LegacyBattleMenuContextAdvanceStatus status) {
    ...
    if (eax == 1U) {
        ...
        edx = static_cast<u32>(reset.value_53bf22);
        ecx = eax + 4U;
        edx += 5U;
        if (signed_bits(ecx) <= signed_bits(edx)) {
            eax = ecx;
        ...
        if (eax >= permissions.size()) {
            return stop(
                LegacyBattleMenuContextAdvanceStatus::permission_typed_stop
            );
        }
    ...
    if (bindings.message_state == 2U) {
        eax = input.action_category_index + 1U;
        input.action_category_index = eax;
        if (signed_bits(eax) >= 3) {
            input.action_category_index = 0U;
        }
