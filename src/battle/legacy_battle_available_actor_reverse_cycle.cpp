...
LegacyBattleAvailableActorCycleResult
reverse_cycle_legacy_battle_available_actor(
    ...
    while (true) {
        const u32 candidate =
            kLegacyBattleActorCyclePhysicalCandidates[candidate_index];
        --candidate_index;
        if (static_cast<i32>(candidate_index) < 0) {
            candidate_index = 3U;
        }
        ...
        ++failed_count;
        if (static_cast<i32>(failed_count) >= 4) {
