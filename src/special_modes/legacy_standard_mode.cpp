...
advance_legacy_standard_mode_guardian_selection(
    ...
    const auto invoke =
        [&state,
         &ports,
         &result](const LegacyStandardModeGuardianSelectionTarget target) {
            result.legacy_return_value =
                ports.invoke_guardian_selection(target, state);
            ++result.helper_call_count;
            result.last_target = target;
        };
    ...
    invoke(LegacyStandardModeGuardianSelectionTarget::refresh_attribute_cache);
    result.legacy_return_value =
        ports.execute_guardian_sample_command(0x2EU, state.sample_owner);
    ++result.helper_call_count;
    state.mode_flags |= 0x30U;
...
handle_legacy_standard_mode_guardian_input(
    ...
    if (availability.available && state.total_record_count > 0x0AU &&
        ...
        if (mode == 1U) {
            if (input.cursor_y < 0x74U && input.cursor_y > 0x66U) {
                static_cast<void>(
                    invoke(LegacyStandardModeGuardianInputTarget::cycle_left)
                );
                return result;
            }
    ...
    if ((input.buttons & 3U) != 0U && input.cursor_y < 0x1D4U &&
        ...
        if (mode == 0U) {
            ...
            if (item_present) {
                ...
                static_cast<void>(
                    invoke(LegacyStandardModeGuardianInputTarget::switch_party)
                );
            }
            return result;
        }
    ...
    if ((input.buttons & 4U) != 0U) {
        static_cast<void>(
            invoke(LegacyStandardModeGuardianInputTarget::commit_interaction)
        );
    }
    return result;
}
