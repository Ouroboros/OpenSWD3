...
void append_header(
    std::vector<u8>& bytes,
    const LegacyImageCommandStreamHeader& header
) {
...
[[nodiscard]] LegacyImageCommandStreamResult expand_indexed_stream(
    ...
    result.header.format = static_cast<u16>(
        (result.header.format & 0x8010U) | 0x0010U
    );
    ...
    while (true) {
        ...
        const std::size_t row_bytes =
            result.bytes.size() - output_row_offset;
        write_u16_at(
            result.bytes,
            output_row_offset,
            static_cast<u16>(row_bytes)
        );
    }
}
        ...
        if (format == 16U) {
            ...
            while (column < pixels_per_row) {
                ...
                if (pixel == kLegacyImageMarker8000 ||
                    ...
                    const u16 tag = marker == kLegacyImageMarker8000
                        ? 0x8000U
                        : 0xC000U;
                    append_u16(
                        result.bytes,
            ...
            while (column < pixels_per_row) {
                ...
                if (pixel == kLegacyIndexedMarker8000 ||
                    ...
                    const u16 tag = pixel == kLegacyIndexedMarker8000
                        ? 0x8000U
                        : 0xC000U;
                    append_u16(
                        result.bytes,
                ...
                while (column + literal_count < pixels_per_row) {
                    const u8 candidate =
                        pixels[source_offset + literal_count];
...
LegacyImageCommandStreamResult decode_legacy_image_command_stream(
    const std::span<const u8> command_stream
) {
    LegacyImageCommandStreamResult result;
    result.status = read_header(command_stream, result.header);
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (depth == 8U) {
                const u8 marker = tag == 0x8000U
                    ? kLegacyIndexedMarker8000
                ...
                const u16 marker = tag == 0x8000U
                    ? kLegacyImageMarker8000
...
convert_legacy_image_command_stream_literals_in_place(
    ...
    if (decoded_header != nullptr) {
        *decoded_header = header;
        decoded_header->format = static_cast<u16>(
            decoded_header->format & kConverterDepthMask
        );
    }
...
convert_legacy_embedded_palette_image_command_stream(
    ...
    for (std::size_t index = 0U; index < palette.size(); ++index) {
        ...
        static_cast<void>(read_u16(
            palette_and_command_stream,
            offset,
            palette[index]
        ));
    }

    result = expand_indexed_stream(
        command_stream,
        palette,
        pixel_conversion
    );
