...
[[nodiscard]] constexpr i32 truncate_10_10_product(
    const i32 left,
    const i32 right
) noexcept {
...
[[nodiscard]] constexpr i32 signed_remainder_10_10(
    const i32 value
) noexcept {
    return value % 0x400;
}
...
[[nodiscard]] bool run_write_bounds(
    ...
    const i32 left_distance = wrapping_subtract(
        wrapping_subtract(destination_word, scaled_run),
        clip_left_word
    );
    const i32 right_distance = wrapping_subtract(
        destination_word,
        clip_right_word
    );
    if (left_distance >= 0) {
    ...
    upper_bound = right_distance > 0
        ? wrapping_subtract(
              wrapping_add(clip_right_word, scaled_run),
              1
          )
        : wrapping_add(destination_word, scaled_run);
    return true;
...
[[nodiscard]] LegacyScaledRleWriteStatus draw_row(
    ...
    while (true) {
        ...
        const i32 scaled_run = truncate_10_10_product(
            horizontal_step,
            run
        );
        i32 lower_bound{};
        ...
        if (literal) {
            ...
            if (can_write) {
                ...
                for (i32 index = 0;
                    ...
                    while (horizontal_phase >= 0x400) {
                        horizontal_phase = wrapping_subtract(
                            horizontal_phase,
                            0x400
                        );
                        reached_terminal_bound =
            ...
            horizontal_phase = wrapping_add(
                horizontal_phase,
                wrapping_multiply(horizontal_step, scaled_run)
            );
        }
...
[[nodiscard]] LegacyScaledRleWriteStatus write_scaled_rle(
    ...
    while (true) {
        ...
        vertical_phase = wrapping_add(
            vertical_phase,
            transform.vertical_step_10_10
        );
        while (vertical_phase >= 0x400) {
            if (destination_y >= clip.top) {
                vertical_phase = signed_remainder_10_10(
                    vertical_phase
                );
                goto render_rows;
            }
