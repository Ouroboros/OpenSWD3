...
LegacyGlyphAtlasProvider::LegacyGlyphAtlasProvider(
    ...
    for (std::size_t index = 0; index < sections_.size(); ++index) {
        ...
        const compat::u16 width = read_u16_le(atlas_bytes_, descriptor_offset);
        const compat::u16 height =
            read_u16_le(atlas_bytes_, descriptor_offset + 2U);
        const compat::u16 row_bytes =
            read_u16_le(atlas_bytes_, descriptor_offset + 4U);
        const compat::u16 mask_bytes =
            read_u16_le(atlas_bytes_, descriptor_offset + 6U);
        const compat::u32 data_offset =
            read_u32_le(atlas_bytes_, descriptor_offset + 8U);
        const compat::u32 data_size =
            read_u32_le(atlas_bytes_, descriptor_offset + 12U);
        const auto expected_geometry = kLegacyGlyphAtlasGeometries[index];
        const compat::u16 expected_row_bytes =
            static_cast<compat::u16>((expected_geometry[0] + 7U) / 8U);
        const compat::u16 expected_mask_bytes =
            static_cast<compat::u16>(expected_row_bytes * expected_geometry[1]);
        const std::size_t expected_data_size =
            static_cast<std::size_t>(kLegacyGlyphAtlasKeyCount) *
            expected_mask_bytes;

        if (width != expected_geometry[0] || height != expected_geometry[1] ||
...
LegacyGlyphProviderStatus LegacyGlyphAtlasProvider::provide_glyph_mask(
    ...
    const compat::u16 expected_key = static_cast<compat::u16>(
        ...
        static_cast<compat::u16>(static_cast<compat::u16>(second_byte) << 8U)
    );
    if ((first_byte < 0x80U &&
    ...
    const auto section = std::ranges::find_if(
        sections_, [glyph_width, glyph_height](const Section& candidate) {
    ...
    const std::size_t source_offset = section->data_offset +
        static_cast<std::size_t>(
            legacy_glyph_atlas_key_index(first_byte, second_byte)
        ) * section->mask_bytes;
    std::ranges::copy(
