...
public:
    ...
    [[nodiscard]] LegacyWorldDirectionProbeStatus validate() const noexcept {
        ...
        const std::size_t cell_count = static_cast<std::size_t>(map_width_) *
            static_cast<std::size_t>(map_height_);
        if (cell_count >
                std::numeric_limits<std::size_t>::max() / bytes_per_cell ||
    ...
    [[nodiscard]] bool
    read(const std::int64_t cell_index, u32& value) const noexcept {
...
[[nodiscard]] bool merge_masked_cell(
    const SurfaceReader& reader,
...
[[nodiscard]] bool scan_clear_stripe(
    const SurfaceReader& reader,
...
[[nodiscard]] i32
wrapping_add_unsigned(const u32 left, const i32 right) noexcept {
    return std::bit_cast<i32>(left + std::bit_cast<u32>(right));
}

void apply_horizontal_rule(
    const DirectionRule& rule,
    const u8 occupancy_mask,
    const u32 clearance_mask,
    i32& delta_x,
    i32& delta_y
) noexcept {
...
compute_legacy_world_directional_occupancy_mask(
    ...
    const u32 map_width,
    const u32 map_height,
    const u32 anchor_cell_index,
    const u32 footprint_width,
    const u32 footprint_height,
    const u32 cell_mask
) noexcept {
    ...
    if (footprint_width <= 2U) {
        for (std::size_t offset = 0U; offset < 3U; ++offset) {
            if (!merge_masked_cell(
                ...
                result.status = LegacyWorldDirectionProbeStatus::
        ...
        if (footprint_width == 2U &&
            (!merge_masked_cell(
                 reader, upper_left + 3, cell_mask, samples[3U]
             ) ||
             !merge_masked_cell(
                 reader, lower_left + 3, cell_mask, samples[11U]
             ))) {
            result.status =
        ...
        for (u32 offset = 0U; offset < footprint_width + 2U; ++offset) {
            if (!merge_masked_cell(
                    reader, upper_left + offset, cell_mask, samples[offset]
                ) ||
                !merge_masked_cell(
                    reader, lower_left + offset, cell_mask, samples[8U + offset]
                )) {
                result.status = LegacyWorldDirectionProbeStatus::
                    surface_access_out_of_bounds;
                return result;
    ...
    if (footprint_height <= 2U) {
        for (std::size_t offset = 0U; offset < 3U; ++offset) {
            const auto row_offset =
                static_cast<std::int64_t>(offset) * row_stride;
            if (!merge_masked_cell(
                    reader,
                    upper_left + row_offset,
                    cell_mask,
                    samples[16U + offset]
                ) ||
                !merge_masked_cell(
                    reader,
                    upper_right + row_offset,
                    cell_mask,
                    samples[24U + offset]
                )) {
                result.status = LegacyWorldDirectionProbeStatus::
        ...
        if (footprint_height == 2U &&
            (!merge_masked_cell(
                 reader, upper_left + 3 * row_stride, cell_mask, samples[19U]
             ) ||
             !merge_masked_cell(
                 reader, upper_right + 3 * row_stride, cell_mask, samples[27U]
             ))) {
            result.status =
                LegacyWorldDirectionProbeStatus::surface_access_out_of_bounds;
            return result;
...
LegacyWorldSurfaceClearanceResult compute_legacy_world_surface_clearance_mask(
    ...
    const i32 start_y,
    const u32 span_length,
    const LegacyWorldClearanceAxis axis
) noexcept {
    ...
    for (u32 distance = 0U; distance <= 2U; ++distance) {
        if (current_selected >= 0) {
            bool clear{};
            if (!scan_clear_stripe(
                    reader, negative_start, cell_step, span_length, clear
                )) {
                result.status = LegacyWorldDirectionProbeStatus::
                    surface_access_out_of_bounds;
                return result;
        ...
        if (positive_bound_test < selected_bound) {
            bool clear{};
            if (!scan_clear_stripe(
                    reader, positive_start, cell_step, span_length, clear
                )) {
                result.status = LegacyWorldDirectionProbeStatus::
                    surface_access_out_of_bounds;
                return result;
...
adjust_legacy_world_direction_for_obstacles(
    ...
    if (result.delta_x != 0 && result.delta_y == 0) {
        const DirectionRule rule = result.delta_x == -1
            ? DirectionRule{-1, 2U, 1U, 0x80U, 0x40U, 0x20U}
            : DirectionRule{
        ...
        const auto clearance = compute_legacy_world_surface_clearance_mask(
            ...
            std::bit_cast<i32>(role.world_y >> 4U),
            role.action.field_30,
        ...
        apply_horizontal_rule(
            rule, occupancy.mask, clearance.mask, result.delta_x, result.delta_y
        );
        return result;
    }
    ...
    const DirectionRule rule = same_sign
        ? (result.delta_x == -1 ? DirectionRule{-1, 1U, 0x80U, 2U, 0U, 0U}
                                : DirectionRule{1, 0x10U, 8U, 0x20U, 0U, 0U})
        : (result.delta_x == -1 ? DirectionRule{-1, 0x40U, 0x80U, 0x20U, 0U, 0U}
                                : DirectionRule{1, 4U, 8U, 2U, 0U, 0U});
    apply_diagonal_rule(
        rule, occupancy.mask, same_sign, result.delta_x, result.delta_y
    );
    result.status = LegacyWorldDirectionProbeStatus::completed;
    return result;
}
