...
[[nodiscard]] constexpr i32
wrapping_shift_left(const i32 value, const u32 count) noexcept {
...
[[nodiscard]] i32 spatial_distance(
    const LegacyWorldRoleRecord& left, const LegacyWorldRoleRecord& right
) noexcept {
    const i32 delta_x = wrapping_subtract(
        std::bit_cast<i32>(left.world_x), std::bit_cast<i32>(right.world_x)
    );
    const i32 delta_y = wrapping_subtract(
        std::bit_cast<i32>(left.world_y), std::bit_cast<i32>(right.world_y)
    );
    const u32 square_sum =
        static_cast<u32>(wrapping_multiply(delta_x, delta_x)) +
...
[[nodiscard]] constexpr i32
spatial_pan(const i32 target_x, const i32 listener_x) noexcept {
    return wrapping_shift_left(wrapping_subtract(target_x, listener_x), 6U) /
        0x200;
}

[[nodiscard]] constexpr i16
vertical_offset(const u32 target_y, const u32 listener_y) noexcept {
    const u16 difference = static_cast<u16>(
        static_cast<u16>(target_y) - static_cast<u16>(listener_y)
    );
...
LegacyWorldSpatialAudioResult update_legacy_world_spatial_audio(
    ...
    LegacyWorldSpatialAudioPorts& ports
) noexcept {
