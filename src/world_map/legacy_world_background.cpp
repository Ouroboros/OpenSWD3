...
[[nodiscard]] PixelRectangle
refresh_rectangle(const rendering::LegacyFramebuffer &framebuffer,
                  const LegacyWorldBackgroundView &view) noexcept {
...
[[nodiscard]] bool read_direct_pixel(const LegacyWorldBackgroundSource &source,
                                     const u16 tile_index, const u32 tile_x,
                                     const u32 tile_y, u16 &value) noexcept {
  ...
  const std::size_t pixel_offset =
      tile_offset +
      (static_cast<std::size_t>(tile_y) * kLegacyWorldTilePixels + tile_x) * 2U;
  ...
  value = static_cast<u16>(
      ...
      static_cast<u16>(static_cast<u16>(source.tile_bytes[pixel_offset + 1U])
                       << 8U));
  return true;
}

[[nodiscard]] bool read_indexed_pixel(const LegacyWorldBackgroundSource &source,
                                      const u16 tile_index, const u32 tile_x,
                                      const u32 tile_y, u8 &palette_index,
                                      u16 &value) noexcept {
  constexpr std::size_t kIndexedHeaderBytes = 0x200U;
  constexpr std::size_t kTileBytes = 0x100U;
  const std::size_t pixel_offset =
      kIndexedHeaderBytes + static_cast<std::size_t>(tile_index) * kTileBytes +
...
LegacyWorldBackgroundRenderResult
render_legacy_world_background(rendering::LegacyFramebuffer &framebuffer,
  ...
  if (source.tile_layer_offset > source.tile_indices.size() ||
    ...
    result.status = LegacyWorldBackgroundRenderStatus::tile_grid_out_of_bounds;
    return result;
  }
  if (cell_count > std::numeric_limits<std::size_t>::max() / 4U ||
      source.cell_flags.size() < cell_count * 4U) {
    result.status = LegacyWorldBackgroundRenderStatus::cell_grid_out_of_bounds;
    return result;
  }
  if (source.pixel_layout == LegacyWorldBackgroundPixelLayout::indexed_8 &&
      source.palette.size() < 256U) {
    result.status = LegacyWorldBackgroundRenderStatus::palette_out_of_bounds;
    return result;
  }
  ...
  for (i32 screen_y = first_screen_y, cell_y = first_cell_y;
    ...
    for (i32 screen_x = first_screen_x, cell_x = first_cell_x;
      ...
      if (screen_x + static_cast<i32>(kLegacyWorldTilePixels) <= redraw.left) {
        continue;
      }
      ...
      const bool transparent = (flags & kLegacyWorldCellTransparent) != 0U;
      if (transparent) {
      ...
      const u16 tile_index =
          source
              .tile_indices[static_cast<std::size_t>(source.tile_layer_offset) +
                            cell_index];
      ...
      const i32 clipped_right = std::min(
          screen_x + static_cast<i32>(kLegacyWorldTilePixels), redraw.right);
      const i32 clipped_bottom = std::min(
          screen_y + static_cast<i32>(kLegacyWorldTilePixels), redraw.bottom);
      for (i32 destination_y = clipped_top; destination_y < clipped_bottom;
           ++destination_y) {
        std::span<u16> row =
            framebuffer.row_pixels(static_cast<u32>(destination_y));
        const u32 tile_y = static_cast<u32>(destination_y - screen_y);
        for (i32 destination_x = clipped_left; destination_x < clipped_right;
             ++destination_x) {
          const u32 tile_x = static_cast<u32>(destination_x - screen_x);
          u16 pixel{};
          if (source.pixel_layout ==
              LegacyWorldBackgroundPixelLayout::direct_16) {
            if (!read_direct_pixel(source, tile_index, tile_x, tile_y, pixel)) {
              result.status =
            ...
            u8 palette_index{};
            if (!read_indexed_pixel(source, tile_index, tile_x, tile_y,
                                    palette_index, pixel)) {
              result.status =
                  LegacyWorldBackgroundRenderStatus::tile_source_out_of_bounds;
              return result;
