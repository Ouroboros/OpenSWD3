...
[[nodiscard]] u16 read_u16(const LegacyWorldObjectSlot &slot,
                           const std::size_t offset) noexcept {
  return static_cast<u16>(slot.bytes[offset]) |
         static_cast<u16>(static_cast<u16>(slot.bytes[offset + 1U]) << 8U);
}

void write_u16(LegacyWorldObjectSlot &slot, const std::size_t offset,
...
void copy_path(const LegacyWorldPathfindingResult &path,
...
[[nodiscard]] bool
clear_surface(LegacyWorldStoryPathResult &result,
              const LegacyWorldRoleRecord &role,
              const LegacyWorldStoryPathRuntime &runtime) noexcept {
  const auto cleared =
...
[[nodiscard]] bool
mark_surface(LegacyWorldStoryPathResult &result,
             const LegacyWorldRoleRecord &role,
             const LegacyWorldStoryPathRuntime &runtime) noexcept {
  const auto marked =
...
[[nodiscard]] bool relocate_role(LegacyWorldStoryPathResult &result,
                                 const LegacyWorldRoleRecord &role,
                                 LegacyWorldStoryPathRuntime &runtime,
                                 const i32 first_row = 0) noexcept {
  result.spatial_status = relocate_legacy_role_spatially_by_guid(
      *runtime.spatial_index, runtime.roles, role.guid, role.flags & 3U,
      first_row, true);
...
[[nodiscard]] bool reset_selected_motion(LegacyWorldStoryPathResult &result,
                                         const u32 role_index,
                                         const LegacyWorldRoleRecord &role,
                                         LegacyWorldStoryPathRuntime &runtime,
                                         const bool recenter) noexcept {
  ...
  std::ranges::fill(
      runtime.selected_arrival_bytes.first(kLegacyWorldGuidOneArrivalByteCount),
      u8{});
  if (recenter && (*runtime.scene_render_flags & 2U) == 0U) {
    recenter_legacy_world_camera(role, runtime.role_surface.map_width,
                                 runtime.map_height, *runtime.camera);
  }
  return true;
}

[[nodiscard]] std::size_t
find_role_slot(const LegacyWorldStoryPathRuntime &runtime,
...
void update_map_cell(LegacyWorldRoleRecord &role,
                     const LegacyWorldStoryPathRuntime &runtime) noexcept {
...
[[nodiscard]] bool
move_role_directly(LegacyWorldStoryPathResult &result, const u32 role_index,
                   LegacyWorldRoleRecord &role, const u16 destination_x,
                   const u16 destination_y,
                   LegacyWorldStoryPathRuntime &runtime,
                   const bool retain_path_ownership) noexcept {
...
[[nodiscard]] bool
align_selected_role(LegacyWorldStoryPathResult &result, const u32 role_index,
                    LegacyWorldRoleRecord &role,
                    LegacyWorldStoryPathRuntime &runtime) noexcept {
  if (role_index != runtime.selected_role_index) {
    return true;
  }
  const i32 first_row = static_cast<i32>(role.world_y >> 4U) - 1;
  const u32 step_x = std::bit_cast<u32>(runtime.movement->camera_x_transition) +
                     std::bit_cast<u32>(runtime.movement->player_x_transition);
  const u32 step_y = std::bit_cast<u32>(runtime.movement->camera_y_transition) +
...
[[nodiscard]] bool align_nonselected_role_from_slot(
    LegacyWorldStoryPathResult &result, LegacyWorldRoleRecord &role,
...
[[nodiscard]] bool
inside_story_view(const LegacyWorldRoleRecord &role,
...
[[nodiscard]] bool
finish_scheduled_path(LegacyWorldStoryPathResult &result, const u32 role_index,
  ...
  while (true) {
    ...
    ++cursor;
    role.world_x +=
        std::bit_cast<u32>(kSubCellStepX[direction] * kOffscreenPathStepScale);
    role.world_y +=
        std::bit_cast<u32>(kSubCellStepY[direction] * kOffscreenPathStepScale);
  ...
  if (direction == 0xFFU) {
    ...
    if (action_id != 0xFFFFU) {
      role.action.action_id =
          static_cast<u32>(static_cast<i32>(std::bit_cast<i16>(action_id)));
    }
    if (base_variant != 0xFFFFU) {
      role.action.base_variant =
          static_cast<u32>(static_cast<i32>(std::bit_cast<i16>(base_variant)));
    }
    if (variant_delta != 0xFFFFU) {
      role.action.variant_delta =
          static_cast<u32>(static_cast<i32>(std::bit_cast<i16>(variant_delta)));
    }
  } else {
  ...
  try {
    ...
    if (result.existing_slot_found) {
      ...
      if ((slot.bytes[kPathFlagsOffset] & 0x0FU) == 1U) {
        ...
        write_u16(slot, kBaseVariantOffset,
                  static_cast<u16>(request.base_variant));
        write_u16(slot, kVariantDeltaOffset,
    ...
    if (((role.world_x | role.world_y) & 0x0FU) != 0U) {
      if (request.role_index == runtime.selected_role_index) {
        if (!align_selected_role(result, request.role_index, role, runtime)) {
          return result;
        }
      } else if (result.existing_slot_found &&
                 !align_nonselected_role_from_slot(
                     result, role, runtime.active_object_slots[slot_index],
                     runtime)) {
        return result;
      }
    }
    ...
    if (slot_index == kLegacyWorldActiveObjectSlotCount) {
      if (!move_role_directly(result, request.role_index, role,
    ...
    LegacyWorldObjectSlot &slot = runtime.active_object_slots[slot_index];
    write_u16(slot, kActionIdOffset, static_cast<u16>(request.action_id));
    write_u16(slot, kBaseVariantOffset, static_cast<u16>(request.base_variant));
    write_u16(slot, kVariantDeltaOffset,
              static_cast<u16>(request.variant_delta));
    ...
    if (!finish_scheduled_path(result, request.role_index, role, slot,
                               runtime)) {
      return result;
    }
    result.legacy_return_value = 1;
...
LegacyWorldStoryPathResult
query_legacy_world_story_path(LegacyWorldStoryPathRuntime &runtime,
                              const u32 role_index) noexcept {
  LegacyWorldStoryPathResult result;
  if (!runtime_is_available(runtime)) {
    result.status = LegacyWorldStoryPathStatus::runtime_unavailable;
  ...
  if ((role.flags & kCollisionBypassFlag) == 0U) {
    const auto occupancy = compute_legacy_world_directional_occupancy_mask(
        runtime.role_surface.surface_grid, runtime.role_surface.map_width,
        runtime.map_height, role.map_cell_pointer_32, role.action.field_2c,
        role.action.field_30, kStoryCollisionMask);
    result.directional_probe_status = occupancy.status;
    if (occupancy.status != LegacyWorldDirectionProbeStatus::completed) {
      result.status = LegacyWorldStoryPathStatus::directional_probe_failed;
      return result;
    }
...
LegacyWorldStoryPathResult
complete_legacy_world_story_path(LegacyWorldStoryPathRuntime &runtime,
                                 const u32 role_index) noexcept {
  LegacyWorldStoryPathResult result;
  if (!runtime_is_available(runtime)) {
    result.status = LegacyWorldStoryPathStatus::runtime_unavailable;
