...
[[nodiscard]] u32 bounded_role_count(
...
[[nodiscard]] u32
limited_scan_extent(const u32 coordinate, const u32 map_extent) noexcept {
    u32 extent = 5U;
    if (coordinate <= 5U) {
        extent = coordinate;
    }
...
[[nodiscard]] bool scan_north_west(
    LegacyWorldFacingRoleQueryResult& result,
    const std::span<const LegacyWorldRoleRecord> roles,
    const u32 role_count,
    const u32 tile_x,
...
[[nodiscard]] bool scan_north(
    LegacyWorldFacingRoleQueryResult& result,
    const std::span<const LegacyWorldRoleRecord> roles,
    const u32 role_count,
    const u32 tile_x,
    const u32 tile_y,
    const u32 player_width,
    const u32 y_extent
) noexcept {
    ...
    for (u32 y = 1U; y <= y_extent; ++y) {
        for (u32 x = 0U; x < width; ++x) {
            if (query_tile(
                    result, roles, role_count, tile_x + x - 1U, tile_y - y
                )) {
                return true;
            }
        }
...
[[nodiscard]] bool scan_north_east(
    LegacyWorldFacingRoleQueryResult& result,
    const std::span<const LegacyWorldRoleRecord> roles,
    const u32 role_count,
    const u32 tile_x,
...
[[nodiscard]] bool scan_west(
    ...
    for (u32 x = 1U; x <= x_extent; ++x) {
        for (u32 y = 0U; y < height; ++y) {
            if (query_tile(
                    result, roles, role_count, tile_x - x, tile_y + y - 1U
                )) {
                return true;
            }
        }
...
[[nodiscard]] bool scan_east(
    ...
    for (u32 distance = 0U; distance < x_extent; ++distance, ++candidate_x) {
        for (u32 y = 0U; y < height; ++y) {
            if (query_tile(
                    result, roles, role_count, candidate_x, tile_y + y - 1U
                )) {
                return true;
            }
        }
...
[[nodiscard]] bool scan_south_west(
    LegacyWorldFacingRoleQueryResult& result,
    const std::span<const LegacyWorldRoleRecord> roles,
    const u32 role_count,
    const u32 tile_x,
...
[[nodiscard]] bool scan_south(
    ...
    for (u32 distance = 0U; distance < y_extent; ++distance, ++candidate_y) {
        for (u32 x = 0U; x < width; ++x) {
            if (query_tile(
                    result, roles, role_count, tile_x + x - 1U, candidate_y
                )) {
                return true;
            }
        }
...
[[nodiscard]] bool scan_south_east(
    LegacyWorldFacingRoleQueryResult& result,
    const std::span<const LegacyWorldRoleRecord> roles,
    const u32 role_count,
    const u32 tile_x,
...
LegacyWorldFacingRoleQueryResult find_legacy_world_facing_role(
    ...
    switch (selector) {
    case 0U:
        found = scan_north_west(
            result, roles, scan_role_count, tile_x, tile_y, x_extent, y_extent
        );
        break;
    case 1U:
        found = scan_north(
