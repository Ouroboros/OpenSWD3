...
[[nodiscard]] u16 pack_bgr888(
    const u32 color, const rendering::LegacyPixelConversionState& format
) noexcept {
    return static_cast<u16>(rendering::legacy_pack_color_pair(
        format,
        static_cast<i32>((color >> 3U) & 0x1FU),
...
[[nodiscard]] bool color_at(
    const std::array<u32, 16U>& source,
    const u16 index,
...
[[nodiscard]] constexpr rendering::LegacyBlitClipRectangle
...
accepted_text_status(const rendering::LegacyTextDrawStatus status) noexcept {
...
public:
    ...
    [[nodiscard]] bool load_frame_piece(
        ...
        try {
            return ports_.load_frame_piece(
                static_cast<u16>(resource_id),
                static_cast<u16>(piece_index),
                piece
            );
        } catch (...) {
            return false;
...
caption_length(const std::span<const u8> text) noexcept {
...
LegacyWorldDialogRuntimeState::LegacyWorldDialogRuntimeState()
    : text_surface(
          rendering::LegacySurfaceGeometry{
...
void LegacyWorldDialogRuntimePorts::clear_text_surface() noexcept {
    if (surface_active_) {
        ...
        std::ranges::fill(
            state_.text_surface.physical_pixels(), transparent_pixel
        );
    }
}
...
void LegacyWorldDialogRuntimePorts::draw_dialog_indicator(
    ...
    auto& action =
        request.kind == story_scene::LegacyDialogIndicatorKind::end_dialog
        ? state_.end_dialog_action
        : state_.next_page_action;
    rendering::LegacyFramePiece piece;
    try {
        if (!action_ports_.load_frame_piece(
                action.field_4a, action.field_4c, piece
            )) {
            return;
        }
        const i32 x = wrapping_subtract(
            wrapping_subtract(
                request.panel.right, from_bits(action.draw_offset_x)
            ),
            4
        );
        const i32 y = wrapping_subtract(
            wrapping_subtract(
                request.panel.bottom, from_bits(action.draw_offset_y)
            ),
            8
        );
...
void LegacyWorldDialogRuntimePorts::draw_dialog_caption(
    ...
    const i32 background_width =
        wrapping_add(wrapping_multiply(byte_length, 8), 0x1C);
    ...
    for (i32 row = 0; row < 15; ++row) {
        ...
        auto destination = framebuffer_.row_pixels(static_cast<u32>(y))
                               .subspan(
    ...
    static_cast<void>(rendering::draw_legacy_text(
        framebuffer_,
        *text_16_.glyph_cache,
        *text_16_.glyph_provider,
        text_state,
...
void LegacyWorldDialogRuntimePorts::release_message_owner(
    ...
    if (role_index < roles_.size()) {
        ...
        role_index == kLegacyWorldTalkMapEventSource && talk_context_ != nullptr
    ) {
...
void LegacyWorldDialogRuntimePorts::restore_text_destination(
    i32, i32
) noexcept {}
...
bool LegacyWorldDialogRuntimePorts::draw_segment(
    ...
    u16 secondary{};
    if (!color_at(
            kLegacyPrimaryBgr888,
            request.foreground_index,
            pixel_conversion_,
            foreground
        ) ||
        !color_at(
            kLegacySecondaryBgr888,
    ...
    try {
        return accepted_text_status(
            rendering::draw_legacy_text(
                state_.text_surface,
                *text_20_.glyph_cache,
                *text_20_.glyph_provider,
                text_state,
                rendering::LegacyTextDrawRequest{
                    .destination_x = request.destination_x,
                    .destination_y = request.destination_y,
