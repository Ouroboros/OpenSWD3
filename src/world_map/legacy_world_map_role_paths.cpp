...
[[nodiscard]] i32 read_i16_as_i32(const LegacyWorldObjectSlot &slot,
                                  const std::size_t offset) noexcept {
...
[[nodiscard]] u32 scaled_step_bits(const i32 value, const bool double_once,
                                   const bool double_twice) noexcept {
...
[[nodiscard]] bool clear_and_move_surface_cell(
    LegacyWorldMapRolePathResult &result, LegacyWorldRoleRecord &role,
    const LegacyWorldRoleSurfaceContext &surface_context,
    const u8 direction) noexcept {
  const auto cleared =
...
[[nodiscard]] bool refresh_cell_flags(
  ...
  if (refresh_legacy_world_role_cell_flags(role,
...
void recenter_camera(LegacyWorldMapRolePathResult &result,
                     const LegacyWorldRoleRecord &role,
                     const LegacyRoleSpatialIndex &spatial_index,
                     const LegacyWorldRoleSurfaceContext &surface_context,
                     LegacyWorldCameraRect &camera) noexcept {
  recenter_legacy_world_camera(role, surface_context.map_width,
                               spatial_index.map_height, camera);
  ++result.camera_recenter_count;
}

[[nodiscard]] bool create_automatic_talk_context(
    LegacyWorldMapRolePathResult &result, const LegacyWorldRoleRecord &role,
    const LegacyRoleSpatialIndex &spatial_index,
    const LegacyWorldRoleSurfaceContext &surface_context,
  ...
  const auto occupancy = compute_legacy_world_directional_occupancy_mask(
      surface_context.surface_grid, surface_context.map_width,
      spatial_index.map_height, role.map_cell_pointer_32, role.action.field_2c,
      role.action.field_30, kAutomaticTalkCellMask);
...
[[nodiscard]] bool handle_arrival(
    LegacyWorldMapRolePathResult &result, const u32 role_index,
    LegacyWorldRoleRecord &role, LegacyWorldObjectSlot &slot,
    const u8 runtime_flags, LegacyWorldMovementRuntimeState &movement,
    LegacyWorldCameraRect &camera, LegacyWorldMapRolePathState &state,
    const LegacyRoleSpatialIndex &spatial_index,
    const LegacyWorldRoleSurfaceContext &surface_context,
    LegacyWorldMapRolePathPorts &path_ports) noexcept {
  ...
  if (slot.bytes[kPathFlagsOffset] < 0x80U) {
    ...
    if (!path_ports.complete_role_path(role_index)) {
      result.status = LegacyWorldMapRolePathStatus::path_completion_port_failed;
      return false;
    }
  ...
  if (role.guid == 1U) {
    ...
    if ((runtime_flags & kCameraRecenterSuppressedFlag) == 0U) {
      recenter_camera(result, role, spatial_index, surface_context, camera);
    }
  }
...
LegacyWorldMapRolePathResult advance_legacy_world_map_role_paths(
    ...
    const LegacyWorldRoleSurfaceContext &surface_context,
    const u32 selected_role_index, const u8 runtime_flags,
  ...
  if (selected_role_index >= roles.size()) {
    result.status = LegacyWorldMapRolePathStatus::invalid_selected_role_index;
    return result;
  }
  ...
  for (LegacyWorldObjectSlot &slot : state.active_object_slots) {
    ...
    if (role.action.wait_remaining == 0U) {
      ...
      if (direction_offset >= slot.bytes.size()) {
        result.status = LegacyWorldMapRolePathStatus::path_byte_out_of_range;
        return result;
      }
      const u8 direction = slot.bytes[direction_offset];
      if (direction != 0xFFU && direction >= kCellStepX.size()) {
        result.status = LegacyWorldMapRolePathStatus::direction_out_of_range;
        return result;
      }

      const bool base_double = role.action.field_94 == 0U;
      const bool flag_double = (role.flags & kDoublePathStepFlag) != 0U;
      const u32 step_x = scaled_step_bits(read_i16_as_i32(slot, kStepXOffset),
                                          base_double, flag_double);
      const u32 step_y = scaled_step_bits(read_i16_as_i32(slot, kStepYOffset),
                                          base_double, flag_double);
      if (direction != 0xFFU) {
      ...
      if (((role.world_x | role.world_y) & 0x0FU) == 0U) {
        ...
        if (!relocate_aligned_role(result, role, roles, spatial_index) ||
            !handle_arrival(result, role_index, role, slot, runtime_flags,
                            movement, camera, state, spatial_index,
                            surface_context, path_ports) ||
            !clear_and_move_surface_cell(result, role, surface_context,
                                         direction)) {
          return result;
        }

        const u16 current_cursor = read_u16_le(slot, kPathCursorOffset);
        write_u16_le(slot, kPathCursorOffset,
                     static_cast<u16>(current_cursor + 1U) |
      ...
      if (!create_automatic_talk_context(result, role, spatial_index,
                                         surface_context, state.talk_context)) {
        return result;
      }
    }
