...
[[nodiscard]] bool
cm_cache_is_ready(const LegacyCmCacheLoadStatus status) noexcept {
    return status == LegacyCmCacheLoadStatus::ready_hit ||
        status == LegacyCmCacheLoadStatus::ready_generated;
}

[[nodiscard]] bool
ensure_cache_directory(const std::filesystem::path& directory) noexcept {
    if (directory.empty()) {
        return false;
    }
...
void decode_indexed_palette(
    ...
    for (std::size_t index = 0U; index < kIndexedPaletteEntries; ++index) {
...
LegacyWorldRenderSessionResult load_legacy_world_render_session(
    ...
    return load_legacy_world_render_session(
        request, map_source, cm_cache_source, {}
    );
}
...
LegacyWorldRenderSessionResult load_legacy_world_render_session(
    ...
    if (pixel_bits == 16U) {
        ...
        rendering::legacy_convert_pixels_forward(
            request.pixel_conversion, &result.session.transparent_pixel, 1
        );
    } else if (pixel_bits == 8U) {
        if (result.session.cm_cache.cache_bytes.size() < kIndexedPaletteBytes) {
            result.status =
        ...
        result.status = LegacyWorldRenderSessionStatus::unsupported_pixel_bits;
        return result;
    }
