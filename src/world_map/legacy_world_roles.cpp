...
constexpr std::array<i32, 16U> kAdditiveOffsets{0, 2, 4,   6,   8,  10, 12, 0,
                                                0, 0, -12, -10, -8, -6, -4, -2};
...
void record_draw(const rendering::LegacyBlitExecutionStatus status,
                 LegacyWorldRoleDrawResult &result) noexcept {
...
[[nodiscard]] i32 normal_x(const LegacyWorldRoleRecord &role,
                           const LegacyActionRecord &action,
                           const LegacyWorldRenderCamera camera) noexcept {
  return wrapping_add(
      wrapping_subtract(wrapping_subtract(static_cast<i32>(role.field_28),
                                          field_as_i32(action.draw_offset_x)),
                        camera.left),
      field_as_i32(role.world_x));
}

[[nodiscard]] i32 normal_y(const LegacyWorldRoleRecord &role,
                           const LegacyActionRecord &action,
                           const LegacyWorldRenderCamera camera) noexcept {
  return wrapping_add(
      wrapping_subtract(wrapping_subtract(static_cast<i32>(role.field_2a),
                                          field_as_i32(action.draw_offset_y)),
                        camera.top),
      field_as_i32(role.world_y));
}

void draw_ghost(const LegacyWorldRoleRecord &role,
  ...
  const i32 base_x =
      wrapping_subtract(wrapping_subtract(field_as_i32(role.world_x),
                                          field_as_i32(action.draw_offset_x)),
                        state.camera.left);
  const i32 base_y =
      wrapping_subtract(wrapping_subtract(field_as_i32(role.world_y),
                                          field_as_i32(action.draw_offset_y)),
                        state.camera.top);
...
terminated_label(const std::span<const u8> bytes,
                 bool &terminator_found) noexcept {
...
LegacyWorldRoleDrawResult
draw_legacy_world_role(LegacyWorldRoleRecord &role,
                       const LegacyWorldRoleRenderState &state,
                       rendering::LegacyRleRowJitterState &jitter,
                       LegacyWorldRoleRenderPorts &ports) {
  ...
  if (role.action.field_58 != 0U) {
    ports.play_positional_sample(role.action.field_58,
                                 field_as_i32(role.world_x),
                                 field_as_i32(role.world_y));
  ...
  if (role.field_3c != 0U) {
    ...
    ++result.frame_requests;
    if (!ports.load_frame(overlay->field_4a, overlay->field_4c,
                          overlay_frame)) {
      result.status = LegacyWorldRoleDrawStatus::frame_load_failed;
      role.action.field_89 = static_cast<u8>(jitter.phase_bytes);
      return result;
    ...
    const i32 overlay_y = wrapping_add(
        wrapping_add(
            wrapping_subtract(
                wrapping_subtract(
                    wrapping_subtract(static_cast<i32>(role.field_2a),
    ...
    record_draw(ports.draw_frame(overlay_frame, overlay_request, jitter),
                result);
  ...
  if ((role.flags & kLegacyWorldRoleParticleBit) != 0U &&
    ...
    ports.emit_role_particles(field_as_i32(role.world_x),
                              field_as_i32(role.world_y), role.guid);
  ...
  const i32 label_y = wrapping_subtract(
      wrapping_subtract(field_as_i32(role.world_y),
                        field_as_i32(role.action.draw_offset_y)),
      state.camera.top);
  ports.draw_label(label, label_x, label_y,
                   ports.label_color(role.path_payload_relation), 4U);
...
LegacyWorldRoleRenderRuntimePorts::LegacyWorldRoleRenderRuntimePorts(
    ...
    LegacyWorldRoleExternalPorts &external_ports) noexcept
...
void LegacyWorldRoleRenderRuntimePorts::play_positional_sample(
    const u16 sound_id, const i32 world_x, const i32 world_y) noexcept {
...
LegacyWorldRoleRenderRuntimePorts::draw_frame(
  ...
  return rendering::blit_legacy_copy_paths(
             framebuffer_, current_clip(raster_), frame.source,
...
void LegacyWorldRoleRenderRuntimePorts::draw_label(
    const std::span<const u8> bytes, const i32 x, const i32 y, const u16 color,
    const u32 style) noexcept {
  external_ports_.draw_label(bytes, x, y, color, style);
}

LegacyWorldRolesResult
draw_legacy_world_roles(const LegacyRoleSpatialIndex &spatial_index,
  ...
  const std::span<const LegacyWorldRoleRecord> role_view{roles.data(),
                                                         roles.size()};
  for (const std::size_t group : kGroupOrder) {
    ...
    for (u32 scan = 0U; scan < 70U; ++scan, ++row) {
      ...
      while (role_index != kLegacySpatialNoRole) {
        ...
        const LegacyWorldRoleDrawResult draw =
            draw_legacy_world_role(role, render_state, jitter, render_ports);
        ...
        if (static_cast<u16>(role.field_2c) != 0U) {
          ...
          const LegacyWorldSpatialAudioResult audio =
              update_legacy_world_spatial_audio(role, role_view, audio_state,
                                                audio_ports);
          ...
          if (audio.status != LegacyWorldSpatialAudioStatus::completed) {
            result.status = LegacyWorldRolesStatus::spatial_audio_failed;
            return result;
          }
        }
