...
LegacyWorldPlayerControlResult prepare_legacy_world_player_control(
    ...
    if (request.camera_x_transition != 0U ||
        request.player_x_transition != 0U ||
        request.camera_y_transition != 0U ||
        request.player_y_transition != 0U || request.input_suppression != 0U ||
        request.special_mode_state != 0U) {
        return result;
    }
