...
[[nodiscard]] constexpr u32 wrapping_scaled_add(const u32 value,
                                                const u32 scale) noexcept {
...
void update_action(LegacyWorldFacingTalkResult &result,
                   LegacyActionRecord &action,
                   LegacyWorldFacingTalkPorts &ports) noexcept {
...
LegacyWorldFacingTalkResult coordinate_legacy_world_facing_talk(
    ...
    LegacyWorldTalkContext &talk_context, u32 &one_shot_interaction_state,
    LegacyWorldFacingTalkPorts &ports) noexcept {
  ...
  const LegacyWorldFacingRoleQueryResult query = find_legacy_world_facing_role(
      roles, static_cast<u32>(roles.size()), request.player_index,
      request.delta_x, request.delta_y, request.map_width, request.map_height);
