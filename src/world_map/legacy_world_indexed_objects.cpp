...
[[nodiscard]] constexpr i32
wrapping_multiply(const i32 left, const u16 right) noexcept {
...
[[nodiscard]] bool intersect_rectangles(
    const Rectangle& first, const Rectangle& second, Rectangle& intersection
) noexcept {
...
[[nodiscard]] i32 parallax_offset(
    const u16 factor,
    const i32 viewport_start,
    const i32 viewport_end,
    const i32 object_start,
    const i32 relative_intersection_start
) noexcept {
    if (factor == 8U) {
...
LegacyWorldIndexedObjectRuntimeDrawPorts::draw_object(
    const LegacyWorldIndexedObject& object,
...
LegacyWorldIndexedObjectDrawResult draw_legacy_world_indexed_objects(
    ...
    LegacyWorldIndexedObjectDrawPorts& ports
) noexcept {
    ...
    for (u32 ordinal = 0U; ordinal < 31U; ++ordinal) {
        ...
        for (auto iterator = objects.rbegin(); iterator != objects.rend();
            ...
            if (!intersect_rectangles(
                    viewport_rectangle, object_rectangle, intersection
                )) {
                continue;
            }
            ...
            const i32 relative_right = wrapping_add(
                relative_left,
                wrapping_subtract(intersection.right, intersection.left)
            );
            const i32 relative_bottom = wrapping_add(
                relative_top,
                wrapping_subtract(intersection.bottom, intersection.top)
            );
            ports.set_clip(
            ...
            const i32 horizontal_offset = parallax_offset(
                object.horizontal_factor,
                viewport.left,
                viewport.right,
                object_rectangle.left,
                relative_left
            );
            const i32 vertical_offset = parallax_offset(
                object.vertical_factor,
                viewport.top,
                viewport.bottom,
            ...
            result.last_blit_status = ports.draw_object(
                object,
                wrapping_subtract(relative_left, horizontal_offset),
