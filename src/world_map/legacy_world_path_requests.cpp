...
wrapping_add(const u32 left, const i32 right) noexcept {
...
[[nodiscard]] u32
clamped_view_start(const u32 value, const u32 margin) noexcept {
    const u32 candidate = value - margin;
    return std::bit_cast<i32>(candidate) < 0 ? 0U : candidate;
}

[[nodiscard]] bool inside_follower_view(
...
[[nodiscard]] bool preadvance_party_role(
    LegacyWorldPartyPathPreparationResult& result,
    LegacyWorldRoleRecord& role,
    LegacyWorldObjectSlot& slot,
...
[[nodiscard]] bool finish_party_slot(
    LegacyWorldPartyPathPreparationResult& result,
    LegacyWorldRoleRecord& role,
    LegacyWorldObjectSlot& slot,
    const LegacyWorldRoleSurfaceContext& surface_context,
    const u32 map_height,
    LegacyWorldPartyPathPorts& ports,
    const LegacyWorldRoleRecord& selected_role
) noexcept {
...
[[nodiscard]] bool prepare_one_party_path(
    ...
    const std::size_t history_index =
        static_cast<std::size_t>(party_index) * 2U;
    ...
    const u16 existing_cursor = static_cast<u16>(
        read_u16_le(slot, kPathCursorOffset) & kPathCursorMask
    );
...
LegacyWorldRolePathRequestResult request_legacy_world_role_path(
    const u32 role_index,
    const std::span<const u8> path_command,
    const std::span<LegacyWorldRoleRecord> roles,
    const LegacyWorldRoleSurfaceContext& surface_context,
    const u32 map_height,
    ...
    if (path.legacy_return_value == 1) {
        if (!path_fits_slot(path)) {
            result.status =
                LegacyWorldRolePathRequestStatus::path_does_not_fit_slot;
            return result;
        }
...
LegacyWorldPartyPathPreparationResult prepare_legacy_world_party_paths(
    ...
    for (u32 role_index = 1U; role_index < roles.size(); ++role_index) {
        ...
        if ((flags & kSpatialRoleFlag) == 0U ||
            (flags & kPartyRoleFlag) == 0U) {
            continue;
        }
        ++result.eligible_roles;
        if (!prepare_one_party_path(
                result,
                node_pool,
                role_index,
                party_role_count,
