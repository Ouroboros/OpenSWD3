...
[[nodiscard]] bool
checked_add(const u32 left, const u32 right, u32& result) noexcept {
...
LegacyWorldSpecialFrameLoader::LegacyWorldSpecialFrameLoader(
    ...
    : archive_path_(std::move(archive_path)), map_offset_(map_offset),
      records_(records), pixel_conversion_(pixel_conversion) {}

bool LegacyWorldSpecialFrameLoader::load_special_frame(
    const u16 variant_index, asset_runtime::LegacyTswRuntimeFrame& frame
) {
    frame = {};
    ...
    if (!checked_add(map_offset_, record.relative_offset, record_offset) ||
        ...
        record_offset >
            static_cast<u32>(std::numeric_limits<compat::i32>::max()) ||
    ...
    if (file.seek_begin_one_based(std::bit_cast<compat::i32>(record_offset)) ==
        0U) {
    ...
    const std::span<const u8> header{
        record_bytes.data(), kSpecialFrameHeaderSize
    };
    ...
    const auto decompression_status =
        resource_io::decompress_legacy_resource_block(
            std::span<const u8>{record_bytes}.subspan(
                kSpecialFrameHeaderSize, compressed_size
            ),
            decompressed,
            actual_output_size
        );
    ...
    if (depth == 16U) {
        ...
        const auto conversion_status =
            rendering::convert_legacy_image_command_stream_literals_in_place(
                decompressed, pixel_conversion_, &command_header
            );
        ...
        auto converted =
            rendering::convert_legacy_embedded_palette_image_command_stream(
                decompressed, pixel_conversion_
            );
        if (converted.status !=
            rendering::LegacyImageCommandStreamStatus::completed) {
            last_status_ = LegacyWorldSpecialFrameStatus::conversion_failed;
