...
[[nodiscard]] bool valid_framebuffer(
...
void set_partial_clip(
    ...
    rendering::set_legacy_clip_rectangle(
        raster,
        wrapping_subtract(state.partial_focus_x, 0xC0),
...
[[nodiscard]] bool execute_common_tail(
    const LegacyWorldFrameState& state,
    LegacyWorldFramePorts& ports,
    LegacyWorldFrameCompositionResult& result
) noexcept {
    if (!execute_stage(
            ports, result, LegacyWorldFrameStage::packed_row_effects_00414e50
        ) ||
        !execute_stage(
            ports, result, LegacyWorldFrameStage::timed_ui_update_0042ed40
        ) ||
        !execute_stage(
            ports, result, LegacyWorldFrameStage::role_head_sprites_00414ce0
        )) {
        return false;
    }
    ...
    if (!indicator_requires_control || query_control(ports, result, 0x2EU)) {
        if (!execute_stage(
                ports, result, LegacyWorldFrameStage::world_indicator_004149b0
            )) {
            return false;
        }
    ...
    return execute_stage(
               ports, result, LegacyWorldFrameStage::frame_color_update_004146f0
           ) &&
        execute_stage(
               ports, result, LegacyWorldFrameStage::timed_messages_004153d0
        );
}

}  // namespace

LegacyWorldFrameCompositionResult compose_legacy_world_frame(
    rendering::LegacyFramebuffer& framebuffer,
    rendering::LegacyRasterGeometryState& raster,
    ...
    if (state.ani_activity_active) {
        ...
        if (!execute_stage(
                ports, result, LegacyWorldFrameStage::ani_activity_004154a0
            )) {
            set_full_clip(raster, result);
            return result;
        }
        ...
        if ((state.runtime_flags & kLegacyWorldFrameClearOnly) != 0U) {
            ...
            if (result.background.status !=
                ...
                set_full_clip(raster, result);
                result.status =
                    LegacyWorldFrameCompositionStatus::background_failed;
                return result;
            }
            ...
            if (!query_service(ports, result, 0x48U) &&
                ...
                if (!execute_stage(
                        ports, result, LegacyWorldFrameStage::ani_drift_004161c0
                    ) ||
                    !execute_stage(
                        ports,
                        result,
                    ...
                    !execute_stage(
                        ports, result, LegacyWorldFrameStage::ani_spark_004167b0
                    ) ||
                    !execute_stage(
                        ports,
                        result,
