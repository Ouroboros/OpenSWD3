...
[[nodiscard]] u32 radial_distance_difference(
    const LegacyWorldRoleRecord &current,
    const LegacyWorldRoleRecord &target) noexcept {
...
resolve_path_command(const std::span<const u8> path_database,
  ...
  const std::int64_t command_offset =
      ...
      static_cast<std::int64_t>(std::bit_cast<compat::i32>(
          role.path_word_index)) * static_cast<std::int64_t>(sizeof(u16));
  if (command_offset < 0 ||
      !range_available(path_database, static_cast<std::size_t>(command_offset),
                       sizeof(u16))) {
    return {LegacyWorldPathScriptStatus::path_command_out_of_range, 0U};
  }
  return {LegacyWorldPathScriptStatus::completed,
          static_cast<std::size_t>(command_offset)};
}

[[nodiscard]] bool transfer_path_cursor(
    const std::span<const u8> path_database, LegacyWorldRoleRecord &role,
    const u32 target_relative_offset) noexcept {
  const std::size_t directory_offset =
      kPathDatabasePayloadOffset +
      static_cast<std::size_t>(role.path_data_id) * sizeof(u32);
  if (!range_available(path_database, directory_offset, sizeof(u32))) {
  ...
  role.path_word_index = std::bit_cast<u32>(
      std::bit_cast<compat::i32>(byte_difference) >> 1U);
...
void advance_cursor(LegacyWorldRoleRecord &role,
                    LegacyWorldPathScriptResult &result,
                    const u32 word_count) noexcept {
  role.path_word_index += word_count;
  result.cursor_words_advanced += word_count;
}

void record_action_update(LegacyWorldRoleRecord &role,
                          LegacyWorldPathScriptResult &result,
...
[[nodiscard]] LegacyWorldObjectSlot *find_role_slot(
    ...
    const std::span<LegacyWorldObjectSlot> object_slots) noexcept {
  for (LegacyWorldObjectSlot &slot :
       object_slots.first(std::min(object_slots.size(),
...
[[nodiscard]] bool initialize_random_walk_slot(
    ...
    LegacyWorldPathScriptResult &result) noexcept {
  if (object_slots.size() < kLegacyWorldActiveObjectSlotCount) {
  ...
  if (selected == nullptr) {
    for (LegacyWorldObjectSlot &slot :
      ...
      selected = &slot;
      write_slot_u16(slot, kRoleIndexOffset, static_cast<u16>(role_index));
...
LegacyWorldPathScriptResult run_legacy_world_path_script(
  ...
  for (;;) {
    ...
    case 10U: {
      if (!range_available(path_database, command.offset, 4U * sizeof(u16))) {
        result.status = LegacyWorldPathScriptStatus::path_command_truncated;
        return result;
      }
      if (runtime.shared_script_state == nullptr) {
      ...
      if (threshold <= clock) {
        if (!transfer_path_cursor(
                path_database, role,
                read_u32_le(path_database,
                            command.offset + 2U * sizeof(u16)))) {
          result.status = LegacyWorldPathScriptStatus::path_command_out_of_range;
          return result;
        }
    ...
    case 16U: {
      ...
      while (true) {
        ...
        if (query_legacy_world_story_flag(*runtime.shared_script_state, flag)) {
          ++set_count;
        }
        ++flag_count;
      ...
      const bool take = opcode == 15U ? set_count == flag_count
                                      : set_count >= 1U;
      if (take) {
        if (!transfer_path_cursor(
                path_database, role,
    ...
    case 23U: {
      ...
      u32 &variable = runtime.shared_script_state->script_variables[index];
      if (opcode == 19U) {
        variable = value;
        advance_cursor(role, result, 3U);
        break;
      ...
      if (!range_available(path_database, command.offset, 5U * sizeof(u16))) {
        result.status = LegacyWorldPathScriptStatus::path_command_truncated;
        return result;
      }
      const bool take = opcode == 22U ? variable >= value : variable <= value;
      if (take) {
        if (!transfer_path_cursor(
                path_database, role,
                read_u32_le(path_database,
                            command.offset + 3U * sizeof(u16)))) {
          result.status = LegacyWorldPathScriptStatus::path_command_out_of_range;
          return result;
        }
        ++result.conditional_transfers;
    ...
    case 24U: {
      ...
      const ResolvedRoleSelector target_selector = resolve_role_selector(
          roles, read_u16_le(path_database, command.offset + sizeof(u16)),
      ...
      if (LegacyWorldObjectSlot *slot = find_role_slot(target_index, object_slots);
          slot != nullptr) {
        const u8 kind = static_cast<u8>(slot->bytes[kPathFlagsOffset] & 0x0FU);
        if (kind == 2U) {
          ...
          if (((target.world_x | target.world_y) & 0x0FU) != 0U) {
            ...
            while ((target.world_x & 0x0FU) != 0U) {
              target.world_x -= std::bit_cast<u32>(
                  static_cast<compat::i32>(kSubCellStepX[direction]));
            }
            while ((target.world_y & 0x0FU) != 0U) {
              target.world_y -= std::bit_cast<u32>(
                  static_cast<compat::i32>(kSubCellStepY[direction]));
            }
            if (runtime.spatial_index != nullptr) {
              static_cast<void>(relocate_legacy_role_spatially_by_guid(
                  *runtime.spatial_index, roles, target.guid,
                  target.flags & 3U, original_row, true));
            }
          }
          slot->bytes.fill(0xFFU);
        }
    ...
    case 29U: {
      ...
      role.world_x =
          static_cast<u32>(read_u16_le(path_database,
                                       command.offset + sizeof(u16)))
          << 4U;
      role.world_y =
          static_cast<u32>(read_u16_le(path_database,
                                       command.offset + 2U * sizeof(u16)))
      ...
      if (runtime.spatial_index != nullptr) {
        static_cast<void>(relocate_legacy_role_spatially_by_guid(
            *runtime.spatial_index, roles, role.guid, role.flags & 3U, 0,
            true));
      }
      return result;
    }
    ...
    case 30U:
      ...
      if (!initialize_random_walk_slot(role_index, role, surface_context,
                                       map_height, object_slots, runtime,
                                       result)) {
        return result;
      }
      return result;
    ...
    case 31U:
      ...
      if (const u16 value =
              read_u16_le(path_database, command.offset + 3U * sizeof(u16));
          value != 0xFFFFU) {
    ...
    case 32U:
      ...
      ports.play_positional_sample(
          read_u16_le(path_database, command.offset + sizeof(u16)),
          signed_field(role.world_x), signed_field(role.world_y));
      advance_cursor(role, result, 2U);
      return result;
    ...
    case 36U: {
      ...
      if (!range_available(path_database, command.offset,
                           word_count * sizeof(u16))) {
        result.status = LegacyWorldPathScriptStatus::path_command_truncated;
        return result;
      }
      ...
      if (opcode == 36U) {
        ...
        const std::size_t cell =
            static_cast<std::size_t>(candidate_y) * surface_context.map_width +
            candidate_x;
        const std::size_t byte_offset = cell * sizeof(u32);
        if (range_available(surface_context.surface_grid, byte_offset,
                            2U * sizeof(u32)) &&
            (read_u32_le(surface_context.surface_grid, byte_offset) &
             0x40000000U) == 0U &&
            (read_u32_le(surface_context.surface_grid,
                         byte_offset + sizeof(u32)) & 0x40000000U) == 0U) {
      ...
      const auto request = request_legacy_world_role_path(
          role_index, path_command, roles, surface_context, map_height,
    ...
    case 34U: {
      ...
      payload.assign(path_database.begin() +
      ...
      const u32 words = static_cast<u32>((terminator - command.offset + 2U) / 2U);
...
LegacyWorldPathRoleFrameAction select_legacy_world_path_role_frame_action(
    const LegacyWorldRoleRecord &role, const u32 role_index,
    const u32 controlled_role_index) noexcept {
...
LegacyWorldPathScriptScanResult run_legacy_world_path_scripts(
  ...
  for (u32 role_index = 1U; role_index < roles.size(); ++role_index) {
    ...
    result.last_role_result = run_legacy_world_path_script(
        ...
        object_slots, node_pool, state, runtime, ports);
...
std::span<const u8> resolve_legacy_world_path_label(
    const LegacyWorldPathScriptState &state, const u32 token) noexcept {
