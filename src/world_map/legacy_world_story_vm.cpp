...
LegacyWorldStoryVmResult step_legacy_world_story_vm(
    ...
    for (u32 dispatch_count = 0U; dispatch_count < kInstructionLimit;
        ...
        case OP_112_WAIT_PACKED_ROW_AND_ROLE_HEAD_ACTIONS:
            if (runtime.packed_row_effects == nullptr) {
                result.status = LegacyWorldStoryVmStatus::runtime_unavailable;
                return result;
            }
            if (runtime.packed_row_effects->empty()) {
                if (runtime.role_head_actions == nullptr) {
                    result.status =
                        LegacyWorldStoryVmStatus::runtime_unavailable;
                    return result;
                }
                if (runtime.role_head_actions->empty()) {
                    context.instruction_offset =
                        static_cast<u16>(context.instruction_offset + 2U);
                }
            }
        ...
        case OP_115_SET_MUSIC_STREAM_VOLUME: {
            if (!has_bytes(state.window, ip, 4U)) {
                result.status = LegacyWorldStoryVmStatus::operand_out_of_range;
                return result;
            }
            const u32 raw_level = read_u16(state.window, ip + 2U);
            const u32 level = raw_level > 11U ? 11U : raw_level;
        ...
        case OP_122_CLEAR_SPEED_MODE:
            if (runtime.speed_mode == nullptr) {
                result.status = LegacyWorldStoryVmStatus::runtime_unavailable;
                return result;
            }
            *runtime.speed_mode = 0U;
            context.instruction_offset =
                static_cast<u16>(context.instruction_offset + 2U);
            state.previous_opcode = result.opcode;
            continue;
        ...
        case OP_124_CLEAR_TEXT_CONTROL_BIT25:
            state.text_control_flags &= 0xFDFFFFFFU;
            context.instruction_offset =
                static_cast<u16>(context.instruction_offset + 2U);
            state.previous_opcode = result.opcode;
            continue;
        ...
        case OP_134_ADJUST_PARTY_MEMBER_RESOURCES: {
            ...
            if (static_cast<i16>(resources.current_first) <= 0) {
                ...
                write_u16(
                    state.window, ip + 10U, OP_144_REQUEST_SPECIAL_MODE_4_OR_5
                );
            }
        ...
        case OP_144_REQUEST_SPECIAL_MODE_4_OR_5:
            ...
            *runtime.special_mode_state = 0x80000004U;
            if (!has_bytes(state.window, ip, 3U)) {
                result.status = LegacyWorldStoryVmStatus::operand_out_of_range;
                return result;
            }

            if (state.window[ip + 2U] == 0U) {
                *runtime.special_mode_state = 0x80000004U;
            } else if (state.window[ip + 2U] == 1U) {
                *runtime.special_mode_state = 0x80000005U;
            }
            ...
            *runtime.high_priority_state = 0U;
            if (!ports.reset_input_menu_and_save_previews()) {
                result.status = LegacyWorldStoryVmStatus::runtime_unavailable;
                return result;
            }
            ...
            *runtime.high_priority_submode = 0U;
            if (runtime.high_priority_auxiliary == nullptr) {
                result.status = LegacyWorldStoryVmStatus::runtime_unavailable;
                return result;
            }
        ...
        case OP_151_CONFIGURE_ANI_FOLLOWER_TARGET: {
            ...
            const i32 target_x = std::bit_cast<i32>(
                static_cast<u32>(static_cast<i32>(
                    static_cast<i16>(read_u16(state.window, ip + 2U))
                ))
                << 4U
            ...
            auto& follower = *runtime.ani_follower;
            follower.target_x = target_x;

            if (!has_bytes(state.window, ip + 4U, sizeof(u16))) {
                result.status = LegacyWorldStoryVmStatus::operand_out_of_range;
                return result;
            }
            follower.target_y = std::bit_cast<i32>(
                static_cast<u32>(static_cast<i32>(
                    static_cast<i16>(read_u16(state.window, ip + 4U))
                ))
                << 4U
        ...
        case OP_152_WAIT_ANI_FOLLOWER_TARGET: {
            ...
            const auto& follower = *runtime.ani_follower;
            if (follower.current_x == follower.target_x &&
                follower.current_y == follower.target_y) {
                context.instruction_offset =
                    static_cast<u16>(context.instruction_offset + 2U);
            }
            state.previous_opcode = result.opcode;
        ...
        case OP_155_RELOAD_CURRENT_WORLD_SESSION: {
            if (runtime.current_logical_map_id != 22U) {
                const auto& selected = roles[controlled_role_index];
