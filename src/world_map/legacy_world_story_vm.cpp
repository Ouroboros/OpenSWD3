...
constexpr std::array<u16, 6U> kInitialSetFlags{
    1U, 3U, 4U, 10U, 30U, 70U};
...
[[nodiscard]] constexpr u16 read_u16(
    const std::span<const u8> bytes, const std::size_t offset) noexcept {
  return static_cast<u16>(
      static_cast<u16>(bytes[offset]) |
      static_cast<u16>(static_cast<u16>(bytes[offset + 1U]) << 8U));
}

[[nodiscard]] constexpr u32 read_u32(
    const std::span<const u8> bytes, const std::size_t offset) noexcept {
...
[[nodiscard]] constexpr u16 read_object_u16(
    const LegacyWorldObjectSlot &slot, const std::size_t offset) noexcept {
...
[[nodiscard]] constexpr bool has_bytes(const std::span<const u8> bytes,
...
[[nodiscard]] constexpr u32 current_file_number(
    ...
    const LegacyWorldStoryVmState &state) noexcept {
...
void record_action_update(LegacyWorldStoryVmResult &result,
...
[[nodiscard]] std::span<const u8> nul_terminated_name(
    const std::span<const u8, 16U> name) noexcept {
  const auto end = std::ranges::find(name, u8{});
  return name.first(static_cast<std::size_t>(end - name.begin()));
}

void replace_name_prefix(std::array<u8, 32U> &destination,
                         const std::span<const u8> expected,
                         const std::span<const u8> replacement) noexcept {
  ...
  if (expected.empty() || destination_size < expected.size() ||
      !std::ranges::equal(
          expected,
          std::span<const u8>{destination}.first(expected.size()))) {
    return;
  }
  ...
  std::memmove(destination.data() + replacement_size,
               destination.data() + tail_begin, copied_tail);
  std::ranges::copy(replacement.first(replacement_size), destination.begin());
  destination.back() = 0U;
}

[[nodiscard]] LegacyWorldStoryVmStatus load_name_record(
    LegacyWorldStoryVmState &state, const std::span<const u8> maps_payload,
    const u16 record_index, const std::span<const u8, 16U> first_name,
    const std::span<const u8, 16U> second_name) noexcept {
  ...
  replace_name_prefix(state.speaker_name, kDefaultFirst,
                      nul_terminated_name(first_name));
  replace_name_prefix(state.speaker_name, kDefaultSecond,
                      nul_terminated_name(second_name));
  return LegacyWorldStoryVmStatus::yielded;
}

[[nodiscard]] std::size_t find_dialog_end(
    const std::span<const u8> bytes, const std::size_t start) noexcept {
...
[[nodiscard]] std::size_t find_dialog_action_slot(
    const LegacyWorldDialogRuntimeState &resources,
    const u16 action_id) noexcept {
...
[[nodiscard]] LegacyWorldStoryVmStatus enqueue_dialog(
    ...
    const std::span<u8> window, const std::size_t ip,
    const u16 opcode,
    const std::span<LegacyWorldRoleRecord> roles, const u32 controlled_role_index,
  ...
  try {
    ...
    if (state.speaker_name.front() != 0U) {
      ...
      const auto caption_end = std::ranges::find(state.speaker_name, u8{});
      message.caption.assign(state.speaker_name.begin(), caption_end);
    }
    message.text.assign(window.begin() + static_cast<std::ptrdiff_t>(text_offset),
    ...
    record.width = static_cast<u16>(static_cast<u32>(columns) * kDialogScale);
    ...
    if (mode_one) {
      ...
      i32 left = std::bit_cast<i32>(role.world_x) - half_width - camera_left;
      i32 top = std::bit_cast<i32>(role.world_y) - half_height - camera_top;
...
[[nodiscard]] LegacyWorldStoryVmStatus wait_for_role(
    ...
    const u32 controlled_role_index, const u16 selector) noexcept {
  u16 status{};
  if (selector == kContextSelector) {
    status = context.field_26;
  } else {
    const u16 resolved = selector == kCurrentSourceSelector
                             ? context.source_guid
                             : selector;
    u32 role_index{};
    if (!resolve_role_index(roles, resolved, controlled_role_index,
                            role_index)) {
      return LegacyWorldStoryVmStatus::role_not_found;
    }
...
[[nodiscard]] bool next_action_targets_role(
    const std::span<const u8> window, const std::size_t next_ip,
    const std::span<const LegacyWorldRoleRecord> roles,
    const u32 controlled_role_index, const u32 role_index) noexcept {
  ...
  u32 next_role_index{};
  return resolve_role_index(roles, read_u16(window, next_ip + 2U),
                            controlled_role_index, next_role_index) &&
...
[[nodiscard]] LegacyWorldStoryVmStatus clear_role_from_scene(
    ...
    const std::span<LegacyWorldObjectSlot,
                    kLegacyWorldActiveObjectSlotCount> active_object_slots,
    const LegacyWorldStoryVmRuntime &runtime, const u16 raw_selector,
    LegacyWorldStoryVmResult &result) noexcept {
  ...
  if (clear_legacy_world_role_surface_occupancy(
          role, runtime.role_surface).status !=
...
[[nodiscard]] LegacyWorldStoryVmStatus set_role_position_and_release(
    ...
    const std::span<LegacyWorldObjectSlot,
                    kLegacyWorldActiveObjectSlotCount> active_object_slots,
    const LegacyWorldStoryVmRuntime &runtime, const u16 raw_selector,
    const u16 world_x, const u16 world_y) noexcept {
  u32 role_index{};
  if (!resolve_role_index(roles, raw_selector, controlled_role_index,
                          role_index)) {
    return LegacyWorldStoryVmStatus::role_not_found;
  }
  ...
  if (tile_x >= runtime.role_surface.map_width || tile_y >= runtime.map_height) {
    return LegacyWorldStoryVmStatus::role_surface_failed;
  }
  role.map_cell_pointer_32 =
      tile_y * runtime.role_surface.map_width + tile_x;
  ...
  if (role_index == controlled_role_index) {
    ...
    if ((*runtime.scene_render_flags & 2U) == 0U) {
      recenter_legacy_world_camera(
          role, runtime.role_surface.map_width, runtime.map_height,
          *runtime.camera);
    }
  }
...
[[nodiscard]] LegacyWorldStoryVmStatus start_absolute_camera_move(
    const std::span<const u8> window, const std::size_t ip,
    const LegacyWorldStoryVmRuntime &runtime) noexcept {
  ...
  const i32 target_x = static_cast<i32>(static_cast<i16>(
                           read_u16(window, ip + 2U))) * 16;
  const i32 target_y = static_cast<i32>(static_cast<i16>(
                           read_u16(window, ip + 4U))) * 16;
  ...
  const auto derive_step = [](const i32 remaining, const u16 requested,
...
[[nodiscard]] LegacyWorldStoryVmStatus finish_talk_source(
    const LegacyWorldTalkContext &context,
    const std::span<LegacyWorldRoleRecord> roles,
    const u32 controlled_role_index, LegacyWorldStoryVmResult &result,
    LegacyWorldStoryVmPorts &ports) {
  ...
  u32 role_index{};
  if (!resolve_role_index(roles, context.source_guid, controlled_role_index,
                          role_index)) {
    return LegacyWorldStoryVmStatus::role_not_found;
  }
  auto &role = roles[role_index];
...
LegacyWorldStoryVmResult step_legacy_world_story_vm(
  ...
  if (context.talk_data_offset == 0U) {
    const auto &controlled_role = roles[controlled_role_index];
    if (((controlled_role.world_x | controlled_role.world_y) & 0x0FU) != 0U) {
      result.status = LegacyWorldStoryVmStatus::yielded;
      return result;
    }
    if (context.source_guid != kContextSelector) {
      u32 source_role_index{};
      if (!resolve_role_index(roles, context.source_guid,
                              controlled_role_index, source_role_index)) {
        result.status = LegacyWorldStoryVmStatus::role_not_found;
        return result;
      }
      roles[source_role_index].flags |= 0x00080000U;
    }
    const auto loaded = ports.load_story_window(
        context.talk_script_id, state.window, true);
    ...
    record_action_update(result, roles[controlled_role_index].action, ports);
  } else if (!state.window_loaded ||
  ...
  for (u32 dispatch_count = 0U; dispatch_count < kInstructionLimit;
    ...
    case 6U: {
      ...
      result.status = enqueue_dialog(
          context, state, state.window, ip, result.opcode, roles,
          controlled_role_index, dialogs, dialog_resources, camera_left,
          camera_top, result, ports);
      return result;
    }
    ...
    case 14U: {
      ...
      result.status = wait_for_role(
          context, roles, controlled_role_index, read_u16(state.window, ip + 2U));
      return result;
    }
    ...
    case 20U: {
              ...
              *runtime.story_paths,
              LegacyWorldStoryPathRequest{
                  ...
                  .destination_x = static_cast<u16>(
                      (tile_x == 0xFFFFU ? selected.world_x >> 4U : tile_x)
                      << 4U),
                  .destination_y = static_cast<u16>(
                      (tile_y == 0xFFFFU ? selected.world_y >> 4U : tile_y)
                      << 4U),
      ...
      for (u16 record_index = 0U; record_index < count; ++record_index) {
        const std::size_t record =
            ip + 4U + static_cast<std::size_t>(record_index) * 6U;
        u32 role_index{};
        if (!resolve_role_index(roles, read_u16(state.window, record),
                                controlled_role_index, role_index)) {
          result.status = LegacyWorldStoryVmStatus::role_not_found;
          return result;
        }
    ...
    case 21U: {
      ...
      if (query_legacy_world_story_flag(state,
    ...
    case 26U:
      ...
      if (result.opcode == 25U) {
        set_legacy_world_story_flag(state, read_u16(state.window, ip + 2U));
      } else {
        clear_legacy_world_story_flag(state,
                                      read_u16(state.window, ip + 2U));
      }
      context.instruction_offset =
          static_cast<u16>(context.instruction_offset + 4U);
    ...
    case 38U: {
      ...
      result.status = clear_role_from_scene(
          ...
          read_u16(state.window, ip + 2U), result);
    ...
    case 51U:
      ...
      if (runtime.camera_pan->remaining_x != 0 ||
          ...
          runtime.camera_pan->step_x != 0 || runtime.camera_pan->step_y != 0) {
        result.status = LegacyWorldStoryVmStatus::yielded;
        return result;
      }
      context.instruction_offset =
    ...
    case 52U: {
      ...
      color.step_blue = (color.target_blue - color.current_blue) / duration;
      context.instruction_offset =
    ...
    case 67U: {
      ...
      if ((operand & 0x8000U) == 0U) {
        ...
        write_u16(state.window, ip + 2U,
                  static_cast<u16>(operand | 0x8000U));
      } else if (runtime.current_tick - state.wait_started_at >
                 state.wait_duration) {
        write_u16(state.window, ip + 2U,
                  static_cast<u16>(operand & 0x7FFFU));
        context.instruction_offset =
            static_cast<u16>(context.instruction_offset + 4U);
        continue;
      }
    ...
    case 70U:
      result.status = start_absolute_camera_move(state.window, ip, runtime);
      if (result.status != LegacyWorldStoryVmStatus::yielded) {
        return result;
      }
      context.instruction_offset =
    ...
    case 85U: {
      ...
      ports.begin_story_video(std::span<const u8>{state.window}.subspan(
          ip + 2U, end - ip - 4U));
      context.instruction_offset = static_cast<u16>(end);
      result.status = LegacyWorldStoryVmStatus::yielded;
      return result;
    ...
    case 161U: {
      ...
      const i32 story_id = static_cast<i16>(read_u16(state.window, ip + 2U));
      const auto loaded = ports.load_story_window(story_id, state.window, false);
      result.load_status = loaded.status;
      if (loaded.status != resource_io::LegacyTalkWindowStatus::ready) {
        result.status = LegacyWorldStoryVmStatus::load_failed;
        return result;
