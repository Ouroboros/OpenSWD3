...
constexpr char kEventErrorFormat[] =
    "\xBF\xF9\xBB\x7E\xAA\xBA\xA6\x61\xB9\xCF\xBC\x40\xB1\xA1" "\xB8\xB9\xBD\x58 (%d)";
...
[[nodiscard]] bool draw_marker_rectangle(
...
[[nodiscard]] LegacyWorldDebugOverlayStatus draw_collision_grid(
    rendering::LegacyFramebuffer& framebuffer,
        ...
        for (i32 row = 0; row < kMarkerRows; ++row) {
            ...
            for (i32 column = 0; column < kMarkerColumns; ++column) {
                ...
                if (cell_x < 0 || cell_y < 0 ||
                    ...
                    return LegacyWorldDebugOverlayStatus::
                ...
                u32 flags{};
                if (!read_cell_flags(
                        background.cell_flags, cell_index, flags
                    )) {
                    return LegacyWorldDebugOverlayStatus::
                        cell_grid_out_of_bounds;
                }
                ...
                if (!draw_marker_rectangle(
                        framebuffer, x, y, 15 - inset, colors[pass], result
                    )) {
                    return LegacyWorldDebugOverlayStatus::invalid_framebuffer;
                }
            }
...
void record_text_result(
    const rendering::LegacyTextDrawResult draw,
    LegacyWorldDebugOverlayResult& result
) noexcept {
...
[[nodiscard]] const char* cycle_mode(
    const u32 flags, const u32 enable_mask, const u32 repeat_mask
) noexcept {
...
void draw_event_debug(
    ...
    const u32 world_x =
        wrapping_add(to_bits(camera_left), state.mouse_screen_x);
    ...
    for (std::size_t index = 1U; index < roles.size(); ++index) {
        ...
        if (result.nearby_roles >= 2U) {
            draw_text(
                ports,
                std::span<const u8>{
                    reinterpret_cast<const u8*>(kRoleOverlap),
                    sizeof(kRoleOverlap)
                },
                0x190,
                state.text_color,
        ...
        static_cast<void>(format_and_draw(
            ports,
            0x1A0,
            state.role_text_color,
            result,
            kRoleSummaryFormat,
            static_cast<i32>(role.guid),
            from_bits(role.action.action_id),
        ...
        static_cast<void>(format_and_draw(
            ...
            static_cast<i32>(role.guid),
            static_cast<i32>(role.talk_script_id),
            static_cast<i32>(role.path_data_id),
            role.flags,
...
void draw_diagnostic_text(
    ...
    const i32 map_mouse_x = wrapping_add(
        arithmetic_shift_right_4(camera_left),
        from_bits(state.mouse_screen_x >> 4U)
    );
    const i32 map_mouse_y = wrapping_add(
        arithmetic_shift_right_4(camera_top),
    ...
    static_cast<void>(format_and_draw(
        ...
        from_bits(state.fps_fused),
        from_bits(state.fps_keyboard_repeat_delay),
        from_bits(state.fps_keyboard_repeat_period),
        from_bits(state.fps_ipa)
    ));
    static_cast<void>(format_and_draw(
        ports,
        0x30,
        ...
        result,
        "MapID[%d] MapCyc[%d] %d",
        from_bits(state.map_id),
        from_bits(state.map_cycle),
    ...
    static_cast<void>(format_and_draw(
        ...
        state.unlock_value,
        state.ui_point_value,
        from_bits(state.game_time),
        from_bits(state.frame_state_counter & 0x7FFFFFFFU),
        from_bits(state.battle_mode),
        from_bits(state.button_input)
    ));
    static_cast<void>(format_and_draw(
        ports,
        0x50,
        ...
        from_bits(state.talk_guid & 0xFFFFU),
        from_bits(state.talk_id & 0xFFFFU),
        from_bits(state.debug_value_1),
        from_bits(state.debug_value_2)
    ));
    ...
    static_cast<void>(format_and_draw(
        ...
        state.text_color,
        result,
        "[%d/%d][%d/%d][playing/%d][mode/%s]," "\xB3\xF5\xB4\xBA%s,\xBC\x40\xB1\xA1%s",
        from_bits(state.scene_position),
        from_bits(state.scene_limit),
        from_bits(state.story_position),
        from_bits(state.story_limit),
        from_bits(state.playing_value & 0xFFU),
        active_mode,
        scene_mode,
        story_mode
    ));
...
LegacyWorldDebugOverlayResult draw_legacy_world_debug_overlay(
    ...
    if (state.collision_grid_visible == 1U) {
        ...
        result.status = draw_collision_grid(
            framebuffer,
            background,
            camera_left,
            camera_top,
