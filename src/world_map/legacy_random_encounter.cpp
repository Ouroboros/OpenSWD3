...
[[nodiscard]] LegacyWorldEncounterResult finish_world_encounter(
    LegacyWorldEncounterState& state, const LegacyWorldEncounterResult result
) noexcept {
...
LegacyEncounterRegionSourceResult load_legacy_encounter_regions(
    const std::span<const u8> game_data, const u32 map_id
) {
    ...
    try {
        ...
        while (has_range(game_data, cursor, 2U)) {
            ...
            if (!has_range(
                    game_data, cursor, kLegacyEncounterRegionSourceRecordSize
                )) {
                result.status =
...
LegacyWorldEncounterResult coordinate_legacy_world_encounter(
    ...
    if (state.temporary_battle_request != 0U) {
        return finish_world_encounter(
            state, {LegacyWorldEncounterOutcome::battle_request_already_pending}
        );
    }
