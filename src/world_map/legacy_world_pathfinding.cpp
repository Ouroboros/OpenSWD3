...
[[nodiscard]] i32 legacy_cell_index(const i32 world_x, const i32 world_y,
                                    const u32 map_width) noexcept {
  return wrapping_add(wrapping_multiply(tile_coordinate(world_y),
                                        std::bit_cast<i32>(map_width)),
                      tile_coordinate(world_x));
}

[[nodiscard]] double legacy_distance(const i32 x, const i32 y,
                                     const i32 target_x,
                                     const i32 target_y) noexcept {
  const i32 delta_x = wrapping_subtract(x, target_x);
  const i32 delta_y = wrapping_subtract(y, target_y);
  const i32 squared = wrapping_add(wrapping_multiply(delta_x, delta_x),
                                   wrapping_multiply(delta_y, delta_y));
  if (squared < 0) {
...
[[nodiscard]] constexpr u8 path_direction(const PathNode &from,
                                          const PathNode &to) noexcept {
  const i32 delta_x =
...
constexpr std::array<u32, 8U> kClosedImprovementDirection{4U, 5U, 6U, 7U,
                                                          0U, 1U, 2U, 3U};
...
struct LegacyWorldPathfinder::Impl {
  ...
  validate_request(const LegacyWorldPathfindingRequest &candidate) noexcept {
    ...
    const std::size_t cells =
        static_cast<std::size_t>(candidate.map_width) * candidate.map_height;
  ...
  [[nodiscard]] bool read_surface_cell(const i32 index, u32 &value) noexcept {
    const std::uint64_t cells =
        static_cast<std::uint64_t>(request->map_width) * request->map_height;
  ...
  [[nodiscard]] bool target_is_passable() noexcept {
    u32 value{};
    if (!read_surface_cell(legacy_cell_index(request->target_x,
                                             request->target_y,
                                             request->map_width),
                           value)) {
      return false;
  ...
  [[nodiscard]] PathNode *find_in_list(PathNode *sentinel,
                                       const i32 cell_index) const noexcept {
  ...
  void consider_neighbour(PathNode *parent, const i32 world_x,
                          const i32 world_y, const i32 heuristic_target_x,
                          const i32 heuristic_target_y, const u32 direction) {
    ...
    if (PathNode *existing = find_in_list(open_sentinel, cell_index)) {
      ...
      if (candidate_cost < existing->path_cost) {
        ...
        existing->total_cost = candidate_cost + existing->heuristic_cost;
      }
      return;
    }
    ...
    node->heuristic_cost = legacy_distance(world_x, world_y, heuristic_target_x,
                                           heuristic_target_y);
  ...
  [[nodiscard]] bool expand(PathNode *node, const i32 heuristic_target_x,
                            const i32 heuristic_target_y) {
    const auto occupancy = compute_legacy_world_directional_occupancy_mask(
        request->surface_grid, request->map_width, request->map_height,
        std::bit_cast<u32>(node->cell_index), footprint_width, footprint_height,
        collision_mask);
    ...
    for (u32 direction = 0U; direction < 8U; ++direction) {
      ...
      consider_neighbour(node, wrapping_add(node->world_x, kStepX[direction]),
                         wrapping_add(node->world_y, kStepY[direction]),
                         heuristic_target_x, heuristic_target_y, direction);
    }
    return true;
  }

  void initialize_search(const i32 start_x, const i32 start_y,
                         const i32 target_x, const i32 target_y) {
...
void LegacyWorldPathfinder::set_collision_mask(
    const u32 collision_mask) noexcept {
...
LegacyWorldPathfinder::find_path(const LegacyWorldPathfindingRequest &request) {
  ...
  if (legacy_cell_index(request.start_x, request.start_y, request.map_width) ==
      legacy_cell_index(request.target_x, request.target_y,
                        request.map_width)) {
    impl_->success_flag = false;
    impl_->clear_nodes();
    return result;
  ...
  impl_->clear_nodes();
  impl_->initialize_search(request.start_x, request.start_y, request.target_x,
                           request.target_y);
