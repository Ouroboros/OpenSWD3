...
[[nodiscard]] constexpr compat::i32 wrapping_add(
    const compat::i32 left, const compat::i32 right) noexcept {
...
LegacyPictureActionResult update_draw_legacy_picture_actions(
    ...
    const compat::i32 camera_left, const compat::i32 camera_top,
  ...
  for (auto current = nodes.begin(); current != nodes.end();) {
    ...
    if (!action_ports.load_frame_piece(current->action.field_4a,
      ...
      result.last_blit_status = action_ports.draw_frame_piece(
          piece,
          wrapping_subtract(static_cast<compat::i32>(current->screen_x),
                            current->action.draw_offset_x),
          wrapping_subtract(static_cast<compat::i32>(current->screen_y),
    ...
    if (current->action.field_58 != 0U) {
      audio_ports.play_positional_sample(
          ...
          wrapping_add(static_cast<compat::i32>(current->screen_x),
                       camera_left),
          wrapping_add(static_cast<compat::i32>(current->screen_y),
                       camera_top));
