...
[[nodiscard]] constexpr compat::i16
wrapping_multiply_word(const compat::i16 value,
                       const compat::u32 multiplier) noexcept {
  return std::bit_cast<compat::i16>(static_cast<compat::u16>(
      static_cast<compat::u32>(std::bit_cast<compat::u16>(value)) *
      multiplier));
}
  ...
  for (auto current = nodes.begin(); current != nodes.end();) {
    ...
    if (!action_ports.load_frame_piece(current->action.field_4a,
      ...
      result.last_blit_status = action_ports.draw_frame_piece(
          piece,
          wrapping_subtract(static_cast<compat::i32>(current->current_x),
                            field_as_i32(current->action.draw_offset_x)),
          wrapping_subtract(static_cast<compat::i32>(current->y),
    ...
    if ((motion_bits & 0x7FFFU) == 0U) {
      const compat::i32 distance = static_cast<compat::i32>(current->target_x) -
    ...
    current->current_x =
        wrapping_add_word(current->current_x,
