...
[[nodiscard]] u32 read_i16_bits(const LegacyWorldObjectSlot &slot,
                                const std::size_t offset) noexcept {
...
[[nodiscard]] bool
remove_aligned_party_role(LegacyWorldPartyRoleActionsResult &result,
...
[[nodiscard]] bool
move_surface_occupancy(LegacyWorldPartyRoleActionsResult &result,
                       LegacyWorldRoleRecord &role,
...
[[nodiscard]] bool refresh_cell_flags(
    LegacyWorldPartyRoleActionsResult &result, LegacyWorldRoleRecord &role,
...
void update_action(LegacyWorldPartyRoleActionsResult &result,
                   LegacyWorldRoleRecord &role,
...
LegacyWorldPartyRoleActionsResult advance_legacy_world_party_role_actions(
  ...
  if (party_role_count > party_object_slots.size()) {
    result.status = LegacyWorldPartyRoleActionsStatus::invalid_party_role_count;
    return result;
  }
  ...
  for (u32 party_index = 1U; party_index < party_role_count; ++party_index) {
    ...
    if (role_index >= roles.size()) {
      result.status = LegacyWorldPartyRoleActionsStatus::invalid_role_index;
      return result;
    }
    ...
    if (direction_offset >= slot.bytes.size()) {
      result.status = LegacyWorldPartyRoleActionsStatus::path_byte_out_of_range;
      return result;
    }
    ...
    if (role.action.wait_remaining == 0U) {
      ...
      if (((role.world_x | role.world_y) & 0x0FU) == 0U) {
        ...
        if (!remove_aligned_party_role(result, role, roles, spatial_index) ||
            !move_surface_occupancy(result, role, surface_context, direction)) {
          return result;
        }
