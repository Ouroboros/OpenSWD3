...
public:
    ...
    [[nodiscard]] bool load_countdown_piece(
        ...
        return ports_.load_frame_piece(
            record_.field_4a, record_.field_4c, piece
        );
    }
...
[[nodiscard]] bool accepted_countdown_status(
    const rendering::LegacyCountdownDisplayStatus status
) noexcept {
...
void sync_frame_camera(
    LegacyWorldFrameCoordinatorState& state, const LegacyWorldCameraRect& camera
) noexcept {
    state.frame_runtime.frame.camera_left =
        std::bit_cast<compat::i32>(camera.left);
    state.frame_runtime.frame.camera_top =
        std::bit_cast<compat::i32>(camera.top);
...
LegacyWorldFrameCoordinatorResult run_legacy_world_frame(
    ...
    LegacyWorldOuterFramePorts& outer_ports
) noexcept {
    ...
    advance_legacy_world_player_and_camera(
        roles[state.player_role_index], camera, state.movement
    );
    ...
    result.map_role_paths = advance_legacy_world_map_role_paths(
        ...
        spatial_index,
        role_surface,
        state.player_role_index,
        state.frame_runtime.frame.runtime_flags,
        state.movement,
        camera,
        state.map_role_paths,
    ...
    if (result.map_role_paths.status !=
        ...
        result.status =
            LegacyWorldFrameCoordinatorStatus::map_role_paths_failed;
        return result;
    }
    ...
    CountdownPieceProvider countdown_pieces{
        state.countdown_action, frame_ports.flagged_roles
    };
    ...
    result.player_post_frame = advance_legacy_world_player_post_frame(
        roles[state.player_role_index],
        roles,
        spatial_index,
        state.movement,
        state.player_post_frame,
        role_surface,
        frame_ports.flagged_roles
    );
    ...
    if (result.player_post_frame.status !=
        ...
        result.status =
            LegacyWorldFrameCoordinatorStatus::player_post_frame_failed;
        return result;
    }
    ...
    const bool should_restore = !selection_words.empty() &&
    ...
    restore_legacy_world_viewport_after_selection_scroll(
        ...
        LegacyWorldViewportRestoreState{
            .first_selection_word = selection_words.empty()
                ? kLegacyWorldSelectionSentinel
                : std::bit_cast<compat::u16>(selection_words.front()),
            .map_id = state.map_id,
