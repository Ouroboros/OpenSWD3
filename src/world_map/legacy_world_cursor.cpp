...
[[nodiscard]] constexpr compat::i32 wrapping_add(const compat::i32 left,
...
[[nodiscard]] constexpr compat::i32 wrapping_subtract(
    const compat::i32 left, const compat::u32 right) noexcept {
...
[[nodiscard]] constexpr bool edge_visible_for_talk(
    const LegacyWorldCursorFrameInput &input) noexcept {
...
LegacyWorldCursorResult update_draw_legacy_world_cursor(
    LegacyWorldCursorState &state, const LegacyWorldCursorFrameInput &input,
  ...
  if (!ports.load_frame_piece(state.cursor_action.field_4a,
                              state.cursor_action.field_4c, piece)) {
    result.status = LegacyWorldCursorStatus::cursor_frame_unavailable;
    return result;
  }

  result.last_cursor_blit_status = ports.draw_frame_piece(
      piece, wrapping_subtract(input.mouse_x, state.cursor_action.draw_offset_x),
