...
LegacyLmfOffset1cDirectory legacy_lmf_read_offset1c_directory(
    ...
    if (!file.open(
        ...
        result.status = LegacyLmfOffset1cDirectoryStatus::file_open_failed;
        return result;
    }

    const compat::u32 directory_position = map_offset + header.offset_1c;
    if (file.seek_begin_one_based(
            std::bit_cast<compat::i32>(directory_position)
        ) == 0U) {
    ...
    const compat::u32 record_count = read_u32(directory_window, 0U);
    if (std::bit_cast<compat::i32>(record_count) <= 0) {
        if (!checked_add(header.offset_1c, 4U, result.end_offset)) {
            result.status =
                LegacyLmfOffset1cDirectoryStatus::record_data_out_of_range;
            return result;
        }

        result.status = LegacyLmfOffset1cDirectoryStatus::ready;
        return result;
    }
    ...
    result.records.reserve(record_count);
    for (compat::u32 record = 0U; record < record_count; ++record) {
        constexpr std::size_t kFixedRecordSize = 10U;
        if (directory_window.size() - cursor < kFixedRecordSize) {
            result.status =
                LegacyLmfOffset1cDirectoryStatus::record_data_out_of_range;
            return result;
        ...
        if (name_end == directory_window.cend()) {
            result.status = LegacyLmfOffset1cDirectoryStatus::unterminated_name;
            return result;
        }
        parsed.name_bytes_with_terminator.assign(
            directory_window.cbegin() + static_cast<std::ptrdiff_t>(cursor),
    ...
    const compat::u32 cursor_u32 = static_cast<compat::u32>(cursor);
    if (!checked_add(header.offset_1c, cursor_u32, result.end_offset)) {
        result.status =
            LegacyLmfOffset1cDirectoryStatus::record_data_out_of_range;
        return result;
    }
