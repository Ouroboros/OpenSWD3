...
void test_loader_state_counter_reset_and_idle_rng(
    ...
    test.expect_equal(
        effect.state().slots[0U].active_flags,
        i16{3},
        "scene reset preserves an active slot"
    );
    test.expect_equal(
        effect.state().slots[95U].fixed_x,
        i16{123},
    ...
    test.expect_equal(
        idle.status, LegacyAniSparkStatus::ready, "idle update completes"
    );
    test.expect_false(
        idle.scanned_slots, "zero counters skip the 96-slot scan"
    );
    test.expect_equal(
        random.index(),
        std::size_t{2U},
    ...
    test.expect_equal(
        services.call_count, u32{0U}, "random 478 does not query service 22"
    );
    ...
    test.expect_equal(
        final_slot_result.visited_active_count,
        u32{1U},
        "the scan reaches physical slot 95"
    );
    test.expect_equal(
        final_slot.active_flags,
        i16{0},
...
void test_trigger_service_and_creation_vector(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        services.call_count, u32{1U}, "the service port is called once"
    );
    test.expect_equal(
    ...
    test.expect_equal(
        slot.fixed_x, i16{5712}, "random x 357 is stored in sixteenth pixels"
    );
    test.expect_equal(slot.fixed_y, i16{0}, "new particles begin at row zero");
    test.expect_equal(
        slot.horizontal_step, i16{0}, "right-half zero drift remains zero"
    );
    test.expect_equal(
        slot.vertical_step,
        i16{1},
    ...
    test.expect_equal(
        slot.phase, i16{0}, "creation clears the horizontal phase"
    );
    test.expect_equal(
        slot.active_flags, i16{1}, "creation writes active word one"
    );
...
void test_nine_pixel_kernel_and_state_advance(openswd3::test::Context& test) {
    LegacyAniSparkEffect effect;
    ...
    test.expect_equal(
        result.visited_active_count, u32{1U}, "one active slot is visited"
    );
    test.expect_equal(
    ...
    test.expect_equal(
        row_100[101U], u16{0x7FFFU}, "center receives intensity 31"
    );
    test.expect_equal(
        row_100[100U], u16{0x3DEFU}, "left neighbor receives intensity 15"
    );
    test.expect_equal(
        row_100[102U], u16{0x3DEFU}, "right neighbor receives intensity 15"
    );
    test.expect_equal(
        row_99[101U], u16{0x3DEFU}, "upper neighbor receives intensity 15"
    );
    test.expect_equal(
        row_101[101U], u16{0x3DEFU}, "lower neighbor receives intensity 15"
    );
    test.expect_equal(
        row_99[100U],
        u16{0x1CE7U},
    ...
    test.expect_equal(
        slot.fixed_y, i16{102}, "frame y advances by the vertical step"
    );
    test.expect_equal(slot.phase, i16{5}, "phase advances once per frame");
    test.expect_equal(
        slot.remaining_height,
        i16{478},
    ...
    test.expect_equal(
        slot.active_flags, i16{1}, "positive remaining height stays active"
    );
    test.expect_equal(
        effect.state().previous_live_count,
        i16{1},
...
void test_flattened_x_boundary_and_bottom_count_quirk(
    ...
    static_cast<void>(flattened_effect.update(
        flattened_random, flattened_framebuffer, pixel_format, services
    ));
    test.expect_equal(
    ...
    test.expect_equal(
        bottom_result.visited_active_count,
        u32{1U},
        "bit zero alone marks a slot active"
    );
    test.expect_equal(
        bottom_result.adjusted_pixel_count,
        u32{0U},
        "row 479 is excluded from the nine-pixel kernel"
    );
    test.expect_equal(
        bottom.active_flags, i16{0}, "row 479 clears the complete active word"
    );
    test.expect_equal(
        bottom.fixed_y, i16{480}, "state still advances after the bounds clear"
    );
    test.expect_equal(
        bottom.phase, i16{0}, "phase 28 advances and wraps to zero"
    );
    test.expect_equal(
    ...
    test.expect_equal(
        bottom_effect.state().previous_live_count,
        i16{1},
    ...
    test.expect_equal(
        bottom.fixed_x, i16{-32765}, "fixed x multiply-add wraps at 16 bits"
    );
    test.expect_equal(
        bottom.remaining_height,
        i16{-1},
    ...
    test.expect_equal(
        bottom.active_flags, i16{0}, "nonpositive height clears active"
    );
    test.expect_equal(
        bottom_effect.state().previous_live_count,
        i16{0},
