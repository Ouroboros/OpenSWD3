...
void test_layout_fields_and_origin_clamp(openswd3::test::Context& test) {
    ...
    test.expect_equal(node.state().damping_shift, 4U, "+0x18 starts at four");
    test.expect_equal(
        node.state().active_field_index, 0U, "+0x1c starts at field zero"
    );
    test.expect_equal(
        node.field(0U).size(),
        std::size_t{40000U},
    ...
    test.expect_equal(
        node.source_snapshot().size(),
        std::size_t{307200U},
        "+0x24 owns a full 640 by 480 snapshot"
    );
    test.expect_true(
        std::ranges::all_of(
            node.field(0U), [](const i16 value) { return value == 0; }
        ),
        "constructor clears both work fields"
    );

    test.expect_equal(
        node.set_origin(600, 470),
        i32{279},
    ...
    test.expect_equal(
        node.state().origin_x, i32{-9}, "original setter has no lower x clamp"
    );
    test.expect_equal(
        node.state().origin_y, i32{-7}, "original setter has no lower y clamp"
    );
}
...
void test_snapshot_and_paired_warp(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        node.capture(framebuffer),
        LegacyDeformationStatus::ready,
        "capture accepts the full source surface"
    );
    test.expect_true(
        std::ranges::equal(node.source_snapshot(), original),
    ...
    test.expect_equal(
        framebuffer[17U], u16{1017U}, "next row advances by framebuffer width"
    );
    test.expect_equal(
        framebuffer[28U], u16{1028U}, "last interior pixel is emitted"
    );
    test.expect_equal(
        framebuffer[8U], u16{0xFFFFU}, "left exterior pixel is untouched"
    );
    test.expect_equal(
        framebuffer[13U], u16{0xFFFFU}, "right exterior pixel is untouched"
    );
    test.expect_equal(
        framebuffer[41U], u16{0xFFFFU}, "bottom exterior row is untouched"
    );

    std::array<u16, 47U> too_small{};
    test.expect_equal(
        node.capture(too_small),
        LegacyDeformationStatus::framebuffer_too_small,
        "short capture is rejected at the modern memory boundary"
    );
    test.expect_equal(
        node.apply(too_small),
        LegacyDeformationStatus::framebuffer_too_small,
...
void test_exact_field_advance(openswd3::test::Context& test) {
    ...
    for (std::size_t index = 0U; index < node.field(0U).size(); ++index) {
        node.field(0U)[index] =
            static_cast<i16>(static_cast<i32>(index) * 3 - 20);
        node.field(1U)[index] = static_cast<i16>(10 - static_cast<i32>(index));
    }
    ...
    test.expect_false(
        result.complete, "a nonzero narrowed destination keeps the node alive"
    );
    test.expect_equal(
        node.state().active_field_index,
        1U,
        "advance toggles +0x1c before reading the fields"
    );

    constexpr std::array<i16, 30U> expected{
        10, 9,  8,  7,  6,  5,  4,  2,  6,   13,  19,  -1,  -2,  37,  45,
        52, 59, -7, -8, 76, 85, 91, 98, -13, -14, -15, -16, -17, -18, -19,
    };
    test.expect_true(
        std::ranges::equal(node.field(1U), expected),
...
void test_radial_injection_and_random_coordinates(
    ...
    test.expect_equal(
        node.field(0U)[5U + 5U * 12U],
        i16{10},
        "radial center receives radius times strength"
    );
    test.expect_equal(
        node.field(0U)[4U + 5U * 12U],
        i16{5},
        "axis neighbor receives truncated radial falloff"
    );
    test.expect_equal(
        node.field(0U)[4U + 4U * 12U],
        i16{2},
        "diagonal uses x87-style square-root falloff"
    );
    const i32 injected_sum =
        std::accumulate(node.field(0U).begin(), node.field(0U).end(), i32{0});
    test.expect_equal(
        injected_sum,
        i32{38},
    ...
    test.expect_equal(
        std::accumulate(
            clipped.field(0U).begin(), clipped.field(0U).end(), i32{0}
        ),
        i32{22},
...
void test_sentinel_list_update_and_removal(openswd3::test::Context& test) {
    ...
    test.expect_true(
        framebuffer == original, "zero-field nodes preserve the framebuffer"
    );
    ...
    auto active = std::make_unique<LegacyDeformationNode>(active_configuration);
    LegacyCrtRng random;
    test.expect_equal(
        active->inject(4, 4, 2, 5, random).status,
        LegacyDeformationStatus::ready,
    ...
    test.expect_equal(
        retained.processed, std::size_t{1U}, "one active node is processed"
    );
    test.expect_equal(
        retained.removed, std::size_t{0U}, "nonzero field remains linked"
    );
    test.expect_equal(
