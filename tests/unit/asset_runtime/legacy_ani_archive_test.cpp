...
[[nodiscard]] SyntheticAni make_synthetic_ani() {
    ...
    write_u32(
        ani.bytes, 0x18U, static_cast<u32>(ani.frame_one_commands.size() + 12U)
    );
    ...
    std::ranges::copy(
        compressed_one,
        ani.bytes.begin() + static_cast<std::ptrdiff_t>(frame_one_offset)
    );
    ...
    std::ranges::copy(
        compressed_three,
        ani.bytes.begin() + static_cast<std::ptrdiff_t>(frame_three_offset)
    );
...
void test_synthetic_chain_and_spans(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        archive.open(tree.path("synthetic.Ani")),
        LegacyAniOpenStatus::ready,
        "synthetic ANI opens"
    );
    test.expect_equal(
        archive.header().frame_count, 3U, "frame count is parsed"
    );
    test.expect_equal(
        archive.header().initial_span_count, 1U, "first span count is parsed"
    );

    const auto first = archive.load_frame(1U);
    test.expect_equal(
        first.status, LegacyAniFrameLoadStatus::ready, "first frame loads"
    );
    test.expect_equal(
        first.mode,
        LegacyAniFrameLoadMode::first_load,
        "first frame excludes its trailer"
    );
    test.expect_equal(
        first.node.record_seek_base,
        0x18U,
        "legacy node stores physical offset minus twelve"
    );
    test.expect_equal(
        first.node.one_based_frame, 1U, "frame numbering is one based"
    );
    test.expect_true(
    ...
    test.expect_equal(
        first.palette[1U], u16{0x7C00U}, "RGB palette red becomes RGB555"
    );
    test.expect_equal(
        first.palette[2U], u16{0x03E0U}, "RGB palette green becomes RGB555"
    );
    test.expect_equal(
        first.palette[3U], u16{0x001FU}, "RGB palette blue becomes RGB555"
    );

    std::vector<u8> framebuffer(640U * 480U * 2U, 0U);
    const auto rendered = apply_legacy_ani_spans(
        first.command_stream,
        first.node.span_count,
        first.palette,
        framebuffer,
        1280U,
        archive.header().display_height
    );
    test.expect_equal(
        rendered.status, LegacyAniSpanStatus::completed, "indexed span renders"
    );
    ...
    test.expect_equal(
        read_u16(framebuffer, 4U), u16{0x7C00U}, "first palette pixel"
    );
    test.expect_equal(
        read_u16(framebuffer, 6U), u16{0x03E0U}, "second palette pixel"
    );
    test.expect_equal(
        read_u16(framebuffer, 8U), u16{0x001FU}, "third palette pixel"
    );

    const auto cached_first = archive.load_frame(1U);
    test.expect_equal(
        cached_first.status,
        LegacyAniFrameLoadStatus::ready,
    ...
    test.expect_equal(
        cached_first.decompression_status,
        LegacyLzo1xStatus::input_not_consumed,
        "cached trailer produces original input-not-consumed"
    );
    test.expect_true(
        std::ranges::equal(
            cached_first.command_stream, synthetic.frame_one_commands
        ),
    ...
    test.expect_true(
        second.command_stream.empty(), "zero-payload frame has no span stream"
    );

    const auto cached_second = archive.load_frame(2U);
    test.expect_equal(
        cached_second.status,
        LegacyAniFrameLoadStatus::ready,
    ...
    test.expect_true(
        std::ranges::equal(
            third.command_stream, synthetic.frame_three_commands
        ),
        "last frame excludes overlapping palette bytes"
    );
    test.expect_equal(
        archive.cached_frame_count(),
        std::size_t{3U},
    ...
    test.expect_equal(
        past_end.status,
        LegacyAniFrameLoadStatus::frame_past_end,
        "frame after declared count ends playback"
    );
    test.expect_equal(
        past_end.legacy_return_value,
        1U,
...
void test_raw_span_conversion_and_boundaries(openswd3::test::Context& test) {
    ...
    const auto result = apply_legacy_ani_spans(
        command, 1U, {}, destination, 16U, 480U, conversion
    );
    test.expect_equal(
        result.status, LegacyAniSpanStatus::completed, "raw 16-bit span renders"
    );
    test.expect_equal(
        read_u16(destination, 2U),
        u16{0xF800U},
        "raw red is converted to RGB565"
    );
    test.expect_equal(
        read_u16(destination, 4U),
        u16{0x07C0U},
    ...
    test.expect_equal(
        ...
        LegacyAniSpanStatus::invalid_pitch,
        "zero pitch is isolated"
    );
    test.expect_equal(
    ...
    test.expect_equal(
        apply_legacy_ani_spans(
            command, 1U, {}, std::span<u8>{destination}.first(4U), 16U, 480U
        )
            .status,
...
void test_open_and_frame_safety(openswd3::test::Context& test) {
    ...
    LegacyAniArchive archive;
    test.expect_equal(
        archive.open(tree.path("missing.Ani")),
    ...
    test.expect_equal(
        archive.open(tree.path("bad-magic.Ani")),
...
void test_real_ani_files(
    openswd3::test::Context& test, const std::filesystem::path& video_root
) {
    ...
    for (const RealAniExpectation expected : kRealAniFiles) {
        LegacyAniArchive archive;
        test.expect_equal(
            archive.open(video_root / expected.name),
            LegacyAniOpenStatus::ready,
            "real ANI opens"
        );
        test.expect_equal(
            archive.header().frame_count,
        ...
        for (u32 frame = 1U; frame <= expected.frame_count; ++frame) {
            ...
            if (frame == 1U) {
                test.expect_equal(
                    fnv1a64(loaded.command_stream),
                    expected.first_frame_fnv1a,
                    "real first-frame command hash"
                );
            }
            const auto rendered = apply_legacy_ani_spans(
                loaded.command_stream,
                loaded.node.span_count,
                loaded.palette,
                framebuffer,
                1280U,
                archive.header().display_height
            ...
            test.expect_equal(
                rendered.status,
                LegacyAniSpanStatus::completed,
                "real ANI spans stay in the framebuffer"
            );
            test.expect_equal(
                rendered.spans_written,
                loaded.node.span_count,
    ...
    test.expect_equal(
        total_frames, std::uint64_t{5312U}, "all real frames are traversed"
    );
    test.expect_equal(
        nonempty_frames,
        std::uint64_t{4623U},
