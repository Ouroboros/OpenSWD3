...
[[nodiscard]] std::vector<u8> synthetic_archive() {
    ...
    write_u32(bytes, kBlockOffset + 10U,
              static_cast<u32>(kTableSize + kVariant0.size()));
    std::ranges::copy(
        kVariant0,
        bytes.begin() + static_cast<std::ptrdiff_t>(kBlockOffset + kTableSize));
    std::ranges::copy(kVariant2, bytes.begin() + static_cast<std::ptrdiff_t>(
                                                     kBlockOffset + kTableSize +
                                                     kVariant0.size()));
    return bytes;
}
...
void test_lazy_open_full_keys_and_two_caches(openswd3::test::Context& test) {
    ...
    test.expect_equal(first.status, LegacyActRuntimeStatus::ready,
                      "first query loads selected stream");
    test.expect_false(first.cache_hit, "first query misses cache");
    test.expect_true(std::ranges::equal(first.stream, kVariant0),
                     "runtime returns exact selected bytes");
    test.expect_equal(runtime.cached_stream_bytes(), 4U,
                      "cache counts stream bytes only");
    test.expect_equal(runtime.index_cache_entry_count(), std::size_t{1U},
    ...
    test.expect_true(repeated.stream.data() == first_pointer,
                     "cached query returns a borrowed stable view");

    const auto same_block_direct = runtime.load_direct(1U, 2U);
    test.expect_equal(same_block_direct.status, LegacyActRuntimeStatus::ready,
                      "direct query loads another variant");
    test.expect_true(std::ranges::equal(same_block_direct.stream, kVariant2),
                     "direct query owns exact stream bytes");
    test.expect_equal(runtime.index_cache_entry_count(), std::size_t{1U},
                      "direct query reuses index metadata cache");
    test.expect_equal(runtime.stream_cache_entry_count(), std::size_t{1U},
                      "direct query does not enter stream cache");

    const auto second_archive = runtime.load_direct(3001U, 0U);
    test.expect_equal(second_archive.status, LegacyActRuntimeStatus::ready,
                      "quotient selects the second ACT archive");
    test.expect_equal(runtime.index_cache_entry_count(), std::size_t{2U},
                      "index key includes archive group");

    const auto high_action = runtime.query_cached(0x00010001U, 0U);
    test.expect_false(high_action.cache_hit,
                      "action ID cache key keeps all 32 bits");
    test.expect_equal(high_action.status,
    ...
    test.expect_false(high_variant.cache_hit,
                      "variant cache key keeps all 32 bits");
    test.expect_equal(high_variant.physical_status,
    ...
    test.expect_equal(runtime.stream_cache_entry_count(), std::size_t{0U},
                      "close clears selected streams");
    test.expect_equal(runtime.index_cache_entry_count(), std::size_t{0U},
...
void test_absent_stream_and_special_bucket(openswd3::test::Context& test) {
    ...
    test.expect_true(repeated.cache_hit,
                     "original leaves failed selected-stream node cached");
    test.expect_equal(runtime.cached_stream_bytes(), 0U,
                      "failed node contributes no stream bytes");

    const auto special = runtime.query_cached(0xFFFFU, 3U);
    test.expect_equal(special.status,
                      LegacyActRuntimeStatus::action_id_out_of_range,
                      "ACT FFFF has no separate physical loader");
    test.expect_equal(runtime.stream_bucket_entry_count(3U), std::size_t{1U},
                      "FFFF uses variant modulo ten for selected cache");
    runtime.close();
}

void populate_eviction_shape(LegacyActRuntime& runtime,
    ...
    for (const u32 action : kActions) {
        ...
        test.expect_equal(loaded.status, LegacyActRuntimeStatus::ready,
                          "eviction fixture stream loads");
    }
    test.expect_equal(runtime.cached_stream_bytes(), 28U,
                      "fixture counts seven four-byte streams");
    test.expect_equal(runtime.stream_bucket_entry_count(0U), std::size_t{3U},
                      "bucket zero starts longest");
    test.expect_equal(runtime.stream_bucket_entry_count(1U), std::size_t{2U},
                      "bucket one count");
    test.expect_equal(runtime.stream_bucket_entry_count(2U), std::size_t{2U},
                      "bucket two count");
}
...
void test_original_eviction_order(openswd3::test::Context& test) {
    ...
    test.expect_true(surviving.cache_hit,
                     "non-selected bucket is searched after eviction");
    test.expect_equal(runtime.stream_bucket_entry_count(0U), std::size_t{0U},
    ...
    test.expect_equal(runtime.cached_stream_bytes(), 16U,
    ...
    test.expect_true(mru.query_cached(11U, 2U).cache_hit,
                     "hit moves appended node to bucket head");
    mru.set_cache_limit(8U);
    test.expect_true(mru.query_cached(11U, 2U).cache_hit,
                     "tail eviction preserves the MRU head");
    test.expect_equal(mru.find_cached(1U, 2U).status,
                      LegacyActRuntimeStatus::cache_miss,
...
void test_initialization_failure(openswd3::test::Context& test) {
    ...
    test.expect_equal(runtime.query_cached(1U, 0U).status,
    ...
    test.expect_false(runtime.is_initialized(),
                      "failed initialization closes partial files");
}
