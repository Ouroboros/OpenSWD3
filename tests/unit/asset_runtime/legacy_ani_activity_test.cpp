...
[[nodiscard]] std::vector<u8> make_one_frame_ani() {
    ...
    std::ranges::copy(compressed, bytes.begin() +
...
void test_reveal_snapshot_playback_and_default_end(
    ...
    const std::filesystem::path& ani_path
) {
    LegacyAniActivity activity;
    const auto started = activity.start(ani_path, 0U, 0x40U, 0x08U);
    test.expect_equal(started.status, LegacyAniActivityStartStatus::ready,
                      "ANI activity starts");
    test.expect_equal(activity.state().active_extent, 640U,
                      "header +0x0e is the activity sentinel");
    test.expect_equal(activity.state().phase, i32{-13},
                      "normal activity begins at negative thirteen");
    test.expect_equal(activity.state().process_flags, 0x42U,
                      "start sets process bit one");
    ...
    test.expect_equal(result.path, LegacyAniActivityPath::reveal_frame,
                      "negative phase renders reveal frame");
    test.expect_equal(result.phase_after, i32{-12},
                      "reveal phase increments once");
    test.expect_equal(read_u16(framebuffer, 100U * 1280U + 20U),
                      u16{0x7C00U}, "reveal applies current ANI span");

    std::ranges::fill(framebuffer, 0xA5U);
    result = activity.update(framebuffer, 1280U, blockers, ports);
    test.expect_equal(result.phase_after, i32{-11},
                      "second reveal phase increments once");
    ...
    result = activity.update(framebuffer, 1280U, blockers, ports);
    test.expect_equal(result.path, LegacyAniActivityPath::snapshot_saved,
    ...
    test.expect_true(framebuffer == snapshot,
                     "blocked restore is byte exact");
    blockers.first = 0U;
    std::ranges::fill(framebuffer, 0x22U);
    result = activity.update(framebuffer, 1280U, blockers, ports);
    test.expect_equal(result.path,
                      LegacyAniActivityPath::snapshot_restored_on_resume,
                      "first resumed call only restores framebuffer");
    test.expect_equal(activity.state().phase, i32{-11},
                      "resume restore does not advance phase");
    test.expect_true(framebuffer == snapshot,
                     "resume restore is byte exact");

    activity.state().phase = 1;
    std::ranges::fill(framebuffer, 0U);
    result = activity.update(framebuffer, 1280U, blockers, ports);
    test.expect_equal(result.path, LegacyAniActivityPath::playback_frame,
                      "positive phase reloads one-based frame");
    test.expect_equal(result.phase_after, i32{2},
                      "playback increments requested frame");
    result = activity.update(framebuffer, 1280U, blockers, ports);
    test.expect_equal(result.path, LegacyAniActivityPath::playback_exhausted,
                      "frame past end enters phase ten thousand");
    test.expect_equal(result.legacy_return_value, 0U,
                      "past-end transition itself returns zero");

    std::ranges::fill(framebuffer, 0x7AU);
    result = activity.update(framebuffer, 1280U, blockers, ports);
    test.expect_equal(result.path, LegacyAniActivityPath::ending_frame,
                      "default ending begins after exhausted transition");
    test.expect_equal(result.legacy_return_value, 1U,
                      "ending phase returns one");
    test.expect_equal(ports.redraw_count, 1U,
                      "default ending redraws scene once per call");
    test.expect_equal(ports.redraw_active_extent, 0U,
                      "redraw observes inactive ANI sentinel");
    test.expect_equal(ports.redraw_scene_flags & 1U, 0U,
    ...
    result = activity.update(framebuffer, 1280U, blockers, ports);
    test.expect_equal(result.path, LegacyAniActivityPath::finalized,
    ...
    test.expect_equal(ports.service_ids, std::vector<u32>{0x23U},
                      "cleanup calls service 0x23");
    test.expect_equal(ports.finalize_active_extent, 0U,
                      "service observes cleared active sentinel");
    test.expect_equal(ports.finalize_phase, i32{0},
                      "service observes cleared phase");
    test.expect_equal(ports.finalize_process_flags, 0x40U,
                      "service observes process bit one cleared");
}

void test_effect_end_and_boundaries(openswd3::test::Context& test,
                                    const std::filesystem::path& ani_path) {
    LegacyAniActivity activity;
    auto started = activity.start(ani_path, 0x03U, 0U, 0U);
    test.expect_equal(started.status, LegacyAniActivityStartStatus::ready,
                      "effect activity starts");
    test.expect_equal(activity.state().phase, i32{1},
    ...
    test.expect_equal(result.ending_adjustment, i32{0},
                      "first effect amount is zero");
    test.expect_equal(ports.effect_pixel_counts, std::vector<u32>{0x4B000U},
                      "effect receives all legacy pixels");
    test.expect_equal(ports.effect_framebuffer_size,
    ...
    test.expect_equal(result.ending_adjustment, i32{-30},
                      "last effect amount preserves signed minus thirty");
    test.expect_equal(ports.redraw_count, 0U,
                      "effect ending does not use scene redraw branch");
    test.expect_equal(ports.effect_values.back(),
    ...
    result = activity.update(std::span<u8>{framebuffer}.first(100U), 1280U,
                             blockers, ports);
    test.expect_equal(result.status,
                      LegacyAniActivityStatus::framebuffer_too_small,
                      "short framebuffer is isolated");
    test.expect_equal(activity.state().phase, phase,
                      "boundary failure does not advance phase");
    result = activity.update(framebuffer, 1279U, blockers, ports);
    test.expect_equal(result.status, LegacyAniActivityStatus::invalid_pitch,
...
void test_start_failure(openswd3::test::Context& test,
    ...
    test.expect_false(activity.is_active(),
                      "failed start leaves activity inactive");
}
