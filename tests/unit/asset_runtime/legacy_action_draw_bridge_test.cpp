...
void test_normal_bridge_reads_updated_record(openswd3::test::Context& test) {
    ...
    const auto result = update_draw_legacy_action(
        record,
    ...
    test.expect_equal(
        result.status, LegacyActionDrawStatus::ready, "normal bridge completes"
    );
    test.expect_equal(
        result.action_update_count, u32{1U}, "action updates once"
    );
    test.expect_equal(
        result.frame_request_count, u32{1U}, "updated frame is requested once"
    );
    test.expect_equal(result.draw_count, u32{1U}, "frame draws once");
    test.expect_equal(
        ports.loads.front(),
        std::pair<u16, u16>{0x1357U, 0x2468U},
    ...
    test.expect_equal(
        ports.draws.front().flags,
        0x81234567U,
        "full flags are read after the action update"
    );
    test.expect_equal(
        ports.draws.front().opacity_step,
        i32{0xFE},
...
void test_direct_bridge_truncates_slots(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        result.status, LegacyActionDrawStatus::ready, "direct bridge completes"
    );
    test.expect_equal(
        result.action_update_count, u32{0U}, "direct bridge skips action update"
    );
    test.expect_equal(
        ports.loads.front(),
        std::pair<u16, u16>{0x1357U, 0x2468U},
    ...
    test.expect_equal(
        ports.draws.front().x, i32{-20}, "direct bridge preserves x"
    );
    test.expect_equal(
        ports.draws.front().y, i32{30}, "direct bridge preserves y"
    );
    test.expect_equal(
        ports.draws.front().flags, u32{0U}, "direct bridge clears draw flags"
    );
    test.expect_equal(
        ports.draws.front().opacity_step, i32{0}, "direct bridge clears opacity"
    );
}
...
void test_masked_bridge_reads_updated_flags(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        result.status, LegacyActionDrawStatus::ready, "masked bridge completes"
    );
    test.expect_equal(
        ports.draws.front().flags,
        0x80000103U,
...
void test_failures_stop_at_original_boundaries(openswd3::test::Context& test) {
    ...
    const auto stopped =
        update_draw_legacy_action(record, 1, 2, update_failure);
    test.expect_equal(
        stopped.status,
    ...
    test.expect_true(
        update_failure.draws.empty(), "failed action update performs no draw"
    );
    ...
    test.expect_true(
        load_failure.draws.empty(), "failed TSW query performs no draw"
    );

    FakePorts draw_failure;
    draw_failure.draw_status = LegacyBlitExecutionStatus::malformed_source;
    const auto diagnosed =
        update_draw_legacy_action(record, 1, 2, draw_failure);
    test.expect_equal(
        diagnosed.status,
        LegacyActionDrawStatus::ready,
        "legacy callers do not branch on the blit result"
    );
    test.expect_equal(
        diagnosed.blit_failure_count,
        u32{1U},
...
void test_real_act_tsw_and_blitter(
    ...
    test.expect_equal(
        result.draw_count, u32{1U}, "real bridge submits one blit"
    );
    test.expect_equal(
        result.blit_failure_count,
        u32{0U},
