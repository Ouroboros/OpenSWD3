...
[[nodiscard]] std::vector<u8> make_patterned_framebuffer() {
    ...
    for (std::size_t index = 0U; index < framebuffer.size(); ++index) {
        ...
        framebuffer[index] = static_cast<u8>(
            row * 7U + column * 3U + 0x5AU
        );
    }
    return framebuffer;
}
...
void test_reset_and_guard_paths(openswd3::test::Context& test) {
    ...
    test.expect_true(
        std::ranges::all_of(effect.state().pixel_offsets,
                            [](const auto value) { return value == 0U; }),
        "reset clears all 64 source offsets"
    );
    test.expect_true(
        std::ranges::all_of(effect.state().copy_width_bytes,
                            [](const i16 value) { return value == 4; }),
        "0x00411D00 initializes all 64 widths to four"
    );
    test.expect_true(
        std::ranges::all_of(effect.state().copy_row_counts,
                            [](const i16 value) { return value == 1; }),
        "0x00411D00 initializes all 64 row counts to one"
    );
    test.expect_equal(effect.state().frame_counter, std::uint16_t{0U},
    ...
    test.expect_equal(disabled.status, LegacyAniRowCopyStatus::disabled,
...
void test_initialization_and_fixed_framebuffer_vector(
    ...
    test.expect_equal(result.status, LegacyAniRowCopyStatus::ready,
                      "first enabled frame completes");
    test.expect_equal(result.refresh, LegacyAniRowCopyRefresh::initialized,
                      "zero counter initializes the first 48 slots");
    test.expect_equal(effect.state().frame_counter, std::uint16_t{1U},
                      "counter increments after parameter generation");
    test.expect_equal(random.index(), std::size_t{38U},
                      "48 triples preserve bounded RNG consumption and wrap");

    constexpr std::array<i16, 8U> expected_rows{
        64, 10, 4, 41, 26, 60, 11, 15,
    };
    constexpr std::array<i16, 8U> expected_widths{
        120, 132, 142, 102, 134, 108, 82, 80,
    };
    constexpr std::array<std::uint32_t, 8U> expected_offsets{
        639U, 51840U, 103040U, 154240U,
        205441U, 256640U, 721U, 51921U,
    };
    test.expect_true(
        std::ranges::equal(
            std::span{effect.state().copy_row_counts}.first(8U),
            expected_rows
        ),
    ...
    test.expect_true(
        std::ranges::equal(
            std::span{effect.state().pixel_offsets}.first(8U),
            expected_offsets
        ),
        "first offset vector keeps column-major six-band traversal"
    );
    test.expect_equal(effect.state().copy_row_counts[48U], i16{1},
                      "initialization leaves inactive row slots untouched");
    test.expect_equal(effect.state().copy_width_bytes[63U], i16{4},
                      "initialization leaves inactive width slots untouched");
    test.expect_equal(effect.state().pixel_offsets[63U], std::uint32_t{0U},
...
void test_refresh_cycle_and_sixty_four_slot_quirk(
    ...
    test.expect_equal(offsets.refresh, LegacyAniRowCopyRefresh::offsets,
                      "counter four refreshes offsets");
    constexpr std::array<std::uint32_t, 6U> expected_offsets{
        640U, 51839U, 103039U, 154239U, 205440U, 256639U,
    };
    test.expect_true(
        std::ranges::equal(
    ...
    test.expect_equal(offsets_effect.state().pixel_offsets[48U],
                      std::uint32_t{0U},
                      "offset refresh writes only 48 slots");
    test.expect_equal(offsets_random.index(), std::size_t{96U},
    ...
    test.expect_equal(rows.refresh, LegacyAniRowCopyRefresh::row_counts,
    ...
    test.expect_equal(rows_effect.state().copy_row_counts[61U], i16{47},
                      "row refresh writes inactive slot 61");
    test.expect_equal(rows_effect.state().copy_row_counts[62U], i16{76},
                      "row refresh writes inactive slot 62");
    test.expect_equal(rows_effect.state().copy_row_counts[63U], i16{45},
                      "row refresh writes all 64 slots");
    test.expect_equal(rows_random.index(), std::size_t{128U},
    ...
    test.expect_equal(gap.refresh, LegacyAniRowCopyRefresh::none,
                      "counter twelve is the fourth-cycle refresh gap");
    test.expect_equal(gap_random.index(), std::size_t{0U},
    ...
    test.expect_equal(widths.refresh, LegacyAniRowCopyRefresh::widths,
    ...
    test.expect_equal(widths_effect.state().copy_width_bytes[61U], i16{92},
                      "width refresh writes inactive slot 61");
    test.expect_equal(widths_effect.state().copy_width_bytes[62U], i16{86},
                      "width refresh writes inactive slot 62");
    test.expect_equal(widths_effect.state().copy_width_bytes[63U], i16{128},
                      "width refresh writes all 64 slots");
    test.expect_equal(widths_random.index(), std::size_t{128U},
                      "64 bounded widths consume 128 raw words");
}

void test_forward_row_copy_and_counter_wrap(
    openswd3::test::Context& test
) {
    LegacyAniRowCopyEffect effect;
    ...
    test.expect_equal(copied.copied_rows, std::uint32_t{2U},
    ...
    test.expect_equal(framebuffer[5U], original[5U],
                      "byte before the unaligned copy remains unchanged");
    test.expect_equal(framebuffer[12U], original[12U],
    ...
    test.expect_equal(wrap.refresh, LegacyAniRowCopyRefresh::none,
                      "0xffff performs no scheduled refresh");
    test.expect_equal(wrapped.state().frame_counter, std::uint16_t{0U},
                      "frame counter wraps at 16 bits");
    const auto reinitialized =
        wrapped.update(true, framebuffer, wrapped_random);
    test.expect_equal(reinitialized.refresh,
                      LegacyAniRowCopyRefresh::initialized,
                      "wrapped zero counter reinitializes active slots");
    test.expect_equal(wrapped.state().copy_width_bytes[63U], i16{9},
                      "zero-counter initialization preserves inactive widths");
    test.expect_equal(wrapped.state().copy_row_counts[63U], i16{2},
                      "zero-counter initialization preserves inactive rows");
    test.expect_equal(wrapped.state().pixel_offsets[63U],
                      std::uint32_t{123U},
