...
public:
    ...
    void write(const char* name, const std::span<const u8> bytes) const {
        std::ofstream output{root_ / name,
...
[[nodiscard]] std::vector<u8> synthetic_archive() {
    ...
    write_u32(bytes, descriptor_offset + 0x00U,
              static_cast<u32>(payload_offset - kBlockOffset));
    write_u32(bytes, descriptor_offset + 0x04U,
              static_cast<u32>(compressed.size()));
    write_u32(bytes, descriptor_offset + 0x08U,
              static_cast<u32>(kStream8.size()));
...
void test_lazy_open_route_and_conversion(openswd3::test::Context& test) {
    ...
    test.expect_equal(runtime.find_cached(1U, 0U).status,
                      LegacyTswRuntimeStatus::cache_miss,
                      "cache-only lookup does not initialize or load");
    test.expect_false(runtime.is_initialized(),
                      "cache-only miss preserves lazy state");

    const LegacyTswQueryResult first = runtime.query_cached(1U, 0U);
    test.expect_equal(first.status, LegacyTswRuntimeStatus::ready,
                      "first cached query loads");
    test.expect_true(runtime.is_initialized(), "first query opens all archives");
    test.expect_false(first.cache_hit, "first query is a miss");
    test.expect_true(first.frame.auxiliary_stream.empty(),
                     "physical TSW auxiliary pointer stays null");
    test.expect_true(first.frame.palette.empty(),
                     "sub_401C70 releases the external palette");
    test.expect_equal(first.frame.width, u16{47U}, "descriptor width is kept");
    test.expect_equal(first.frame.height, u16{95U}, "descriptor height is kept");
    ...
    test.expect_true(std::ranges::equal(first.frame.primary_stream,
                                        expected.bytes),
                     "cached frame is the exact converted command stream");
    test.expect_equal(runtime.cached_primary_bytes(),
    ...
    test.expect_equal(repeated.status, LegacyTswRuntimeStatus::ready,
                      "repeat query succeeds");
    test.expect_true(repeated.cache_hit, "repeat query hits cache");
    test.expect_true(repeated.frame.primary_stream.data() == first_pointer,
                     "cache returns a borrowed stable view");

    const LegacyTswQueryResult truncated =
        runtime.query_cached(0x00010001U, 0x00010000U);
    test.expect_true(truncated.cache_hit,
                     "both four-byte ABI slots truncate to low 16 bits");
    test.expect_true(runtime.find_cached(0x00010001U, 0x00010000U).cache_hit,
    ...
    test.expect_equal(runtime.cache_entry_count(), std::size_t{0U},
...
void test_special_resource_and_failures(openswd3::test::Context& test) {
    ...
    test.expect_equal(unavailable.query_cached(0xFFFFU, 13U).status,
    ...
    test.expect_equal(runtime.bucket_entry_count(3U), std::size_t{1U},
    ...
    test.expect_true(repeated.cache_hit, "special key also truncates both slots");
    test.expect_equal(loader.calls, std::size_t{1U},
                      "special cache hit skips loader");

    loader.fail = true;
    test.expect_equal(runtime.load_direct(0xFFFFU, 14U).status,
    ...
    test.expect_equal(missing.query_cached(1U, 0U).status,
                      LegacyTswRuntimeStatus::archive_open_failed,
                      "missing six-file set fails initialization");
    test.expect_false(missing.is_initialized(),
                      "failed initialization closes partial state");
}

void populate_eviction_shape(LegacyTswRuntime& runtime,
                             FakeSpecialLoader& loader,
    ...
    for (const u16 variant : kVariants) {
        ...
        test.expect_equal(loaded.status, LegacyTswRuntimeStatus::ready,
                          "eviction fixture frame loads");
    }
    test.expect_equal(loader.calls, std::size_t{7U},
                      "fixture has seven distinct keys");
    test.expect_equal(runtime.cached_primary_bytes(), 28U,
                      "fixture counts four bytes per special frame");
    test.expect_equal(runtime.bucket_entry_count(0U), std::size_t{3U},
                      "bucket zero starts longest");
    test.expect_equal(runtime.bucket_entry_count(1U), std::size_t{2U},
                      "bucket one count");
    test.expect_equal(runtime.bucket_entry_count(2U), std::size_t{2U},
                      "bucket two count");
}
...
void test_original_bucket_eviction(openswd3::test::Context& test) {
    ...
    test.expect_false(reloaded_tail.cache_hit,
                      "eviction runs before lookup and removes the LRU tail");
    test.expect_equal(first_loader.calls, std::size_t{8U},
                      "evicted requested frame is loaded again");
    test.expect_equal(before_hit.cached_primary_bytes(), 28U,
    ...
    test.expect_true(surviving_hit.cache_hit,
                     "query hits a non-selected bucket after eviction");
    test.expect_equal(one_bucket.bucket_entry_count(0U), std::size_t{0U},
                      "selected longest bucket is drained continuously");
    test.expect_equal(one_bucket.bucket_entry_count(1U), std::size_t{2U},
                      "eviction does not recompute a new longest bucket");
    test.expect_equal(one_bucket.bucket_entry_count(2U), std::size_t{2U},
                      "second non-selected bucket also survives");
    test.expect_equal(one_bucket.cached_primary_bytes(), 16U,
