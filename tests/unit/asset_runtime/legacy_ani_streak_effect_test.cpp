...
void test_reset_boundary_and_idle_rng(openswd3::test::Context& test) {
    ...
    effect.reset();
    test.expect_equal(effect.state().slots[47U].active_flags, i16{0},
                      "reset clears the first 48 physical slots");
    test.expect_equal(effect.state().slots[48U].active_flags, i16{9},
                      "reset leaves slot 48 untouched");
    test.expect_equal(effect.state().slots[63U].fixed_x, i16{123},
    ...
    test.expect_equal(idle.status, LegacyAniStreakStatus::ready,
                      "idle update completes");
    test.expect_false(idle.scanned_slots,
                      "zero target and live count skip the 64-slot scan");
    ...
    test.expect_equal(services.call_count, u32{0U},
                      "random 478 does not query service eight");
...
void test_trigger_service_and_creation_vector(
    ...
    test.expect_equal(result.service_query_count, u32{1U},
                      "random 953 queries service eight");
    test.expect_equal(services.call_count, u32{1U},
                      "service port is called exactly once");
    ...
    test.expect_equal(slot.vertical_step, i16{22},
                      "random vertical step 6 receives the plus-16 bias");
    test.expect_equal(slot.trail_limit, i16{23},
                      "random trail 15 receives the plus-eight bias");
    test.expect_equal(slot.remaining_frames, i16{32},
    ...
    test.expect_equal(slot.active_flags, i16{1},
                      "new slot writes active word one");
...
void test_trail_pixels_and_state_advance(openswd3::test::Context& test) {
    ...
    test.expect_equal(result.packed_color_count, u32{23U},
                      "row zero is skipped from the inclusive 24-point trail");
    test.expect_equal(result.adjusted_pixel_count, u32{23U},
    ...
    test.expect_equal(effect.state().slots[0U].fixed_x, i16{5712},
                      "zero horizontal drift keeps fixed x");
    test.expect_equal(effect.state().slots[0U].fixed_y, i16{22},
                      "outer state advances by vertical step once");
    test.expect_equal(effect.state().slots[0U].remaining_frames, i16{31},
                      "outer state decrements lifetime once");
    test.expect_equal(effect.state().slots[1U].fixed_x, i16{7296},
                      "second creation keeps the next bounded x value");
    test.expect_equal(effect.state().slots[1U].horizontal_step, i16{-2},
                      "right-half positive drift is negated");
    test.expect_equal(effect.state().slots[1U].vertical_step, i16{21},
                      "second creation keeps RNG call order");
    test.expect_equal(effect.state().slots[1U].trail_limit, i16{10},
                      "second creation keeps the final bounded value");
    test.expect_equal(framebuffer.row_pixels(0U)[357U], u16{0U},
                      "strict base-pointer comparison skips row zero");
    test.expect_equal(framebuffer.row_pixels(1U)[357U], u16{0x0842U},
                      "first drawn row adds grayscale channel value two");
    test.expect_equal(framebuffer.row_pixels(15U)[357U], u16{0x7BDEU},
                      "intensity 30 remains below channel saturation");
    test.expect_equal(framebuffer.row_pixels(16U)[357U], u16{0x7FFFU},
                      "intensity 32 saturates all effective channels");
    test.expect_equal(framebuffer.row_pixels(23U)[357U], u16{0x7FFFU},
...
void test_active_bit_wrap_and_bottom_count_quirk(
    ...
    test.expect_equal(result.adjusted_pixel_count, u32{0U},
                      "row at fixed canvas end is not adjusted");
    test.expect_equal(slot.active_flags, i16{0},
                      "canvas-end row clears the complete active word");
    test.expect_equal(slot.fixed_x, i16{32},
                      "x advances by low-word horizontal times vertical");
    test.expect_equal(slot.fixed_y, i16{496},
                      "y advances even after active is cleared");
    test.expect_equal(slot.remaining_frames, i16{1},
    ...
    test.expect_equal(slot.fixed_x, i16{-32767},
                      "fixed x addition wraps at 16 bits");
    test.expect_equal(slot.remaining_frames, i16{0},
                      "lifetime reaches zero after a skipped trail");
    test.expect_equal(slot.active_flags, i16{0},
                      "zero lifetime clears active");
    test.expect_equal(effect.state().previous_live_count, i16{0},
