...
void test_physical_groups_reset_and_initialization(
    ...
    test.expect_equal(
        sizeof(LegacyAniDirectionalMotionSlot),
        std::size_t{0x10U},
        "the motion group retains its 0x10-byte stride"
    );
    test.expect_equal(
        sizeof(LegacyAniDirectionalColorSlot),
        std::size_t{0x10U},
        "the color group retains its 0x10-byte stride"
    );
    test.expect_equal(
        sizeof(LegacyAniDirectionalTimingSlot),
        std::size_t{0x10U},
    ...
    test.expect_equal(
        kLegacyAniDirectionalUpdatedSlotCount,
        std::size_t{2U},
    ...
    test.expect_equal(
        effect.state().color[0U].current_offset,
        i32{-7},
        "motion reset preserves the remote color group"
    );
    test.expect_equal(
        effect.state().timing[0U].frame_counter,
        i32{9},
    ...
    test.expect_equal(
        initialized.initialized_slot_count,
        u32{4U},
        "all four physical slots initialize"
    );
    test.expect_equal(
        initialized.random_call_count,
        u32{24U},
    ...
    test.expect_equal(
        effect.state().timing[0U].current_interval,
        i32{6},
...
void test_disabled_path(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        result.status,
        LegacyAniDirectionalStatus::disabled,
    ...
    test.expect_equal(
        ...
        kLegacyAniDirectionalServiceId,
        "the gate uses service id five"
    );
    test.expect_equal(
        random.index(), std::size_t{0U}, "disabled path consumes no RNG state"
    );
    test.expect_true(
        ports.draws.empty(), "disabled path produces no asset or draw work"
    );
}

void test_inclusive_bounds_without_respawn(openswd3::test::Context& test) {
    const auto expect_outside =
        [&test](const i32 x, const i32 y, const char* message) {
            LegacyAniDirectionalEffect effect;
            ...
            const auto result = effect.update(
                kConfiguration, kFrame, random, services, action, ports
            );
            test.expect_equal(
                result.status, LegacyAniDirectionalStatus::ready, message
            );
            test.expect_equal(
            ...
            test.expect_equal(
                effect.state().motion[0U].world_x,
                x,
                "failed respawn leaves outside x unchanged"
            );
            test.expect_equal(
                effect.state().motion[0U].world_y,
                y,
...
void test_inside_timer_motion_color_and_draw(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        result.status,
        LegacyAniDirectionalStatus::ready,
    ...
    test.expect_equal(
        result.moved_slot_count, u32{1U}, "only the expired frame counter moves"
    );
    test.expect_equal(
    ...
    test.expect_equal(
        effect.state().timing[0U].target_interval,
        i32{1},
    ...
    test.expect_equal(
        effect.state().color[0U].target_offset,
        i32{-3},
    ...
    test.expect_equal(
        effect.state().motion[1U].world_x,
        i32{300},
        "an unexpired timer preserves slot-two x"
    );
    test.expect_equal(
        effect.state().motion[1U].world_y,
        i32{400},
        "an unexpired timer preserves slot-two y"
    );
    test.expect_equal(
        effect.state().timing[1U].frame_counter,
        i32{1},
    ...
    test.expect_equal(
        ports.action_ids,
        std::vector<u32>{
            kLegacyAniDirectionalActionId, kLegacyAniDirectionalActionId
        },
        "both updates force action 0x232b"
    );
    test.expect_equal(
        ports.variants,
        std::vector<u32>{10U, 11U},
    ...
    test.expect_equal(
        ports.loads[1U],
        std::pair<u16, u16>{101U, 201U},
        "second update advances the same shared action record"
    );
    test.expect_equal(
        ports.draws[0U].x, i32{75}, "first draw subtracts action and camera x"
    );
    test.expect_equal(
        ports.draws[0U].y, i32{144}, "first draw subtracts action and camera y"
    );
    test.expect_equal(
        ports.draws[1U].x,
        i32{269},
    ...
    test.expect_equal(
        ports.draws[0U].flags, u32{4U}, "four variants select draw flags four"
    );
    test.expect_equal(
        ports.draws[0U].opacity_step,
        i32{7},
        "action byte 0x8a is zero-extended into opacity"
    );
    test.expect_equal(
        ports.draws[0U].color_offset,
        i32{-2},
        "the first current color reaches all RGB channels"
    );
    test.expect_equal(
        ports.draws[1U].color_offset,
        i32{5},
...
void test_four_spawn_directions(openswd3::test::Context& test) {
    ...
    for (u32 direction = 0U; direction < 4U; ++direction) {
        ...
        const auto result = effect.update(
            configuration, kFrame, random, services, action, ports
        );
        ...
        test.expect_equal(
            effect.state().timing[0U].variant,
            i32{23},
...
void test_i32_wrap_and_alternate_draw_flags(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        result.moved_slot_count, u32{1U}, "only the first signed timer expires"
    );
    test.expect_equal(
        effect.state().motion[0U].world_x,
        i32{-2147483576},
    ...
    test.expect_equal(
        ports.draws.size(), std::size_t{2U}, "both in-range slots still draw"
    );
    test.expect_equal(
        ports.draws[0U].flags,
        u32{0x2CU},
        "variant counts other than four select flags 0x2c"
    );
    test.expect_equal(
        ports.draws[1U].flags,
        u32{0x2CU},
...
void test_invalid_direction_and_random_bounds(openswd3::test::Context& test) {
    ...
    const auto direction_result = invalid_direction.update(
    ...
    test.expect_equal(
        direction_result.invalid_direction_count,
        u32{1U},
        "directions above three take the switch default"
    );
    test.expect_equal(
        direction_result.random_call_count,
        u32{2U},
        "invalid direction skips x and velocity RNG calls"
    );
    test.expect_equal(
        invalid_direction.state().motion[0U].world_x,
        i32{-640},
        "invalid direction preserves outside motion state"
    );
    test.expect_equal(
        invalid_direction.state().motion[0U].world_y,
        i32{77},
        "invalid direction preserves outside y"
    );
    test.expect_equal(
        invalid_direction.state().timing[0U].variant,
        i32{23},
    ...
    test.expect_equal(
        bound_result.random_call_count,
        u32{1U},
        "the outside probability roll still precedes the failure"
    );
    test.expect_equal(
        invalid_bound.state().timing[0U].target_interval,
        i32{10},
    ...
    test.expect_equal(
        initialization_result.random_call_count,
        u32{1U},
...
void test_asset_failure_guards(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        update_failure.random_call_count,
        u32{1U},
    ...
    const auto load_failure = effect.update(
        kConfiguration, kFrame, load_random, services, action, load_ports
    );
    test.expect_equal(
        load_failure.status,
...
void test_real_act_tsw_and_blitter(
    ...
    test.expect_equal(
        result.draw_count, u32{1U}, "the inside real slot submits one frame"
    );
    test.expect_equal(
        result.skipped_outside_slot_count,
        u32{1U},
