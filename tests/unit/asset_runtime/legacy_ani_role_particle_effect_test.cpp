...
public:
    [[nodiscard]] bool resolve_role_position(
        const u16 role_selector, i16& world_x, i16& world_y
    ) override {
        ++call_count;
...
void test_physical_layout_pool_and_reset(openswd3::test::Context& test) {
    test.expect_equal(
        sizeof(LegacyAniRoleParticleNode),
        std::size_t{0x10U},
        "particle nodes retain the original 0x10-byte fields"
    );
    test.expect_equal(
        sizeof(LegacyAniRoleParticleEmitter),
        std::size_t{0x10U},
    ...
    test.expect_true(
        std::ranges::all_of(
            ...
            [](const LegacyAniRoleParticleEmitter& emitter) {
                return emitter.head_token == 0U && emitter.role_selector == 0;
            }
        ),
...
void test_cull_and_pre_loop_order(openswd3::test::Context& test) {
    const auto expect_culled = [&test](
                                   const i32 x, const i32 y, const char* message
                               ) {
        LegacyAniRoleParticleEffect effect;
        effect.emitters()[0U].role_selector = 7;
        ...
        const auto result = effect.update(
            x, y, 7, 1, kViewport, random, positions, action, ports
        );
        test.expect_equal(
            result.status, LegacyAniRoleParticleStatus::culled, message
        );
        test.expect_equal(
            random.index(),
            std::size_t{0U},
        ...
        test.expect_equal(
            ports.update_count, u32{0U}, "early cull performs no action update"
        );
        test.expect_equal(
            positions.call_count, u32{0U}, "early cull performs no role lookup"
        );
    };
    ...
    const auto no_match = effect.update(
        -63, 100, 8, 1, kViewport, inside_random, positions, action, ports
    );
    test.expect_equal(
        no_match.status,
        LegacyAniRoleParticleStatus::ready,
        "the coordinate immediately inside the margin proceeds"
    );
    test.expect_equal(
        no_match.random_call_count,
        u32{1U},
...
void test_four_emitters_spawn_rng_and_common_fade(
    ...
    const auto result = effect.update(
        100, 200, 7, 1, kViewport, random, positions, action, ports
    );
    test.expect_equal(
        result.status,
        LegacyAniRoleParticleStatus::ready,
    ...
    test.expect_equal(
        result.draw_count, u32{2U}, "new nodes draw in their creation frame"
    );
    test.expect_equal(
...
void test_special_map_fade_and_ignored_blit(openswd3::test::Context& test) {
    ...
    const auto result = effect.update(
        ...
        200,
        7,
        kLegacyAniRoleParticleSpecialMapId,
        kViewport,
        random,
        positions,
        action,
...
void test_delete_by_copy_and_tail_release(openswd3::test::Context& test) {
    ...
    LegacyAniRoleParticleNode* const current =
    ...
    FakePorts ports;
    LegacyActionRecord action = make_action_record();
    const auto copied = effect.update(
        100, 200, 7, 1, kViewport, random, positions, action, ports
    );
    test.expect_equal(
        copied.updated_node_count,
        u32{2U},
        "the expired node and copied successor are both visited"
    );
    test.expect_equal(
        copied.removed_node_count,
        u32{1U},
        "copy deletion releases exactly the successor allocation"
    );
    test.expect_equal(
        copied.copied_successor_count,
        u32{1U},
    ...
    const auto tail = effect.update(
        100, 200, 7, 1, kViewport, tail_random, positions, action, ports
    );
    test.expect_equal(
        tail.removed_node_count,
        u32{1U},
        "an expired tail releases the current allocation"
    );
    test.expect_equal(
        tail.copied_successor_count,
        u32{0U},
...
void test_modern_failure_guards(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        role_failure.status,
        LegacyAniRoleParticleStatus::role_lookup_failed,
        "an invalid world role index does not become host OOB access"
    );
    test.expect_equal(
        role_failure.random_call_count,
        u32{1U},
    ...
    const auto corrupt_result = corrupt.update(
        100,
        200,
        7,
        1,
...
void test_real_act_tsw_and_blitter(
    ...
    const auto result = effect.update(
        320, 240, 7, 1, kViewport, random, positions, action, ports
    );
    test.expect_equal(
        result.status,
        LegacyAniRoleParticleStatus::ready,
    ...
    test.expect_equal(
        result.draw_count, u32{1U}, "the fixed real vector submits one frame"
    );
    test.expect_equal(
        result.blit_failure_count,
        u32{0U},
