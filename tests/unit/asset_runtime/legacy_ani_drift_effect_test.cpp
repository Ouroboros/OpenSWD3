...
public:
  ...
  draw_frame_piece(const LegacyFramePiece &piece, const i32 destination_x,
                   const i32 destination_y, const u32 flags) noexcept override {
...
void test_initialization_and_selective_reset(openswd3::test::Context &test) {
  ...
  test.expect_equal(effect.state().slots.size(), kLegacyAniDriftSlotCount,
                    "the physical state owns four 0x10-byte slots");
  test.expect_true(std::ranges::all_of(effect.state().slots,
  ...
  test.expect_true(std::ranges::all_of(effect.action_records(),
                                       [](const LegacyActionRecord &record) {
                                         return record.action_id ==
                                                kLegacyAniDriftActionId;
                                       }),
  ...
  test.expect_equal(effect.state().slots[0U].x, kLegacyAniDriftInactiveX,
                    "scene reset rewrites the first x sentinel");
  test.expect_equal(effect.state().slots[3U].x, kLegacyAniDriftInactiveX,
                    "scene reset rewrites the fourth x sentinel");
  test.expect_equal(effect.state().slots[0U].y, i32{2},
                    "scene reset preserves y");
  test.expect_equal(effect.state().slots[0U].velocity_x, i32{3},
                    "scene reset preserves horizontal velocity");
  test.expect_equal(effect.state().slots[0U].velocity_y, i32{4},
                    "scene reset preserves vertical velocity");
  test.expect_equal(effect.action_records()[0U].field_94, 0x12345678U,
...
void test_disabled_path(openswd3::test::Context &test) {
  ...
  test.expect_equal(result.status, LegacyAniDriftStatus::disabled,
  ...
  test.expect_equal(services.last_service_id, kLegacyAniDriftServiceId,
                    "the query uses service id six");
  test.expect_equal(random.index(), std::size_t{0U},
                    "disabled path consumes no random words");
  test.expect_true(ports.variants.empty(),
                   "disabled path updates no action record");
  test.expect_equal(effect.state().slots[0U].x, kLegacyAniDriftInactiveX,
...
void test_bounds_respawn_rng_variants_and_draw_coordinates(
  ...
  const auto result = effect.update(100, 80, 10, 20, random, services, ports);
  test.expect_equal(result.status, LegacyAniDriftStatus::ready,
                    "four respawns complete");
  test.expect_equal(result.respawn_count, u32{4U},
                    "inclusive outer bounds invalidate all four slots");
  test.expect_equal(result.perturbation_count, u32{0U},
                    "respawn path skips probability perturbation");
  test.expect_equal(result.action_update_count, u32{4U},
                    "all four new slots update an action in the same frame");
  test.expect_equal(result.frame_request_count, u32{4U},
                    "all four new slots request a TSW frame");
  test.expect_equal(result.draw_count, u32{4U},
                    "all four new slots draw in the creation frame");
  test.expect_equal(random.index(), std::size_t{24U},
                    "four respawns consume twelve bounded values");

  test.expect_equal(ports.variants[0U], u32{55U},
                    "zero vertical and positive x select variant 55");
  test.expect_equal(ports.variants[1U], u32{56U},
                    "positive vertical and negative x select variant 56");
  test.expect_equal(ports.variants[2U], u32{53U},
                    "negative vertical and positive x select variant 53");
  test.expect_equal(ports.variants[3U], u32{55U},
                    "fourth spawn selects the same positive zero-row variant");
  test.expect_true(std::ranges::all_of(ports.action_ids,
                                       [](const u32 action_id) {
                                         return action_id ==
                                                kLegacyAniDriftActionId;
                                       }),
  ...
  test.expect_equal(ports.loads[3U], std::pair<u16, u16>{103U, 203U},
                    "updated +0x4a/+0x4c form the fourth TSW key");

  test.expect_equal(ports.draws[0U].x, i32{-84},
                    "first draw subtracts action offset and camera x");
  test.expect_equal(ports.draws[0U].y, i32{393},
                    "first draw subtracts action offset and camera y");
  test.expect_equal(ports.draws[1U].x, i32{1643},
                    "negative-x spawn starts beyond the right map edge");
  test.expect_equal(ports.draws[1U].y, i32{855},
                    "second draw uses its own action y offset");
  test.expect_equal(ports.draws[2U].x, i32{-86},
                    "third draw uses its own action x offset");
  test.expect_equal(ports.draws[2U].y, i32{414},
                    "third draw uses random y 456");
  test.expect_equal(ports.draws[3U].x, i32{-87},
                    "fourth draw uses its own action x offset");
  test.expect_equal(ports.draws[3U].y, i32{1127},
                    "fourth draw uses random y 1170");
  test.expect_equal(ports.draws[0U].flags, u32{0x30U},
                    "draw uses record +0x18 flags");

  const auto &slots = effect.state().slots;
  test.expect_equal(slots[0U].x, i32{-61},
                    "first positive velocity moves after drawing");
  test.expect_equal(slots[0U].y, i32{433},
                    "first zero vertical velocity preserves y");
  test.expect_equal(slots[0U].velocity_x, i32{3},
                    "first random horizontal velocity is three");
  test.expect_equal(slots[0U].velocity_y, i32{0},
                    "first random vertical velocity is zero");
  test.expect_equal(slots[1U].x, i32{1662},
                    "negative velocity moves right-edge spawn left");
  test.expect_equal(slots[1U].y, i32{897},
                    "second vertical velocity moves after drawing");
  test.expect_equal(slots[2U].x, i32{-63},
                    "third positive velocity moves from the left edge");
  test.expect_equal(slots[2U].y, i32{455},
                    "third negative vertical velocity applies");
  test.expect_equal(slots[3U].x, i32{-63}, "fourth positive velocity applies");
  test.expect_equal(slots[3U].y, i32{1170},
                    "fourth zero vertical velocity preserves y");
}
...
void test_active_probability_clamps_zero_quirk_and_valid_edges(
  ...
  LegacyAniDriftEffect effect;
  effect.state().slots = {
      LegacyAniDriftSlot{.x = 100, .y = 100, .velocity_x = 1, .velocity_y = 0},
      LegacyAniDriftSlot{.x = 200, .y = 200, .velocity_x = -5, .velocity_y = 3},
      LegacyAniDriftSlot{.x = 300, .y = 300, .velocity_x = 5, .velocity_y = 3},
      LegacyAniDriftSlot{.x = 400, .y = 400, .velocity_x = 0, .velocity_y = -3},
  };
  LegacySecondaryRng random;
  random.seed(39U);
  FakeServices services;
  ...
  test.expect_equal(result.respawn_count, u32{0U},
                    "in-range slots do not respawn");
  test.expect_equal(result.perturbation_count, u32{2U},
                    "probabilities 953 and 343 perturb two slots");
  test.expect_equal(random.index(), std::size_t{16U},
                    "two perturbed and two steady slots consume eight values");
  test.expect_equal(effect.state().slots[0U].velocity_x, i32{1},
                    "zero x result becomes the negated minus-one delta");
  test.expect_equal(effect.state().slots[0U].velocity_y, i32{-1},
                    "first vertical delta is minus one");
  test.expect_equal(effect.state().slots[2U].velocity_x, i32{5},
                    "positive x velocity remains clamped at five");
  test.expect_equal(effect.state().slots[2U].velocity_y, i32{3},
                    "positive y velocity remains clamped at three");
  test.expect_equal(ports.variants[0U], u32{53U},
                    "perturbed minus-one row with positive x is 53");
  test.expect_equal(ports.variants[1U], u32{56U},
                    "steady plus-three row with negative x is 56");
  test.expect_equal(ports.variants[2U], u32{57U},
                    "clamped plus-three row with positive x is 57");
  test.expect_equal(ports.variants[3U], u32{52U},
                    "steady minus-three row with zero x is 52");
  ...
  test.expect_equal(edge_ports.draws.size(), std::size_t{4U},
                    "the four immediately-inside bounds remain drawable");
  test.expect_equal(edges.state().slots[0U].x, i32{-119},
                    "x minus 119 is inside the strict bound");
  test.expect_equal(edges.state().slots[1U].x, i32{1719},
                    "maximum x minus one is inside the strict bound");
  test.expect_equal(edge_random.index(), std::size_t{8U},
...
void test_failures_ignored_blit_and_table_guard(openswd3::test::Context &test) {
  ...
  test.expect_equal(update_failure.failed_slot, u32{1U},
                    "failure reports the second physical slot");
  test.expect_equal(update_failure.draw_count, u32{1U},
                    "only the completed first slot draws");
  test.expect_equal(update_failure_effect.state().slots[0U].x, i32{-61},
                    "completed slot moves before the next failure");
  test.expect_equal(update_failure_effect.state().slots[1U].x, i32{1664},
  ...
  test.expect_equal(load_failure.draw_count, u32{0U},
                    "failed frame lookup performs no draw");
  test.expect_equal(load_failure_effect.state().slots[0U].x, i32{-64},
  ...
  test.expect_equal(ignored_blit.status, LegacyAniDriftStatus::ready,
  ...
  test.expect_equal(ignored_blit.draw_count, u32{4U},
                    "later slots still draw after a blit failure");
  test.expect_equal(blit_failure_effect.state().slots[0U].x, i32{-61},
  ...
  const auto guarded = guarded_effect.update(100, 80, 0, 0, guarded_random,
                                             services, guarded_ports);
  test.expect_equal(guarded.status,
                    LegacyAniDriftStatus::invalid_vertical_velocity,
                    "modern boundary isolates a corrupt negative table index");
  test.expect_true(guarded_ports.variants.empty(),
...
void test_real_act_tsw_and_blitter(openswd3::test::Context &test,
  ...
  LegacyAniDriftEffect effect;
  effect.state().slots = {
      LegacyAniDriftSlot{.x = 100, .y = 100, .velocity_x = 1, .velocity_y = -3},
      LegacyAniDriftSlot{.x = 200, .y = 120, .velocity_x = -1, .velocity_y = 0},
      LegacyAniDriftSlot{.x = 300, .y = 140, .velocity_x = 1, .velocity_y = 3},
      LegacyAniDriftSlot{.x = 400, .y = 160, .velocity_x = -1, .velocity_y = 1},
  };
  LegacySecondaryRng random;
  random.seed(11U);
  ...
  test.expect_equal(result.draw_count, u32{4U},
                    "real path submits all four blits");
