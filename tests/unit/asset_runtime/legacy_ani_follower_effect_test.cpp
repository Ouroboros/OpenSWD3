...
void test_default_state_and_clip_helper(openswd3::test::Context& test) {
    ...
    openswd3::rendering::set_legacy_clip_rectangle(raster, -5, -6, 700, 900);
    ...
    openswd3::rendering::set_legacy_clip_rectangle(raster, 700, 500, 600, 400);
    test.expect_equal(raster.clip_left, i32{700}, "left has no upper clamp");
    test.expect_equal(raster.clip_top, i32{500}, "top has no upper clamp");
    test.expect_equal(
        raster.clip_width, i32{-100}, "right below left keeps a negative width"
    );
    test.expect_equal(
        raster.clip_height,
        i32{-100},
...
void test_draw_order_clip_bug_and_movement(openswd3::test::Context& test) {
    ...
    FakePorts ports;
    const auto result =
        openswd3::asset_runtime::update_draw_legacy_ani_follower(
            true, state, action_record, ports
        );

    test.expect_equal(
        result.status,
        LegacyAniFollowerStatus::ready,
    ...
    test.expect_equal(
        ...
        kLegacyAniFollowerActionId,
        "shared action record retains action 0x232b"
    );
    test.expect_equal(
        action_record.base_variant,
        kLegacyAniFollowerSecondVariant,
    ...
    test.expect_equal(
        ports.clips[0U].left, i32{95}, "first clip left subtracts half width"
    );
    test.expect_equal(
        ports.clips[0U].top,
        i32{195},
    ...
    test.expect_equal(
        ports.clips[0U].right, i32{103}, "first clip bug adds half height to x"
    );
    test.expect_equal(
        ports.clips[0U].bottom, i32{203}, "first clip bottom adds half height"
    );
    test.expect_equal(
        ports.clips[1U].left, i32{-92}, "second clip is current x minus 192"
    );
    test.expect_equal(
        ports.clips[1U].top, i32{8}, "second clip is current y minus 192"
    );
    test.expect_equal(
        ports.clips[1U].right, i32{292}, "second clip is current x plus 192"
    );
    test.expect_equal(
        ports.clips[1U].bottom, i32{392}, "second clip is current y plus 192"
    );

    test.expect_equal(
        ports.draws[0U].x, i32{95}, "first draw centers with half width"
    );
    test.expect_equal(
        ports.draws[0U].y, i32{197}, "first draw centers with half height"
    );
    test.expect_equal(
        ports.draws[0U].flags, u32{0U}, "first draw uses zero flags"
    );
    test.expect_equal(
        ports.draws[1U].x, i32{90}, "second draw centers with its width"
    );
    test.expect_equal(
        ports.draws[1U].y, i32{194}, "second draw centers with its height"
    );
    test.expect_equal(
        ports.draws[1U].flags, u32{0x2CU}, "second draw uses flags 0x2c"
    );

    test.expect_equal(
        state.current_x, i32{105}, "movement happens after both draws"
    );
    test.expect_equal(
        state.current_y, i32{195}, "both velocity components apply once"
    );
    test.expect_equal(
        state.velocity_x, i32{5}, "nonmatching x keeps its velocity"
    );
    test.expect_equal(
        state.velocity_y, i32{-5}, "nonmatching y keeps its velocity"
    );
}
...
void test_equality_rules_and_wrap(openswd3::test::Context& test) {
    ...
    static_cast<void>(openswd3::asset_runtime::update_draw_legacy_ani_follower(
        true, already_at_target, action_record, ports
    ));
    test.expect_equal(
        already_at_target.current_x,
        i32{10},
        "both coordinates equal skips movement"
    );
    test.expect_equal(
        already_at_target.velocity_x,
        i32{3},
        "skipped movement does not clear stale x velocity"
    );
    test.expect_equal(
        already_at_target.velocity_y,
        i32{-4},
    ...
    test.expect_equal(
        one_axis_equal.current_x,
        i32{12},
        "one equal axis still receives its velocity"
    );
    test.expect_equal(
        one_axis_equal.current_y, i32{1}, "other axis reaches its target"
    );
    test.expect_equal(
        one_axis_equal.velocity_x,
        i32{2},
        "x velocity survives after moving away from target"
    );
    test.expect_equal(
        one_axis_equal.velocity_y,
        i32{0},
    ...
    static_cast<void>(openswd3::asset_runtime::update_draw_legacy_ani_follower(
        true, wrapped, action_record, ports
    ));
    test.expect_equal(
        wrapped.current_x,
    ...
    test.expect_equal(
        wrapped.velocity_x, i32{0}, "wrapped equality clears x velocity"
    );
}
...
void test_failure_and_ignored_blit_paths(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        disabled.status,
        LegacyAniFollowerStatus::disabled,
        "service 0x13 false returns immediately"
    );
    test.expect_true(
        disabled_ports.updates.empty(), "disabled path makes no external calls"
    );
    test.expect_equal(
        state.current_x, i32{1}, "disabled path does not move state"
    );
    ...
    test.expect_equal(
        failed_update.failed_variant,
        kLegacyAniFollowerFirstVariant,
        "failure reports the first variant"
    );
    test.expect_true(
        update_failure.loads.empty(),
    ...
    test.expect_true(
        load_failure.draws.empty(), "failed frame lookup performs no draw"
    );

    FakePorts blit_failure;
    blit_failure.failed_draw_call = 1U;
    const auto ignored_blit =
        openswd3::asset_runtime::update_draw_legacy_ani_follower(
...
void test_real_act_tsw_and_blitter(
    ...
    test.expect_equal(
        result.frame_request_count, u32{2U}, "real path loads both TSW frames"
    );
    test.expect_equal(
        result.draw_count, u32{2U}, "real path submits both blits"
    );
    test.expect_equal(
    ...
    test.expect_equal(
        action_record.field_4c, u16{0U}, "real variant 79 resolves frame zero"
    );
    test.expect_equal(
        tsw_runtime.cache_entry_count(),
        std::size_t{2U},
    ...
    test.expect_equal(
        raster.clip_top, i32{48}, "real second clip top is current y minus 192"
    );
    test.expect_equal(
        raster.clip_width, i32{384}, "real second clip width is 384"
    );
    test.expect_equal(
        raster.clip_height, i32{384}, "real second clip height is 384"
    );
    test.expect_equal(
        state.current_x,
        i32{320},
    ...
    test.expect_equal(
        state.current_y, i32{240}, "default y position remains fixed"
    );
}
