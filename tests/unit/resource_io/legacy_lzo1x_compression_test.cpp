...
[[nodiscard]] CompressionOutput compress(
    ...
    destination.resize(std::min<std::size_t>(
        destination.size(),
...
void test_short_match_and_tail_literals(openswd3::test::Context& test) {
    ...
    for (std::size_t tail_size = 0U; tail_size <= kTailBytes.size(); ++tail_size) {
        ...
        std::vector<u8> expected{
            0x02U, 'A', 'A', 'A', 'A', 'A',
            0x37U, static_cast<u8>(tail_size), 0x00U,
        };
        expected.insert(
            expected.end(),
            kTailBytes.begin(),
    ...
    constexpr std::array<u8, 15> kLongMatchExpected{
        0x02U, 'A', 'A', 'A', 'A', 'A',
        0x20U, 0x00U, 0x00U, 0x34U, 0x00U, 0x00U,
        0x11U, 0x00U, 0x00U,
    };
    expect_exact(test, long_match_source, kLongMatchExpected);
}

[[nodiscard]] std::vector<u8> make_distance_source(
...
void test_dictionary_widths(openswd3::test::Context& test) {
    ...
    const CompressionOutput output_14 = compress(
        source,
        openswd3::resource_io::compress_legacy_lzo1x_14
    );
    const CompressionOutput output_15 = compress(
        source,
        openswd3::resource_io::compress_legacy_lzo1x_15
    );
    ...
    test.expect_equal(output_14.result.bytes_written, 518U, "14-bit output size");
    test.expect_equal(
        fnv1a64(output_14.bytes),
        0x28C584A5307464A9ULL,
        "14-bit output bytes"
    );
    ...
    test.expect_equal(output_15.result.bytes_written, 517U, "15-bit output size");
    test.expect_equal(
        fnv1a64(output_15.bytes),
        0x4C542BBD551818C9ULL,
        "15-bit output bytes"
    );
    test.expect_false(
...
void test_destination_boundary(openswd3::test::Context& test) {
    ...
    for (const Compressor compressor : kCompressors) {
        const CompressionOutput exact = compress(
            kSource,
            compressor,
            kExpected.size()
        );
        ...
        const CompressionOutput short_output = compress(
            kSource,
            compressor,
            kExpected.size() - 1U
        );
