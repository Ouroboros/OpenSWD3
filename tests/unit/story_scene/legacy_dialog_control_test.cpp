...
public:
  [[nodiscard]] bool
  close_role_dialog_action(const openswd3::compat::u16 role_index) noexcept
      override {
  ...
  void close_detached_dialog() noexcept override { ++detached_close_count; }
...
void test_interactive_confirm_arms_then_closes(
...
void test_page_selection_one_advances_and_two_closes(
  ...
  const auto close = advance_legacy_dialog_control(
      close_record,
      LegacyDialogControlInput{.initial_selection_state = 2U}, close_state);
  test.expect_true(
...
void test_direct_rectangle_fade_and_sentinel(
    openswd3::test::Context &test) {
  ...
  const auto held = advance_legacy_dialog_control(
      sentinel, LegacyDialogControlInput{.current_tick = 50U},
      sentinel_state);
  test.expect_true(
...
void test_choice_chain_and_transition_gates(
    openswd3::test::Context &test) {
...
void test_apply_page_advance_restores_saved_style(
  ...
  const auto result = apply_legacy_dialog_control_action(
      message, LegacyDialogControlAction::advance_page, control, close,
      ports);
  test.expect_true(
  ...
  const auto invalid = apply_legacy_dialog_control_action(
      ...
      ports);
  test.expect_equal(
      invalid.status, LegacyDialogControlApplyStatus::source_out_of_bounds,
      "modern page advance isolates an invalid legacy pointer boundary");
}

void test_apply_close_preserves_flagged_counter(
    openswd3::test::Context &test) {
  LegacyDialogMessage message;
  ...
  const auto result = apply_legacy_dialog_control_action(
      message, LegacyDialogControlAction::close_message, control, close,
      ports);
  test.expect_true(
      result.message_closed &&
          ...
          control.selection_state == 0U &&
          control.advance_signal_state == 0U,
  ...
  const auto detached_result = apply_legacy_dialog_control_action(
      detached, LegacyDialogControlAction::close_message, control, close,
      ports);
  test.expect_true(detached_result.message_closed &&
                       ports.detached_close_count == 1U,
