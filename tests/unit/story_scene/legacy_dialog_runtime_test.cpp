...
public:
  [[nodiscard]] bool begin_text_surface(const i32 width,
                                        const i32 height) noexcept override {
  ...
  void end_text_surface() noexcept override { calls.push_back(Call::end); }

  [[nodiscard]] bool resolve_role_anchor(
      const u16 role_index, i32 &world_x, i32 &world_y) noexcept override {
  ...
  void set_dialog_clip(
      const LegacyDialogRectangle &rectangle) noexcept override {
    calls.push_back(Call::clip);
    clips.push_back(rectangle);
  }

  void draw_dialog_panel(
      const LegacyDialogPanelDrawRequest &request) noexcept override {
  ...
  void composite_text_surface(
      const LegacyDialogCompositeRequest &request) noexcept override {
    calls.push_back(Call::composite);
    composite = request;
  }

  void draw_dialog_indicator(
      const LegacyDialogIndicatorRequest &request) noexcept override {
    calls.push_back(Call::indicator);
    indicator_kind = request.kind;
  }

  void draw_dialog_caption(
      const LegacyDialogCaptionRequest &request) noexcept override {
  ...
  void restore_text_destination(const i32 width,
                                const i32 height) noexcept override {
  ...
  void draw_selected_choice_background(
      const LegacyDialogChoiceBackgroundRequest &) noexcept override {}
...
void test_empty_chain_is_exact_early_return(openswd3::test::Context &test) {
  ...
  test.expect_true(
      result.status == LegacyDialogRuntimeStatus::idle &&
          ports.calls.empty(),
...
void test_complete_message_order_and_dimensions(
  ...
  test.expect_true(
      result.status == LegacyDialogRuntimeStatus::completed &&
          ...
          result.composite_count == 1U &&
          result.indicator_draw_count == 1U &&
          ...
          ports.caption_size == 4U &&
          ports.caption_action == 0xABCDEFU &&
...
void test_close_is_composited_then_removed(openswd3::test::Context &test) {
  ...
  test.expect_true(
      result.status == LegacyDialogRuntimeStatus::completed &&
          ...
          state.messages.empty() && close < composite && composite < release &&
          ports.closed_role == 3U && ports.released_role == 3U &&
