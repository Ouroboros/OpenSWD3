...
using openswd3::battle::LegacyBattleActorIdleStateRequest;
using openswd3::battle::LegacyBattleActorIdleStateStatus;
using openswd3::battle::LegacyBattleActorIdleStateView;
using openswd3::compat::u32;

[[nodiscard]] LegacyBattleActorIdleStateRequest request() {
    return {
        .actor_token = 0x005029D0U,
        .entry_eax = 0x11112222U,
        .entry_edx = 0x33334444U,
        .entry_esp = 0x80001000U,
        .entry_return_address = 0x0045691CU,
        .entry_flags = {
            .carry = true,
            .parity = false,
...
void test_battle_actor_idle_state(openswd3::test::Context& test) {
    {
        ...
        action.group_a_action_execution[2U].idle_state_latch = 0x11223344U;
        (*startup.group_b_lifecycle)[3U].action_execution.idle_state_latch =
            0x55667788U;
        const auto owners = openswd3::battle::LegacyBattleActorIdleStateOwners{
            .action = &action,
            .startup = &startup,
        };
        const auto group_a =
            openswd3::battle::resolve_legacy_battle_actor_idle_state(
                owners,
        ...
        test.expect_true(
            group_a.latch ==
                    &action.group_a_action_execution[2U].idle_state_latch &&
            ...
            "idle-state resolver aliases only canonical Group-A action and Group-B lifecycle owners"
        );
    }

    {
        ...
        const auto result =
            openswd3::battle::query_legacy_battle_actor_idle_state(
                {.latch = &latch}, entry
            );
        test.expect_true(
            result.status == LegacyBattleActorIdleStateStatus::completed &&
                ...
                result.field_token == entry.actor_token + 0x2AB4U &&
            ...
            "idle-state getter returns the full dword and plain-RET machine state without changing flags"
        );
    }

    {
        ...
        test.expect_true(
            result.status == LegacyBattleActorIdleStateStatus::completed &&
                ...
                result.latch_reads == 1U && result.return_address_reads == 1U &&
                flags_equal(result.flags, entry.entry_flags),
            "zero idle-state value is loaded without synthesizing TEST or XOR flags"
        );
    }

    {
        ...
        const auto missing_stop =
            openswd3::battle::query_legacy_battle_actor_idle_state(
                LegacyBattleActorIdleStateView{}, request()
            );
        ...
        test.expect_true(
            field_stop.status ==
                    LegacyBattleActorIdleStateStatus::latch_read_typed_stop &&
                ...
                return_stop.return_eip == 0x004786A6U &&
