...
protected:
    [[nodiscard]] std::optional<bool> prepare_definition_record(
        ...
        if (load_reply.definition == nullptr) {
            return false;
        }
        std::copy(
            load_reply.definition->cbegin(),
            load_reply.definition->cend(),
            destination.begin()
        );
        return true;
    }
...
void test_battle_group_b_action_item_option(openswd3::test::Context& test) {
...

    {
        ...
        test.expect_true(
            all_failed && output == 0xAAAAAAAAU && port.copy_requests.empty(),
        ...
        for (u32 selector = 0U; selector < 3U; ++selector) {
            ...
            test.expect_true(
                result.status ==
                    ...
                    port.seek_calls == 3U && port.read_calls == 3U &&
                    port.release_calls == 1U && output == 0x1111U &&
                    text[0U] == 'A' && text[1U] == 'B' && text[2U] == 0U &&
                    text[3U] == 0xEEU && port.copy_requests.size() == 1U &&
...

    {
        ...
        OptionPort port;
        auto unterminated = std::make_shared<std::array<u8, 0xA4>>();
        unterminated->fill('X');
        port.load_reply.definition = unterminated;
        std::array<u8, 24> text{};
        text.fill(0xEEU);
        u32 output{};
        const auto result =
