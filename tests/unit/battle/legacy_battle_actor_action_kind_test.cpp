...
using openswd3::battle::LegacyBattleActorActionKindRequest;
using openswd3::battle::LegacyBattleActorActionKindStatus;
using openswd3::battle::LegacyBattleActorActionKindView;
using openswd3::compat::u16;
using openswd3::compat::u32;

[[nodiscard]] LegacyBattleActorActionKindRequest request() {
    return {
        .actor_token = 0x005029D0U,
        .entry_eax = 0xA5A51234U,
        .entry_edx = 0x33334444U,
        .entry_esp = 0x80001000U,
        .entry_return_address = 0x004539EBU,
        .entry_flags = {
            .carry = true,
            .parity = false,
...
void test_battle_actor_action_kind(openswd3::test::Context& test) {
    {
        ...
        action.group_a_action_execution[2U].action_kind = 0x1122U;
        (*startup.group_b_lifecycle)[3U].action_composition.action_kind =
            0x5566U;
        const auto owners = openswd3::battle::LegacyBattleActorActionKindOwners{
            .action = &action,
            .startup = &startup,
        };
        const auto group_a =
            openswd3::battle::resolve_legacy_battle_actor_action_kind(
                owners,
        ...
        test.expect_true(
            group_a.action_kind ==
                    &action.group_a_action_execution[2U].action_kind &&
                *group_a.action_kind == 0x1122U &&
                group_b.action_kind ==
                    &(*startup.group_b_lifecycle)[3U]
                         .action_composition.action_kind &&
                *group_b.action_kind == 0x5566U &&
                missing.action_kind == nullptr &&
                invalid.action_kind == nullptr,
            "action-kind resolver aliases only canonical Group-A action-execution and Group-B lifecycle owners"
        );
    }

    {
        const u16 action_kind = 0xCDEFU;
        const auto entry = request();
        const auto result =
            openswd3::battle::query_legacy_battle_actor_action_kind(
                {.action_kind = &action_kind}, entry
            );
        test.expect_true(
            result.status == LegacyBattleActorActionKindStatus::completed &&
                ...
                result.field_token == entry.actor_token + 0x2A6CU &&
                result.action_kind_reads == 1U &&
            ...
            "action-kind getter replaces only AX and preserves the plain-RET machine state and flags"
        );
    }

    {
        ...
        const auto zero_result =
            openswd3::battle::query_legacy_battle_actor_action_kind(
                {.action_kind = &zero}, entry
            );
        const auto one_result =
            openswd3::battle::query_legacy_battle_actor_action_kind(
                {.action_kind = &one}, entry
            );
        test.expect_true(
            ...
            "zero and one action kinds replace AX without zero extension, sign extension, or synthetic flags"
        );
    }

    {
        const u16 action_kind = 0xBEEFU;
        auto field_request = request();
        field_request.access.action_kind_readable = false;
        const auto field_stop =
            openswd3::battle::query_legacy_battle_actor_action_kind(
                {.action_kind = &action_kind}, field_request
            );
        const auto missing_stop =
            openswd3::battle::query_legacy_battle_actor_action_kind(
                LegacyBattleActorActionKindView{}, request()
            );
        ...
        const auto return_stop =
            openswd3::battle::query_legacy_battle_actor_action_kind(
                {.action_kind = &action_kind}, return_request
            );
        test.expect_true(
            field_stop.status ==
                    LegacyBattleActorActionKindStatus::
                ...
                field_stop.return_eip == 0x004786B0U &&
                field_stop.action_kind_reads == 0U &&
                ...
                return_stop.return_eip == 0x004786B7U &&
                return_stop.action_kind_reads == 1U &&
