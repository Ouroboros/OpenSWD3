...
void test_complete_contract_catalog(openswd3::test::Context& test) {
    ...
    for (std::size_t index = 0U; index < kExpected.size(); ++index) {
        test.expect_equal(
            contracts[index].site,
            kExpected[index].site,
            "presentation address order is exact"
        );
        test.expect_equal(
            contracts[index].source,
            kExpected[index].source,
        ...
        full_count +=
            contracts[index].rectangle_contract == full_surfaces ? 1U : 0U;
        dynamic_count += contracts[index].rectangle_contract ==
                dynamic_source_and_destination
            ? 1U
            : 0U;
        fixed_count +=
            contracts[index].rectangle_contract == fixed_bink_639x479 ? 1U : 0U;
    }
...
void test_full_surface_dispatch(openswd3::test::Context& test) {
    ...
    const auto result = openswd3::rendering::submit_legacy_presentation(
        LegacyPresentationSite::steady_world, ports
    );
    ...
    test.expect_false(
        result.request.has_source_rectangle, "NULL source RECT remains absent"
    );
    test.expect_false(
        result.request.has_destination_rectangle,
...
void test_dynamic_rectangles(openswd3::test::Context& test) {
    ...
    auto result = openswd3::rendering::submit_legacy_presentation(
        LegacyPresentationSite::battle_vertical_displacement_part_1, ports
    );
    test.expect_equal(
        result.status,
    ...
    test.expect_equal(
        ports.requests.size(), 0U, "missing rectangles do not reach the backend"
    );
    ...
    test.expect_true(
        result.request.has_source_rectangle, "dynamic source RECT is present"
    );
    test.expect_equal(
        result.request.source_rectangle,
        rectangles.source,
...
void test_bink_and_failure_paths(openswd3::test::Context& test) {
    RecordingPorts ports;
    auto result = openswd3::rendering::submit_legacy_presentation(
        LegacyPresentationSite::bink_video, ports
    );
    ...
    result = openswd3::rendering::submit_legacy_presentation(
        LegacyPresentationSite::pause_overlay, ports
    );
    test.expect_equal(
        result.status,
    ...
    result = openswd3::rendering::submit_legacy_presentation(
        static_cast<LegacyPresentationSite>(0xDEADBEEFU), ports
    );
    test.expect_equal(
        result.status,
    ...
    test.expect_equal(
        ports.requests.size(), 2U, "unknown site does not reach the backend"
    );
}
...
void test_full_surface_primary_composition(openswd3::test::Context& test) {
    ...
    for (openswd3::compat::u32 row = 0; row < 3U; ++row) {
        for (openswd3::compat::u32 column = 0; column < 4U; ++column) {
            source.row_pixels(row)[column] =
                static_cast<openswd3::compat::u16>(row * 10U + column + 1U);
        }
    }
    ...
    const auto status = openswd3::rendering::compose_legacy_primary_surface(
        primary, request, LegacyPresentationSources{.game_framebuffer = &source}
    );
    ...
    for (openswd3::compat::u32 row = 0; row < 3U; ++row) {
        test.expect_true(
            std::ranges::equal(source.row_pixels(row), primary.row_pixels(row)),
...
void test_primary_composition_rejections(openswd3::test::Context& test) {
    ...
    LegacyFramebuffer short_primary(
        LegacySurfaceGeometry{
            .pitch_bytes = 8,
            .width = 4,
            .height = 3,
