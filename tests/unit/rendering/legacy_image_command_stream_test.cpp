...
const std::vector<u8> kRaw16{
    0x9FU, 0x31U, 0x9FU, 0x31U, 0x33U, 0x33U,
    0x44U, 0x44U, 0x6BU, 0x02U, 0x6BU, 0x02U,
    0x55U, 0x55U, 0x9FU, 0x31U, 0x6BU, 0x02U,
    0x66U, 0x66U, 0x67U, 0x66U, 0x68U, 0x66U,
};

const std::vector<u8> kStream16{
    0xFFU, 0xFFU, 0x06U, 0x00U, 0x02U, 0x00U, 0x10U, 0x00U,
    0x0EU, 0x00U,
    0x02U, 0x80U,
    0x02U, 0x00U, 0x33U, 0x33U, 0x44U, 0x44U,
    0x02U, 0xC0U,
    0x00U, 0x00U,
    0x14U, 0x80U,
    0x01U, 0x00U, 0x55U, 0x55U,
    0x01U, 0x80U,
    0x01U, 0xC0U,
    0x03U, 0x00U, 0x66U, 0x66U, 0x67U, 0x66U, 0x68U, 0x66U,
    0x00U, 0x00U,
    0x00U, 0x00U,
};
...
void test_exact_encoders_and_decoders(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        encoded16.bytes,
        kStream16,
        "sub_4014F0 exact word command stream"
    );
    ...
    test.expect_equal(
        decoded16.bytes,
        kRaw16,
        "sub_4019A0 word decoder output"
    );
    ...
    test.expect_equal(
        decoded8.bytes,
        kRaw8,
        "sub_4019A0 byte decoder output"
    );
}
...
void test_literal_pixel_conversion(openswd3::test::Context& test) {
    ...
    const std::vector<u8> expected{
        ...
        0x0EU, 0x00U,
        0x02U, 0x80U,
        0x02U, 0x00U, 0x53U, 0x66U, 0x84U, 0x88U,
        0x02U, 0xC0U,
        0x00U, 0x00U,
        0x14U, 0x80U,
        0x01U, 0x00U, 0x95U, 0xAAU,
        0x01U, 0x80U,
        0x01U, 0xC0U,
        0x03U, 0x00U, 0xC6U, 0xCCU, 0xC7U, 0xCCU, 0xC8U, 0xCCU,
        0x00U, 0x00U,
        0x00U, 0x00U,
    };
    test.expect_equal(
...
void test_indexed_palette_conversion(openswd3::test::Context& test) {
    ...
    const std::vector<u8> expected_pixels{
        0x9FU, 0x31U, 0x9FU, 0x31U, 0x54U, 0x24U,
        0x1FU, 0xF8U, 0x6BU, 0x02U, 0x6BU, 0x02U,
        0x1FU, 0x00U, 0x9FU, 0x31U, 0x6BU, 0x02U,
        0xC0U, 0x07U, 0x53U, 0x66U, 0x84U, 0x88U,
    };
    test.expect_equal(
        decoded.status,
        LegacyImageCommandStreamStatus::completed,
    ...
    for (std::size_t index = 0U; index < palette.size(); ++index) {
        embedded[index * 2U] =
            static_cast<u8>(palette[index] & 0x00FFU);
        embedded[index * 2U + 1U] =
            static_cast<u8>(palette[index] >> 8U);
    }
...
void test_current_lmf_command_stream(
    ...
    const auto lookup = openswd3::resource_io::legacy_lmf_lookup_map(
        archive_path,
        72U
    );
    test.expect_equal(
    ...
    const auto map_header =
        openswd3::resource_io::legacy_lmf_read_map_header(
            archive_path,
            lookup.map_offset
        );
    test.expect_equal(
    ...
    const auto directory = openswd3::resource_io::
    ...
    test.expect_true(
        decoded_header.width == 1072U &&
            decoded_header.height == 1024U &&
