...
public:
    ...
    void run_legacy_command(
        ...
        events.push_back({Call::legacy_command, selector, std::string(payload)});
    }
...
int main() {
    ...
    test.expect_false(
        openswd3::app::run_nonempty_command_line_path(std::nullopt, empty_ports),
        "null command line continues normal startup"
    );
    test.expect_false(
        openswd3::app::run_nonempty_command_line_path(std::string_view{}, empty_ports),
        "empty command line continues normal startup"
    );
    test.expect_true(empty_ports.events.empty(), "empty paths call no command port");

    RecordingCommandLinePorts command_ports;
    test.expect_true(
        openswd3::app::run_nonempty_command_line_path("5payload", command_ports),
    ...
    test.expect_true(
        openswd3::app::run_nonempty_command_line_path("/x", wrapped_selector_ports),
