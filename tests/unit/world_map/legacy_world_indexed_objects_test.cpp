...
void test_loader_normalization_matches_00426519_00426620(
    ...
    test.expect_true(
        ...
        "word coordinates wrap after the original 16-bit shift and byte fields " "zero extend"
    );
    test.expect_equal(
        read_u16(object.command_stream, 12U),
        u16{0xF800U},
...
void test_invalid_stream_is_a_checked_load_failure(
    ...
    test.expect_true(
        result.status ==
                LegacyWorldIndexedObjectPreparationStatus::
...
public:
    ...
    [[nodiscard]] LegacyBlitExecutionStatus draw_object(
...
[[nodiscard]] LegacyWorldIndexedObject make_runtime_object(
    const u16 ordinal,
    const u16 width_tag,
    const u16 left,
    const u16 top,
    const u16 right,
    const u16 bottom,
    const u16 horizontal_factor,
    const u16 vertical_factor
) {
...
void test_reverse_link_order_ordinal_scan_clip_and_parallax(
    ...
    test.expect_true(
        ...
        "prepended list order wins within an ordinal and both " "parallax formulas match"
    );
    test.expect_true(
        ports.clips ==
            std::vector<ClipCall>{
        ...
        "world intersections become viewport-relative clips and " "restore full clip"
    );
}
...
void test_draw_failure_still_restores_full_clip(openswd3::test::Context& test) {
    ...
    test.expect_true(
        ...
        "a checked blitter failure remains visible after the " "original full-clip restore"
    );
}
