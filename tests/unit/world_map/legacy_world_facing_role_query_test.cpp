...
void test_invalid_player(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        result.tile_query_count, u32{0U}, "invalid player does not probe tiles"
    );
}
...
void test_cardinal_scan_order(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        west.role_index, u32{3U}, "west scans outward from the adjacent column"
    );
    const auto east =
        find_legacy_world_facing_role(roles, 6U, 0U, 1, 0, 40U, 30U);
    test.expect_equal(
        east.role_index, u32{4U}, "east begins at player tile plus player width"
    );
    const auto south =
        find_legacy_world_facing_role(roles, 6U, 0U, 0, 1, 40U, 30U);
    test.expect_equal(
...
void test_no_match_query_count(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        result.role_index,
        kLegacyWorldFacingRoleNotFound,
