...
public:
    openswd3::rendering::LegacyGlyphProviderStatus provide_glyph_mask(
        const openswd3::rendering::LegacyRawCharacter&,
        i32,
        i32,
...
public:
    ...
    openswd3::rendering::LegacyBlitExecutionStatus draw_frame_piece(
        const openswd3::rendering::LegacyFramePiece&,
...
void test_prime_and_live_surface_adapter(openswd3::test::Context& test) {
    ...
    const auto primed = openswd3::world_map::prime_legacy_world_dialog_runtime(
        state, action_ports
    );
    ...
    const u16 transparent_text_pixel = state.text_surface.physical_pixels()[1U];
    ...
    ports.draw_selected_choice_background(
        openswd3::story_scene::LegacyDialogChoiceBackgroundRequest{
            ...
            .width = 22,
            .height = 22,
            .surface_width = openswd3::story_scene::kLegacyDialogSurfaceWidth,
        }
    );
    ...
    test.expect_true(
        text_ready && primed.action_update_count == 10U &&
            ...
            state.caption_actions[3].action_id == 0x233BU && anchor_x == 120 &&
            anchor_y == 80 && !invalid_surface && surface && text_drawn &&
            state.choice_border.phase == 1U &&
...
void test_action_failures_remain_observable(openswd3::test::Context& test) {
    ...
    const auto result = openswd3::world_map::prime_legacy_world_dialog_runtime(
        state, action_ports
    );
    test.expect_true(
