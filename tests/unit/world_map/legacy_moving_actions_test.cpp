...
[[nodiscard]] LegacyMovingActionNode make_node(const u32 id, const float x,
                                               const float y) {
...
void test_exact_layout_and_frame_order(openswd3::test::Context &test) {
  test.expect_equal(sizeof(LegacyMovingActionNode), std::size_t{0xB4U},
                    "moving PicPaint node retains its full physical size");
  test.expect_equal(offsetof(LegacyMovingActionNode, action),
                    std::size_t{0x00U},
                    "the action record starts at node zero");
  test.expect_equal(offsetof(LegacyMovingActionNode, position_x),
                    std::size_t{0xA8U},
                    "float position keeps the producer offset");
  test.expect_equal(offsetof(LegacyMovingActionNode, next_pointer_32),
  ...
  test.expect_true(
      result.visited_count == 4U && result.action_update_failure_count == 1U &&
      ...
      "0x00414B60 preserves update, strict visibility, draw and retirement "
      "order");
  test.expect_true(
      ports.updated_ids == std::vector<u32>{1U, 2U, 3U, 4U} &&
          ports.loads == std::vector<std::pair<u16, u16>>{{0x1201U, 0x21U},
                                                          {0x1203U, 0x23U},
                                                          {0x1204U, 0x24U}} &&
  ...
  test.expect_true(nodes.size() == 3U && nodes.front().action.action_id == 2U &&
...
void test_strict_target_window_edges(openswd3::test::Context &test) {
  ...
  test.expect_true(result.frame_request_count == 0U &&
                       result.removed_count == 1U && nodes.size() == 1U &&
                       nodes.front().action.action_id == 4U,
