...
std::vector<u8> make_grid() {
    return std::vector<u8>(
        static_cast<std::size_t>(kMapWidth) * kMapHeight * 4U, 0U
    );
}

void set_cell(
    std::vector<u8>& grid, const u32 x, const u32 y, const u32 value
) {
    const std::size_t offset =
        (static_cast<std::size_t>(y) * kMapWidth + x) * 4U;
...
u8 expected_direction_bits_for_cell(
...
void test_eight_direction_occupancy_bits(openswd3::test::Context& test) {
    ...
    for (const auto& probe : probes) {
        ...
        set_cell(
            grid,
            static_cast<u32>(static_cast<i32>(kAnchorX) + probe.x),
...
void test_footprint_folding_and_zero_quirk(openswd3::test::Context& test) {
    ...
    test.expect_true(
        ...
        "the modern boundary rejects values that overflow the " "original six-slot arrays"
    );
}
        ...
        for (u32 height = 1U; height <= 6U; ++height) {
            for (i32 y = -1; y <= static_cast<i32>(height); ++y) {
                for (i32 x = -1; x <= static_cast<i32>(width); ++x) {
                    ...
                    set_cell(
                        grid,
                        static_cast<u32>(static_cast<i32>(kAnchorX) + x),
...
void test_surface_clearance_mask(openswd3::test::Context& test) {
    ...
    const auto open = compute_legacy_world_surface_clearance_mask(
        ...
        kMapWidth,
        kMapHeight,
        static_cast<i32>(kAnchorX),
        static_cast<i32>(kAnchorY),
        2U,
...
void test_cardinal_adjustment(openswd3::test::Context& test) {
    ...
    test.expect_true(
        unchanged.status == LegacyWorldDirectionProbeStatus::completed &&
...
void test_diagonal_adjustment(openswd3::test::Context& test) {
    ...
    test.expect_true(
        stopped.delta_x == 0 && stopped.delta_y == 0,
    ...
    test.expect_true(
        slide_right.delta_x == 1 && slide_right.delta_y == 0,
