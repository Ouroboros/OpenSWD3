...
public:
    ...
    [[nodiscard]] openswd3::rendering::LegacyTimedMessageResult update_and_draw(
        std::list<openswd3::rendering::LegacyTimedMessage>& messages, const u16
    ) noexcept override {
        return {
...
public:
    ...
    update_action_record(LegacyActionRecord& record) override {
        ...
        if (record.action_id ==
            openswd3::world_map::kLegacyWorldCursorActionId) {
    ...
    [[nodiscard]] bool load_frame_piece(
        const u16, const u16 frame_index, LegacyFramePiece& piece
    ) override {
...
public:
    ...
    [[nodiscard]] bool
    load_frame(const u16, const u16, LegacyWorldRoleFrame&) override {
        return false;
    }
...
struct Fixture {
    ...
    Fixture() {
        ...
        const std::size_t spatial_rows = static_cast<std::size_t>(map_height) +
...
void test_complete_frame_exact_order_and_state(openswd3::test::Context& test) {
    ...
    test.expect_true(
        fixture.outer_ports.text_configuration_calls == 1U &&
            fixture.outer_ports.configured_background == 0xFFFEU &&
            fixture.outer_ports.configured_secondary == 0U,
        "the debug overlay configures the 16-point renderer at " "the exact assembly slot"
    );
    test.expect_true(
        fixture.state.movement.camera_x_transition == 0 &&
        ...
        "the four movement transitions clear only after " "post-present bookkeeping"
    );
    test.expect_true(
        fixture.framebuffer.row_pixels(8U)[400U] == 0x1234U &&
        ...
        std::ranges::none_of(
            ...
            [](const u32 event) {
                return event >= kCountdownEventBase &&
                    event < kCountdownEventBase + 0x100U;
            }
        ),
...
void test_party_count_and_alignment_gates(openswd3::test::Context& test) {
...

    {
        ...
        test.expect_true(
            ...
            "unaligned player skips aligned bookkeeping while the " "unconditional validation gate still executes"
        );
    }
}
...
void test_checked_failures_stop_at_the_original_slot(
...

    {
        ...
        test.expect_true(
            result.status ==
                ...
                !result.presentation_requested &&
                fixture.roles[1].world_x == 128U,
            "invalid party-slot ownership stops after player motion and before " "00414570"
        );
    }
...

    {
        ...
        test.expect_true(
            result.status ==
                    LegacyWorldFrameCoordinatorStatus::countdown_failed &&
                result.countdown.status ==
                    openswd3::rendering::LegacyCountdownDisplayStatus::
                        piece_unavailable &&
