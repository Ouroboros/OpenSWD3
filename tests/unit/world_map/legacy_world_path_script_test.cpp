...
[[nodiscard]] u32
read_surface_u32(const std::span<const u8> bytes, const u32 cell) noexcept {
...
struct Fixture {
    ...
    void set_script(const std::span<const u16> words) noexcept {
        for (std::size_t index = 0U; index < words.size(); ++index) {
            write_u16(
                path_database, 0x220U + index * sizeof(u16), words[index]
            );
        }
    }

    [[nodiscard]] auto run() {
        return run_legacy_world_path_script(
            1U,
            path_database,
            roles,
...
void test_wait_opcode_preserves_frame_boundaries(
    ...
    test.expect_true(
        first.status == LegacyWorldPathScriptStatus::completed &&
            first.opcodes_dispatched == 2U && first.waits_set == 1U &&
            first.waits_decremented == 1U &&
            first.cursor_words_advanced == 2U &&
            fixture.roles[1].path_word_index == 2U &&
            fixture.roles[1].path_wait_remaining == 1U,
        "opcode 4 falls through to opcode 5, which consumes one " "wait frame"
    );

    const auto second = fixture.run();
    test.expect_true(
        second.opcodes_dispatched == 1U && second.waits_decremented == 1U &&
            fixture.roles[1].path_word_index == 2U &&
    ...
    test.expect_true(
        third.opcodes_dispatched == 1U && third.cursor_words_advanced == 1U &&
...
void test_request_and_movement_handshake(openswd3::test::Context& test) {
    ...
    test.expect_true(
        request.status == LegacyWorldPathScriptStatus::completed &&
            ...
            read_slot_u16(slot, 0x16U) == 4U &&
            read_slot_u16(slot, 0x18U) == 0U,
        "opcode 7 immediately falls through to opcode 8 and arms four-pixel " "movement"
    );
    ...
    test.expect_true(
        arrival.status == LegacyWorldPathScriptStatus::completed &&
        ...
        "terminal direction clears the object slot, advances " "opcode 8, and reaches opcode 0 in the same call"
    );
}
...

    {
        ...
        test.expect_true(
            result.status == LegacyWorldPathScriptStatus::completed &&
                ...
                read_slot_u16(slot, 0x02U) == 0x8000U &&
                slot.bytes[0x1AU] == 9U && read_slot_u16(slot, 0x16U) == 0U &&
                read_slot_u16(slot, 0x18U) == 0U,
            "blocked opcode-8 movement preserves the exact stall and " "cursor updates"
        );
    }
...
void test_action_wait_and_audio_handlers(openswd3::test::Context& test) {
    ...
    test.expect_true(
        base_result.opcodes_dispatched == 1U &&
    ...
    test.expect_true(
        direction_result.opcodes_dispatched == 1U &&
    ...
    test.expect_true(
        wait_result.waits_decremented == 1U &&
    ...
    test.expect_true(
        action_result.action_updates == 1U &&
    ...
    test.expect_true(
        sample_result.opcodes_dispatched == 1U &&
...
void test_clock_flag_and_transfer_handlers(openswd3::test::Context& test) {
    {
        ...
        test.expect_true(
            waiting.opcodes_dispatched == 1U && waiting.action_updates == 1U &&
                fixture.roles[1].path_word_index == 0U,
        ...
        test.expect_true(
            elapsed.opcodes_dispatched == 2U && elapsed.action_updates == 0U &&
                fixture.roles[1].path_word_index == 3U,
...

    {
        ...
        test.expect_true(
            taken.conditional_transfers == 1U &&
        ...
        test.expect_true(
            skipped.conditional_transfers == 0U &&
...

    {
        ...
        test.expect_true(
            ...
            "a transfer before the current PATH directory entry preserves the " "original signed 32-bit word cursor"
        );
    }

    {
        ...
        test.expect_true(
            inverted_result.conditional_transfers == 1U &&
                fixture.roles[1].path_word_index == 9U,
...

    {
        ...
        fixture.roles[1].path_word_index = 0U;
        constexpr std::array<u16, 5U> empty_any{16U, 0xFFFFU, 0x28U, 0U, 5U};
        fixture.set_script(empty_any);
        const auto empty_result = fixture.run();
        test.expect_true(
            empty_result.conditional_transfers == 0U &&
                fixture.roles[1].path_word_index == 5U,
        ...
        test.expect_true(
            any_result.conditional_transfers == 1U &&
                fixture.roles[1].path_word_index == 9U,
...

    {
        ...
        const auto result = fixture.run();
        test.expect_true(
            result.opcodes_dispatched == 3U &&
                !query_legacy_world_story_flag(fixture.shared_state, 12U) &&
                fixture.roles[1].path_word_index == 5U,
    ...
    {
        Fixture fixture;
        constexpr std::array<u16, 7U> script{25U, 3U, 0x2CU, 0U, 5U, 0U, 5U};
        fixture.set_script(script);
        fixture.shared_state.script_clock_origin = 7U;
        fixture.shared_state.script_clock = 10U;
        const auto equal = fixture.run();
        test.expect_true(
            equal.conditional_transfers == 0U &&
                fixture.roles[1].path_word_index == 5U,
        ...
        test.expect_true(
            greater.conditional_transfers == 1U &&
                fixture.roles[1].path_word_index == 7U,
        ...
        constexpr std::array<u16, 7U> wrapped{25U, 3U, 0x2CU, 0U, 5U, 0U, 5U};
        fixture.set_script(wrapped);
        const auto wrapped_result = fixture.run();
        test.expect_true(
            wrapped_result.conditional_transfers == 1U &&
                fixture.roles[1].path_word_index == 7U,
...
void test_variable_handlers(openswd3::test::Context& test) {
    ...
    test.expect_true(
        fixture.shared_state.script_variables[2] == 1001U &&
            fixture.roles[1].path_word_index == 4U,
    ...
    static_cast<void>(fixture.run());
    test.expect_equal(
        fixture.shared_state.script_variables[2],
        u32{1000U},
    ...
    test.expect_true(
        ge_result.conditional_transfers == 1U &&
            fixture.roles[1].path_word_index == 7U,
        "opcode 22 takes the variable equality edge"
    );

    fixture.roles[1].path_word_index = 0U;
    constexpr std::array<u16, 7U> less_equal{23U, 2U, 499U, 0x2CU, 0U, 5U, 5U};
    fixture.set_script(less_equal);
    const auto le_result = fixture.run();
    test.expect_true(
        le_result.conditional_transfers == 0U &&
    ...
    test.expect_true(
        invalid_result.invalid_variable_indices == 1U &&
            fixture.roles[1].path_word_index == 0U,
...
void test_role_mutation_and_random_walk_handlers(
...
) {
    {
        ...
        const auto result = fixture.run();
        test.expect_true(
            result.opcodes_dispatched == 2U && target.world_x == (8U << 4U) &&
                target.path_data_id == 9U && target.path_word_index == 0U &&
                fixture.roles[1].path_word_index == 4U &&
                slot.bytes[0U] == 0xFFU &&
...

    {
        ...
        static_cast<void>(fixture.run());
        test.expect_true(
            read_slot_u16(slot, 0x08U) == 0xFFFFU &&
...

    {
        ...
        static_cast<void>(fixture.run());
        test.expect_equal(
            fixture.roles[2].flags & 0x0000C000U,
            u32{0xC000U},
        ...
        static_cast<void>(fixture.run());
        test.expect_true(
            (fixture.roles[2].flags & 0x0000C000U) == 0U &&
                read_surface_u32(
                    fixture.surface, fixture.roles[2].map_cell_pointer_32
                ) == 0U,
...

    {
        ...
        test.expect_equal(
            disabled.status,
            LegacyWorldPathScriptStatus::invalid_role_index,
...

    {
        ...
        test.expect_true(
            result.action_updates == 1U &&
                ...
                read_surface_u32(fixture.surface, new_cell) == 0x10000100U,
...

    {
        ...
        test.expect_true(
            result.opcodes_dispatched == 1U &&
                ...
                read_slot_u16(slot, 0x00U) == 1U &&
                (slot.bytes[0x1BU] & 0x0FU) == 1U && slot.bytes[0x1CU] == 1U &&
...

    {
        ...
        const auto& slot = fixture.slots[0];
        test.expect_true(
            slot.bytes[0x1CU] == 4U && read_slot_u16(slot, 0x16U) == 0xFFFCU &&
...

    {
        ...
        const auto& slot = fixture.slots[0];
        test.expect_true(
            (slot.bytes[3U] & 0x80U) != 0U &&
                fixture.roles[1].action.base_variant == 0U &&
                read_slot_u16(slot, 0x16U) == 0xFFFFU &&
                read_slot_u16(slot, 0x18U) == 0xFFFFU,
            "opcode 30 idles without rewriting step fields when the " "random value exceeds sixty"
        );
    }
}
...
void test_follow_and_label_handlers(openswd3::test::Context& test) {
    {
        ...
        const auto result = fixture.run();
        test.expect_true(
            result.path_requests == 1U && result.opcodes_dispatched == 2U &&
...

    {
        ...
        const auto result = fixture.run();
        test.expect_true(
            result.opcodes_dispatched == 1U && result.path_requests == 0U &&
                fixture.roles[1].path_word_index == 0U,
    ...
    {
        constexpr std::array<u16, 8U> expected_x{
            9U, 0U, 7U, 9U, 10U, 6U, 8U, 8U
        };
        constexpr std::array<u16, 8U> expected_y{
            4U, 0U, 6U, 4U, 5U, 5U, 3U, 7U
        };
        for (u32 direction = 0U; direction < 8U; ++direction) {
            ...
            test.expect_true(
                result.path_requests == 1U &&
                    fixture.path_state.camera_target_x ==
                        expected_x[direction] &&
                    fixture.path_state.camera_target_y ==
...

    {
        ...
        const auto result = fixture.run();
        const auto payload =
            openswd3::world_map::resolve_legacy_world_path_label(
        ...
        static_cast<void>(fixture.run());
        test.expect_true(
            fixture.roles[1].path_payload_pointer_32 == 0U &&
                fixture.roles[1].path_payload_relation == 0U &&
...
void test_ignored_path_request_failures(openswd3::test::Context& test) {
    ...
    const auto result = run_legacy_world_path_script(
        ...
        kMapHeight,
        std::span<LegacyWorldObjectSlot>{},
        fixture.node_pool,
        fixture.path_state,
        LegacyWorldPathScriptRuntime{
...
void test_shared_path_clock_cadence(openswd3::test::Context& test) {
    ...
    test.expect_true(
        state.script_clock_frame_counter == 20U && state.script_clock == 0U,
        "the PATH clock does not advance during the first 20 frames"
    );

    advance_legacy_world_script_clock(state);
    test.expect_true(
        state.script_clock_frame_counter == 0U && state.script_clock == 1U,
    ...
    advance_legacy_world_script_clock(state);
    test.expect_equal(
        state.script_clock, u32{0U}, "the PATH clock wraps after 1000"
    );
}
...
void test_sub_405430_surface_restore_branches(openswd3::test::Context& test) {
    Fixture fixture;
    constexpr std::array<u16, 2U> script{5U, 0U};
    fixture.set_script(script);
    std::array<LegacyWorldRoleRecord, 6U> roles{};
    ...
    test.expect_true(
        ...
        "sub_405430 restores suspended roles and updates eligible no-PATH " "roles while excluding the controlled role"
    );
}

void test_current_path_entries(
    openswd3::test::Context& test,
    ...
    for (const auto& source : decoded.database.role_sources) {
        ...
        test.expect_true(
            result.status == LegacyWorldPathScriptStatus::completed &&
                result.last_opcode == 5U && result.opcodes_dispatched == 1U &&
                roles[1].path_word_index ==
                    static_cast<u32>(source.path_word_index) + 1U,
            "every current nonzero initial PATH entry executes the " "restored opcode-5 cursor rule"
        );
        ++verified;
    }
    test.expect_equal(
    ...
    if (directory_count != 802U ||
        payload_offset +
                static_cast<std::size_t>(directory_count) * sizeof(u32) >
            path.size()) {
        return;
    }
    ...
    for (const auto& source : decoded.database.role_sources) {
        if (source.path_data_id != 0U) {
            enqueue(
                source.path_data_id, static_cast<u32>(source.path_word_index)
            );
        }
    }
    ...
    while (!pending.empty()) {
        ...
        if (cursor.path_id >= directory_count) {
            std::cerr << "PATH invalid id=" << cursor.path_id
                      << " word=" << cursor.word_index << '\n';
            ++invalid_commands;
            continue;
        }
        const u32 script_relative = read_u32(
            path,
            payload_offset +
                static_cast<std::size_t>(cursor.path_id) * sizeof(u32)
        );
        ...
        if (opcode >= opcode_count) {
            std::cerr << "PATH unsupported id=" << cursor.path_id
                      << " word=" << cursor.word_index << " opcode=" << opcode
                      << '\n';
            ++unsupported_opcodes;
            continue;
        }
        ...
        const auto words_available = [&](const u32 words) {
            return command_offset +
                static_cast<std::size_t>(words) * sizeof(u16) <=
                path.size();
        };
        ...
        const auto transfer = [&](const std::size_t operand_word) {
            ...
            const u32 target_relative =
                read_u32(path, command_offset + operand_word * sizeof(u16));
        ...
        case 34U: {
            ...
            advance(
                static_cast<u32>(
                    (terminator - command_offset + sizeof(u16)) / sizeof(u16)
                )
            );
            break;
        }
