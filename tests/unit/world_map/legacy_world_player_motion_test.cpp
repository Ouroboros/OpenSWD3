...
void test_movement_bounds(openswd3::test::Context& test) {
    ...
    const auto centered = compute_legacy_world_movement_bounds(
        player,
        LegacyWorldCameraRect{100U, 100U, 740U, 580U},
        100U,
        100U
    );
    test.expect_true(
        centered.camera_left && centered.camera_right &&
            centered.camera_up && centered.camera_down &&
            centered.player_left && centered.player_right &&
            centered.player_up && centered.player_down,
    ...
    const auto origin = compute_legacy_world_movement_bounds(
        player,
        LegacyWorldCameraRect{},
        40U,
        30U
    );
    test.expect_true(
        !origin.camera_left && !origin.camera_up &&
            !origin.player_left && !origin.player_up &&
            origin.player_right && origin.player_down,
...
void test_idle_state(openswd3::test::Context& test) {
    ...
    test.expect_true(
        player.action.base_variant == 0x34U &&
            ...
            state.no_input_frame_count == 1U &&
            state.idle_phase == 0U &&
            state.idle_action_age == 130U &&
...
void test_coordinate_advance(openswd3::test::Context& test) {
    ...
    test.expect_true(
        player.world_x == 97U && player.world_y == 203U &&
            camera.left == 7U && camera.right == 27U &&
            camera.top == 20U && camera.bottom == 40U,
