...
std::vector<u8>
make_grid(const u32 width = kMapWidth, const u32 height = kMapHeight) {
...
void set_cell(
    ...
    const std::size_t offset =
        (static_cast<std::size_t>(y) * map_width + x) * 4U;
...
LegacyWorldPathfindingRequest make_request(
...
void test_direct_and_diagonal_routes(openswd3::test::Context& test) {
    ...
    test.expect_true(
        straight.status == LegacyWorldPathfindingStatus::completed &&
            straight.legacy_return_value == 1 && straight.path_length == 2U &&
            straight.path[0] == 7U && straight.path[1] == 7U &&
            straight.path[2] == 0xFFU,
    ...
    test.expect_true(
        statistics.available_nodes == statistics.allocated_nodes &&
...
void test_obstacle_detour_and_collision_mask(openswd3::test::Context& test) {
    ...
    test.expect_true(
        failure.status == LegacyWorldPathfindingStatus::completed &&
            failure.legacy_return_value == 0 && failure.path[0] == 0xFFU,
    ...
    const auto allowed =
        mask_override.find_path(make_request(grid, 5, 5, 7, 5));
    test.expect_true(
...
void test_legacy_early_returns_and_object_state(openswd3::test::Context& test) {
    ...
    test.expect_true(
        same_position.legacy_return_value == 1 &&
            same_position.path_length == 0U && same_position.path[0] == 0xFFU &&
    ...
    test.expect_true(
        unchecked.status == LegacyWorldPathfindingStatus::completed &&
    ...
    test.expect_true(
        first.legacy_return_value == 1 && pathfinder.legacy_success_flag() &&
...
void test_boundaries_and_legacy_path_limit(openswd3::test::Context& test) {
    ...
    for (u32 x = 0U; x < long_width; ++x) {
        set_cell(
            long_grid, long_width, x, 0U, kLegacyWorldPathDefaultCollisionMask
        );
        set_cell(
            long_grid, long_width, x, 2U, kLegacyWorldPathDefaultCollisionMask
        );
    }
    set_cell(
        long_grid, long_width, 0U, 1U, kLegacyWorldPathDefaultCollisionMask
    );
    set_cell(
        long_grid,
        long_width,
