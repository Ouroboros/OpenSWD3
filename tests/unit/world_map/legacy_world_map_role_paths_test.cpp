...
void write_cell(std::span<u8> surface, const u32 cell_index,
...
[[nodiscard]] u32 read_cell(std::span<const u8> surface,
                            const u32 cell_index) noexcept {
  const std::size_t offset = static_cast<std::size_t>(cell_index) * 4U;
...
struct Fixture {
  ...
  [[nodiscard]] LegacyWorldMapRolePathResult run(const u8 runtime_flags = 0U) {
...
void test_arrival_replays_exact_state_order(openswd3::test::Context &test) {
  ...
  fixture.actions.update_status = LegacyActionUpdateStatus::stream_load_failed;
  ...
  test.expect_true(
      ...
      "the 72-slot loop preserves movement, arrival, diagnostic and camera "
      "order");
  test.expect_true(
      role.world_x == 400U && role.world_y == 320U &&
          role.map_cell_pointer_32 == new_cell && role.flags == 0x02008100U &&
          role.path_wait_remaining == 0U && role.action.wait_remaining == 0U &&
  ...
  test.expect_true(
      ...
      "action validation sees arrival overrides before selected-role "
      "recentering");
}
...
void test_wait_and_skip_gates_keep_their_slots(openswd3::test::Context &test) {
  {
    ...
    const auto result = fixture.run();
    test.expect_true(result.status == LegacyWorldMapRolePathStatus::completed &&
                         result.roles_moved == 0U &&
                         result.aligned_updates == 0U &&
                         result.action_update_count == 1U &&
                         result.camera_recenter_count == 1U &&
                         fixture.roles[1].world_x == 384U &&
                         read_u16(fixture.state.active_object_slots[0],
                                  kPathCursorOffset) == 0U,
                     "a live action wait bypasses motion but retains update "
                     "and camera slots");
  }
...
void test_unaligned_motion_defers_cell_bookkeeping(
  ...
  test.expect_true(
      ...
      "sub-cell movement changes coordinates/action only until 16-pixel "
      "alignment");
}
...
void test_guid_one_arrival_resets_motion_and_recenters(
  ...
  for (const u8 runtime_flags : std::array<u8, 2U>{0U, 2U}) {
    ...
    test.expect_true(
        ...
        "GUID one arrival clears four transitions and 0x200 bytes before "
        "camera gates");
  }
}
...
void test_automatic_talk_and_failure_boundary(openswd3::test::Context &test) {
  {
    ...
    write_cell(fixture.surface, Fixture::kOldCell - Fixture::kMapWidth - 1U,
               0x100U);
...

  {
    ...
    test.expect_true(
        result.status ==
            ...
            result.cursor_advances == 0U && result.action_update_count == 0U &&
            fixture.roles[1].world_x == 400U &&
            fixture.roles[1].map_cell_pointer_32 == Fixture::kOldCell &&
            read_cell(fixture.surface, Fixture::kOldCell) == 0x10000100U,
        "an unconnected story/path owner stops at sub_42D920 without faking "
        "later state");
  }
}
