...
public:
    ...
    u32
    find_collision_role_at_tile(const u32 tile_x, const u32 tile_y) override {
        queried_tiles.push_back(Tile{tile_x, tile_y});
        if (hit_query.has_value() && queried_tiles.size() == *hit_query) {
...
void test_all_nine_selector_paths(openswd3::test::Context& test) {
    ...
    const std::vector<DirectionCase> cases{
        {-1,
         ...
         {{9U, 19U}, {10U, 19U}, {11U, 19U}, {9U, 19U}, {9U, 20U}}},
        {0, -1, {480U, 481U, 482U}, {{10U, 19U}, {11U, 19U}, {12U, 19U}}},
        {1,
         -1,
         {481U, 482U, 483U, 483U, 486U},
         {{11U, 19U}, {12U, 19U}, {13U, 19U}, {14U, 19U}, {14U, 20U}}},
        {-1, 0, {499U, 502U}, {{9U, 20U}, {9U, 21U}}},
        {0, 0, {}, {}},
        {1, 0, {503U, 506U}, {{13U, 20U}, {13U, 21U}}},
        {-1,
         1,
         {539U, 540U, 541U, 519U, 522U},
         {{9U, 22U}, {10U, 22U}, {11U, 22U}, {9U, 21U}, {9U, 22U}}},
        {0, 1, {540U, 541U, 542U}, {{10U, 22U}, {11U, 22U}, {12U, 22U}}},
        {1,
         1,
         {541U, 542U, 543U, 503U, 506U},
         {{11U, 22U}, {12U, 22U}, {13U, 22U}, {13U, 21U}, {13U, 22U}}},
    };

    for (const auto& direction : cases) {
        RecordingPorts ports;
        const auto result = check_legacy_movement_collision(
            role, direction.delta_x, direction.delta_y, 20U, ports
        );
...
void test_map_event_stops_before_role_query(openswd3::test::Context& test) {
    const LegacyWorldRoleRecord role = make_role();
    RecordingPorts ports;
    ports.cell_values[481U] = 0xA1B2C35AU;

    const auto result =
        check_legacy_movement_collision(role, 0, -1, 20U, ports);
    ...
    test.expect_equal(
        result.event_code, u32{0x5AU}, "only the low map-cell byte is returned"
    );
    test.expect_equal(
        result.hit_role_index,
        kLegacyMovementCollisionNoRole,
...
void test_role_hit_returns_zero_and_stops(openswd3::test::Context& test) {
    ...
    const auto result = check_legacy_movement_collision(role, 1, 0, 20U, ports);
...
void test_selector_alias_and_default(openswd3::test::Context& test) {
    ...
    const auto alias =
        check_legacy_movement_collision(role, 2, 0, 20U, alias_ports);
    test.expect_equal(
    ...
    const auto outside =
        check_legacy_movement_collision(role, 5, 0, 20U, default_ports);
    test.expect_true(
...
void test_zero_extent_and_wrapping(openswd3::test::Context& test) {
    ...
    const auto result =
        check_legacy_movement_collision(role, -1, -1, 1U, ports);
    test.expect_equal(
        ports.read_cells,
    ...
    test.expect_equal(
        result.event_code, u32{0U}, "wrapped reads retain x86 dword arithmetic"
    );
}
...
void test_session_grid_and_role_adapter(openswd3::test::Context& test) {
    ...
    roles[1].flags = kLegacyRoleSpatiallyActiveFlag | kLegacyRoleCollisionFlag;
    roles[1].action.field_2c = 1U;
    roles[1].action.field_30 = 1U;
    std::vector<openswd3::compat::u8> cells(800U * 4U);

    const auto role_hit =
        check_legacy_movement_collision(roles, 2U, 0U, 1, 0, cells, 20U);
    test.expect_equal(
        role_hit.status,
        LegacyMovementCollisionStatus::completed,
    ...
    const auto map_event =
        check_legacy_movement_collision(roles, 2U, 0U, 0, -1, cells, 20U);
    test.expect_equal(
...
void test_session_adapter_boundaries(openswd3::test::Context& test) {
    ...
    const auto invalid_role =
        check_legacy_movement_collision(roles, 1U, 1U, 0, -1, short_cells, 20U);
    test.expect_equal(
        invalid_role.status,
    ...
    const auto invalid_cell =
        check_legacy_movement_collision(roles, 1U, 0U, 0, -1, short_cells, 20U);
    test.expect_equal(
        invalid_cell.status,
        LegacyMovementCollisionStatus::invalid_map_cell,
