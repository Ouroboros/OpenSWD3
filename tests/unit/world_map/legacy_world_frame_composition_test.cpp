...
public:
  ...
  void draw_decorated_number(const i32 right, const i32 bottom, const u32 style,
                             const u32 value) noexcept override {
...
void test_normal_exact_order(openswd3::test::Context &test) {
  ...
  const auto result = compose_legacy_world_frame(
      framebuffer, raster, background.source(),
      LegacyWorldFrameState{.decorated_value = 123456U}, ports);
  ...
  const std::vector<u32> expected_queries{
      0x0FU, 0x13U, 0x48U, 0x13U, 0x48U, 0x13U, 0x0BU,
      0x48U, 0x48U, 0x51U, 0x0AU, 0x09U, 0x51U,
  };
  test.expect_true(
...
void test_service_13_reclips_after_pre_background(
  ...
  const auto result =
      compose_legacy_world_frame(framebuffer, raster, background.source(),
...
void test_clear_only_short_circuit(openswd3::test::Context &test) {
  ...
  const auto result = compose_legacy_world_frame(
      framebuffer, raster, LegacyWorldBackgroundSource{},
...
void test_current_map_24(openswd3::test::Context &test,
                         const std::filesystem::path &archive_path) {
  const auto map = load_legacy_world_map(archive_path, 24U);
  test.expect_equal(map.status, LegacyWorldMapLoadStatus::ready,
  ...
  const auto cache = load_legacy_cm_cache(LegacyCmCacheRequest{
  ...
  test.expect_equal(
      cache.status, LegacyCmCacheLoadStatus::ready_generated,
