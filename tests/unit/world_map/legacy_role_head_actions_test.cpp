...
void test_exact_layout_and_frame_order(openswd3::test::Context& test) {
    test.expect_equal(
        sizeof(LegacyRoleHeadActionNode),
        std::size_t{0xB4U},
        "role-head node retains its full physical size"
    );
    test.expect_equal(
        offsetof(LegacyRoleHeadActionNode, current_x),
        std::size_t{0x98U},
        "current X keeps the producer offset"
    );
    test.expect_equal(
        offsetof(LegacyRoleHeadActionNode, horizontal_motion),
        std::size_t{0x9AU},
        "motion word keeps its exact offset"
    );
    test.expect_equal(
        offsetof(LegacyRoleHeadActionNode, y),
        std::size_t{0x9EU},
        "fixed Y keeps its exact offset"
    );
    test.expect_equal(
        offsetof(LegacyRoleHeadActionNode, next_pointer_32),
        std::size_t{0xB0U},
    ...
    test.expect_true(
        nodes.size() == 5U && nodes.front().current_x == 2 &&
            ...
            nodes.back().current_x == 102 &&
            nodes.back().horizontal_motion == 6,
...
void test_ballistic_boundaries_are_inclusive(openswd3::test::Context& test) {
    ...
    test.expect_true(
        result.removed_count == 2U && nodes.empty(),
