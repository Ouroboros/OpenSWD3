...
void test_inactive_state_returns_without_mutation(
  ...
  test.expect_true(!advance_legacy_world_camera_pan(camera, state),
                   "two zero remaining axes skip sub_414570");
  test.expect_true(camera.left == 10U && camera.top == 20U &&
                       camera.right == 650U && camera.bottom == 500U &&
                       state.step_x == 7 && state.step_y == -9,
...
void test_signed_steps_and_axis_completion(openswd3::test::Context &test) {
  ...
  test.expect_true(
      camera.left == 5U && camera.right == 645U && camera.top == 0xFFFFFFFFU &&
          camera.bottom == 479U && state.remaining_x == 4 &&
          state.remaining_y == 0 && state.step_x == 4 && state.step_y == 0,
      "signed steps use 32-bit wrap and clear only a completed axis");

  test.expect_true(advance_legacy_world_camera_pan(camera, state),
                   "the unfinished x axis advances on the following frame");
  test.expect_true(camera.left == 9U && camera.right == 649U &&
...
void test_active_peer_preserves_noncanonical_axis(
  ...
  test.expect_true(camera.left == 2U && camera.right == 2U &&
                       camera.top == 1U && camera.bottom == 1U &&
                       state.remaining_x == -2 && state.remaining_y == 0 &&
                       state.step_x == 2 && state.step_y == 0,
