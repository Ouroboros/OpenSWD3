...
void write_cell(std::vector<u8> &surface, const u32 index,
...
[[nodiscard]] std::array<u8, 6U> make_path_command(const u16 x,
                                                   const u16 y) noexcept {
  return {7U,
          0U,
          static_cast<u8>(x),
          static_cast<u8>(x >> 8U),
...
struct RolePathFixture {
  ...
  [[nodiscard]] auto run(const std::span<const u8> command) {
    return request_legacy_world_role_path(1U, command, roles,
...
void test_ordinary_role_path_legacy_returns(openswd3::test::Context &test) {
  {
    ...
    const auto result = fixture.run(make_path_command(7U, 5U));
    test.expect_true(result.path_found && result.slot_index == 0x20U &&
...

  {
    ...
    const auto result = fixture.run(make_path_command(7U, 5U));
    test.expect_true(!result.free_slot_found && result.slot_index == 72U &&
                         result.legacy_return_value == 1,
...

  {
    ...
    test.expect_true(result.free_slot_found &&
                         !result.target_in_legacy_bounds &&
                         result.legacy_return_value == 0 &&
...
struct PartyPathFixture {
  ...
  [[nodiscard]] auto run(const bool collision_disabled = false) {
    ...
    return prepare_legacy_world_party_paths(roles, spatial,
                                            LegacyWorldRoleSurfaceContext{
                                                .map_width = kMapWidth,
                                                .selected_guid = roles[1].guid,
                                                .surface_grid = surface,
                                            },
                                            1U, 2U, party_indices, slots,
                                            history, camera, node_pool, ports);
  }
};
...
void test_party_path_generation_and_reuse(openswd3::test::Context &test) {
  {
    ...
    test.expect_equal(read_u16(slot, 0x04U), u16{7U << 4U},
                      "party slot target x");
    ...
    test.expect_equal(slot.bytes[kPathBytesOffset], u8{7U},
                      "party first path direction");
    test.expect_equal(fixture.roles[2].action.base_variant, u32{8U},
                      "party movement action base");
    test.expect_equal(fixture.roles[2].action.variant_delta, u32{3U},
...
void test_party_offscreen_preadvance_and_terminal_action(
  ...
  test.expect_equal(fixture.roles[2].world_x, u32{(5U << 4U) + 8U},
                    "off-screen follower final x");
  test.expect_equal(fixture.roles[2].world_y, u32{5U << 4U},
                    "off-screen follower final y");
  test.expect_equal(fixture.roles[2].map_cell_pointer_32,
                    u32{5U * kMapWidth + 5U},
                    "off-screen follower final map cell");
  test.expect_equal(fixture.roles[2].action.base_variant, u32{0U},
                    "terminal follower action base");
  test.expect_equal(fixture.roles[2].action.variant_delta, u32{6U},
                    "terminal follower copies selected role facing");
  test.expect_equal(read_u16(slot, 0x02U), u16{0x8002U},
                    "terminal follower cursor and frame gate");
  test.expect_equal(read_u16(slot, 0x16U), u16{0U}, "terminal follower step x");
  test.expect_equal(read_u16(slot, 0x18U), u16{0U}, "terminal follower step y");
}
  ...
  for (const bool collision_disabled : std::array<bool, 2U>{false, true}) {
    ...
    test.expect_true(
        result.status == LegacyWorldPartyPathPreparationStatus::completed &&
            ...
            result.movement_slots_blocked == (collision_disabled ? 0U : 1U) &&
            result.movement_slots_enabled == (collision_disabled ? 1U : 0U) &&
