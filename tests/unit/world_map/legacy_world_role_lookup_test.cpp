...
void test_guid_scan_and_wrapper(openswd3::test::Context &test) {
  ...
  test.expect_true(lookup_legacy_world_role_by_guid(roles, 9U, role_index) &&
                       role_index == 1U,
                   "0x0040C100 stores a successful lookup result");
  test.expect_true(!lookup_legacy_world_role_by_guid(roles, 8U, role_index) &&
...
void test_index_to_guid_boundary(openswd3::test::Context &test) {
  ...
  test.expect_true(valid.status == LegacyWorldRoleGuidStatus::ready &&
                       valid.guid == 0xBEEFU,
                   "0x0040C060 returns the low-word GUID for a valid index");

  const auto invalid = legacy_world_role_guid_at(roles, 2U);
  test.expect_equal(
      invalid.status, LegacyWorldRoleGuidStatus::invalid_role_index,
