  ...
  for (u32 tile = 0U; tile < tile_count; ++tile) {
    for (u32 y = 0U; y < 16U; ++y) {
      for (u32 x = 0U; x < 16U; ++x) {
        const u16 pixel = static_cast<u16>(tile * 0x1000U + y * 16U + x + 1U);
        const std::size_t offset = static_cast<std::size_t>(tile) * 0x200U +
...
void test_unaligned_edges_and_flags(openswd3::test::Context &test) {
  ...
  test.expect_true(
      result.status == LegacyWorldBackgroundRenderStatus::completed &&
          result.visited_cells == 41U * 31U && result.transparent_cells == 1U &&
          result.hidden_cells == 1U,
...
void test_partial_refresh(openswd3::test::Context &test) {
  ...
  const auto result =
      render_legacy_world_background(framebuffer, map.source(),
...
void test_current_map_24(openswd3::test::Context &test,
  ...
  test.expect_equal(cache.status, LegacyCmCacheLoadStatus::ready_generated,
