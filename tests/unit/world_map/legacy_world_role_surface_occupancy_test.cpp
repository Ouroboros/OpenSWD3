...
void write_cell(std::vector<u8>& bytes, const std::size_t index,
                const u32 value) {
...
[[nodiscard]] u32 read_cell(const std::vector<u8>& bytes,
...
void test_exact_masks_and_special_footprints(openswd3::test::Context& test) {
    ...
    const auto cleared = clear_legacy_world_role_surface_occupancy(role, context);
    test.expect_true(
...
void test_generic_path_revisits_anchor(openswd3::test::Context& test) {
    ...
    const auto result = mark_legacy_world_role_surface_occupancy(
        role,
        LegacyWorldRoleSurfaceContext{4U, 0xFFFFU, surface});
