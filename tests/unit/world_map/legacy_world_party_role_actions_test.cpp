...
void test_aligned_path_removes_spatial_role_and_moves_surface(
  ...
  test.expect_true(role.world_x == 400U && role.world_y == 304U &&
                       role.map_cell_pointer_32 == Fixture::kOldCell + 1U &&
                       role.action.variant_delta == 3U &&
                       role.flags == 0x20A00100U &&
                       read_u16(slot, kPathCursorOffset) == 0x8001U,
                   "aligned follower motion updates direction, cell cursor and "
                   "projected flags");
  test.expect_true(
      ...
      "aligned party movement removes without spatial reinsertion and "
      "transfers occupancy");
}
...
void test_inactive_and_waiting_slots_still_update_action(
    openswd3::test::Context &test) {
  {
    ...
    test.expect_true(
        ...
        "an inactive cursor skips path bytes but still refreshes the party "
        "action");
  }

  {
    ...
    test.expect_true(
        ...
        "the path byte is fetched before the wait gate but not indexed while "
        "waiting");
  }
}
...
void test_modern_ownership_guards(openswd3::test::Context &test) {
...

  {
    ...
    test.expect_equal(result.status,
                      ...
                      "the original pre-wait path-byte read remains an "
