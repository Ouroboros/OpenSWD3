...
public:
    ...
    resolve_label_bytes(const u32 token) noexcept override {
        ...
        return label_available ? std::span<const u8>{label}
                               : std::span<const u8>{};
    }
...
void test_main_draw_contract(openswd3::test::Context& test) {
    ...
    LegacyRleRowJitterState jitter;
    const auto result =
        draw_legacy_world_role(role, make_state(), jitter, ports);
    ...
    test.expect_true(
        draw.request.destination_x == 65 && draw.request.destination_y == 146 &&
            draw.request.flags == 0x80000003U &&
            draw.request.opacity_step == 5 && draw.group_before == 2 &&
            draw.request.auxiliary.size() == 3U && draw.phase_before == 8U &&
            role.action.field_89 == 12U,
...
void test_inclusive_cull_and_service(openswd3::test::Context& test) {
    ...
    auto left = draw_legacy_world_role(
        role, LegacyWorldRoleRenderState{}, jitter, ports
    );
    role.world_x = 960U;
    auto right = draw_legacy_world_role(
        role, LegacyWorldRoleRenderState{}, jitter, ports
    );
    test.expect_true(
    ...
    auto outside = draw_legacy_world_role(
        role, LegacyWorldRoleRenderState{}, jitter, ports
    );
    test.expect_true(
        !outside.horizontally_visible && ports.draws.size() == 2U,
        "ordinary role culling excludes values beyond the boundary"
    );

    role.world_x = 0U;
    ports.service_0b = true;
    auto service = draw_legacy_world_role(
        role, LegacyWorldRoleRenderState{}, jitter, ports
    );
    test.expect_true(
        service.suppressed_by_service && ports.loads.size() == 2U,
...
void test_ghost_pass(openswd3::test::Context& test) {
    ...
    test.expect_true(
        result.ghost_drawn && result.main_drawn && result.draw_count == 2U &&
            ports.draws.size() == 2U,
        "flash bit and low counter phase add the ghost pass"
    );
    const auto& ghost = ports.draws[0].request;
    test.expect_true(
        ghost.destination_x == 56 && ghost.destination_y == 154 &&
            ...
            ports.draws[0].group_before == 6 && ghost.auxiliary.empty(),
        "0x004145F0 phase-one geometry flags and colors are exact"
    );
    test.expect_true(
        ports.draws[0].phase_before == 20U &&
            ports.draws[1].phase_before == 8U && role.action.field_89 == 12U &&
...
void test_additive_overlay_particles_and_label(openswd3::test::Context& test) {
    ...
    test.expect_true(
        additive.flags == 0x80000013U && additive.red_offset == 5 &&
    ...
    test.expect_true(
        overlay.destination_x == 73 && overlay.destination_y == 171 &&
            overlay.flags == 0x55U && overlay.auxiliary.empty(),
...
void test_checked_failures_and_blit_diagnostics(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        label.status,
        LegacyWorldRoleDrawStatus::label_missing_terminator,
...
void test_spatial_traversal_order_padding_and_audio(
    ...
    test.expect_true(
        ...
        "0x00413870 scans 70 rows in each physical group including bottom " "padding"
    );
    test.expect_true(
        render_ports.loads ==
...
void test_spatial_traversal_checked_failures(openswd3::test::Context& test) {
    ...
    const auto draw_failure = draw_legacy_world_roles(
        spatial,
        roles,
        LegacyWorldRoleRenderState{},
        audio_state,
    ...
    const auto bad_link = draw_legacy_world_roles(
        spatial,
        roles,
        LegacyWorldRoleRenderState{},
        audio_state,
    ...
    const auto short_index = draw_legacy_world_roles(
        spatial,
        roles,
        LegacyWorldRoleRenderState{},
        audio_state,
...
void test_real_tsw_and_blitter(
    ...
    LegacyWorldRoleRenderRuntimePorts ports{
        tsw_runtime, framebuffer, raster, effects, external_ports
    };
    LegacyWorldRoleRecord role{};
    role.flags = 0x00008000U;
    role.world_x = 320U;
    ...
    const auto result = draw_legacy_world_role(
        role, LegacyWorldRoleRenderState{}, jitter, ports
    );
    test.expect_true(
        result.status == LegacyWorldRoleDrawStatus::completed &&
    ...
    test.expect_equal(
        framebuffer_hash,
        std::uint64_t{0xA4766C928B05DC88ULL},
