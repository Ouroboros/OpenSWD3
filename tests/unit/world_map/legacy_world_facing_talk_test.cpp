...
void make_role(
    LegacyWorldRoleRecord& role, const u32 tile_x, const u32 tile_y
) {
...
void test_role_talk(openswd3::test::Context& test) {
    ...
    test.expect_equal(
        result.role_index, u32{1U}, "the north target is selected"
    );
    test.expect_equal(
        result.facing,
        u32{1U},
    ...
    test.expect_equal(
        result.action_update_count, u32{2U}, "target then player are refreshed"
    );
    test.expect_equal(
    ...
    test.expect_equal(
        ports.updated[0], &roles[1].action, "first refresh targets NPC"
    );
    test.expect_equal(
        ports.updated[1], &roles[0].action, "second refresh targets player"
    );
    test.expect_equal(
    ...
    test.expect_equal(
        roles[1].action.one_shot_variant_delta,
        u32{6U},
    ...
    test.expect_equal(
        talk.talk_data_offset, 0x11223344U, "Talk data offset is copied"
    );
    test.expect_equal(
        talk.talk_script_id, u16{0x5678U}, "Talk script is copied"
    );
    test.expect_equal(
        talk.instruction_offset, u16{0x9ABCU}, "Talk cursor is copied"
    );
    test.expect_equal(talk.source_guid, u16{0x2468U}, "Talk GUID is copied");
    test.expect_equal(
        talk.world_x, 0xAAAAAAAAU, "facing role path leaves Talk X stale"
    );
    test.expect_equal(
        talk.world_y, 0xBBBBBBBBU, "facing role path leaves Talk Y stale"
    );
    test.expect_equal(
        one_shot, u32{0U}, "successful role Talk clears one-shot state"
    );
}
...
void test_gates(openswd3::test::Context& test) {
    ...
    u32 one_shot = 8U;
    RecordingPorts ports;
    auto busy = coordinate_legacy_world_facing_talk(
        {0U, 0, -1, 40U, 30U}, roles, talk, one_shot, ports
    );
    test.expect_false(
        busy.talk_created, "occupied Talk skips the facing query"
    );
    test.expect_equal(
        busy.tile_query_count, u32{0U}, "occupied Talk probes no tiles"
    );

    talk.source_guid = kLegacyWorldTalkIdleSource;
    roles[1].interaction_gate = 1U;
    auto gated = coordinate_legacy_world_facing_talk(
        {0U, 0, -1, 40U, 30U}, roles, talk, one_shot, ports
    );
    test.expect_false(
        gated.talk_created, "target interaction gate blocks Talk"
    );
    test.expect_true(
        gated.tile_query_count != 0U, "target gate is checked after query"
    );
    test.expect_equal(
        one_shot, u32{8U}, "failed Talk leaves one-shot state intact"
    );
