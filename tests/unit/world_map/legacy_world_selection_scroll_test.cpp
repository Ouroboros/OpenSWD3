...
void test_entry_gates(openswd3::test::Context &test) {
  ...
  test.expect_true(camera.left == 10U && camera.top == 20U &&
                       state.cursor_word_index == 6U &&
                       state.frames_remaining == 9 && state.saved_left == 77U,
...
void test_scroll_and_frame_tail_restore(openswd3::test::Context &test) {
  ...
  test.expect_true(camera.left == 107U && camera.top == 191U &&
                       camera.right == 747U && camera.bottom == 671U,
